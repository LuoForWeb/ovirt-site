...
class Upload
{
    ...
    private $config = [
        ...
        'subName'      => ['date', 'Y-m-d'],
        //保存根路径
        'rootPath'     => './web_ng/api/data/uploadfile/',
        // 保存路径
        'savePath'     => '',
     ...
     * @param string $driver       要使用的上传驱动
     *                             LOCAL-本地上传驱动，FTP-FTP上传驱动
     * @param $driverConfig 上传驱动
     */
...
/* 调整配置，把字符串配置参数转换为数组 */
    ...
    public function upload($files = '')
    {
        ...
        if (empty($files)) {
            $this->error = xphp_get_lang('WEB_UPLOAD_NO_FILE');
            return false;
        }
    ...
    //删除垃圾据
            ...
            if (in_array($ext, ['gif', 'jpg', 'jpeg', 'bmp', 'png', 'swf'])) {
                ...
                if (empty($imginfo) || ('gif' == $ext && empty($imginfo['bits']))) {
                    $this->error = xphp_get_lang('WEB_UPLOAD_ILLEGAL_IMAGE_FILE');
                    continue;
                }
            }
    ...
    private function check($file)
    {
        ...
        /* 无效上传 */
        if (empty($file['name'])) {
            $this->error = xphp_get_lang('WEB_UPLOAD_UNKNOWN_ERROR');
        }

        /* 检查是否合法上传 */
        if (!is_uploaded_file($file['tmp_name'])) {
            $this->error = xphp_get_lang('WEB_UPLOAD_ILLEGAL_UPLOAD_FILE');
            return false;
        }

        /* 检查文件大小 */
        if (!$this->checkSize($file['size'])) {
            $this->error = xphp_get_lang('WEB_UPLOAD_FILE_SIZE_ERROR');
            return false;
        }
        ...
        if (!$this->checkMime($file['type'])) {
            $this->error = xphp_get_lang('WEB_UPLOAD_FILE_MINE_TYPE_ERROR');
            return false;
        }

        /* 检查文件后缀 */
        if (!$this->checkExt($file['ext'])) {
            $this->error = xphp_get_lang('WEB_UPLOAD_FILE_SUFFIX_ERROR');
            return false;
        }
    ...
    private function error($errorNo)
    {
        switch ($errorNo) {
            case 1:
                $this->error = xphp_get_lang('WEB_UPLOAD_FILE_ERROR_TIPS1');
                break;
            case 2:
                 $this->error = xphp_get_lang('WEB_UPLOAD_FILE_ERROR_TIPS2');
                break;
            case 3:
                $this->error = xphp_get_lang('WEB_UPLOAD_PORTION_FILE');
                break;
            case 4:
                $this->error = xphp_get_lang('WEB_UPLOAD_NO_FILE_UPLOAD');
                break;
            case 6:
                $this->error = xphp_get_lang('WEB_UPLOAD_NOT_FIND_DIRECTORY');
                break;
            case 7:
                $this->error = xphp_get_lang('WEB_UPLOAD_WRITE_FILE_FAIL');
                break;
            default:
                $this->error = xphp_get_lang('WEB_UPLOAD_FILE_UNKNOWN_ERROR');
        }
    }
    ...
    private function getSaveName($file)
    {
        ...
        //保持文件名不变
            ...
            if (empty($savename)) {
                $this->error = xphp_get_lang('WEB_UPLOAD_NAME_FILE_ERROR');
                return false;
            }
        }
