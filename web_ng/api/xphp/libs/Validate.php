...
use Closure;
use xphp\exception\ValidateException;
use xphp\helper\Str;
use xphp\validate\ValidateRule;
...
class Validate
{
     ...
     * @return bool
     */
    public function ip($value, string $rule = ''): bool
    {
        $filter = [FILTER_VALIDATE_IP, FILTER_FLAG_IPV4|FILTER_FLAG_IPV6];
        if ($rule == 'ipv4') {
            $filter = [FILTER_VALIDATE_IP, FILTER_FLAG_IPV4];
        } elseif ($rule == 'ipv6') {
            $filter = [FILTER_VALIDATE_IP, FILTER_FLAG_IPV6];
        }
