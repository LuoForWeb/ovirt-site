...
class Sockets
{
    ...
    {
        $socket = xphp_get_config('socket');
        //初始化SOCKET信息
        $this->IP = $socket['IP'];
        $this->PORT = $socket['Port'];
        $this->recvByte = $socket['recvByte'];
        $this->recTimeout = $socket['recTimeout'];
        $this->sendTimeout = $socket['sendTimeout'];
        //初始化包头信息
        $this->magic_number = $socket['BdHeader']['magic_number'];
        $this->sequence_number = $socket['BdHeader']['sequence_number'];
        $this->source_type = $socket['BdRouterHeader']['source_type'];
        $this->message_level1_type = $socket['BdRouterHeader']['message_level1_type'];
        $this->message_level1_log_type = $socket['BdLogRouterHeader']['message_level1_type'];
        $this->message_level2_type = $socket['BdRouterHeader']['message_level2_type'];
        //初始化操作信息
    ...
    {
        //初始化操作类
        $this->opHandler = (new Op());

        $this->SOCKET = socket_create(AF_INET, SOCK_STREAM, SOL_TCP);

        if ($this->SOCKET === false) {
            //             echo "socket_create() failed.\n reason: " . socket_strerror(socket_last_error()) . "\n";
            $this->opHandler->writeLog('PF_SOCKET_CREATE_ERROR', xphp_get_config('log')['LOG_INFO']['ERROR']);
        ...
        if ($result === false) {
            ...
            $this->opHandler->writeLog('PF_SOCKET_CONNECT_ERROR', xphp_get_config('log')['LOG_INFO']['ERROR']);
    ...
    private function sendMsg(string $msg)
    {
        ...
        while (true) {
            ...
            if ($sent === false) {
                $this->opHandler->writeLog('PF_SOCKET_SEND_ERROR', xphp_get_config('log')['LOG_INFO']['ERROR']);
                return false;
            }
    ...
    private function recvMsg($length)
    {
        ...
        if ($buf === false) {
            $this->opHandler->writeLog('PF_SOCKET_RECV_ERROR', xphp_get_config('log')['LOG_INFO']['ERROR']);
            return false;
        }
        return $data;
    }
     ...
     * @return string
     */
    private function bdHeader(string $msg)
    {
     ...
     * @return string
     */
    private function bdRouterHeader(string $msg, $module, $nodeuuid)
    {

        $source_uuid = xphp_uuid();
        $length = strlen($msg);
        ...
        $packStr = pack(
            ...
            $module,
            $length,
            $this->message_level1_type,
            $this->message_level2_type
        );
        return $packStr . $msg;
    }
     ...
     * @param string $opName
     */
    private function setOpCodeInfo(int $module, string $opName)
    {
        //按模块实例化opcode对象
        $class = xphp_get_config('module')['MODULE_OPCODE_TYPE'][$module];

        $opcodeHandler = new $class();

        $opcodeInfo = $opcodeHandler->getOpcodeInfo($opName);
        if (empty($opcodeInfo)) {
            //如果在特定模块下没有找到,使用平台PUBLIC
            $module = xphp_get_config('module')['MODULE_TYPE']['PF'];
            $class = xphp_get_config('module')['MODULE_OPCODE_TYPE'][$module];
            $opcodeHandler = new $class();
            ...
            if (empty($opcodeInfo)) {
                ...
                $module = xphp_get_config('module')['MODULE_TYPE']['PFPRIVATE'];
                $class = xphp_get_config('module')['MODULE_OPCODE_TYPE'][$module];
                $opcodeHandler = new $class();
                $opcodeInfo = $opcodeHandler->getOpcodeInfo($opName);
            }
    ...
    {
        return xphp_get_config('module')['MODULE_TYPE']['PF'] == $module ? 6 : 4;
    }
     ...
     * @param int $sync 1同步模式 2异步
     * @param string $opName 操作名
     * @return string
     */
    private function groupMsg($msg, int $module, int $submodule_type, int $sync, string $opName, string $nodeuuid)
    {
        ...
        return $this->bdHeader($bdRouterHeader);
     ...
     */
    private function sendAndGetMsg($msg, $module, $submodule_type, $sync, $opName, string $nodeuuid)
    {
        //连接socket
        $this->socketConnect();
    ...
    private function checkRecvMsg($rece)
    {
        ...
        $bdTargetConnLostRouterHeader = xphp_get_config('socket')['BdTargetConnLostRouterHeader'];
        if (
            $moduleHeader[5] == $bdTargetConnLostRouterHeader['message_level1_type'] &&
            $moduleHeader[6] == $bdTargetConnLostRouterHeader['message_level2_type']
        ) {
            exit((new Response())->returns(
                (new Op())->muOpResult(
                    false,
                    xphp_get_lang('WEB_TIPS_SYSTEM_SERVICE_ERROR'),
                    xphp_get_lang('WEB_TIPS_SYSTEM_SERVICE_ERROR_TIP'),
                    'warning',
                    $this->getErrorCode('PF_SOCKET_SYSTEM_SERVICE_ERROR')
                )
            )
            );
        }
...
//             var_dump($revMsg);
        ...
        if (
            $opName == "NODE_SYS_OP_PREAD_FILE" || $opName == "EXCH_BACKUP_POINT_OP_CODE_READ_EXPORT_ITEM_DATA"
    ...
    private function asyncMsg($msg, $module, $submodule_type, $sync, $command, $opName, $nodeuuid)
    {
        ...
        //exchange获取身份验证码、添加组织单独处理
        if (
            $opName == 'M365_COMMON_OP_CODE_LOGIN_AZURE_AD' || $opName == 'M365_COMMON_OP_CODE_ADD_ORGANIZATION'
            || $opName == 'M365_ON_PREMISES_OP_CODE_ADD_ORGANIZATION' || $opName == 'M365_COMMON_OP_CODE_SYNC_META_FILE'
        ) {
            return $opID;
        } else {
     ...
     * @return array $result
     */
    private function getOpStatus(int $opID): array
    {
        $result = array(
            'result' => false,
            'errorCode' => 0,
        ...
        do {
            ...
            if ($opInfo) {
                ...
                if (xphp_get_config('app')['OP_STATUS']['SUCCEED'] == $status) {
                    //操作成功
                    $result['result'] = TRUE;
                    return $result;
                } elseif (xphp_get_config('app')['OP_STATUS']['FAILURE'] == $status) {
                    $this->opHandler->writeLog('PF_SOCKET_OP_STATUS_ERROR: ' . $errorCode, xphp_get_config('log')['LOG_INFO']['ERROR']);
                    $result['errorCode'] = $errorCode;
                    return $result;
                    //操作失败
                } elseif (xphp_get_config('app')['OP_STATUS']['RUNNING'] == $status) {
                    //正在运行
                    sleep(1);
                    continue;
                }
            } else {
                $this->opHandler->writeLog('PF_SOCKET_GET_OP_STATUS_ERROR', xphp_get_config('log')['LOG_INFO']['ERROR']);
        ...
        $this->opHandler->writeLog('PF_SOCKET_GET_OP_TIMEOUT', xphp_get_config('log')['LOG_INFO']['ERROR']);
     ...
     */
    public function opMsg(string $opName, $msg, int $module, int $submodule_type, bool $sync, bool $command, string $nodeuuid, $timeout = 0)
    {
        ...
        if ($sync) {
            $sync = xphp_get_config('socket')['MSG_MODE_TYPE']['SYNC'];
            return $this->syncMsg($msg, $module, $submodule_type, $sync, $command, $opName, $nodeuuid);
        } else {
            $sync = xphp_get_config('socket')['MSG_MODE_TYPE']['RSYNC'];
     ...
     * @return int
     */
    private function getErrorCode(string $errorName): int
    {

        $error = xphp_get_config('error', 'errorCode');
        return array_search($errorName, $error);
    }
