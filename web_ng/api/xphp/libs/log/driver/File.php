...
class File
{
    ...
    public function save(array $log = [], $depr = true)
    {
        ...
        $destination = $this->config['path'] . date('Ymd') . '/' . date('H') . '.log';
        ...
        $runtime = round(microtime(true) - XPHP_START_TIME, 10);
        $reqs = $runtime > 0 ? number_format(1 / $runtime, 2) : '∞';
        $time_str = ' [运行时间：' . number_format($runtime, 6) . 's][吞吐率：' . $reqs . 'req/s]';
        $memory_use = number_format((memory_get_usage() - XPHP_START_MEM) / 1024, 2);
        ...
        // 添加上请求头等信息 php执行脚本可能不会存在 会导致意外的报错
