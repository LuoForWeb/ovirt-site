...
class Index extends Base
{
    ...
    public function getJobLogList($params, $isExport = false)
    {
        ...
        $sqlData = "SELECT
                        ...
                        btl.submodule_type,
                        btl.error_code,
                        unix_timestamp( btl.op_time ) op_time,
                        btl.description_key,
                        btl.description_param,
                        btl.log_level 
                    FROM
                        bd_task_log btl
                        LEFT JOIN bd_user bu ON btl.user_uuid = bu.user_uuid 
                    WHERE
                        btl.running_flag = {$FLAG['UNSET']} ";
        ...
        if ($moduleType == $MODULE_TYPE['VM'] && !empty($hypervisor)) {
            //虚拟化类型
            $sql .= " and btl.submodule_type = '{$hypervisor}' ";
        } else if ($moduleType == $MODULE_TYPE['DB'] && !empty($dbType)) {
            //数据库类型
        ...
        //模块类型
        if (!empty($moduleType)) {
            if ($subModuleType == 3 && $moduleType == $MODULE_TYPE['VM']) { //公有云
                $publicCloud = "(" . implode(',', $vm_group['publiccloud']) . ")";
                $sql .= " and btl.module_type = '{$moduleType}' and btl.submodule_type in  " . $publicCloud . " ";
            } else if ($subModuleType == 2 && $moduleType == $MODULE_TYPE['VM']) { // 私有云
                $privateCloud = "(" . implode(',', $vm_group['openstack']) . ")";
                $sql .= " and btl.module_type = '{$moduleType}' and btl.submodule_type in  " . $privateCloud . " ";
            } else if ($subModuleType == 1 && $moduleType == $MODULE_TYPE['VM']) { // 虚拟机
                $publicCloud = "(" . implode(',', $vm_group['publiccloud']) . ")";
                $vmHypervisor = array_diff(xphp_get_config('vm', 'VMHYPERVISORTYPE'), $vm_group['publiccloud'], $vm_group['openstack']);
                ...
                if ($moduleType != $MODULE_TYPE['DB'] && !empty($subModuleType)) {
        ...
        foreach ($data as $d) {
            ...
            if ($d['module_type'] == $MODULE_TYPE['VM']) {
                $sub_module_type = $this->getVmSubModuleType($d['submodule_type']);
            } else if ($d['module_type'] == $MODULE_TYPE['DB']) {
                $sub_module_type = '';
            }
            $records["rows"][] = array(
                'num' => ++$offset,
                ...
                'sub_module_type_value' => $d['module_type'] == $MODULE_TYPE['FS'] ? $d['submodule_type'] : $this->getVmSubModuleType($d['submodule_type']),
    ...
    public function getSystemLogList($params, $isExport = false)
    {
        ...
        if ($this->checkEmpty($opType)) {
            ...
            } else if ($opType == 'SYSTEM_USER_LOGIN') {
                $sql .= " and (bsl.description_key = 'SYSTEM_USER_LOGIN_SUCCESS' OR bsl.description_key = 'SYSTEM_USER_LOGIN_FAILURE')";
            } else if ($opType == 'SYSTEM_USER_LOGINOUT') {
                $sql .= " and bsl.description_key like '%SYSTEM_USER_LOGINOUT_%' ";
            } else if ($opType == 'UI_PLATFORM_SYSTEM_SET') {
                $sql .= " and (bsl.description_key not like '%BD_SYSTEMLOG_DESC_KEY_NODE%' 
                    AND bsl.description_key not like '%BD_SYSTEMLOG_DESC_KEY_AGENT_%' 
                    AND bsl.description_key not like '%BD_SYSTEMLOG_DESC_KEY_STORAGE%' 
                    AND bsl.description_key not like '%BD_SYSTEMLOG_DESC_KEY_PRODUCTION_STORAGE%' 
                    AND bsl.description_key not like '%SYSTEM_USER%' 
                    AND bsl.description_key not like '%SYSTEM_USER_LOGIN%' 
                    )";
            } else {
    ...
    public function deleteSystemLog($params)
    {
        ...
        // 判断下是否在最低删除期限内
        $ids = "(" . implode(',', $idArray) . ")";
        $data = $this->dbSelect("SELECT unix_timestamp(op_time) op_time from bd_system_log where id in {$ids}");
        // 操作权限判断
        $userArr = implode(',', array_unique(array_column((array) $data, 'user_uuid')));
        $this->checkAuthByUserUuid($userArr, xphp_get_config('user', 'USER_AUTH')['log']);
        // 判断下是否符合保留策略
    ...
    {
        $sql = "SELECT id, task_uuid, description_key, description_param, ha_type, ha_rr_type, ha_time, error_code, error_detail, log_level, task_name FROM bd_cluster_ha_log WHERE 1=1 ";
    ...
    public function deleteHaLog($params): array
    {
        ...
        // 验证是否能够删除
        $validateRet = $this->validateDeleteHaLog($params['ha_log_id_list']);
    ...
    private function getOpType($des)
    {
        ...
        if ($des == 'SYSTEM_USER_LOGIN_SUCCESS' || $des == 'SYSTEM_USER_LOGIN_FAILURE') {
