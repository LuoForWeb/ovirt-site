 ...
 * @LastEditTime: 2026-01-28 16:11:03
 * @Version: 2.0
...
class Alarm extends Base
{
    ...
    public function getJobAlarmList($params = [], $isExport = false): array
    {
        ...
        foreach ($data as $d) {
            ...
            // 公有云替换描述中的"虚拟机"为"实例"
            if (in_array($d['submodule_type'], $vmGroup['publiccloud'])) {
            ...
            if ($d['module_type'] == $moduleTypeArr['VM']) {
            ...
            } else if ($d['module_type'] == $moduleTypeArr['DB']) {
                $subModuleTypeAli = '';
            }
            $rows[] = [
    ...
    public function getJobAlarmDetail($id)
    {
        ...
        $pushParams['desription'] = $d['description_key'];
    ...
    private function sendSystemNotice($emailFlag, $smsFlag, $systemAlarmID)
    {
        ...
        if ($emailFlag) {
            ...
            $EmailInfo = $this->getSystemEmailParams($systemAlarmID);
            $emailResult = (new Notice())->sendEmail($EmailInfo);
            $this->updateNoticeResult($systemAlarmID, $type, $NOTICE_TYPE['EMAIL'], $emailResult);
            return $emailResult;
        }
        if ($smsFlag) {
            //短信通知
            $SmsInfo = $this->getSystemSmsParams($systemAlarmID);
            $smsResult = (new Notice())->sendSms($SmsInfo);
            $this->updateNoticeResult($systemAlarmID, $type, $NOTICE_TYPE['SMS'], $smsResult);
            return $smsResult;
        }
    }
    /**
    ...
    private function getSystemWechatParams($systemAlarmID)
    {
        ...
        $desription = (new LogInfo())->getLogDescriptionNotice(
            $LOG_TYPE['SYSTEM'],
            $dataInfo['error_code'],
    ...
    private function getSystemEmailParams($systemAlarmID)
    {
        ...
        $desription = (new LogInfo())->getLogDescriptionNotice($LOG_TYPE['SYSTEM'], $data[0]['error_code'], $data[0]['description_key'], $data[0]['description_param'], "");
    ...
    public function getManagerEmailAndTelephone()
    {
        ...
        $data = $this->dbSelect($sql, array(xphp_get_config('usr', 'USERTYPE')['manager'], xphp_get_config('three_powers', 'THREE_POWERS_USER')['sysadmin']));
        $email = [];
        $telephone = [];
        foreach ($data as $d) {
            $email[] = $d['email'];
    ...
    private function getSystemEmailBody($data)
    {
        ...
        if ($data['description_param']) {
            ...
            $desArr = explode('%s', $desStr);
            foreach ($desArr as $k => $v) {
                $title .= $v . (new LogInfo())->getEachParamsDes($param[$k], $param, false);
            }
        }
    ...
    private function getSystemSmsParams($systemAlarmID)
    {
        ...
        $desription = (new LogInfo())->getLogDescription(
            $LOG_TYPE['SYSTEM'],
            $data[0]['error_code'],
    ...
    private function getTaskEmailBodyList($taskType, $sub_module_type, $moduleType, $taskuuid, $historyUuid)
    {
        ...
        $item_name = [
            ...
            "4-0" => xphp_get_lang('UI_DB_DATABASE_NAME') . '/' . xphp_get_lang('UI_OVERVIEW_PUBLIC_CLOUD_INSTANCE_NAME'),
    ...
    private function getTaskSmsParams($taskAlarmID)
    {
        ...
        $desription = (new LogInfo())->getLogDescriptionNotice(xphp_get_config('log', 'LOGTYPE')['TASK'], $data[0]['error_code'], $data[0]['description_key'], $data[0]['description_param'], $data[0]['task_name']);
        $params = [
