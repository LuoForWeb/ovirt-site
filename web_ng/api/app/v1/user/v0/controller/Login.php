...
class Login extends Base
{
    ...
    * 登录入口
    ...
    public function index()
    {
        ...
        $username = v1_decrypt_js_rsa($this->param['username']);
        $password = v1_decrypt_js_rsa($this->param['password']);
        $lockpage = !empty($this->param['page']) ? v1_decrypt_js_rsa($this->param['page']) : '';  //锁定页面调用
        $lockcount = !empty($this->param['count']) ? v1_decrypt_js_rsa($this->param['count']) : 0; //锁定页面输入密码的次数
        ...
        if (getCache('login_faild_num') > 3 && !empty($this->param['very_code'])) {
            ...
            $message = '';
            // 需要验证验证码
            if(empty(xphp_get_cache('code'))){
                $needReturnError = true;
                $message = xphp_get_lang('UI_LOGIN_VERIFY_CODE_EXPIRED');

            }elseif($this->param['very_code'] !== xphp_get_cache('code')) {
                $needReturnError = true;
...
//        $this->checkParams('login');
    ...
    public function auth()
    {
        ...
        if ($result['result'] == 1) {
            //验证成功，error_code返回0
            $tokenid = v1_my_encrype($username . '|' . $password . '|' . time());
            $info = array(
                'error_code' => 0,
    ...
    {
        $tokenid = v1_my_decrypt($this->param['token_id']);
        ...
        if (1 == $loginResult['result']) {
            ...
            $redirectUrl = '/index.php?csrf_token=' . urlencode(getCache('__token__')) .
                ...
                '&username=' . v1_encrypt_js_rsa($username) . '&password=' . v1_encrypt_js_rsa($password);
    ...
    * 退出登录/锁定屏幕
    ...
    {
        $result = $this->logic()->getDomainServerList();
        $this->success('', $result);
    }
}
