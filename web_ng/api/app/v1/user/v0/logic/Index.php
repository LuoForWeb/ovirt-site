...
class Index extends Base
{
    ...
    public function getUserList($params = []): array
    {
        ...
        $sql = "select distinct bu.user_uuid, bu.user_name, bu.user_type,
            ...
            unix_timestamp(bu.last_login_time) last_login_time, bu.lock_flag,bu.manager_uuid, bu.create_user_uuid, bu.user_level,
    ...
    public function editUser(array $params)
    {
        ...
        $sql = "select password, email, telephone, user_uuid, user_level, `position` from bd_user where user_uuid = ?";
        $data = $this->dbSelect($sql, array($userUUID));
...
from bd_user_group bug left join mt_user_group_tenant mugt on bug.user_group_uuid = mugt.user_group_uuid 
    ...
    public function editSelfInfo($params)
    {
        ...
        if(!empty($username)){
            ...
            $params[] = $username;
            $chaneMsg[] = xphp_get_lang('UI_LOGIN_USERNAME');
        }
        if(!empty($email)){
            $setClauses[] = "email = ?";
            $params[] = $email;
        ...
        if(!empty($language)){
            $setClauses[] = "language = ?";
            $params[] = $language;
           // $chaneMsg[] = xphp_get_lang('UI_USER_LANG');
        }
        if(!empty($password)){
            $setClauses[] = "password = ?";
        ...
        if(!empty($customePassword)){
            ...
            $chaneMsg[] = xphp_get_lang('UI_USER_CUSTOM_PASSWORD');
        }
        ...
        if ($result) {
            $msgs = !empty($chaneMsg) ? ' "' . implode('，', $chaneMsg) . '"' : '';
            $this->systemLog('SYSTEM_USER_EDIT_INFO_SUCCESS', [$msgs]);
            if ($oldlang != $language) {
                session_start();
