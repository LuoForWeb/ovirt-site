...
class Login extends Base
{
    ...
    public function loginVerifys(string $username, string $password)
    {
        ...
        $token = v1_encrype(json_encode($token));
...
//        $password = md5($password);
        ...
        if (!empty($tenantuuid)) {
            ...
            //AD域用户
            $username = v1_escape_wildcard($username);
    ...
    public function loginVerify(
        ...
        $users = dbSelect($sql, $sqlParams);
        $token['domain_uuid'] = !empty($users[0]['domain_uuid']) ? $users[0]['domain_uuid'] : '';
        $token = v1_encrype(json_encode($token));
        ...
        if (2 == $lockFlag) {
            ...
            if (!$check) {
                ...
                // 用户被锁定
                $errorCode = v1_get_error_num('PF_USER_LOGIN_LOCK_ERROR');
                $this->loginLog(
                    $userUUID,
                    $username,
        ...
        if (intval($users[0]['user_type']) == 2) {
            ...
            if ($conn) {
                ...
                if ($bd) {
                    ...
                    $errorCode = v1_get_error_num('PF_USER_USER_PASS_ERROR');
                    $this->loginLog(
                        $userUUID,
                        $username,
                        'SYSTEM_USER_LOGIN_FAILURE',
    ...
    private function accessRestrictLogin()
    {
        ...
        foreach ($whiteIPListData as $item) {
            ...
            if (trim($item['start_ip']) == trim($item['end_ip'])) {
                if (v1_parse_flag_to_bool($item['permanent_access_flag']) && trim($ipAddress) == trim($item['start_ip'])) {
                    // 永久时效
                    $listFlag = true;
                    break;
                } else {
                ...
                if ($ipVersionFlag) {
                    if (ip2long($ipAddress) >= ip2long($item['start_ip']) && ip2long($ipAddress) <= ip2long($item['end_ip'])) {
                        if (v1_parse_flag_to_bool($item['permanent_access_flag'])) {
                            // 永久时效
                            $listFlag = true;
                            break;
                        } else {
    ...
    private function sendEmail($useruuid, $userName, $email)
    {
        ...
        $cryptToken = v1_encrype($jsonToken);  //加密
        ...
        $attachment = '';  // 附件
        (new \app\v1\user\v0\logic\Index())->resetPwdSendEmail($title, $email, $message, $attachment, $userName);
    }

    /**
     * 获取域服务器列表
     */
    public function getDomainServerList()
    {
        $sql = "select domain_uuid, domain_name from bd_domain_server";
        ...
        if(!empty($data)){
            ...
            $result['rows'][] = array(
                'uuid' => '',
                'name' => xphp_get_lang('UI_USER_LOCATION')
            );
        }
        return $result;
    }
