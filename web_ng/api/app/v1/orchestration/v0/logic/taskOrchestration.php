 ...
 * @LastEditTime: 2025-12-03 10:30:15
 * @Version: 1.0
...
class taskOrchestration extends Base
{
    ...
    public function addOrchestrationList($params)
    {
        ...
        $time_strategy = $params['task_orchestration_plan']['time_strategy'] != [] ? $this->groupTimeStrategy($backup_mode['FULL'], $params['task_orchestration_plan']['time_strategy']) : [];
    ...
    { // 参数验证和处理
        ...
        if (v1_auth_need_check_look()) {
            $user_uuid_arr = v1_auth_get_users(xphp_get_config('user', 'USER_AUTH')['current_job']);
            return " AND bt.user_uuid IN ({$user_uuid_arr})";
        }
        // 分页
