...
class MachineOsRecover extends Recover
{
   /**
    ...
    public function getSyncTimepoint($params)
    {
        ...
        $fulluuidList = array();
        $unfullList = array();
        $full_backup_mode = xphp_get_config('task', 'BACKUP_MODE')['FULL'];
        foreach ($pointData as $point) {
            if ($point['backup_mode'] == $full_backup_mode) {
...
//         }
        ...
        foreach ($pointData as $op) {
            ...
            foreach ($allStorageData as $item) {
                if($item['timepoint_uuid'] == $op['timepoint_uuid']){
            ...
            if ($op['backup_mode'] == $full_backup_mode) {
