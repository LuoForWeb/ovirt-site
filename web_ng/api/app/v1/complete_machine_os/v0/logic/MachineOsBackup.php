...
class MachineOsBackup extends Backup
{
    ...
    */
        ...
        foreach ($data as $device) {
            ...
            if(empty($mount_point)){
             ...
             if($device['removable_flag'] == 1){
                ...
                // $nocheck = true;//不选中
                $checked = false; //不选中
                $name .= '<i class="viconfont vicon-yidongyingpan" data-toggle="tooltip" data-placement="right" data-html="true" title="'.xphp_get_lang('WEB_OS_REMOVE_DEVICE').'"></i>' ;
            ...
            if($device['is_bitLocker'] == 1){
                ...
                //给name添加一个锁的标签
                $name .= '<i class="viconfont vicon-ge_lock" data-toggle="tooltip" data-placement="right" data-html="true" title="'.xphp_get_lang('WEB_MACHINE_OS_ENCRYPTED').'"></i>' ;
            }
            ...
            if($logic_flag){
                $name .= '<i class="viconfont vicon-luojifenqu" data-toggle="tooltip" data-placement="right" data-html="true" title="'.xphp_get_lang('WEB_MACHINE_LOGIC_VOL').'"></i>' ;
            }
            //判断扩展分区
            ...
            if($extend_flag){
                $name .= '<i class="viconfont vicon-kuozhanfenqu" data-toggle="tooltip" data-placement="right" data-html="true" title="'.xphp_get_lang('WEB_MACHINE_EXTEND_VOL').'"></i>' ;
                //如果是扩展分区则不显示勾选框
                $nocheck = true;
            }
            //如果是磁盘层并且is_boot为1
            if($device['is_boot'] == 1 && isset($device['part_table_type'])){
                //给name添加一个引导磁盘的的标签
                $name .= '<i class="viconfont vicon-boot" data-toggle="tooltip" data-placement="right" data-html="true" title="'.xphp_get_lang('WEB_MACHINE_OS_BOOT').'"></i>' ;
            }
            //获取磁盘阵列
            //如果磁盘阵列不为空
            $disk_array = "";
            ...
            if(!empty($disk_array)){
                $name .= '<i class="viconfont vicon-cipanzhenlie" data-toggle="tooltip" data-placement="right" data-html="true" title="'.$disk_array.'"></i>' ;
            }
    ...
    public function createBackupJob($params){
        ...
        //GFS保留策略
        $pfMsg['gfs_strategy_item_list'] = $params['strategyInfo']['reserve']['reserveInfo']['gfs_strategy_item_list'];
        //发送消息
        //得到备份创建操作码
        ...
        if($data){
            $info = array(
                ...
                'reserve_strategy' => array(
                    ...
                    'gfs_info' => $this->getGfsStrategyInfo($task_uuid),
                ),
                //获取传输策略
    ...
    public function diskTransitionList($data, $pid = 0, $params = array(), $motionFlag = false){
        ...
        foreach ($data as $device) {
            ...
            if(!empty($mount_point)){
                ...
                //如果是可移动设备则增加字样
                if($device['removable_flag'] == 1){
                    $name .= '<i class="viconfont vicon-yidongyingpan" data-toggle="tooltip" data-placement="right" data-html="true" title="'.xphp_get_lang('WEB_MACHINE_OS_REMOVABLE_DEVICE').'"></i>' ;
                    // $name .= "(".xphp_get_lang('WEB_MACHINE_OS_REMOVABLE_DEVICE').")";
                }
                if($first_node ==1 && $is_system_disk_flag ==1){
                    //如果是磁盘层并且是系统磁盘则给出提示文字
                    $name .= '<i class="viconfont vicon-xitong" data-toggle="tooltip" data-placement="right" data-html="true" title="'.xphp_get_lang('WEB_MACHINE_OS_SYSTEM_DISK').'"></i>' ;
