...
class DbcdpJobInfo extends Base
{
    ...
    public function getBasicInfo($params)
    {
        ...
        // 导入/导出线程
        $sqlTaskConfig = "select recovery_timepoint_type, recovery_target_scn, recovery_target_datetime, imp_max_process_num, exp_max_process_num, select_copy_users, sync_direction from cdp_db_dr_task where task_uuid = ?";
        ...
        $return = array(
            ...
            'recovery_timepoint_type' => $dataTaskConfig[0]['recovery_timepoint_type'],
            'recovery_target_scn' => $dataTaskConfig[0]['recovery_target_scn'],
            'recovery_target_datetime' => $dataTaskConfig[0]['recovery_target_datetime']
        );
        return $return;
    }
    ...
    public function historyTaskDetailsHandlerDbCdp($details, $taskType, $info)
    {
        ...
        // 数据表空间目录
        $result[$i]['app_tablespace_dir'] = !empty($details['app_tablespace_dir'])? $details['app_tablespace_dir'] : xphp_get_lang('UI_DB_CDP_BACKUP_SYSTEM_TABLE_SPACE_DIRECTORY');
        // 字典总数量
