...
class Base extends Op
{
    ...
    {
        $OP_CODE = xphp_get_config('point', 'OPERATION_STATUS');
        // 获取操作描述配置
        $OP_DES = xphp_get_desc('Point', 'OPERATE_DES');
        // 0是未操作
        if ($status == 0){
            return $OP_DES[0];
        }
     ...
     * 3 异常
     */
    protected function getTimePointStatus($merge_status, $operation_status, $virus_scan_status, $integrity_check_status){
        // 操作状态码
        $OP_CODE = xphp_get_config('point', 'OPERATION_STATUS');
        // 病毒状态
