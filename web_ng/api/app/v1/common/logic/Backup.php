...
class Backup extends Base
{
            ...
            case 'strategy':
                ...
                foreach ($data as $d) {
                    ...
                    $strategyData[] = array(
                        ...
                        'full_backup_compensation_flag' => v1_parse_bool_to_flag($d['full_backup_compensation_flag']),
                    );
                }
     ...
     * @return {}
     */
    public function getGfsStrategyInfo($taskUUID){
        // 定义 level1_type 映射关系
        $typeMap = [
            1 => 'week',
            2 => 'month',
        ...
        try {
            $dataGFS = $this->dbSelect($sqlGFS, [$taskUUID]);
        } catch (\Exception $e) {
            // 根据实际项目规范决定是否记录日志或抛出异常
            return []; 
        }
        ...
        if (!empty($dataGFS)) {
            foreach ($dataGFS as $d) {
                // 确保必须字段存在
                if (!isset($d['level1_type'], $d['level2_type'], $d['retention_num'])) {
                    continue;
                }
    
                $key = $typeMap[$d['level1_type']] ?? null;
