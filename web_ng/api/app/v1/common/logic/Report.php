...
class Report extends Base
{
    ...
    public function getFileClientInfo()
    {
        ...
        $sql = "select online_flag from bd_agent where agent_type not in (2, 3, 4) and user_uuid = ?";
    ...
    public function getNasClientInfo(): array
    {
        ...
        if (!in_array($_SESSION['userLevel'], [1, 2, 3])) {//userLevel是1 2 3就全部显示
            ...
            $uuid = $resourceHandler->pGetUserAllResource($user['userUuid'], 57);
    ...
    public function pGetTaskNum($module, $subModule = null)
    {
        ...
        $sqlParams = array($module, xphp_get_config('task', 'TASKTYPE')['BACKUP'], $user['userUuid']);
        ...
        switch ($module) {
            case xphp_get_config('module', 'MODULE_TYPE')['KUBERNETES']: // KUBERNETES
                $sqlParams = array($module, xphp_get_config('task', 'TASKTYPE')['KUBE_BACKUP'], $user['userUuid']);
                break;
            case xphp_get_config('module', 'MODULE_TYPE')['FILE_COPY']://文件复制
                $sqlParams = array($module, xphp_get_config('task', 'TASKTYPE')['FILE_COPY'], $user['userUuid']);
                break;
            case xphp_get_config('module', 'MODULE_TYPE')['DB_CDP']://数据库复制    
                $sqlParams = array($module, xphp_get_config('task', 'TASKTYPE')['DB_CDP_SYN'], $user['userUuid']);
                break;
            case xphp_get_config('module', 'MODULE_TYPE')['DB']://数据库保护    
                $sqlParams = array($module, xphp_get_config('task', 'TASKTYPE')['DB_BACKUP'], $user['userUuid']);
                break;
            default:
                break;
        }
    ...
    public function hadoopEmailContent($detailInfo, $message, $alarmInfo, $nodeInfo, $nameInfo)
    {
        ...
        $message = str_replace('hadoop_online', $hadoopAuth, $message);
        $message = str_replace('hadoop_offline', $hadoopOnline, $message);
    ...
    public function k8sEmailContent($detailInfo, $message, $alarmInfo, $nodeInfo, $nameInfo)
    {
        ...
        $backupData = v1_calsize($detailInfo['backupDataSize']['value'], true);
