...
use app\v1\user\v0\logic\Login;
use xphp\db\Op;
use xphp\Request;
use xphp\Response;
...
class Base
{
    ...
    * 根据 apikey 生成对应的用户信息以完成用户信息的获取
    ...
    private function apiRequest()
    {
        ...
        $requestNum = cache($requestNumKey);

        if (empty($requestNum)) {
            // 不存在缓存 写入
            cache($requestNumKey, [1, TIMESTAMP + $expire]);
            return true;
        }
        ...
        cache($requestNumKey, [$requestNum[0] + 1, $requestNum[1]]);
    }
