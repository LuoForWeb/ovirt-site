...
//        $storageVendors = xphp_get_config('resource', 'OBJECT_STORAGE_VENDOR');
        ...
        foreach ($data as $d) {
            switch ($d['vendor']) {
                case 0:
                    $item = array('pId' => 0, 'id' => 1, 'name' => xphp_get_lang('UI_OBS_VENDOR_AWS'), 'title' => xphp_get_lang('UI_OBS_VENDOR_AWS'), 'open' => false, 'nocheck' => $isRecoveryFlag, 'isParent' => true, 'icon' => './img/s3/aws.svg', 'type' => 'obsGroup', 'event_type' => 'obsGroup', 'checkDisabled' => false);
                    if (!$hasPushedFlag[0]) {
                    ...
                    break;
                case 1:
                    $item = array('pId' => 0, 'id' => 2, 'name' => xphp_get_lang('UI_OBS_VENDOR_OSS'), 'title' => xphp_get_lang('UI_OBS_VENDOR_OSS'), 'open' => false, 'nocheck' => $isRecoveryFlag, 'isParent' => true, 'icon' => './img/s3/ali.svg', 'type' => 'obsGroup', 'event_type' => 'obsGroup', 'checkDisabled' => false);
                    if (!$hasPushedFlag[1]) {
                        array_push($firstLevelArr, $item);
                    ...
                    break;
                case 2:
                    $item = array('pId' => 0, 'id' => 3, 'name' => xphp_get_lang('UI_OBS_VENDOR_COS'), 'title' => xphp_get_lang('UI_OBS_VENDOR_COS'), 'open' => false, 'nocheck' => $isRecoveryFlag, 'isParent' => true, 'icon' => './img/s3/tengxun.svg', 'type' => 'obsGroup', 'event_type' => 'obsGroup', 'checkDisabled' => false);
                    if (!$hasPushedFlag[2]) {
                        array_push($firstLevelArr, $item);
                    ...
                    break;
                case 3:
                    $item = array('pId' => 0, 'id' => 4, 'name' => xphp_get_lang('UI_OBS_VENDOR'), 'title' => xphp_get_lang('UI_OBS_VENDOR'), 'open' => false, 'nocheck' => $isRecoveryFlag, 'isParent' => true, 'icon' => './img/s3/huawei.svg', 'type' => 'obsGroup', 'event_type' => 'obsGroup', 'checkDisabled' => false);
                    if (!$hasPushedFlag[3]) {
                        array_push($firstLevelArr, $item);
                    ...
                    break;
                case 4:
                    $item = array('pId' => 0, 'id' => 5, 'name' => 'Ceph S3', 'title' => 'Ceph S3', 'open' => false, 'nocheck' => $isRecoveryFlag, 'isParent' => true, 'icon' => './img/s3/ceph.svg', 'type' => 'obsGroup', 'event_type' => 'obsGroup', 'checkDisabled' => false);
                    if (!$hasPushedFlag[4]) {
                        array_push($firstLevelArr, $item);
                    ...
                    break;
                case 5:
                    $item = array('pId' => 0, 'id' => 6, 'name' => 'Wasabi', 'title' => 'Wasabi', 'open' => false, 'nocheck' => $isRecoveryFlag, 'isParent' => true, 'icon' => './img/s3/wasabi.svg', 'type' => 'obsGroup', 'event_type' => 'obsGroup', 'checkDisabled' => false);
                    if (!$hasPushedFlag[5]) {
                        array_push($firstLevelArr, $item);
                    ...
                    break;
                case 6:
                    $item = array('pId' => 0, 'id' => 7, 'name' => 'MinIO', 'title' => 'MinIO', 'open' => false, 'nocheck' => $isRecoveryFlag, 'isParent' => true, 'icon' => './img/s3/minio.svg', 'type' => 'obsGroup', 'event_type' => 'obsGroup', 'checkDisabled' => false);
                    if (!$hasPushedFlag[6]) {
                        array_push($firstLevelArr, $item);
                    ...
                    break;
                case 7:
                    $item = array('pId' => 0, 'id' => 8, 'name' => xphp_get_lang('UI_OBS_VENDOR_AZURE'), 'title' => xphp_get_lang('UI_OBS_VENDOR_AZURE'), 'open' => false, 'nocheck' => $isRecoveryFlag, 'isParent' => true, 'icon' => './img/s3/azure.svg', 'type' => 'obsGroup', 'event_type' => 'obsGroup', 'checkDisabled' => false);
                    if (!$hasPushedFlag[7]) {
                        array_push($firstLevelArr, $item);
        ...
        if (!empty($editUuid)) {//修改任务
            $sqlObs = "SELECT agent_uuid FROM bd_task_agent_list WHERE task_uuid = ?";
            $dataObs = $this->dbSelect($sqlObs, array($editUuid));
            $editObsUuid = array_column($dataObs, 'agent_uuid');
        }
        if (!empty($data)) {
            foreach ($data as $d) {
                ...
                $nodes[] = array(
                    ...
                    'isParent' => false,
                    'nocheck' => false,
                    'open' => in_array($d['obs_uuid'], $editObsUuid) ?? false,
                    'type' => 'obsItem',
                    'event_type' => 'obsItem',
                    'uuid' => $d['obs_uuid'],
                    ...
                    'checked' => in_array($d['obs_uuid'], $editObsUuid) ?? false,
                );
            }
        }
        ...
        foreach ($result['item_list'] as $d) {
            ...
            if (!empty($params['dir'])) {
                $fileNodes[] = array(
                    ...
                    'file_path' => $filename === '/' ? $d['item_path'] . '/' : $d['item_path'],
                    'more' => false,
    ...
    private function getOnLoadTree($list, $params, $path)
    {
        ...
        foreach ($fileData as $key => $d) {
            ...
            if ($checked && $d['type'] != 1) {
                ...
                $sonList = $this->getOnLoadTree($list, $data, $d['file_path']);
                $fileNodes = array_merge($fileNodes, $sonList);
            }
        }
    ...
    private function groupBackupOBS(array $filelists): array
    {
        ...
        foreach ($filelists as $file) {
            $fileList[] = array(
                ...
                'path_name' => html_entity_decode($file['file_path']),             //文件路径
