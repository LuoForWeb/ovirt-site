...
class TapeController extends Base
{
    ...
    public function delTapeBackupSet(array $params)
    {
        ...
        $taskSql = "SELECT count(btt.id) num 
                    FROM bd_backup_timepoint btt
                        ...
                        inner join ({$subSql}) AS valid_timepoints ON
                            btt.timepoint_uuid = valid_timepoints.timepoint_uuid
                    WHERE
    ...
    public function retrievalTape($param)
    {
        ...
        if ($tapeSerialNumber == '') {
            ...
            $tapeTimepoint = v1_tape_timepoint_sql(" and bd_tape_carriage.lib_name = '{$tapeLibName}'");
            $timepointSql = "SELECT DISTINCT btt.timepoint_uuid, btt.task_uuid 
                                    FROM bd_backup_timepoint btt 
                                        inner join ({$tapeTimepoint}) AS valid_timepoints ON
                                            btt.timepoint_uuid = valid_timepoints.timepoint_uuid";
            $timePoints = $this->dbSelect($timepointSql);
