...
class TapeInfo extends Base
{
    ...
    public function getTapeGroupInfo($params)
    {
        ...
        $sql = "select btg.id, btg.group_uuid, btg.name, btg.reserve_strategy_type, btg.reserve_days,
                ...
                bsr.warning_flag, bsr.use_mode, bsr.warning_type, bsr.warning_value, bsr.node_uuid 
    ...
    public function getBackupTimepoint(array $params)
    {
        ...
        if ($params['offset'] == 0) {
            // 构建时间点表和磁带设备对应临时表
            $delTable = "DROP TABLE IF EXISTS temp_bd_tape_backup_file_web";
            $this->dbExec($delTable);
        }

        $tempTable2 = "CREATE TABLE IF NOT EXISTS temp_bd_tape_backup_file_web AS
                            SELECT SUBSTRING_INDEX(SUBSTRING_INDEX(file_path, '/', 2), '/', -1) 
                                AS timepoint_uuid2, GROUP_CONCAT(DISTINCT tape_serial_number) tape_serial_number
                        FROM bd_tape_backup_file GROUP BY timepoint_uuid2";
        $result = $this->dbQuery($tempTable2);
        if ($result) {
            $buildSql = "ALTER TABLE temp_bd_tape_backup_file_web ADD INDEX idx_timepoint_uuid (timepoint_uuid2)";
        ...
        $fields = "bbt.timepoint,bbt.task_type,bbt.task_name,bbt.task_uuid, bbt.backup_mode,
                    ...
                    tape_agg.tape_serial_number as lib_name,
                CASE
                        bbt.module_type 
                        WHEN 2 THEN
        ...
        $table = " INNER JOIN bd_storage_resource bsr ON bbt.storage_uuid = bsr.storage_uuid
                    ...
                    LEFT JOIN temp_bd_tape_backup_file_web tape_agg ON tape_agg.timepoint_uuid2 = bbt.timepoint_uuid
                    ";
