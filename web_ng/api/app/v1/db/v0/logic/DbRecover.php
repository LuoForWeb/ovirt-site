...
class DbRecover extends Recover
{
    ...
    public function getDbRecoveryJob(string $jobsUuid): array
    {
        ...
        return $this->sendResult('', true, 200, [
            ...
            'node_info' => $this->getNodeNameAndIp($jobInfo['node_uuid']),
    ...
    private function getDbRecoveryInfo(string $jobsUuid, array $jobRecoveryList): array
    {
        ...
        // 查询备份时间信息
        $timepointUuids = "'" . implode("', '", array_column($jobRecoveryList, 'timepoint_uuid')) . "'";
        $sql = "SELECT bbt.timepoint, bbt.timepoint_uuid, bbt.backup_mode, bbt.storage_uuid,
                    ...
                    dbt.cluster_name, dbt.db_name, dbt.instance_name, dbt.db_config,
                    bsr.storage_type
        ...
        foreach ($timepointList as $timepoint) {
            $timepointMap[$timepoint['timepoint_uuid']] = $timepoint;
        }
        ...
        foreach ($jobRecoveryList as $jobRecoveryInfo) {
            ...
            if (isset($timepointMap[$jobRecoveryInfo['timepoint_uuid']])) {
                $timepointInfo = [
                    'timepoint_uuid' => $timepointMap[$jobRecoveryInfo['timepoint_uuid']]['timepoint_uuid'],
                    'timepoint' => $timepointMap[$jobRecoveryInfo['timepoint_uuid']]['timepoint'],
                    'backup_mode' => $timepointMap[$jobRecoveryInfo['timepoint_uuid']]['backup_mode'],
                    'storage_type' => $timepointMap[$jobRecoveryInfo['timepoint_uuid']]['storage_type'],
                    'db_config' => $timepointMap[$jobRecoveryInfo['timepoint_uuid']]['db_config'],
                ];
            }
