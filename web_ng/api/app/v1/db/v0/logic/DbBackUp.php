...
class DbBackUp extends Backup
{
    ...
    private function buildDbBackupJobMsg(array $params): array
    {
        ...
        } elseif ($dbType == $allDbType['TIDB']) {  // TiDB不支持完全备份补偿
            if ($params['time_strategy']['type'] === 'strategy') {
                if ($params['time_strategy']['full_info']) {
                    $params['time_strategy']['full_info']['full_backup_compensation_flag'] = false;
                }
            }
        }
    ...
    public function getDbBackupJob(string $jobsUuid, bool $withEncryptPasswordFlag = false): array
    {
        ...
        $reservedStrategy = $this->getDbBackupTaskReservedStrategy((int) $jobInfo['strategy_id']);
    ...
    private function getDbBackupTaskStorageStrategy(int $strategyId, bool $withEncryptPasswordFlag): array
    {
        $sql = "SELECT deduplication_flag, compressed_flag, encrypted_flag,
                    password_auto_flag, password, compress_method, encrypt_method
     ...
     * @return array
     */
    private function getDbBackupTaskReservedStrategy(int $strategyId): array
    {
