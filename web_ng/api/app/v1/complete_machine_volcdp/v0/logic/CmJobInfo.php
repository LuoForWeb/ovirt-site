...
class CmJobInfo extends JobInfos
{
    ...
    public function getCmCdpTaskSpeed($params)
    {
        ...
        $sql = "SELECT bt.module_type, bri.speed, bri.speed_time, bt.task_status
    ...
    public function getTaskMonitorDeviceInfo($params)
    {
        ...
        if ($taskType == $taskTypeArr['VOL_CDP_BACKUP'] || $taskType == $taskTypeArr['VOL_CDP_REPLICATION']) {
            //备份
            // if (
            //$taskCurrentStage== xphp_get_desc('Volcdp','TASK_RUNNING_STAGE')['IN_TAKEOVER'] ||
            // $taskCurrentStage== xphp_get_desc('Volcdp','TASK_RUNNING_STAGE')['IN_TAKEOVER_STARTING']
            //){
            // //任务阶段处于接管中
            ...
            // }
        } elseif ($taskType == $taskTypeArr['VOL_CDP_RECOVERY'] || $taskType == $taskTypeArr['PLATFORM_RECOVERY']) {
            //恢复
            $taskDevInfo = $this->getRecoverTaskVolInfo($params);

        } elseif ($taskType == $taskTypeArr['VOL_CDP_TAKEOVER']) {
            //接管
            $taskDevInfo = $this->getTakeoverTaskVolInfo($params);
    ...
    private function getTakeoverTaskVolInfo($params)
    {
        ...
        $sql = "SELECT cvt.master_agent_uuid,bt.task_type, bt.task_status, cvt.current_task_running_stage,  
                        cvtti.takeover_standby_agent_uuid,cvtti.takeover_vm_hypervisor,
                        cvtti.takeover_vm_config,cvtti.takeover_timestamp, 
        ...
        foreach ($data as $d) {
            ...
            //接管目标设备
            $takeoverTargetInfo = $this->getBdAgentInfo($d['takeover_standby_agent_uuid'], $takeoverVmHypervisor);
    ...
    public function getBasicInfo($params)
    {
        ...
        $sql = "SELECT 
                    ...
                    vol_task.current_task_running_stage,
                    vol_task.master_agent_uuid,
                    vol_task.standby_agent_uuid,
                    vol_task.recovery_target_agent_uuid,
                    vol_task.auto_takeover_flag, 
                    vol_task.rebuild_partition_flag,
                    vol_task.recovery_data_source,
                    vol_task.monitor_data_io_replication_mode,
                    vol_task.takeover_data_source, 
                    vol_task.auto_fault_resume_flag, 
                    vol_task.mirror_backup_flag,
                    vol_task.backup_mode,
                    vol_task.auto_takeover_enable_flag, 
                    vol_task.full_backup_flag,
                    vol_task.skip_bad_block_flag,
                    bu.user_name, 
                    UNIX_TIMESTAMP(bs.next_start_time) next_start_time,
                    bts.start_time as strategy_start_time
                    FROM 
                    bd_task bt
                    ...
                    INNER JOIN cdp_vol_task vol_task ON bt.task_uuid = vol_task.task_uuid
        ...
        foreach ($data as $d){
            ...
            if($taskType==xphp_get_config( 'task', 'TASKTYPE')['VOL_CDP_RECOVERY']){  //恢复
                ...
                $strategy_start_time = $d['strategy_start_time'];
            }else{
                $standbyAgentUuid = $d['standby_agent_uuid'];
            ...
            if($currentTaskRunningStage == xphp_get_desc('Volcdp','TASK_RUNNING_STAGE')['SERVER_CONS_CHECK'] 
				|| $currentTaskRunningStage == xphp_get_desc('Volcdp','TASK_RUNNING_STAGE')['SERVERDATA_REALTIME_CONS_CHECK']){  //服务端数据一致性校验和实时数
        ...
        $data = $this->dbSelect($sql,array($taskUUID));
        $all_disk_list = json_decode($data[0]['master_agent_detail'],true)['all_disk_list'];

        $dev_uuid_list = [];
        if (is_array($all_disk_list)) {
            foreach ($all_disk_list as $disk) {
                if (isset($disk['dev_uuid'], $disk['dev_name'])
                    && !empty(trim($disk['dev_uuid']))
                    && !empty(trim($disk['dev_name']))
                    && empty($disk['sub_devices'])) {
                    $dev_uuid_list[] = [
                        'dev_uuid' => $disk['dev_uuid'],
                        'dev_name' => $disk['dev_name']
                    ];
                }
                if (isset($disk['sub_devices']) && is_array($disk['sub_devices'])) {
                    foreach ($disk['sub_devices'] as $sub) {
                        if (is_array($sub)) {
                            if(empty($sub['sub_devices'])){
                                $dev_uuid_list[] = [
                                    'dev_uuid' => $sub['dev_uuid'],
                                    'dev_name' => $sub['dev_name']
                                ];
                            }else{
                                foreach ($sub['sub_devices'] as $a) {
                                    $dev_uuid_list[] = [
                                        'dev_uuid' => $a['dev_uuid'],
                                        'dev_name' => $a['dev_name']
                                    ];
        ...
        foreach ($dev_uuid_list as &$list) {
            $sql = "select takeover_target_mount_point,mount_point from cdp_vol_task_vol where vol_uuid = ? and task_uuid = ?";
            $data = $this->dbSelect($sql, array($list['dev_uuid'], $taskUUID));
            $list['mount_point'] = !empty($data[0]['takeover_target_mount_point']) ?$data[0]['takeover_target_mount_point'] : xphp_get_lang('WEB_MACHINE_OS_AUTO_NETWORK');
        }
        unset($list);

        //整机复制备机映射
        $sql = "select dev_name,standby_target_dev_name from cdp_vol_task_disk where task_uuid = ?";
        $data = $this->dbSelect($sql,array($taskUUID));
        $standby_target_dev = [];
        foreach ($data as $d) {
            $standby_target_dev[] = [
                'dev_name' => $d['dev_name'],
                'standby_target_dev_name' => $d['standby_target_dev_name']
            ];
        }
        ...
        $basicInfo = array(
            ...
            'dev_uuid_list' => $dev_uuid_list,
            'standby_target_dev' => $standby_target_dev,
            'flag' => true,
        );
        return $basicInfo;
    }
        ...
        if(xphp_get_config('task','TASKTYPE')['VOL_CDP_BACKUP'] == $task_type || xphp_get_config('task','TASKTYPE')['VOL_CDP_REPLICATION'] == $task_type){
            try {
                $sql = "select takeover_standby_agent_uuid,agent_failback_standby_ip,app_takeover_flag,app_consecutive_failure_num,
                        app_fault_detection_interval,agent_heartbeat_failure_time,takeover_vm_hypervisor,takeover_vm_config,detail 
                ...
                if(!empty($data)){
                    ...
                    $consoleUrl = "";
                    $tempStatus = 0;
                    $standby_agent_uuid =  $data[0]['takeover_standby_agent_uuid'];
                    $prefixStatus = 0;
                    ...
                    if($takeoverAgentType == xphp_get_config('vm','VMHYPERVISORTYPE')['VM_HYPERVISOR_TYPE_UNKNOWN']){
                        ...
                        $sql = "select name,console_url,status,prefix_status from vm_emd where uuid = ? ";
                    ...
                    // }
                    $take_info['takeover_business_ip_map'] = json_decode($data[0]['detail'])->takeover_business_ip_map;
                }
            } catch (Exception $e) {
                $take_info = array();
            }
