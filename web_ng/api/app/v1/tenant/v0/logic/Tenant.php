...
use app\v1\tenant\v0\controller\Index;
use app\v1\user\v0\logic\Index as userIndex;
...
class Tenant extends Base
{
        ...
        if($result){
            ...
            foreach ($roleList as $role){
                ...
                if (!empty($tenantAdmin) && $role['role_name'] == $tenantAdmin[0]['role_name']) {
                    \app\v1\user\v0\logic\Index::instance()->pAddUserRole($adminUuid, array($roleUUID));
                }
            }
            //添加系统操作日志
    ...
    private function addTenantAdminUser($params, $tenantUuid)
    {
        ...
        if(\app\v1\user\v0\logic\Index::instance()->checkCodeExist($code)){
            //验证码重复，需要重新生成
            $code = xphp_random_code(4);
        }

        $sqlExtentionParams = array($userUUID, $quota);
        $sqlParam = array($userUUID, $adminUsername, $params['admin_password'], $params['admin_email'], $userType, $permission,
            $createTime, $createUserName, $createUserUUID, $language, 0, $code);
        ...
        if ($result) {
            //添加用户和租户关系
            (new \app\v1\user\v0\logic\Index())->pAddUserTenant(array($userUUID), $tenantUuid);
        ...
        foreach ($result as $each) {
            ...
            if (!array_key_exists('machinecopy_num', $config)) {
                ...
                $info['rows'][] = array(
                    ...
                    //已备份数据
                    'backup_data' => v1_calSize($this->getTenantBackupData($each['tenant_uuid']), true) . '/' . '--',
                    //配额
                    'quota' => '--',
                    //状态 启用|禁用
            ...
            $info['rows'][] = array(
                ...
                'user_uuid' => $each['user_uuid'],
                //已备份数据
                'backup_data' => v1_calSize($this->getTenantBackupData($each['tenant_uuid']), true) . '/' . $quota,
                //配额
                'quota' => $quota,
                //状态 启用|禁用
                'lock_flag' => intval($each['lock_flag']),
    ...
    public function deleteTenantLogAndAlarm($userList)
    {
        ...
        if (!empty($dataTaskLog)) {
            ...
            // \app\v1\log\v0\logic\Index::instance()->deleteTaskLog($params);
            $t2 = time();
            //间隔1秒检查
            sleep(1 + ($t2 - $t1));
        }
...
//            Alarm::instance()->deleteTaskAlarm($params);
 ...
 /**
                ...
                switch ($allAuthInfo['second_type']) {
					case 1: //全按数量
                        ...
                        if(intval($params['m365_num']) + intval($tenantAuthInfo['m365_num']) > intval($allAuthInfo['module_info']['exchange']['avail_des'])) {
                            exit($this->muOpResult(false, xphp_get_lang('UI_PLATFORM_AUTHOR_BY_NUM'), xphp_get_lang('WEB_TENANT_M365_AUTH_NOT_ENOUGH'). '(Exchange Server)', "warning"));
                        }
                        if(intval($params['m365_num_online']) + intval($tenantAuthInfo['m365_num_online']) > intval($allAuthInfo['module_info']['exchange_online']['avail_des'])) {
                            exit($this->muOpResult(false, xphp_get_lang('UI_PLATFORM_AUTHOR_BY_NUM'), xphp_get_lang('WEB_TENANT_M365_AUTH_NOT_ENOUGH'). '(Exchange Online)', "warning"));
                        }
    ...
    public function getRecoverHost($params){
        ...
        $vmList = \app\v1\resources\v0\logic\Index::instance()->pGetUserResourceVM(xphp_get_user_info()['userUuid'],"tree_id", "desc", $limit = 0, "all", [], $subModule);
...
//            }
            ...
            if (!empty($_SESSION['tenantuuid'])) {
                if (!in_array($d['vcenter_uuid'], $tenantVcenter)) continue;
            }
            //针对全局用户，虚拟化中心不属于该用户,也不是分配的资源直接排除
            if(empty($_SESSION['tenantuuid']) && xphp_get_user_info()['userUuid'] != $d['user_uuid'] && !in_array($d['vcenter_uuid'], $vcenterList)) continue;
    ...
    public function getSyncRecoveryVcenter($params){
        ...
        foreach ($data as $d){
            ...
            if(!empty($_SESSION['tenantuuid']) && !empty($tenantHost)){
                if(!in_array($d['host_uuid'], $tenantHost)) continue;
            }
            if (!$d['host_uuid']) {
                $d['host_uuid'] = $d['uuid'];
            }
        ...
        if (!empty($data)) {
            ...
            foreach ($data as $d) {
                ...
                $info['rows'][] = array(
                    ...
                    'backup_data' => v1_calSize($this->getEachTenantBackupData($d['user_uuid']), true) . '/' . $quota,
                    'lock_flag' => $d['lock_flag'],
                    'create_time' => $d['create_time'],
                    'used_backup_data' => v1_calSize($this->getEachTenantBackupData($d['user_uuid']), true),
    ...
    public function checkTenantAuth($type, $info, $taskuuid = "", $numFlag = false, $tenantUuid = '',$subType = '',$userUuids = []){
        ...
        switch (intval($type)){
            case xphp_get_config('module')['MODULE_TYPE']['VM']:
                switch (intval($subType)){
                    case xphp_get_config('module', 'VM_SUB_MODULE')['VM']:
                        $dataInfo = v1_license_get_auth('vm', $info, $taskuuid, $tenantUuid);
                        //total为-1代表按数量授权无限制
                    ...
                    case xphp_get_config('module', 'VM_SUB_MODULE')['PRIVATE_CLOUD']:
                        $dataInfo = v1_license_get_auth('private_cloud', $info, $taskuuid, $tenantUuid);
                        //total为-1代表按数量授权无限制
                        if ($dataInfo['auth_type'] == 1 && $dataInfo['total'] != -1) {//按数量授权
                    ...
                    case xphp_get_config('module', 'VM_SUB_MODULE')['PUBLIC_CLOUD']:
                        $dataInfo = v1_license_get_auth('cloud', $info, $taskuuid, $tenantUuid);
                        //total为-1代表按数量授权无限制
                        if ($dataInfo['auth_type'] == 1 && $dataInfo['total'] != -1) {//按数量授权
            ...
            case xphp_get_config('module')['MODULE_TYPE']['FS']:
                $dataInfo = v1_license_get_auth('file', $info, $taskuuid, $tenantUuid);
                //total为-1代表按数量授权无限制
                if ($dataInfo['auth_type'] == 1 && $dataInfo['total'] != -1) {//按数量授权
            ...
            case xphp_get_config('module')['MODULE_TYPE']['DB']:
                $dataInfo = v1_license_get_auth('oracle', $info, $taskuuid, $tenantUuid);
                //total为-1代表按数量授权无限制
                if ($dataInfo['auth_type'] == 1 && $dataInfo['total'] != -1) {//按数量授权
            ...
            case xphp_get_config('module')['MODULE_TYPE']['OS']:
                $dataInfo = v1_license_get_auth('os', $info, $taskuuid, $tenantUuid);
                //total为-1代表按数量授权无限制
                if ($dataInfo['auth_type'] == 1 && $dataInfo['total'] != -1) {//按数量授权
            ...
            case xphp_get_config('module')['MODULE_TYPE']['NAS']:
                $dataInfo = v1_license_get_auth('nas', $info, $taskuuid, $tenantUuid);
                //total为-1代表按数量授权无限制
                if ($dataInfo['auth_type'] == 1 && $dataInfo['total'] != -1) {//按数量授权
            ...
            case xphp_get_config('module')['MODULE_TYPE']['M365']:
                switch ($subType){
                    case 1://server
                        $dataInfo = v1_license_get_auth('exchange', $info, $taskuuid, $tenantUuid);
                        //total为-1代表按数量授权无限制
                        if ($dataInfo['auth_type'] == 1 && $dataInfo['total'] != -1) {//按数量授权
                        ...
                        break;
                    case 2://online
                        $dataInfo = v1_license_get_auth('exchange_online', $info, $taskuuid, $tenantUuid);
                        //total为-1代表按数量授权无限制
                        if ($dataInfo['auth_type'] == 1 && $dataInfo['total'] != -1) {//按数量授权
            ...
            case xphp_get_config('module')['MODULE_TYPE']['OBS']:
                $dataInfo = v1_license_get_auth('obs', $info, $taskuuid, $tenantUuid);
                //total为-1代表按数量授权无限制
                if ($dataInfo['auth_type'] == 1 && $dataInfo['total'] != -1) {//按数量授权
            ...
            case xphp_get_config('module')['MODULE_TYPE']['HADOOP']:
                $dataInfo = v1_license_get_auth('hadoop', $info, $taskuuid, $tenantUuid);
                //total为-1代表按数量授权无限制
                if ($dataInfo['auth_type'] == 1 && $dataInfo['total'] != -1) {//按数量授权
            ...
            case xphp_get_config('module')['MODULE_TYPE']['KUBERNETES']:
                $dataInfo = v1_license_get_auth('k8s', $info, $taskuuid, $tenantUuid);
                //total为-1代表按数量授权无限制
                if ($dataInfo['auth_type'] == 1 && $dataInfo['total'] != -1) {//按数量授权
            ...
            case xphp_get_config('module')['MODULE_TYPE']['VOL_CDP']://整机复制
                $dataInfo = v1_license_get_auth('copy_machine', $info, $taskuuid, $tenantUuid);
                //total为-1代表按数量授权无限制
                if ($dataInfo['auth_type'] == 1 && $dataInfo['total'] != -1) {//按数量授权
            ...
            case xphp_get_config('module')['MODULE_TYPE']['FILE_COPY']:
                switch (intval($subType)){//文件复制暂时只支持文件复制和nas复制
                    case xphp_get_config('module', 'SUBMODULE_TYPE')['FS']:
                        $dataInfo = v1_license_get_auth('copy_fs', $info, $taskuuid, $tenantUuid);
                        //total为-1代表按数量授权无限制
                        if ($dataInfo['auth_type'] == 1 && $dataInfo['total'] != -1) {//按数量授权
                    ...
                    case xphp_get_config('module', 'SUBMODULE_TYPE')['NAS']:
                        $dataInfo = v1_license_get_auth('copy_nas', $info, $taskuuid, $tenantUuid);
                        //total为-1代表按数量授权无限制
                        if ($dataInfo['auth_type'] == 1 && $dataInfo['total'] != -1) {//按数量授权
            ...
            case xphp_get_config('module')['MODULE_TYPE']['DB_CDP']://数据库复制
                $dataInfo = v1_license_get_auth('copy_db', $info, $taskuuid, $tenantUuid);
                //total为-1代表按数量授权无限制
                if ($dataInfo['auth_type'] == 1 && $dataInfo['total'] != -1) {//按数量授权
