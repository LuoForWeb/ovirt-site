...
class JobInfo extends JobInfos
{
    ...
    public function getJobInfo(string $jobUuid)
    {
        ...
        return [
            ...
            'total_object_size' => $return['total_object_size'],
            'complate_size' => v1_calsize($return['total_object_completed_size'], true),
            'total_object_completed_size' => $return['total_object_completed_size'],
     ...
     * @return array|bool
     */
    public function getJobFlow(array $params)
    {
        ...
        if (!empty($params['type'])) {
            ...
            // 那么直接查询运行表的最后几分钟数据
            $data = $this->dbSelect(
                "select io_summary_info from bd_history_task_io_summary where history_uuid = ?",
                [$jobUuid]
            );
            if (!empty($data[0]['io_summary_info'])) {
                $speedInfo = json_decode($data[0]['io_summary_info'], true);
                foreach ($speedInfo['data_points'] as $item) {
                    $return[] = [
                        'speed' => $item['speed'],
                        'speed_time' => $item['time'],
                    ];
                }
            }
            return $return;
        ...
        if (!empty($params['all'])) {
            $return = [];
            // 获取之前产生的
            // 这是进入当前任务详情第一次获取所有的历史产生的点
            $task = $this->dbSelect("select node_uuid,task_status from bd_task where task_uuid = ?", [$jobUuid]);
            if ($task[0]['task_status'] != xphp_get_config('task', 'TASKSTATUS')['RUNNING']) {
                // 任务不在运行中
                // 那么直接查询运行表的最后几分钟数据
                $data = $this->dbSelect(
                    "select last_period_io_summary from bd_running_info where task_uuid = ?",
                    [$jobUuid]
                );
                if (!empty($data[0]['last_period_io_summary'])) {
                    $speedInfo = json_decode($data[0]['last_period_io_summary'], true);
                    foreach ($speedInfo['data_points'] as $item) {
                        $return[] = [
                            'speed' => $item['speed'],
                            'speed_time' => $item['time'],
                ...
                // 判断是否是实时模块
                $module = $this->dbSelect('select module_type from bd_task where task_uuid = ?', [$jobUuid]);
                $moduleType = xphp_get_config('module', 'MODULE_TYPE');
                if (!empty($module) && $module[0]['module_type'] == $moduleType['VOL_CDP']) {
                    return $this->getVolFlow($jobUuid);
                }
                $opName = 'NODE_TASK_OP_GET_TASK_HISTORY_IO_SUMMARY';
                $msg = [
                    'task_uuid' => $jobUuid
                ];
                $mbResult = $this->service()->mbNodeMsgs($task[0]['node_uuid'], $opName, $msg, true);
                ...
                foreach ($mbResult['msg']['data_points'] as $item) {
                    $return[] = [
                        'speed' => $item['speed'],
                        'speed_time' => $item['time'],
                    ];
            ...
            if (empty($data[0]['speed_time'])) {
                return [
                    [
                        'speed' => 0,
                        'speed_time' => date('Y-m-d H:i:s')
                    ]
                ];
            }
            return [
                [
                    'speed' => $data[0]['speed'],
                    'speed_time' => date('Y-m-d H:i:s', $data[0]['speed_time']),
                ]
            ];
        }
    }
     ...
     * @return array
     */
    private function getVolFlow(string $jobUuid)
    {

        $task = $this->dbSelect("select node_uuid,task_status from bd_task where task_uuid = ?", [$jobUuid]);
        $taskStatus = xphp_get_config('task', 'TASKSTATUS');
        if ($task[0]['task_status'] == $taskStatus['RUNNING']) {
            // 1、如果任务还在运行中，那么就查时间点数据
            // 获取备份代理ID
            $backupAgentId = $this->getBackupAgentId($jobUuid);
            if (!$backupAgentId) {
                return [];
            }
        } else {
            // 2、如果任务是未运行，那么需要先判断上一次的运行结果
            $sql = "select details from bd_history_task where task_uuid = ? order by id desc limit 1";
            $history = $this->dbSelect($sql, [$jobUuid]);
            if (empty($history)) {
                return [];
            }
            $detail = json_decode($history[0]['details'], true);
            if (in_array($detail[0]['task_status'], [$taskStatus['NETWORK_FAULT'], $taskStatus['ERROR']])) {
                // 失败或网络错误，都不会有流量
                return [];
            }
            // 2.1、如果不是失败（中止或成功都会有时间点），那么order by 时间点的时间，取最近的一条时间点uuid
            // 获取备份代理ID
            $backupAgentId = $this->getBackupAgentId($jobUuid);
            if (!$backupAgentId) {
                return [];
        ...
        return $this->getBackupFlowData($backupAgentId);
    }

    /**
     * 获取备份流量数据（MariaDB优化版）
     * @param int $backupAgentId 备份集id
     * @param int $maxPoints     最大数据点数，默认300
     * @return array 处理后的流量数据
     */
    public function getBackupFlowData($backupAgentId, $maxPoints = 300)
    {

        // 2. 判断使用哪个表
        $hourCount = $this->getTableCount('cdp_vol_backup_agent_hour_level_data_flow', $backupAgentId);
        $useHour = $hourCount >= 180;

        // 3. 获取采样数据
        if ($useHour) {
            $tableName = 'cdp_vol_backup_agent_hour_level_data_flow';
            $isHourTable = true;
        } else {
            $tableName = 'cdp_vol_backup_agent_minute_level_data_flow';
            $isHourTable = false;
        }

        // 4. 获取采样数据
        $rawData = $this->getSampledData($tableName, $backupAgentId, $maxPoints);

        // 5. 处理数据
        return $this->processFlowData($rawData, $isHourTable);
    }

    /**
     * 获取备份代理ID
     * @param string $jobUuid 任务uuid
     * @param string $where   额外的条件
     * @return int
     */
    private function getBackupAgentId(string $jobUuid, string $where = '')
    {
        $sql = "SELECT cvba.id 
            FROM bd_backup_timepoint bbt
            INNER JOIN cdp_vol_backup_agent cvba ON bbt.timepoint_uuid = cvba.timepoint_uuid
            WHERE bbt.task_uuid = ? {$where}
     ...
     * @return int
     */
    private function getTableCount($tableName, $backupAgentId)
    {
        $sql = "SELECT COUNT(*) as cnt FROM {$tableName} WHERE backup_agent_id = ?";
        $result = $this->dbSelect($sql, [$backupAgentId]);
     ...
     * @param string $tableName     表名
     * @param int    $backupAgentId 备份集id
     * @param int    $maxPoints     返回的最大数量
     * @return array
     */
    private function getSampledData(string $tableName, $backupAgentId, $maxPoints)
    {
        // 1. 先获取总记录数
        $totalCount = $this->getTableCount($tableName, $backupAgentId);

        // 如果数据量小于等于最大点数，直接返回全部
        if ($totalCount <= $maxPoints) {
            $sql = "SELECT backup_start_timestamp, data_flow 
                FROM {$tableName}
                WHERE backup_agent_id = ?
                ORDER BY backup_start_timestamp ASC";
            return $this->dbSelect($sql, [$backupAgentId]);
        }
        ...
        $sql = "SELECT 
            ...
            WHERE MOD(rn, ?) = 1  -- 从第一条开始，每隔step条取一条
            ORDER BY backup_start_timestamp ASC
            LIMIT ?";

        return $this->dbSelect($sql, [$backupAgentId, $step, $maxPoints]);
    }

    /**
     * 处理流量数据
     * @param array $rawData     原始数据
     * @param bool  $isHourTable 否是小时表
     * @return array
     */
    private function processFlowData(array $rawData, bool $isHourTable)
    {
        $result = [];

        foreach ($rawData as $row) {
            $speed = (int) $row['data_flow'];

            // 根据表类型转换速度
            if ($isHourTable) {
                $speed = ceil($speed / 3600);  // 小时转秒
            } else {
                $speed = ceil($speed / 60);    // 分钟转秒
            }

            $result[] = [
                'speed_time' => $row['backup_start_timestamp'],
                'speed' => $speed
            ];
        }
    ...
    public function getJobList($params = [], $isExportAll = false)
    {
        ...
        if (!empty($moduleType)) {
            ...
            $whereOrParts = ["bt.task_type = {$tasktypeCfg['PLATFORM_RECOVERY']}"];
        ...
        if (!empty($params['dev_type'])) {
            ...
            $backup_dev_types = []; // 实时备份设备类型
            $replication_dev_types = []; // 实时复制设备类型
            ...
            $devTypeWheres = [];
            $taskTypeFilter = !empty($params['task_type']) ? 'bt.task_type IN (' . implode(',', array_map('intval', explode(',', trim($params['task_type'])))) . ')' : '';

            $backup_conditions = [];
            if (!empty($backup_dev_types)) {
                $dev_in_clause = implode(',', array_unique($backup_dev_types));
                $backup_conditions[] = "cvt.dev_type IN ($dev_in_clause)";
                if ($taskTypeFilter) {
                    $backup_conditions[] = $taskTypeFilter;
                }
                $devTypeWheres[] = '(' . implode(' AND ', $backup_conditions) . ')';
            }

            $replication_conditions = [];
            if (!empty($replication_dev_types)) {
                $dev_in_clause = implode(',', array_unique($replication_dev_types));
                $replication_conditions[] = "cvt.dev_type IN ($dev_in_clause)";
                if ($taskTypeFilter) {
                    $replication_conditions[] = $taskTypeFilter;
                }
                $devTypeWheres[] = '(' . implode(' AND ', $replication_conditions) . ')';
            }
    ...
    public function exportAllHistoryJobs($params)
    {
        ...
        $header = [
            ...
            'finish_time' => xphp_get_lang('UI_STRATEGY_OVER_TIME'),
    ...
    public function getHistoryList($params = [], $isExportAll = false): array
    {
         ...
         */
        if (isset($params['job_uuid']) && $params['job_uuid']) {
            array_push($sqls, "bht.task_uuid = ? ");
            array_push($sqlParams, $params['job_uuid']);
        }
        ...
        foreach ($data as $d) {
            ...
            $records[] = [
                ...
                'job_status_popover' => $this->getJobStatusShowPopover($d['error_code']),
...
ON timetable.minute = sumtable.minute  ORDER BY count asc, minute asc";
     ...
     * @return array
     */
    public function getHistoryDetailHeader(array $params): array
    {

        $sql = "select 
                   bht.task_name,bht.module_type,bht.task_type,bht.start_time,bht.finish_time,bht.error_code,
                   bht.total_object_size,bht.details,bhtis.io_summary_info
                from bd_history_task bht
                left join bd_history_task_io_summary bhtis on bht.history_uuid = bhtis.history_uuid
                where bht.history_uuid = ? limit 1";
        $data = $this->dbSelect($sql, [$params['history_uuid']]);
        if (empty($data)) {
            return [];
        }
        $d = $data[0];
        ...
        $moduleType = xphp_get_config('module', 'MODULE_TYPE');
        $details = json_decode($d['details'], true);
        $speedArr = json_decode($d['io_summary_info'], true);
        $allSize = v1_calsize($d['total_object_size'], true);
        $nullSpace = xphp_get_config('app', 'NULLSPACE');
        //验证任务没有大小
        if (intval($d['task_type']) == $taskTypeArr['SURE_BACKUP']) {
            $allSize = $validSize = $nullSpace;
        } else {
            $validSize = $this->getTotalValidSize(
        ...
        return [
            ...
            'average_speed' => v1_calspeed($speedArr['average_speed'] ?? 0),
            'start_time' => $this->parseDate(strtotime($d['start_time'])),
            'interval_time' => $this->getExecutionDuration(strtotime($d['start_time']), strtotime($d['finish_time']))
        ];
    }
}
