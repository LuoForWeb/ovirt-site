...
return [
    ...
    'REAL_PROTECT_STAGE' => [
        ...
        83 =>  'WEB_TASK_VOL_CDP_TASK_FAILBACK_WAIT_CONVERT_TO_CDP', // 等待恢复回切实时保护，回切阶段接管机触发降级，等待恢复cdp，可停止接管
        90 =>  'WEB_TASK_VOL_CDP_TASK_FAILBACK_IN_SERVER_DATA_CONSISTENCY_CHECK', // 回切数据一致性校验，回切初始阶段已同步区域差异数据校验阶段，展示流量图及校验百分比，可停止接管
        91 =>  'WEB_TASK_VOL_CDP_TASK_FAILBACK_IN_STANDBY_DATA_CONSISTENCY_CHECK',// 回切机数据一致性校验，代理到代理回切数据校验阶段，预留，暂未使用，
        92 =>  'WEB_TASK_VOL_CDP_TASK_FAILBACK_IN_SERVER_REALTIME_CONSISTENCY_CHECK',// 回切实时数据一致性校验，回切实时阶段断点续传差异数据校验阶段，展示流量图及校验进度百分比，可停止接管，但需提醒用户此时回切目标机的数据可能不完整或存在一致性问题
