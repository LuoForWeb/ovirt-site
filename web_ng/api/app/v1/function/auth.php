...
/**
 * v1 版本 获取用户权限函数文件
 * 以  v1_auth_ 开头，单词之间用 _ 下划线隔开，单词用小写
 */
 ...
 * @return array
 */
function v1_auth_is_admin(string $userUuid = ''): array
{
    $return = [
    ...
    if (!empty($userUuid)) {
        ...
        $userHander = new app\v1\user\v0\logic\User();
 ...
 * @return array|string
 */
function v1_auth_manage_user(string $auth, $back = 'array', $userUuid = '')
{
 ...
 * @return bool
 */
function v1_auth_checks(string $userUuid = ''): bool
{
    $checkAuth = v1_auth_is_admin($userUuid);
    if (
 ...
 * @return bool
 */
function v1_auth_need_check_look(string $userUuid = ''): bool
{
    $checkAuth = v1_auth_is_admin($userUuid);
    if (
 ...
 * @return bool
 */
function v1_auth_need_operation(string $userUuid = ''): bool
{
    $checkAuth = v1_auth_is_admin($userUuid);
    if (
        (!xphp_three_powers() && empty($checkAuth['is_admin']) && empty($checkAuth['global_write']))
 ...
 * @return string
 */
function v1_auth_get_source_by_type(
    int $souceType,
    string $field = '',
    ...
    // 关联管理用户判断 存储资源 - 查看?操作
    $authUser = v1_auth_manage_user($auth . $source, 'sql', $userUuid);
 ...
 * @return bool|string
 */
function v1_auth_check_operate(string $sourceUuid, string $auth, int $souceType, string $userUuid = ''): bool
{

    $resourceUuid = v1_auth_get_source_by_type($souceType, '', $auth, 2, $userUuid);
 ...
 * @return string
 */
function v1_auth_get_users(string $auth, int $type = 1, string $userUuid = ''): string
{

    $source = $type == 1 ? '_look' : '_operate';
    // 关联管理用户判断 存储资源 - 查看?操作
    return v1_auth_manage_user($auth . $source, 'sql', $userUuid);
}
