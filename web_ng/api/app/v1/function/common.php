 ...
 * @return string
 */
function v1_calsize($num = 0, bool $valueFlag = false): string
{
    $num = floatval($num);
    if (0 == $num && !$valueFlag) {
 ...
 * @return string
 */
function v1_calsize1000($num = 0, $valueFlag = false)
{
    $num = floatval($num);
    if (0 == $num && !$valueFlag) {
        return xphp_get_config('app')['NULLSPACE'];
 ...
 * @return array
 */
function v1_calsize_to_value_and_unit($num = 0, $valueFlag = false)
{
    $num = intval($num);
    $info = array(
 ...
 * @return string
 */
function v1_calspeed($num = 0)
{
    $speed = v1_calsize($num, true);
    if (0 == $speed) {
        return $speed;
    }
    return $speed . '/s';
 ...
 */
function v1_calpercent($total = 0, $value = 0)
{
 ...
 * @return number|string
 */
function v1_calpercent_value($total = 0, $value = 0)
{
    $total = floatval($total);
    $value = floatval($value);
    if (0 == $total) {
 ...
 * @return array
 */
function v1_object_array(object $array)
{
    if (is_object($array)) {
        $array = (array) $array;
    }
    ...
    if (is_array($array)) {
        foreach ($array as $key => $value) {
            $array[$key] = v1_object_array($value);
        }
    }
    return $array;
}
 ...
 * @return string
 */
function v1_get_user_type_des(int $usertype)
{
 ...
 * @return string
 */
function v1_format_date($timestamp)
{
    if ($timestamp <= 0) {
        return ' ';
    }
 ...
 * @return string
 */
function v1_get_error_des(int $errorInfo)
{
 ...
 * @return int
 */
function v1_get_error_num(string $errorValue)
{
    $error = xphp_get_config('error');
 ...
 * @return string
 */
function v1_get_online_des(int $onlineFlag)
{
 ...
 * @return string
 */
function v1_get_status_des(int $errorCode)
{
 ...
 * @return string
 */
function v1_get_username(string $useruuid)
{
    $opHandler = new Op();
 ...
 * @return string
 */
function v1_formart_time($dayTime)
{
    if (empty($dayTime)) {
        return $dayTime;
    }
 ...
 * @return String
 */
function v1_sec_to_time(int $times)
{
    $result = '00:00:00';
    if ($times > 0) {
        $hour = v1_formart_time_string(floor($times / 3600));
        $minute = v1_formart_time_string(floor(($times - 3600 * $hour) / 60));
        $second = v1_formart_time_string(floor((($times - 3600 * $hour) - 60 * $minute) % 60));
 ...
 * @return String
 */
function v1_sec_to_day_time(int $times)
{
    $result = xphp_get_config('app')['TIMESPACE'];
    if ($times > 0) {
        $dayValue = floor($times / 3600 / 24);
        $day = v1_formart_time_string($dayValue);

        $hour = v1_formart_time_string(floor($times / 3600 % 24));
        $minute = v1_formart_time_string(floor($times / 60 % 60));
        $second = v1_formart_time_string(floor($times % 60));
 ...
 * @return number
 */
function v1_time_to_sec($timeStr)
{
 ...
 * @return string
 */
function v1_formart_time_string($str)
{
    if ($str < 10) {
        return '0' . $str;
    }
 ...
 * @return bool
 */
function v1_send_email(string $toEmail, string $subject, string $body): bool
{
    $email = new Email();
 ...
 * @return boolean
 */
function v1_parse_flag_to_bool($flag)
{
    $flag = intval($flag);
 ...
 * @return int
 */
function v1_parse_bool_to_flag($boolValue)
{
 ...
 * @return array
 */
function v1_array_search(array $searchArray, int $searchColumn, string $searchValue)
{
 ...
 * @return array
 */
function v1_array_sort(array $sortArray, string $sortColumn, string $sortType, int $start, int $length)
{
    if ('desc' == $sortType) {
        $sortType = SORT_DESC;
    } else {
 ...
 * @return array        去重后的数组
 */
function v1_unique_multidim_array(array $array, string $key)
{
    $temparray = array();
    $i = 0;
    $keyarray = array();
 ...
 */
function v1_encrype54(string $plaintext)
{
    # 密钥是 16 进制字符串格式
 ...
 * @return string
 */
function v1_decrypt54(string $ciphertext)
{
    # 密钥是 16 进制字符串格式
    $key = pack('H*', xphp_get_config('app')['SECRET_KEY']);
    $ivsize = mcrypt_get_iv_size(MCRYPT_RIJNDAEL_256, MCRYPT_MODE_CBC);
 ...
 * @return string base64字符串
 */
function v1_encrype($plaintext = '')
{
    if (!(PHP_VERSION_ID >= 70000)) {
        return v1_encrype54($plaintext);
    }
    if (empty($plaintext)) {
        return '';
    }
 ...
 * @return string
 */
function v1_decrypt($ciphertext = '')
{
    if (!(PHP_VERSION_ID >= 70000)) {
        return v1_decrypt54($ciphertext);
    }
    if (empty($ciphertext)) {
        return '';
    }
 ...
 * @return string
 */
function v1_pt_pass_decrypt($ciphertext = '')
{
 ...
 * @return string base64字符串
 */
function v1_pt_pass_encrypt($plaintext = '')
{
 ...
 * @return string
 */
function v1_decrypt_js_rsa($plaintext = '')
{
    $decrypted = '';
 ...
 * @return string
 */
function v1_encrypt_js_rsa($plaintext = '')
{
    $encrypted = '';
 ...
 * @return void
 */
function v1_my_export($expTitle, $expCellName, $expTableData)
{
 ...
 * @throws Exception
 */
function v1_base_export(string $title, array $header, array $data, array $relation)
{
    $objPHPExcel = new Spreadsheet();
 ...
 * @return string base64字符串
 */
function v1_my_encrype($plaintext = '')
{
 ...
 * @return string
 */
function v1_my_decrypt($ciphertext = '')
{
    # 密钥是 16 进制字符串格式
    $key = pack('H*', '0e55cf8bfba0fdf4a53353cd0d7e282e727a3c1e504145164ae97aa95542ca9a');
    $cipher = 'aes-256-cbc';
 ...
 * @return string
 */
function v1_escape_wildcard($strText = '')
{
    if (empty($strText)) {
        return $strText;
    }
 ...
 * @return mixed
 */
function v1_multi_to_twos(array $data = [], $field = 'child', $fields = 'function')
{
    $array = [];
    if (is_array($data)) {
        foreach ($data as $p) {
            if (isset($p[$field])) {
                $array = array_merge($array, v1_multi_to_twos($p[$field], $field, $fields));
            } else {
 ...
 * @return array
 */
function v1_multi_to_two(array $data = [], $keys = 'name', $field = 'child', $fields = 'function'): array
{
    $array = [];
    foreach ($data as $p) {
        if (!empty($p[$field])) {
            foreach (v1_self_multi_get($p, $keys, $field, $fields) as $key => $item) {
                $array[$key] = $item;
            }
        }
    }
 ...
 * @return array
 */
function v1_self_multi_get($p, $keys = 'name', $field = 'child', $fields = 'function'): array
{
    $array = [];
    if (!empty($p[$field])) {
        foreach ($p[$field] as $p2) {
            if (intval($p2['level']) === 10) {
                ...
                $array = array_merge($array, v1_self_multi_get($p2, $keys, $field, $fields));
            }
        }
    }
 ...
 * @return bool
 */
function v1_check_password($pwd)
{
    $myType = 0;
    if ($pwd == null) {
        return false;
 ...
 * @return string
 */
function v1_remove_escape(string $value): string
{
 ...
 * @return array
 */
function v1_get_all_name(array $arr = []): array
{
    ...
    foreach ($arr as $menu) {
        ...
        if (!empty($menu['child']) && $menu['level'] < 10) {
            $return = array_merge($return, v1_get_all_name($menu['child']));
        }
    }
 ...
 * @return void
 */
function v1_secondary_array_sort(array &$array, string $field, int $order = SORT_ASC)
{
 ...
 * @return array
 */
function v1_unique_by_two_keys_manual(array $array, string $key1, string $key2)
{
    $seen = [];
    $result = [];
 ...
 * @return string
 */
function v1_tape_timepoint_sql($where = '')
{
