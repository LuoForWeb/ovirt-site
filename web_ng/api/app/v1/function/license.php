...
/**
 * v1 版本 获取授权系统函数文件
 * 以  v1_license_ 开头，单词之间用 _ 下划线隔开，单词用小写
 *
// 枚举定义
$modules = [
'vm',  // 虚拟机
 ...
 * @return int
 */
function v1_license_get_capacity_used($type = 0, $subsql = ''): int
{
    ...
    $exclude = v1_license_info('exclude_client_replication_capacity_flag');
 ...
 * @return int
 */
function v1_license_get_vm_used(int $licenseType, $subsql = ''): int
{
    ...
    switch ($licenseType) {
        ...
        case $licenseInfo['cpu']:
            // 需要把zstack的也包含进去计算
            $notIn = implode(',', array_values(array_diff(explode(',', $notIn), [26])));
 ...
 * @return int
 */
function v1_license_get_virtualization_used_num(int $mode = 1): int
{
 ...
 * @return int
 */
function v1_license_get_cdp_used(int $authType, $subsql = ''): int
{
    if ($authType == 2) {
        return v1_license_get_capacity_used(2, $subsql);
    }
    $taskType = xphp_get_config('task', 'TASKTYPE');
 ...
 * @return int
 */
function v1_license_get_num_used($module, $uuids = [], $taskuuid = '', $subsql = ''): int
{
    ...
    if (in_array('nas', $module)) {
        ...
        if (!empty($dataNas)) {
            ...
            $ipArr = dbSelect($sqlIp);
            $ipArr = array_column($ipArr, 'ip');
            $num = count($ipArr);
            if (!empty($uuids)) {
                // 需要查询出当前传递的uuids所包含的ip，是否存在于已经使用了的ip里面
                // 查询本次的uuids的ip数组
                $sqlIp = "SELECT DISTINCT ip FROM nas_storage_resource
                    WHERE nas_uuid IN ('" . implode("','", $uuids) . "')";
                $ipArr2 = array_column(dbSelect($sqlIp), 'ip');
                // 取两个数组的交集的数量
                $sameNum = count(array_intersect($ipArr, $ipArr2));
                // 已使用的减去本次包含的相同的数量
                $num = max(0, $num - $sameNum);
            }
    ...
    if (in_array('private_cloud', $module) || in_array('cloud', $module)) {
        ...
        $num += v1_license_get_virtualization_used_num(intval($mode));
    }
    ...
    if (in_array('verify', $module)) {
        ...
        if (xphp_get_config('app', 'SYSTEM_INFO')['vendor'] == 'vdms') {
 ...
 * @return int
 */
function v1_license_get_files_capacity_used(array $module, $uuids = [], $taskUuid = '', $subsql = ''): int
{
    $moduleType = xphp_get_config('module', 'MODULE_TYPE');
 ...
 * @return int
 */
function v1_license_get_used_storages(
    string $module,
    $authType = 1,
    $licenseType = 0,
    $uuids = [],
    ...
    switch ($module) {
        case 'vm':
            ...
            $num = v1_license_get_vm_used($licenseType, $subsql);
            break;
        case 'cdp':
            // cdp的$authType类型有  1个数 2容量
            $num = v1_license_get_cdp_used($authType, $subsql);
            break;
        ...
        case 'oracle_hana': // hala数据库单独计算个数
            $num = v1_license_get_num_used($module, $uuids, $taskuuid, $subsql);
            break;
        default:
            ...
            $methodArr = [
                1 => [
                    'function' => 'v1_license_get_num_used', 'param' => $module,
                    'uuids' => $uuids, 'task_uuid' => $taskuuid
                ],
                2 => [
                    'function' => 'v1_license_get_num_used', 'param' => ['file', 'oracle', 'os'],
                    'uuids' => $uuids, 'task_uuid' => $taskuuid
                ],
                3 => [
                    'function' => 'v1_license_get_num_used', 'param' => ['oracle', 'os'],
                    'uuids' => $uuids, 'task_uuid' => $taskuuid
                ],
                4 => [
                    'function' => 'v1_license_get_files_capacity_used', 'param' => ['file', 'nas', 'hadoop', 'obs'],
                    'uuids' => $uuids, 'task_uuid' => $taskuuid
                ],
            ];
 ...
 * @return array
 */
function v1_license_get_auth($module = '', $uuids = [], $taskuuid = '', $tenantuuid = ''): array
{

    $data = v1_license_info();
    if (empty($data)) {
        // 未查询到授权信息
        if (!empty($module)) {
            return [
    ...
    if ($auth['root_type'] == 1) {
        ...
        $return['capacity'] = [
            ...
            'used' => v1_license_get_capacity_used(0, $subsql),
        ];
        ...
        $return['storage'] = [
            ...
            'used' => v1_license_get_capacity_used(1, $subsql),
        ];
        $return['vol_capacity'] = [
            'auth_type' => 2,
            'license_type' => 0,
            'total' => $auth['cdp_capacity_max'],
            'used' => v1_license_get_capacity_used(2, $subsql),
        ];
    } else {
        $return = [];
        // 非整体容量授权
    ...
    if (!empty($module)) {
        ...
        if ($returns['auth_type'] == 2) {
            $returns['used_des'] = v1_calsize($returns['used']);
            $returns['total_des'] = v1_calsize($returns['total']);
            $returns['avail_des'] = v1_calsize($returns['avail_des']);
        }
    ...
    foreach ($return as $key => &$value) {
        if ($value['used'] != 0) {
            ...
            if ($value['auth_type'] == 2) {
                $value['used_des'] = v1_calsize($value['used'], true);
                $value['total_des'] = v1_calsize($value['total'], true);
                $value['avail_des'] = v1_calsize($value['avail_des'], true);
            }
            continue;
        }
        if ($key == 'third_db_cdp') {
        ...
        if ($key == 'vm' || in_array($value['license_type'], [0, 1])) {
            ...
            $value['used'] = v1_license_get_used_storages(
                $key,
                $value['auth_type'],
                $value['license_type'],
                $uuids,
        ...
        $used = v1_license_get_used_storages(
            $key,
            $value['auth_type'],
            $value['license_type'],
            $uuids,
 ...
 * @return array
 */
function v1_license_get_func(string $type = 'f', int $mode = 0): array
{

    $data = v1_license_info('extension');
    $return = [];
    if (!empty($data)) {
 ...
 * @return array
 */
function v1_license_get_v(int $type = 1): array
{
 ...
 * @return array
 */
function v1_license_get_v2v(array $array = []): array
{
 ...
 * @return bool
 */
function v1_license_v2v(array $array = [], int $goal = 0, int $jobType = 1, int $instant = 0): bool
{
    ...
    if ($need) {
        ...
        // 获取系统的授权
        $license = v1_license_get_auth('v2v');
        if ($license['total'] == -1) {
            // -1 表示无限制
            return true;
        }
 ...
 * @return array
 */
function v1_get_db_cdp_num(): array
{
    $return = [
        'used' => 0,
        'total' => 0,
    ...
    if ($extension['dbcdplic']) {
        ...
        $rpc = new app\v1\db\v0\logic\DbRpc();
        $data = array();
        //是否获取cdp授权信息
        ...
        $dbCDPHandler = new app\v1\db\v0\logic\DbCdp();
 ...
 * @return array
 */
function v1_license_info(string $field = ''): array
{
    if (empty($field)) {
        $field = '*';
    }
...
* 获取授权是否有效
 */
function v1_license_get_expire_days(): array
{
    $return = [
    ...
    $info = v1_license_info('register_time,days');
    if (empty($info)) {
        // 未授权
        return $return;
    }
