...
use app\v1\recovery\v0\service\Service as RecoveryService;
...
use xphp\BLLHandler;
use app\v1\complete_machine_os\v0\service\Service as CMService;
...
class VmRecover extends Recover
{
    ...
    public function getRecoverVcenter(array $params): array
    {
        ...
        $oneHypersiorFlag = $params['one_hypervisor_flag'];    //是否只显示一个虚拟化,原平台恢复用
        ...
        if (v1_auth_checks()) {
            ...
            $resourceUuidSql = v1_auth_get_source_by_type(VmPlatform::instance()->getResourceTypeBySubModule($subModuleType), 'uuid');
...
//                $allVcenterFlag = $settings['recover']['hosttype'] == "0";
        ...
        // 获取可恢复的目标类型
        $recoveryHypervisorArr = v1_license_get_v2v(explode(',', $hypersior));
        ...
        foreach ($data as $d) {
            ...
            if ($instantFlag && (
                    ...
                    || intval($d['hypervisor_type']) == xphp_get_config('vm', 'VMHYPERVISORTYPE')['VM_HYPERVISOR_TYPE_NUTANIX_AHV']
                )) continue;
            ...
            if (in_array($d['hypervisor_type'], xphp_get_config('vm','VMHYPERVISORGROUP')['publiccloud'])
    ...
    public function getSyncRecoveryVcenter(array $params)
    {
        ...
        if (xphp_get_config('vm')['VMHYPERVISORTYPE']['VM_HYPERVISOR_TYPE_HYPERV'] == $hypervisor
        ...
        foreach ($data as $d) {
            ...
            if (!empty($_SESSION['tenantuuid'])) {
                // 获取已分配的和租户自有的
                if (!in_array($d['host_uuid'], $tenantHost) && $_SESSION['tenantuuid'] != $vcenterTenantUuid) continue;
            }
            if (!$d['host_uuid']) {
                $d['host_uuid'] = $d['uuid'];
            }
            ...
            if (xphp_get_config('vm')['VM_TREE_TYPE']['HOST'] == $d['type']) {
                ...
                if ($d['online_flag'] == xphp_get_config('app')['FLAG']['SET'] || $privateCloudFlag) {
                    ...
                    if ($hypervisor == xphp_get_config('vm')['VMHYPERVISORTYPE']['VM_HYPERVISOR_TYPE_SANGFOR_KVM']
            ...
            $node[] = array(
                ...
                "refresh" => false,
                "vcuuid" => $id,
                "chkDisabled" => !$privateCloudFlag && ($this->getOffLine($d['online_flag']) || $this->getUnauthorized($d['authorization_flag'])),
                "hypervisor_des" => xphp_get_config('vm')['VMHYPERVISORDES'][intval($pid)],
                "checked" => $checked,
    ...
    public function getSyncRecoveryVcenterForXhere(array $params, array $data): array
    {
        ...
        foreach ($data as $d) {
            if (!empty($_SESSION['tenantuuid'])) {
                // 获取已分配的和租户自有的
                if (!in_array($d['uuid'], $tenantCluster) && $_SESSION['tenantuuid'] != $vcenterTenantUuid) continue;
            }
            $node[] = array(
                "id" => $d['uuid'],
                "pid" => $d['parent_uuid'],
    ...
    public function getOpenStackNetworkAndStorage(array $params): array
    {
        ...
        $storage = v1_array_sort($storage, 'freesize', 'desc', 0, -1);
...
//         var_dump($hypervisor, $points);
            ...
            foreach ($pointsDetail as $k => $pointItem) {
                ...
                if ($mbResultDisk['result']) {
                    ...
                    foreach ($mbResultMsg['disk_list'] as &$item) {
                        $item['is_bootable'] = v1_parse_flag_to_bool($diskList[$item['disk_key']]['is_system_disk_flag']);
                    }
                }
    ...
    public function groupVMRecoveryPluginMsg(
        ...
        foreach ($target_vm_list as $key => $value) {
            ...
            //设置内存按单位显示的格式
            $target_vm_list[$key]['memory_array'] = v1_calsize_to_value_and_unit($target_vm_list[$key]['memory']);
...
//             $target_vm_list[$key]['openstack_start_mode'] = $value['openstack_start_mode'];
            ...
            if (xphp_get_config('vm', 'VMHYPERVISORTYPE')['VM_HYPERVISOR_TYPE_HYPERV'] == $pointsDetail[$key]['hypervisor']
            ...
            foreach ($diskList as $dkey => $dvalue) {
                $target_vm_list[$key]['disk_list'][$dkey]['virtual_size_unit'] = v1_calsize($dvalue['virtual_size'], true);
                ...
                if (xphp_get_config('vm', 'VMHYPERVISORTYPE')['VM_HYPERVISOR_TYPE_VMWARE'] == $pointsDetail[$key]['hypervisor']
    ...
    public function createRecoverJob(array $params): string
    {
        ...
        $pfMSg['calibreate_strategy_list'] = array(
            "int_create_calibration_algorithm" => v1_parse_bool_to_flag($params['verifyInfo']['is_integrity_check']), //是否生成数据校验
            "integrity_error_handle" => v1_parse_bool_to_flag($params['verifyInfo']['integrity_error_handle']), //校验出错后是否继续
        );
        // 传输压缩
        $pfMSg['source_compression'] = $params['typeInfo']['high']['trasfer']['transfer_compress'];
        // 异步传输
        $pfMSg['async_rw_flag'] = v1_parse_bool_to_flag($params['typeInfo']['high']['trasfer']['async_transfer'] ?? false);
        // 重试策略
...
//                "name" => $d['vcenter_ip'] == $d['nickname'] ? $d['vcenter_ip'] : $d['nickname'] . "(" . $d['vcenter_ip'] . ")",
    ...
    public function createMotionJob(array $params): string
    {
        ...
        $rebuildVMInfo = array(
            ...
            'start_vm_flag' => v1_parse_bool_to_flag($startVMFlag),
        ...
        $pfMSg = array(
            ...
            'encrypt_flag' => v1_parse_bool_to_flag($encryptFlag),
        ...
        foreach ($vms as $vm) {
            ...
            if ($vm['pass_flag']) {
                ...
                if (!empty($data)) {
                    ...
                    $password = v1_pt_pass_encrypt(base64_decode($vm['encrypt_pass']));
...
//        }
    ...
    private function getHostStorageNameText(int $hypervisor, array $storage): string
    {
        ...
        return $storage['storage_name'] . "(" . $storage[$typeStr] .
            ", " . xphp_get_lang('UI_STORAGE_TOTAL_SIZE') . ":" . v1_calsize($storage['total_size'], true) .
            ", " . xphp_get_lang('WEB_PLATFORM_DC_AVAILABLE_SPACE') . ":" . v1_calsize($storage['free_size'], true) . ")";
    }
    ...
    public function getVMRecoveryControlContent(int $hypervisor, string $vcenteruuid, string $hostuuid): array
    {
        ...
        switch ($hypervisor) {
            ...
            case $hypervisorConf['VM_HYPERVISOR_TYPE_LENOVO_AIO']:
                ...
                $network_bus = $this->getControllerTypeDesArr($network_bus);
                break;
            case $hypervisorConf['VM_HYPERVISOR_TYPE_NUTANIX_AHV']:
                //显示控制
                $control = array(
                    'other_nav' => false,                //其他,横选项
                    'vm_version' => false,               //虚拟机版本
                    ...
                    'network_high_config' => true,      //网络是否显示高级配置
                    'network_bus_type' => true,         //网卡类型
                    'network_bus_type_disabled' => true,         //禁用网卡类型
                    'network_ip_setting' => false,       //网卡IP设置
                    'os_type' => true,                    //跨平台恢复是否支持操作系统选择
                );
                ...
                $disk_bus = array(
                    'VM_MIDDLE_CONTROLLER_TYPE_SCSI' => 'SCSI',
                    'VM_MIDDLE_CONTROLLER_TYPE_VIRTIO' => 'PCI', // 磁盘总线类型选择为pci时实际为virtio
                    'VM_MIDDLE_CONTROLLER_TYPE_SATA' => 'SATA',
                    'VM_MIDDLE_CONTROLLER_TYPE_IDE' => 'IDE',
                );
    ...
    private function groupRebuildVMInfo($recoveryPos, $points, $recoverInfo)
    {
        ...
        foreach ($points as $point) {
            ...
            if ($driverInfo && $driverInfo['driver_check_status'] != 8) {
                $extensionInfo['driver_replace_flag'] = v1_parse_bool_to_flag($driverInfo['platform_type'] == 2); // 仅异构需替换驱动
                $extensionInfo['driver_hw_id_map'] = $driverInfo['driver_hw_id_map'];
            } else {
                $extensionInfo['driver_replace_flag'] = v1_parse_bool_to_flag(false);
    ...
    public function groupVMConfig(array $vmConfig, array $point = [], string $cdpDatetime = '', bool $motionFlag = false): string
    {
        ...
        $info = array(
            ...
            'reset_hostname_flag' => v1_parse_bool_to_flag($other['reset_hostname']),
            'new_hostname' => trim($other['new_hostname']),
            'reset_bios_uuid_flag' => v1_parse_bool_to_flag($other['reset_biosuuid']),
    ...
    public function processRecoveryVmNetwork(array $networkList, bool $motionFlag = false): array
    {
        foreach ($networkList as $key => &$value) {
            ...
            $networkList[$key]['reset_nic_list'] = [
                [
                    'reset_network_flag' => v1_parse_bool_to_flag($value['modify_network_flag']),
                    'ip_protocol' => 'ipv4',
    ...
    public function groupExtensionInfo(array $recoverInfo, array $vmConfig = []): string
    {
        if (self::VM_RECOVERY_WAY_SPECIFIC == $recoverInfo['target_way']) {
            $info = array(
                ...
                'password' => v1_pt_pass_decrypt($vmConfig['target_password']),
            ...
            $info = array(
                ...
                'password' => $recoverInfo['password'] ? v1_pt_pass_decrypt($recoverInfo['password']) : v1_pt_pass_decrypt($recoverInfo['node']['password']),
                'controller_ip' => $recoverInfo['controllerip'],
    ...
    private function groupInstantVMInfo(array $pointInfo, array $recoverInfo): array
    {
        ...
        return array(
            ...
            'start_vm_flag' => v1_parse_bool_to_flag($recoverInfo['startvm']),     //启动标志
