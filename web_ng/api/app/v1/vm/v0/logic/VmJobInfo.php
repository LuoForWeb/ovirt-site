...
use app\v1\log\v0\logic\Index as LogHandler;
...
class VmJobInfo extends Base
{
    ...
    public function getBasicInfo(array $params): array
    {
        ...
        foreach ($data as $d) {
            ...
            $basicInfo = array(
                ...
                'totalSize' => $taskStopFlag ? xphp_get_config('app')['NULLSPACE'] : v1_calsize($d['total_object_size'], true),
                'currentSize' => $taskStopFlag ? xphp_get_config('app')['NULLSPACE'] : v1_calsize($d['total_object_completed_size'], true),
            ...
            if (intval($d['module_type']) == xphp_get_config('module', 'MODULE_TYPE')['VM']) {
                ...
                $basicInfo['autoJoinFlag'] = v1_parse_flag_to_bool($auto_join_flag);
            }
        }
        return $basicInfo;
    }
    ...
    public function getVMRunningJobLog(array $params): array
    {
        ...
        $info = array();
        foreach ($data as $d) {
            $des = LogHandler::instance()->getLogDescription(xphp_get_config('log', 'LOGTYPE')['TASK'], $d['error_code'],
                $d['description_key'], $d['description_param'], false, $d['log_level'], $d['module_type'], $d['submodule_type']);
            // 公有云替换描述
            if (in_array($d['submodule_type'], xphp_get_config('vm', 'VMHYPERVISORGROUP')['publiccloud'])) {
                ...
                if (xphp_get_config('task', 'TASKTYPE')['RECOVERY'] == $d['task_type']
    ...
    public function getInstantBaseInfo(array $params): array
    {
        ...
        if (intval($data[0]['new_vm_status']) == xphp_get_config('vm', 'MACHINESTATUS')['POWEREDOFF'] &&
    ...
    public function getVmGrainRecoveryFileDir(array $params): array
    {
        ...
        foreach ($msgFileList as $d) {
            ...
            $filename = $d['name'];
            $filesize = v1_calsize($d['size'], true);
            $fileList[] = array(
...
//        dump($list);
    ...
    public function getTenantHypervisor($tenantuuid = '', $resourceType = 3)
    {
        ...
        // 获取租户下所有资源
        $resourceList = (new \app\v1\resources\v0\logic\Index())->pGetTenantAllResource(
            $tenantuuid,
            $resourceType
        );
        $vcenterUuids = $list = [];
    ...
    public function getUserHypervisor($useruuid = '', $resourceType = 3)
    {
        ...
        // 获取当前用户下所有资源
        $resourceList = (new \app\v1\resources\v0\logic\Index())->pGetUserAllResource(
            $useruuid,
            $resourceType
        );
     ...
     * @return array
     */
    public function getJobVms(array $params, bool $isExportAll = false): array
    {
        $start = $params['offset'];
        $length = $params['limit'];
        $taskUuid = $params['job_uuid'];
        $searchName = $params['search_name'];
        $jobStatus = $params['job_status'];
        ...
        if (isset($jobStatus)) {
            $sql .= " and vml.task_status in ({$jobStatus})";
            $sqlCount .= " and vml.task_status in ({$jobStatus})";
        }
        $sql .= " group by vml.machine_id order by vml.machine_id";
        $paramsArr = array($taskUuid);
        if (!$isExportAll) {
            $sql .= " limit ? , ? ";
            $paramsArr = array($taskUuid, $start, $length);
        }
        $data = $this->dbSelect($sql, $paramsArr);
        ...
        $parallelFlag = in_array($data[0]['hypervisor_type'], $parallelHypervisors);
     ...
     * @throws \Exception
     */
    public function exportDetailsVM(array $params): array
    {
        // 传过来的是数组要转字符串
        $params['job_status'] = $_POST['job_status'] ? implode(",", $_POST['job_status']) : null;
        $exportData = $this->getJobVms($params, true)['rows'];
        $title = xphp_get_lang('UI_VCENTER_MACHINE_LIST');

        $header = [
            'no' => xphp_get_lang('UI_PUBLIC_NUMBER'),
            'vm_name' => xphp_get_lang('UI_BACKUP_REPORT_NAME'),
            'backup_mode' => $params['task_type'] == 1 ? xphp_get_lang('UI_BACKUP_MODE') : xphp_get_lang('UI_EMERGENCY_PLAN_RECOVER_TYPE'),
            'vm_size' => xphp_get_lang('UI_JOB_VM_SIZE'),
            'valid_size' => xphp_get_lang('UI_JOB_VALID_DATA_SIZE'),
            'transport_size' => xphp_get_lang('UI_JOB_TRANSFER_SIZE'),
        ...
        $exportData = array_map(function ($row) {
            ...
            return [
                'no' => $row['no'],
                'vm_name' => $row['name'],
                'backup_mode' => $row['task_type'],
                'vm_size' => $row['size'],
                'valid_size' => $row['valid_size'],
                'transport_size' => $row['transport_size'],
        ...
        $relation = [
            'no' => ['col_name' => 'A', 'width' => 15],
            'vm_name' => ['col_name' => 'B', 'width' => 60],
            'backup_mode' => ['col_name' => 'C', 'width' => 15],
            'vm_size' => ['col_name' => 'D', 'width' => 15],
            'valid_size' => ['col_name' => 'E', 'width' => 15],
            'transport_size' => ['col_name' => 'F', 'width' => 15],
            'write_size' => ['col_name' => 'G', 'width' => 15],
            'speed' => ['col_name' => 'H', 'width' => 15],
            'progress' => ['col_name' => 'I', 'width' => 15],
            'status' => ['col_name' => 'J', 'width' => 15],
            'description' => ['col_name' => 'K', 'width' => 30],
        ];
        ...
        if (
            ...
            if ($copyFlag) {
                ...
                if ($vmTaskStatus == xphp_get_config('vm', 'VmTaskStatus')['RUNNING']) {
                    //虚拟机是运行状态
                    return $parallelFlag ? v1_calsize($vmlCompletedSize, true) : v1_calsize($briCompletedSize, true);
        ...
        if (
            ...
            if ($copyFlag) {
                ...
                if ($vmTaskStatus == xphp_get_config('task', 'TASKSTATUS')['RUNNING']) {
                    return $parallelFlag ? v1_calpercent($vmSize, $vmlCompletedSize) : v1_calpercent($vmSize, $briCompletedSize);
...
//         $this->paramsCheck($strategyID);
        ...
        foreach ($data as $d) {
            ...
            $strategy[] = array(
                ...
                'rollInterval' => v1_sec_to_time(intval($d['roll_interval'])),
    ...
    public function getTransportStrategy($taskuuid, $strategyid)
    {
        ...
        $info['async_transfer'] = isset($detailArr['async_rw_flag']) && v1_parse_flag_to_bool($detailArr['async_rw_flag']);
        // 传输代理实例类型，公有云使用
        $info['agent_type'] = '';
    ...
    protected function getVMBackupModeStrategy($taskuuid)
    {
        ...
        return array(
            ...
            'parseFs' => v1_parse_flag_to_bool($data[0]['parse_fs_flag']),
            'noSwapFile' => v1_parse_flag_to_bool($data[0]['not_backup_swap_file_flag']),
            'noDeletedFile' => v1_parse_flag_to_bool($data[0]['not_backup_deleted_file_flag']),
            'noPartitionGap' => v1_parse_flag_to_bool($data[0]['not_backup_partition_gap_flag']),
            'presnapshot' => v1_parse_flag_to_bool($data[0]['pre_create_snap_flag']),
            'snapshotTimeout' => intval($snapshotTimeout),
            'highspeedDiskCbt' => v1_parse_flag_to_bool($highspeedDiskCbt),
            'keepSnapshot' => v1_parse_flag_to_bool($keepSnapshot),
    ...
    public function getTransportModeDes($hypervisor, $transportPriority)
    {
        ...
        // 部分KVM
        if ($hypervisor == xphp_get_config('vm')['VMHYPERVISORTYPE']['VM_HYPERVISOR_TYPE_INSPUR_VVDK']
    ...
    private function getVerifyModeStrategy(string $taskuuid)
    {
        ...
        return array(
            ...
            'ping_warn_flag' => v1_parse_flag_to_bool(intval($data[0]['ping_warn_flag'])),
            'heartbeat_warn_flag' => v1_parse_flag_to_bool(intval($data[0]['heartbeat_warn_flag'])),
            'screenshot_warn_flag' => v1_parse_flag_to_bool(intval($data[0]['screenshot_warn_flag'])),
