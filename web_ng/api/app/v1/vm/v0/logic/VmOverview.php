    ...
    public function getVMReport(array $params): array
    {
        ...
        if (!empty($data)) {
            ...
            foreach ($data as $d) {
                ...
                //如果是不属于当前用户的虚拟机需要排除
                if (v1_auth_need_check_look() && $userInfo['userUuid'] != $d['user_uuid'] && $userInfo['userType'] != $configUserType['manager']) {
                ...
                $operateFlag = xphp_check_operate($userInfo['userUuid'], $userUuids, $authUser) || !v1_auth_need_check_look();
                ...
                if ($backupFlag && $vmBackupFlag != v1_parse_flag_to_bool($backupFlag)) {
                    $searchCount--;
                    continue;
                }
...
//                    '<input type="checkbox" name="id[]" value="' . $d['tree_id'] . '">',
        ...
        $vminfo = v1_array_sort($vminfo, $sortColumn, $sortType, $start, $length);  //排序
...
//             $sizeInfo = $this->getVmStorageSize($d['vm_uuid'], $d['vcenter_uuid']);
            //备份数据大小
            $info['backupstorage'] = v1_calsize($d['real_size'], true);
            $info['backupstorageSize'] = $d['real_size'];
            //虚拟机备份总大小
            $info['totalsizeDes'] = v1_calsize($d['total_size'], true);
    ...
    public function isHypervisorHaveIp($hypervisor): bool
    {
        ...
        return in_array(intval($hypervisor), $vmHypervisorGroup['openstack'])
            || in_array(intval($hypervisor), [
                ...
                $vmHypervisorType['VM_HYPERVISOR_TYPE_NUTANIX_AHV'],
            ]);
    }
}
