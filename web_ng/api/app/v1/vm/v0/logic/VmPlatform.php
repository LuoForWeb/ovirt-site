...
//        $cloudTypesStr = implode("','", $cloudTypes);
    ...
    public function getVmPlatformsVmTree(array $params): array
    {
        ...
        $speedFlag = $params['speed_flag'] ?? false;
     ...
     * @param bool $speedFlag 快速创建备份任务获取标志
     * @return array
     */
...
//        dump($data);
        ...
        foreach ($data as $d) {
            ...
            if ($loadAllFlag) {
            ...
            } elseif (isset($keyword) || $speedFlag) {
                $node[] = array(
                    ...
                    'chkDisabled' => !$checkEnabled,
                    'online' => $connState,
                    'open' => v1_parse_flag_to_bool($d['is_search_parent'] || $speedFlag),
                ...
                if (($objCount[$pUuid] < $limit || $limit === null || $modifyFlag || $loadOnceFlag) && !$skipFlag || $awsFlag) {
                    $node[] = array(
                        ...
                        "username" => $d['username'],
                        "password" => $d['password'],
                        "passwordDe" => base64_encode(v1_pt_pass_decrypt($d['password'])),
                    );
                }
                ++$objCount[$pUuid];
...
//            }
    ...
    private function deleteVmPlatformsCheck(array $platformUuids): void
    {
        ...
        if (empty($userInfo['tenantuuid']) && $userInfo['userUuid'] == 'a508b813-19c7-eb4e-d6fa-bb61b25a4de9') {
            ...
            $publicCloudFlag = in_array($data[0]['hypervisor_type'],
    ...
    public function getBackupTree(array $params): array
    {
        ...
        foreach ($data as $d) {
            ...
            $node = array(
                ...
                "name" => $name,
                "title" => $name,
                "open" => $params['speed_flag'] && $d['vcenter_uuid'] == $vcenteruuid,
                "isParent" => true,
                "uuid" => $d['vcenter_uuid'],
                "sid" => $d['vcenter_id'],
                "nocheck" => !$checkedFlag,
    ...
    public function getSyncVcenterGroup(array $params): array
    {
        ...
        $typeArr = [xphp_get_config('vm', 'VM_TREE_TYPE')['CLUSTER'], xphp_get_config('vm', 'VM_TREE_TYPE')['FOLDER']];
        $typeStr = implode(",", $typeArr);

        $sql = "WITH RECURSIVE tree_path AS (
            -- 基础查询：获取指定parent_uuid的直接子节点
            SELECT name, uuid, parent_uuid, type, 0 as level, parent_uuid as top_group_uuid
            FROM vm_tree 
            WHERE vcenter_uuid = ? 
            AND parent_uuid IN ($groupUuidsStr) 
            AND type IN ($typeStr)
            
            UNION ALL
            
            -- 递归查询：获取子节点的子节点
            SELECT t.name, t.uuid, t.parent_uuid, t.type, tp.level + 1, tp.parent_uuid as top_group_uuid
            FROM vm_tree t
            INNER JOIN tree_path tp ON t.parent_uuid = tp.uuid
            WHERE t.vcenter_uuid = ?
            AND t.type IN ($typeStr)
        )
        SELECT name, uuid, parent_uuid, type, level, top_group_uuid
        FROM tree_path
        ORDER BY level, name;";
        $dataRegion = $this->dbSelect($sql, [$vcenteruuid, $vcenteruuid]);

        $userList = array();
        foreach ($groupList as $group){
            $sql = "select type from vm_tree where vcenter_uuid = ? and uuid = ?";
            $treeType = $this->dbSelect($sql, [$vcenteruuid, $group['group_uuid']])[0]['type'];
            if (!empty($_SESSION['tenantuuid'])) {
                // 获取已分配的和租户自有的
            ...
            $userList[] = array(
                ...
                "username" => $username,
                "password" => $password,
                "iconSkin" => $treeType == 3 ? $iconskin . "_hostcluster" : $iconskin . "_folder_vm",
            ...
            foreach ($dataRegion as $region) {
                if ($region['top_group_uuid'] != $group['group_uuid']) {
                    continue;
                }
                if ($region['type'] == 1) {
                    // 分组
                    $userList[] = array(
                        ...
                        "isParent" => true,
                        "vcuuid" => $vcenteruuid,
                        "pid" => $region['parent_uuid'],
                        "pId" => $region['parent_uuid'],
                        "hypervisor" => $hypervisor,
                        "hypervisor_des" => xphp_get_config('vm')['VMHYPERVISORDES'][$hypervisor],
                        "name" => $region['name'],
                        "groupuuid" => $region['uuid'],
                    ...
                    $parentName = $this->dbSelect($sql, [$vcenteruuid, $region['parent_uuid']])[0]['name'];
                    $userList[] = array(
                        "type" => 3,
                        "isParent" => false,
                        "vcuuid" => $vcenteruuid,
    ...
    public function getSupportInfo(array $params)
    {
        ...
        foreach ($msg['os_info_list'] as &$archItem) {
            ...
            foreach ($archItem['os_list'] as &$v) {
                ...
                // 如果是英文版环境 os_description里面含有"位" 则替换为bit 这里不能提取语言包  因为"位"是从后端接口获取到的
                if (xphp_get_config('app', 'lang') == 'en-us' && strpos($v['os_description'], '位')) {
                    $v['os_description'] = str_replace('位', 'bit', $v['os_description']);
                }
            }
