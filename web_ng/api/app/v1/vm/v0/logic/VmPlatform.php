...
class VmPlatform extends Base
{
    ...
    public function addVmPlatform(array $params): string
    {
        ...
        //检查参数
        if (xphp_get_config('vm')['VMHYPERVISORTYPE']['VM_HYPERVISOR_TYPE_INSPUR_INCLOUD_SPHERE_KVM'] == $hypervisorType
    ...
    public function getVmPlatformsList(array $params): array
    {
        ...
        if (v1_auth_need_check_look()) {
            $resourceUuidSql = v1_auth_get_users($this->getUserAuthKeyBySubModule($subModule));
        ...
        $vmDes = require APP_PATH . 'v1/description/Vm.php';
        foreach ($datas as $data) {
            ...
            if (in_array($data['hypervisor_type'], $privateCloudType)) {
                // Zstack Cloud用的虚拟化授权
                if (xphp_get_config('vm', 'VMHYPERVISORTYPE')['VM_HYPERVISOR_TYPE_ZSTACK'] == $data['hypervisor_type']) {
...
//        $cloudTypesStr = implode("','", $cloudTypes);
        ...
        if (v1_auth_checks()) {
            ...
            $resourceUuidSql = v1_auth_get_source_by_type($this->getResourceTypeBySubModule($subModule), 'uuid');
            $sqlNew = " EXISTS (select vcenter_uuid from 
                                        (select DISTINCT vcenter_uuid
                                            from vm_tree
                                             where {$resourceUuidSql}
                                        ) v1_auth_all2
                                 where v1_auth_all2.vcenter_uuid = vv.vcenter_uuid) ";
            $sql .= " and {$sqlNew}";
        }
        ...
        $chkDisabled = v1_auth_need_operation();
        // 标记哪些虚拟化有“已分配虚拟机”节点
        ...
        foreach ($data as $d) {
            ...
            if (!in_array($hypervisorType, $hypervisor)) {
                if (!in_array($hypervisorType, v1_license_get_v())) {
                    // 未勾选授权的排除
                    continue;
                }
                //没有hypervisor层时,自动添加
            ...
            if (!empty($userInfo['tenantuuid']) && xphp_get_user_info()['userUuid'] != $d['user_uuid']
            ...
            if (!$platformUuid && !in_array($hypervisorType, v1_license_get_v())) {
                // 未勾选授权的排除
                continue;
            }
        ...
        if ($onlyHypervisorFlag) {
            // 按虚拟化排序
            $tree = v1_array_sort($tree, 'id', 'asc', 0, -1);
        }
    ...
    public function getPlatformCurrentJobs(string $platformUuid): array
    {
        ...
        $order = " order by CASE
                    ...
                    BINARY bt.task_name";
        $sql .= $order;
        $sqlParams = array($platformUuid, xphp_get_config('task', 'TASKTYPE')['BACKUP'],
            xphp_get_user_info()['userUuid']);
        $data = $this->dbSelect($sql, $sqlParams);
        $taskInfo = array();
        foreach ($data as $d) {
    ...
    public function getTreeFromVMTree(
        ...
        if ($allocatedVmFlag) {
            ...
            where vt.display_mode = ? and if(vv.hypervisor_type = ? and vt.uuid = ?, 0, 1) ";
            $sqlParams = array($displayMode, $hypervType, $platformUuid);
        } elseif ($specificVmRecoveryFlag) {
            $sql = "select distinct vv.user_uuid, vv.hypervisor_type, vv.username, vv.password, vt.tree_id, vt.type, vt.name, vt.uuid, vt.parent_uuid, vt.dir_path, vt.conn_state, 
            ...
            if ($parentUuid == $platformUuid) {
                $parentSql = "select vcenter_name from vm_vcenter where vcenter_uuid = ?";
                $parentPath = $this->dbSelect($parentSql, [$parentUuid])[0]['vcenter_name'];
            } else {
                $parentSql = "select dir_path from vm_tree where uuid = ?";
                $parentPath = $this->dbSelect($parentSql, [$parentUuid])[0]['dir_path'];
            }
            $sql = "select distinct vv.user_uuid, vv.hypervisor_type, vv.vcenter_name, vv.vcenter_flag, vt.tree_id, vt.type, vt.name, vt.uuid, vt.parent_uuid, vt.dir_path, vt.conn_state,
                vt.power_state, vt.host_uuid, vt.vcenter_uuid, vt.version, vt.detail as tree_detail, vh.host_name, '' as vml_task_uuid
                from vm_tree vt
                join vm_vcenter vv on vt.vcenter_uuid = vv.vcenter_uuid
                left join vm_host vh on vt.host_uuid = vh.host_uuid and vt.vcenter_uuid = vh.vcenter_uuid
                where vt.vcenter_uuid = ? and vt.dir_path like '{$parentPath}/%' and vt.display_mode = ? and if(vv.hypervisor_type = ? and vt.uuid = ?, 0, 1) and vt.type = 7";
            $sqlParams = array($platformUuid, $displayMode, $hypervType, $platformUuid);
        } else {
        ...
        if (v1_auth_checks()) {
            ...
            $resourceUuidSql = v1_auth_get_source_by_type($this->getResourceTypeBySubModule($subModule), $filed);
        ...
        foreach ($data as $d) {
            ...
            $checkEnabled = true;
            // 检查虚拟化平台的主机授权，右侧加载时不用获取
            if (!$loadAllFlag && !in_array($hypervisor, $vmGroupConf['privatecloud']) && !in_array($hypervisor, $vmGroupConf['publiccloud'])) {
            ...
            if (!$loadAllFlag && $vmFlag && in_array($d['host_uuid'], $disconnectHostUuids)) {
                $checkEnabled = false;
                $connState = false;
            };
            ...
            // Nutanix AHV 的保护域为空时，不允许备份
            if (xphp_get_config('vm', 'VMHYPERVISORTYPE')['VM_HYPERVISOR_TYPE_NUTANIX_AHV'] == $hypervisor) {
                $treeDetail = json_decode($d['tree_detail'], true);
                if (empty($treeDetail['protect_domain'])) {
                    $checkEnabled = false;
                    $checkDisabledType = 'protect_domain_null';
                }
            }
            ...
            if ($allocatedVmFlag) {
                if (xphp_get_user_info()['userUuid'] != $d['user_uuid']
                    ...
                    $node[] = array(
                        ...
                        "chkDisabledType" => $checkDisabledType,
                        "online" => $connState,
                        "open" => $openFlag,
            ...
            if ($loadAllFlag) {
                ...
                $node[] = array(
                    ...
                    'chkDisabled' => !$checkEnabled,
                    'online' => $connState,
                    'open' => v1_parse_flag_to_bool($d['is_search_parent']),
                ...
                if (($objCount[$pUuid] < $limit || $modifyFlag || $awsFlag || $loadOnceFlag) && !$skipFlag) {
                    $node[] = array(
                        'treeId' => $d['vcenter_uuid'] . '_' . $d['uuid'],
                        'id' => $d['uuid'],
                        'uuid' => $d['uuid'],
...
//        dump($parentList);
    ...
    {
        // 任务中in_task需判断vm_machine_list.delete_flag = 2
        $searchParentFlag = v1_parse_bool_to_flag(!empty($dirPathArr));
        if ($taskUuid) {
            $sql = "select distinct vv.user_uuid, vv.hypervisor_type, vv.vcenter_name, vv.vcenter_flag, vt.tree_id, vt.type, vt.name, vt.uuid, vt.parent_uuid, vt.dir_path, vt.conn_state, 
            vt.power_state, vt.host_uuid, vt.vcenter_uuid, vt.version, vt.detail as tree_detail, vml.task_uuid as vml_task_uuid, 
            !isnull(vol.id) as in_current_task, !isnull(vol2.id) as show_current_task, if(!isnull(vml.machine_id) and vml.delete_flag = 2, 1, 0) as in_task,  bt.task_name, {$searchParentFlag} as is_search_parent 
            from vm_tree vt 
            join vm_vcenter vv on vt.vcenter_uuid = vv.vcenter_uuid 
            left join vm_object_list vol on vt.uuid = vol.object_uuid and vt.vcenter_uuid = vol.vcenter_uuid and vol.task_uuid = ? 
            left join vm_object_list vol2 on vt.type != 7 and vol2.dir_path like concat(vt.dir_path, '%') and vt.vcenter_uuid = vol2.vcenter_uuid and vol2.task_uuid = ? 
            ...
            $sql = "select distinct vv.user_uuid, vv.hypervisor_type, vv.vcenter_name, vv.vcenter_flag, vt.tree_id, vt.type, vt.name, vt.uuid, vt.parent_uuid, vt.dir_path, vt.conn_state, 
            vt.power_state, vt.host_uuid, vt.vcenter_uuid, vt.version, vt.detail as tree_detail, vml.task_uuid as vml_task_uuid, 
            if(!isnull(vml.machine_id) and vml.delete_flag = 2, 1, 0) as in_task, bt.task_name, {$searchParentFlag} as is_search_parent  
            from vm_tree vt 
            join vm_vcenter vv on vt.vcenter_uuid = vv.vcenter_uuid 
            left join vm_machine_list vml on vt.type = 7 and vt.uuid = vml.vm_uuid and vt.vcenter_uuid = vml.vcenter_uuid and vml.timepoint_uuid = '' -- 只join备份任务的
            left join bd_task bt on vml.task_uuid = bt.task_uuid
        ...
        if ($platformUuid) {
            $sql .= " and vt.vcenter_uuid = ? and if(vv.hypervisor_type = ? and vt.uuid = ?, 0, 1) ";
     ...
     * @param bool   $vcenterFlag   平台flag
     * @param array  $vmTreeType
     * @param array  $hypervisorTypeConf
     * @return string
     */
    public function getVMTreeName(int $type, string $name, int $hypervisor, string $vcenterName, bool $vcenterFlag, array $vmTreeType, array $hypervisorTypeConf): string
    {
        if ($type == $vmTreeType['OVIRT_FAKE_HOST_FOLDER']) {
            ...
            if ($hypervisorTypeConf['VM_HYPERVISOR_TYPE_HYPERV'] == $hypervisor) {
                //针对于hyper-v单机宿主机
                if ($vcenterFlag == xphp_get_config('app', 'FLAG')['UNSET']) {
                    $name = $vcenterName;
                }
            }
            return $name;
...
//            }
    ...
    private function deleteVmPlatformsCheck(array $platformUuids): void
    {
        ...
        if (empty($userInfo['tenantuuid']) && $userInfo['userUuid'] == 'a508b813-19c7-eb4e-d6fa-bb61b25a4de9') {
            ...
            $submoduleType = $this->getVmPlatformSubModule($platformUuids[0]);
            $authKey = $this->getUserAuthKeyBySubModule($submoduleType);
            $sqlcheck = "select user_uuid, hypervisor_type from vm_vcenter where vcenter_uuid in ( ? )";
            // 获取非分配资源的权限
            if (v1_auth_need_check_look()) {
                $resourceUuidSql = v1_auth_get_users($authKey);
                $sqlcheck .= " and vv.user_uuid in $resourceUuidSql";
            }
            $data = $this->dbSelect($sqlcheck, [$vcenterStr]);
            $userArr = array_unique(array_column($data, 'user_uuid'));
            if (v1_auth_need_check_look() && !xphp_check_operate($authKey, $userArr)) {
                // 没有操作权限
                exit(
                $this->muOpResult(
                    false,
        ...
        $data = $this->dbSelect($sql, $sqlParams);
        if (!empty($data)) {
            if (in_array($data[0]['hypervisor_type'], xphp_get_config('vm')['VMHYPERVISORGROUP']['publiccloud'])
     ...
     * @return array
     */
    private function getHypervisorLevTree(int $hypervisor, int $platformFlag): array
    {
    ...
    public function getSyncVcenterGroup(array $params): array
    {
        ...
        foreach ($groupList as $group){
            if (!empty($_SESSION['tenantuuid'])) {
                // 获取已分配的和租户自有的
                if (!in_array($group['group_uuid'], $tenantGroups) && $_SESSION['tenantuuid'] != $vcenterTenantUuid) continue;
            }
            $username = '';
            $password = '';
        ...
        return [
            're' => $result,
            'op' => $operate,
            'data' => v1_array_sort($userList, 'name', 'asc', 0, -1)
        ];
    }
    ...
    public function getSupportInfo(array $params)
    {
        ...
        foreach ($msg['os_info_list'] as &$archItem) {
            ...
            if (!in_array($archItem['arch_type'], $archArr)) {
                $archItem['os_list'] = v1_array_sort($archItem['os_list'], 'os_description', 'asc', 0, -1);
                ...
                foreach ($finalOsList as &$finalOsItem) {
                    if ($finalOsItem['arch_type'] == $archItem['arch_type']) {
                        ...
                        $finalOsItem['os_list'] = v1_array_sort($osList, 'os_description', 'asc', 0, -1);
                    }
                }
            }
        }
    ...
    public function getVcenterNameInTree(
        ...
        } else if (xphp_get_config('vm','VMHYPERVISORGROUP')['VM_HYPERVISOR_TYPE_HYPERV'] == $hypervisor
...
//            $name = $vcenterip . "(" . $vcentetname . ")";
    ...
    public function getBackupTreeOldInfo(array $params): array
    {
        ...
        $topNode = $this->getBackupTree($params);
        //获取hypervisor层
        $hypervisorLevNode = $this->getHypervisorLevTree($params['hypervisor_type'], 1);
        //获取该任务备份对象
        ...
        if (v1_auth_checks()) {
            $subModule = $this->hypervisorTypeToSubModule($params['hypervisor_type']);
            $resourceUuidSql = v1_auth_get_source_by_type($this->getResourceTypeBySubModule($subModule), 'vol.object_uuid');
        ...
        if (empty($data)) {
            ...
            if (v1_auth_need_check_look()) {
                $subModule = $this->hypervisorTypeToSubModule($params['hypervisor_type']);
                $resourceUuidSql = v1_auth_get_source_by_type($this->getResourceTypeBySubModule($subModule), 'vml.vm_uuid');
                $sql .= " and $resourceUuidSql";
            }
            $data = $this->dbSelect($sql, array($params['job_uuid']));
        }
    ...
    public function getVcenterHostLisenceStatus(string $vcenteruuid): array
    {
        ...
        $vmDes = include APP_PATH . 'v1/description/Vm.php';
        $info = array();
        if ($countHost == $authHost) {
            //全部授权
            $info['status'] = xphp_get_config('vm', 'HOST_LISENCE')['ALL'];
