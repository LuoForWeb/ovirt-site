...
use app\v1\vm\v0\service\Service;
...
class VmJobController extends JobController
{
    ...
    public function addVmToBackupTask(array $params): string
    {
        ...
        foreach ($vmInfo as $info) {
            ...
            $sqlParams = array($taskUuid, $platformUuid, $info['uuid'], $info['name'], $info['version'],
                $info['host_uuid'], $info['dir_path'], xphp_get_config('vm', 'VmTaskStatus')['NEW_ADD']);
            ...
            values (?, ?, ?, ?, ?, ?, ?, ?)";
            $sql2Params = array($taskUuid, $info['uuid'], $platformUuid, xphp_get_config('vm', 'VM_TREE_TYPE')['VM'],
