...
//        $pfMsg['integrity_check_flag'] = intval($params['high_strategy']['integrity_check_flag']);
    ...
    public function getVerifySelectObjectList($params = [])
    {
        ...
        $sql = "select
                ...
                btal.agent_uuid,
                dl.db_name, dl.instance_name, dl.db_uuid,dl.agent_uuid as db_agent_uuid, dl.dir_path,dt.depend_task_uuid,
                ol.agent_uuid as os_agent_uuid,
                nt.nas_uuid,
                mol.organization_uuid,
                kt.cluster_uuid,
                cvt.master_agent_uuid as vol_agent_uuid, cvt.dev_type,
        ...
        foreach ($result as $each) {
            ...
            switch (intval($each['module_type'])){
                ...
                case $module['DB']:
                    ...
                    $db[] = $objectuuid;
                    //获取子任务信息
                    if(!empty($each['depend_task_uuid'])){
                        $dbChild[$each['depend_task_uuid']] = array(
        ...
        foreach ($result as $each) {
            ...
            switch (intval($each['module_type'])){
                ...
                case $module['DB']:
                    ...
                    $objectname = $each['dir_path'];
                    $title = $objectname;
                    $otherInfo = $dbChild[$each['task_uuid']] ?? array();
                    break;
                case $module['OS']:
                    $objectuuid = $each['agent_uuid'];
            ...
            if(!inArray($taskuuid.'_'.$objectuuid, $objectTemp) && !empty($objectuuid) && empty($each['depend_task_uuid'])){
                //处理整机实时和复制子模块类型
            
                if(intval($each['module_type']) == $module['VOL_CDP']){
                    $subtype = intval($each['dev_type']);
                    if(intval($each['task_type']) == 65 ){//处理复制
                        $subtype = 3;
                    }
                }
                $info['rows'][] = array(
    ...
    public function getDeleteTaskObjectInfo($params){
        ...
        $sql = "select  
                ...
                dbt.db_name, dbt.instance_name, dbt.db_uuid, dbt.agent_uuid as db_agent_uuid, dbt.dir_path,dbt.depend_task_uuid,
                ...
                agent.master_agent_uuid as vol_agent_uuid, agent.master_agent_detail, agent.dev_type 
        ...
        //获取当前在运行的所有任务uuid集合
        $sqlTasks = "select task_uuid from bd_task";
        $dataTask = $this->dbSelect($sqlTasks);
        $taskuuidList = array();
        foreach ($dataTask as $d){
            $taskuuidList[] = $d['task_uuid'];
        }
        ...
        foreach ($result as $each) {
            ...
            if(!inArray($each['task_uuid']."_".$objectuuid, $liIDList) && !empty($objectuuid) && empty($each['depend_task_uuid'])){
                ...
                //处理整机实时和复制子模块类型
                $subModule = intval($each['sub_module_type']);
                if(intval($each['module_type']) == $module['VOL_CDP']){
                    $subModule = intval($each['dev_type']);
                    if(intval($each['task_type']) == 65 ){//处理复制
                        $subModule = 3;
                    }
                }
        ...
        switch ($moduleType){
            ...
            case $module['DB']:
                ...
                $timepointSql = "select dbt.timepoint_uuid, bbt.timepoint, bbt.backup_mode, bbt.integrity_check_flag from db_backup_timepoint dbt join bd_backup_timepoint bbt
                    ...
                    where (bbt.task_uuid = ? or dbt.depend_task_uuid = ?) and dbt.db_uuid = ? and bbt.operation_status not in (1, 2) and bbt.available_flag = 1 and bbt.import_flag = 2 
                    order by bbt.id desc limit 0, 1";
                $data = (array)$this->dbSelect($timepointSql, array($taskuuid, $taskuuid, $objectuuid));
                break;
            case $module['OS']:
            ...
            case $module['M365']:
                ...
                $timepointSql = "select mbt.m365_timepoint_uuid as timepoint_uuid, bbt.timepoint, bbt.backup_mode, bbt.integrity_check_flag from m365_backup_timepoint mbt join bd_backup_timepoint bbt
