...
class Visual extends Base
{
    ...
    public function getConfigure()
    {
        ...
        $info = array(
            ...
            'module_show' => empty($config['moduleShow']) ? null : $config['moduleShow'],
        );
        return $info;
    }
    ...
    {
        $taskNum = array(
            'vmprotect' => array('num' => 0,'des'=>xphp_get_lang('UI_PLATFORM_VM_VIRTUAL')),
            'awsprotect' => array('num' => 0,'des'=>xphp_get_lang('WEB_PLATFORM_DES_PUBLIC_CLOUD')),
            'prcloud_protect' => array('num' => 0,'des'=>xphp_get_lang('WEB_PLATFORM_DES_PRIVATE_CLOUD')),
            'complete_machine' => array('num' => 0,'des'=>xphp_get_lang('UI_COMPLETE_MACHINE')),
            'osbackup' => array('num' => 0,'des'=>xphp_get_lang('WEB_PLATFORM_DES_VOL')),
            'fileprotect' => array('num' => 0,'des'=>xphp_get_lang('WEB_PLATFORM_DES_FS')),
            'nas_protect' => array('num' => 0,'des'=>xphp_get_lang('WEB_PLATFORM_DES_NAS')),
            'obs_protect' => array('num' => 0,'des'=>xphp_get_lang('UI_PLATFORM_OBS')),
            'hadoop_protect' => array('num' => 0,'des'=>xphp_get_lang('UI_PLATFORM_HADOOP')),
            'office365_protect' => array('num' => 0,'des'=>'Microsoft365'),
            'k8s_protect' => array('num' => 0,'des'=> xphp_get_lang(('UI_PLATFORM_K8S'))),
            'db_protect' => array('num' => 0,'des'=>xphp_get_lang('WEB_PLATFORM_DES_DB')),
            ...
            switch ($module) {
                case $moduleTypeConf['VM']:
                    ...
                    if($subModuleType  == xphp_get_config('vm', 'VM_SUB_MODULE')['VM'] && $vmflag == false){
                        $taskNum['vmprotect']['num'] = $jobHandler -> getJobList($params)['total'];
                        $vmflag = true;
                    }else if($subModuleType  == xphp_get_config('vm', 'VM_SUB_MODULE')['PUBLIC_CLOUD'] && $publicflag == false){
                        $taskNum['awsprotect']['num'] = $jobHandler -> getJobList($params)['total'];
                        $publicflag =  true;
                    }else if($subModuleType  == xphp_get_config('vm', 'VM_SUB_MODULE')['PRIVATE_CLOUD'] && $privateflag == false){
                         $taskNum['prcloud_protect']['num'] = $jobHandler -> getJobList($params)['total'];
                         $privateflag = true;
                    }
                ...
                case $moduleTypeConf['OS']:
                    ...
                    //这里要区分整机定时和之前旧的
                    if($subModuleType == 0){
                        $taskNum['osbackup']['num'] += 1;
                    }else if($subModuleType == xphp_get_config('module','OS_SUBMODULE_TYPE')['MACHINE_OS']) {
                        $taskNum['complete_machine']['num'] += 1;
                    }
                    break;
    ...
    public function getSystemLisenceInfo()
    {
        ...
        in_array(
            ...
            $info['vmprotect'] = true : $info['vmprotect'] = false;
        //公有云
        in_array(
            'awsprotect',
            $extension['p']
        ) ?
            $info['awsprotect'] = true : $info['awsprotect'] = false;  //公有云
        //私有云
        in_array(
            'prcloud_protect',
            $extension['p']
        ) ?
            $info['prcloud_protect'] = true : $info['prcloud_protect'] = false;  //私有云
        //整机
        in_array(
            'complete_machine',
            $extension['p']
        ) ? $info['complete_machine'] = true : $info['complete_machine'] = false;
        //卷
        in_array(
            'osbackup',
            $extension['p']
        ) ? $info['osbackup'] = true : $info['osbackup'] = false;
        // 文件
        in_array(
            'fileprotect',
            $extension['p']
        ) ?
            $info['fileprotect'] = true : $info['fileprotect'] = false;
        //nas
        in_array(
            'nas_protect',
            $extension['p']
        ) ?
            $info['nas_protect'] = true : $info['nas_protect'] = false;

        in_array(
            'obs_protect',
            $extension['p']
        ) ?
            $info['obs_protect'] = true : $info['obs_protect'] = false; //obs

        in_array(
            'hadoop_protect',
            $extension['p']
        ) ?
            $info['hadoop_protect'] = true : $info['hadoop_protect'] = false; //hadoop

        in_array(
            'office365_protect',
            $extension['p']
        ) ?
            $info['office365_protect'] = true : $info['office365_protect'] = false; // office365
        ...
        in_array(
            ...
            $info['k8s_protect'] = true : $info['k8s_protect'] = false; // k8s


        in_array(
            'db_protect',
            $extension['p']
        ) ? $info['db_protect'] = true : $info['db_protect'] = false;
        ...
        in_array(
            'complete_cdp_backup',
            $extension['p']
        ) ? $info['complete_cdp_backup'] = true : $info['complete_cdp_backup'] = false;

        in_array(
            'vol_cdp_backup',
            $extension['p']
        ) ? $info['vol_cdp_backup'] = true : $info['vol_cdp_backup'] = false;
        ...
        in_array(
            'machine_copy',
            $extension['p']
        ) ? $info['machine_copy'] = true : $info['machine_copy'] = false;

        in_array(
            'vol_cdp_copy',
            $extension['p']
        ) ? $info['vol_cdp_copy'] = true : $info['vol_cdp_copy'] = false;

        in_array(
            'file_copy_protect',
            $extension['p']
        ) ? $info['file_copy_protect'] = true : $info['file_copy_protect'] = false;

        in_array(
            'dbcdpcopy',
            $extension['p']
        ) ? $info['dbcdpcopy'] = true : $info['dbcdpcopy'] = false;
...
WHERE 
    ...
    private function getVmViewInfo()
    {
        ...
        $todayStart = date('Y-m-d') . ' 00:00:00';
        $todayEnd = date('Y-m-d') . ' 23:59:59';
        $sql = 'select sum(total_object_write_size) as size from bd_history_task 
