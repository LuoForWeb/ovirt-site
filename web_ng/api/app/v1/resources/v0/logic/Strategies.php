 ...
 * @LastEditTime: 2026-02-09 14:17:10
 * @Version: 2.0
...
class Strategies extends Base
{
    ...
    public function getStrategyList($params)
    {
        ...
        $length = $params['limit'];
        // 查询策略组
        $sql = "SELECT bsg.strategy_group_uuid, bsg.strategy_group_name, bsg.create_time, bsg.strategy_group_type, bsg.remark, bsg.extra_info, bu.user_name, bsg.user_uuid FROM bd_strategy_group bsg, bd_user bu WHERE bu.user_uuid = bsg.user_uuid ";
        // 查询策略组个数
    ...
    public function insertTimeStrategy($task_uuid, $strategyGroupUUid, $strategyID, $modeType, $strategyInfo)
    {
        ...
        $sql = "INSERT bd_time_strategy ( task_uuid, strategy_group_uuid, strategy_id, mode, strategy_type, days, start_time, roll_flag, roll_interval, roll_end_time, full_backup_compensation_flag) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        $sqlParams = array(
            $task_uuid,
            $strategyGroupUUid,
            $strategyID,
            ...
            !empty($strategyInfo['full_backup_compensation_flag']) ? v1_parse_bool_to_flag($strategyInfo['full_backup_compensation_flag']) : 2,
        );
