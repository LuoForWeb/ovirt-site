...
class Client extends Base
{
    ...
    public function getClientList(array $params = []): array
    {
        ...
        // 操作
        // 1应用配置提取为表头配置了
        $op = [2, 3, 4, 5, 6];  //1 应用配置  2.修改授权 3 刷新客户端 4详情 5日志下载 6代理配置
        ...
        foreach ($clientInfo['data'] as $row) {
            ...
            $rows[] = [
                ...
                'app_des' => $row['app_name'] ?: $nullSpace,
    ...
    private function queryClientList(array $params): array
    {
        ...
        $appTypeDesField = ' CASE ';
        foreach ($allDbTypeDes as $dbType => $dbTypeDes) {
            if ($dbType == 0 || $dbType == 1000) {
                continue;
            }
            $appTypeDesField .= " WHEN app_type = $dbType THEN '$dbTypeDes' ";
        }
        $appTypeDesField .= " ELSE '' END ";

        $appSql = "SELECT *, $appTypeDesField AS app_type_des FROM bd_agent_app ORDER BY app_type ";
        $subSql = "SELECT baa.agent_uuid, GROUP_CONCAT(CONCAT(baa.app_name, '(', baa.app_type_des, ')') SEPARATOR ',\n') AS app_name FROM ($appSql) AS baa GROUP BY baa.agent_uuid ";
        ...
        $ret = [
            ...
            'rows' => array_map(function ($row) use ($clientPackages, $allAgentPackageTypeMap) {
                ...
                return [
                    ...
                    'running_task_flag' => $runningTaskFlag,
                ];
            }, $clientList),
        ];
