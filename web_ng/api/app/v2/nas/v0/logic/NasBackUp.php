...
// use app\v2\resources\v0\logic\Nas;
...
/**
    ...
    public function getNasBackupTree($params = array()): array
    {
        ...
        if(v2_auth_need_check_look()) {
            ...
            $resourceUuidSql = v2_auth_get_source_by_type(xphp_get_config('resource','RESOURCE_TYPE')['NAS'], 'nsr.nas_uuid');
        ...
        if (!empty($data)) {
            foreach ($data as $d) {
                $authflag = $this->checkSystemAuth(1);
                ...
                if (!in_array($d['nas_uuid'], $nas)) {
                    ...
                    $nodes[] = array(
                        ...
                        "authorizationStatus" => $this->checkSystemAuth(1),
                        "checked" => $checked,
                        'snapshotFlag' => $d['snapshot_flag'],
...
// ----------------------------新框架迁移结束--------------------------------------------------
    ...
    //     $pfMsg['permission_operate_flag'] = v2_parse_bool_to_flag($params['highInfo']['permission_operate_flag']); // 是否开启对象存储备份
    //     $pfMsg['skip_file_alarm_flag'] = v2_parse_bool_to_flag($params['highInfo']['skip_file_alarm_flag']); // 是否跳过智能告警
