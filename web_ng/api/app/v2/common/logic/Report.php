...
class Report extends Base
{
     ...
     * 使用子表获取虚拟机备份历史数据
     */
    public function getAllVmHistoryData1($keys = ''): array
    {
        $vmHistoryDataCache = xphp_get_cache($keys);
        ...
        if (empty($vmHistoryDataCache)) {
            ...
            $sql = "SELECT 
                    ...
                    bhtvd.vm_size,
                    bhtvd.vm_valid_size,
                    bhtvd.transport_size,
                    bhtvd.write_size
                FROM 
                    bd_history_task bht
                INNER JOIN bd_history_task_vm_details bhtvd 
                    ON bht.id = bhtvd.history_task_id
                WHERE 
                    bht.module_type = ? 
                    AND bht.task_type = ? 
                    AND bht.error_code = 0";
     ...
     * @return void
     */
    public function getTaskAgentNumber(): int
    {
        $sql = "SELECT 
                    DISTINCT btal.agent_uuid 
                FROM 
                ...
                WHERE 
                    EXISTS (
                        SELECT 1 
                        FROM bd_agent ba 
                        WHERE ba.agent_uuid = btal.agent_uuid 
                        AND ba.agent_type NOT IN (3, 4)
                    )";

        if (v2_auth_need_check_look()) {
            // 三权模式下的操作员只能查看分配存储列表
            // 不是超级管理员也不是全局观察者, 也只能看到分配的资源
            $resourceUuidSql = v2_auth_get_source_by_type(xphp_get_config('resource', 'RESOURCE_TYPE')['CLIENT'], 'btal.agent_uuid');
     ...
     * @return int
     */
    public function getModuleTaskNumber($module)
    {
     ...
     * @return int
     */
    public function getModuleBackupData($moduleType)
    {
     ...
     * @return []
     */
    public function getFileClientInfo()
    {
     ...
     * @return void
     */
    public function getNasClientInfo(): array
    {
        $user = xphp_get_user_info();
        $flag = xphp_get_config('app', 'FLAG');
        $sql = "SELECT nas_status,user_uuid FROM nas_storage_resource";
     ...
     * @return int
     */
    public function pGetTaskNum($module, $subModule = null)
    {
    ...
    {
        $sql = "SELECT sum(completed_size) AS completed_size FROM cdp_db_dr_task_progress_info";
        $data = $this->dbSelect($sql, []);
     ...
     */
    public function pGetProtectData($moduleType, $subModule = null)
    {
        $user = xphp_get_user_info();
    ...
    {
        $writeSize = v2_calsize($size, true);
        $writeInfo = v2_calsize_to_value_and_unit($size, true);
        $info = array(
            'size' => $size,
     ...
     * @return void
     */
    public function getModuleBackupNum($moduleType): int
    {
    ...
    public function vmEmailContent($detailInfo, $message, $alarmInfo, $nodeInfo, $nameInfo)
    {
        ...
        foreach ($detailInfo as $d) {
            ...
            $info = '<tr><td>' . $d['vm_number'] . '</td>' .
                ...
                '<td>' . v2_calsize($d['backup_data'], true) . '</td></tr>';
            $content .= $info;
        }
        ...
        $message = str_replace('backup_data', v2_calsize($backupData, true), $message);
    ...
    public function taskEmailContent($detailInfo, $message, $alarmInfo, $nodeInfo, $nameInfo)
    {
        ...
        $message = str_replace('task_number', $totalCurrentTask, $message);
        $message = str_replace('running_task', $totalTaskNumber, $message);
    ...
    public function storageEmailContent($detailInfo, $message, $alarmInfo, $nodeInfo, $nameInfo)
    {
        ...
        foreach ($detailInfo as $d) {
            ...
            $info = '<tr><td>' . $d['storage_device'] . '</td>' .
                ...
                '<td>' . v2_calsize($d['backup_data'], true) . '</td>' .
                '<td>' . v2_calsize($d['copy_data'], true) . '</td>' .
                '<td>' . v2_calsize($d['archived_data'], true) . '</td></tr>';
            $content .= $info;
        }
        $message = str_replace('display-hide', 'display-show', $message);
        $message = str_replace('tableContent', $content, $message);
        ...
        $message = str_replace('used_storage', v2_calsize($usedStorage, true), $message);
        $message = str_replace('unusedStorage', v2_calsize($unUsedStorage, true), $message);
        $message = str_replace('backupData', v2_calsize($totalBackupData, true), $message);
        $message = str_replace('copyData', v2_calsize($totalCopyData, true), $message);
        $message = str_replace('archivedData', v2_calsize($totalArchiveData, true), $message);
    ...
    public function clientEmailContent($detailInfo, $message, $alarmInfo, $nodeInfo, $nameInfo)
    {
        ...
        $message = str_replace('backup_data', v2_calsize($totalBackupData, true), $message);
    ...
    public function cdpEmailContent($detailInfo, $message, $alarmInfo, $nodeInfo, $nameInfo)
    {
        ...
        $message = str_replace('task_number', $taskNumber, $message);
    ...
    public function appEmailContent($detailInfo, $message, $alarmInfo, $nodeInfo, $nameInfo)
    {
        ...
        $message = str_replace('app_backup', v2_calsize($appBackup, true), $message);
    ...
    public function obEmailContent($detailInfo, $message, $alarmInfo, $nodeInfo, $nameInfo)
    {
        ...
        foreach ($detailInfo as $d) {
            $obTotal += $d['ob_total'];
            $obNormal += $d['ob_normal'];
            $obOffline += $d['ob_offline'];

            $info = '<tr><td>' . $d['ob_total'] . '</td>' .
                '<td>' . $d['ob_normal'] . '</td>' .
                '<td>' . $d['ob_offline'] . '</td></tr>';
            $content .= $info;
        }
        $message = str_replace('display-hide', 'display-show', $message);
        $message = str_replace('#tableContent#', $content, $message);
    ...
    public function k8sEmailContent($detailInfo, $message, $alarmInfo, $nodeInfo, $nameInfo)
    {
        ...
        $backupData = v2_calsize($detailInfo['backupDataSize']['value'], true);
