...
class Base extends Op
{
                ...
                if (in_array($strategyType, [1, 4, 5])) {
                    ...
                    foreach ($speedStrategy['speed'] as $item) {
                        $timeList[] = [
                            'start_time' => $strategyType != 5
                                ? v2_formart_time($item['startTime']) : $item['startTime'],
                            'end_time' => $strategyType != 5 ? v2_formart_time($item['endTime']) : $item['endTime'],
                    ...
                    foreach ($speedStrategy['speed'] as $item) {
                        ...
                        $speedLimitInfos[$days][] = [
                            'start_time' => v2_formart_time($item['startTime']),
                            'end_time' => v2_formart_time($item['endTime']),
                            'speed_limited_value' => $item['value'],
                        ];
                    }
                    $speedLimitInfo = [];
    ...
    public function getSafeConfigStrategy($taskUuid)
    {
        ...
        return [
            'worm_flag' => v2_parse_flag_to_bool($safeConfigData[0]['worm_flag']),
            'worm_protection_time' => intval($safeConfigData[0]['worm_protection_time']),
            'virus_scan_flag' => v2_parse_flag_to_bool($safeConfigData[0]['virus_scan_flag']),
            'virus_scan_config_list' => json_decode($safeConfigData[0]['virus_scan_config_list'], true) ?: [],
            'integrity_check_flag' => v2_parse_flag_to_bool($safeConfigData[0]['integrity_check_flag']),
            ...
            'create_surebackup_after_backup' => v2_parse_flag_to_bool($safeConfigData[0]['create_surebackup_after_backup']),
    ...
    protected function checkAuthByUserUuid(string $userUuid, string $auth)
    {
        ...
        if (v2_auth_need_operation()) {
            ...
            $userUuidArr = v2_auth_get_users($auth, 2);
