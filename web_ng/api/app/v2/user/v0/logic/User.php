...
class User extends Base
{
    ...
    public function addUserStorage(string $userUuid, array $resourceList)
    {
        ...
        $resourceHandler = new \app\v2\resources\v0\logic\Index();
        //关联存储所在节点
    ...
    public function getUserQuotaInfo(): array
    {
        ...
        if ($quota == -1) {
            ...
            $freeSizeDes = v2_calsize($freeSize, true);
            if ($freeSize < 0) {
                $freeSize = 0;
                $freeSizeDes = 0;
            }
            $des = xphp_get_lang('WEB_USERS_QUOTA_TOTAL_SIZE') . ': ' . v2_calsize($quota, true);
