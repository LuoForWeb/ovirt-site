...
class Index extends Base
{
    ...
    public function getUserList($params = []): array
    {
        ...
        if ($num) {
            ...
            foreach ($users as $user) {
                ...
                $backupData = '--';
                $quota = $user['quota'] == -1 ? xphp_get_lang('UI_SETTINGS_AUTH_UNLIMITED') : v2_calSize($user['quota'], true);
                if (!empty($tenantInfo)) {
                    // 租户
                    // 已备份数据
                    $backupData = v2_calSize((new Tenant())->getTenantBackupData($user['tenant_uuid']), true) . '/' . $quota;
                } else {
                    // 非租户
                    $backupData = v2_calSize($this->getUserBackupData($user['user_uuid']), true) . '/' . $quota;
                }
    ...
    public function addUser($params = [])
    {
        ...
        $password = v2_decrypt_js_rsa($params['password']);
        ...
        //修改密码flag
        $editPwdFlag = v2_parse_bool_to_flag($params['force_edit_pass']);
...
SELECT id from mt_user_resource_group where user_uuid in ({$userIdStr})";
    ...
    private function checkNodeByresouce(array $uuid, int $resourceType, int $type, string $userUuid): bool
    {
        ...
        if ($isCheck && !empty($nodeUuid)) {
            // 判断这个节点是否属于用户
            $node = (new \app\v2\resources\v0\logic\Index())->pGetUserAllResource($userUuid, 7);
    ...
    public function delAllocation(array $params): bool
    {
        ...
        if (!empty($tenantUuid)) {
            ...
            if (!empty($this->dbSelect($sqlTenant))) {
                // 表示是租户管理员
                // 获取下面的所有用户
                $userId = (new \app\v2\tenant\v0\logic\Index())->pGetTenantAllUser($tenantUuid);
                if (!empty($userId)) {
    ...
    private function checkDomainUserExist(string $username, string $domainuuid)
    {
        ...
        if (!empty($data)) {
            ...
            $password = $data[0]['password'];
            //解密
            $decrype_password = v2_decrypt($data[0]['password']);
            $port = $data[0]['port'];
            //协议类型 1:ldaps 2:ldap
            ...
            if ($conn) {
                ...
                if ($bd) {
                    ...
                    return [
                        ...
                        'message' => xphp_get_lang('WEB_DOMAIN_SERVER_ADD_USER_PASSWORD_ERROR'),
                        'warning'
                    ];
                }
            } else {
                ...
                return [
                    'success' => false,
                    'title' => xphp_get_lang('WEB_USERS_ADD_USER'),
                    'message' => xphp_get_lang('WEB_DOMAIN_SERVER_ADD_NOT_CONNECT_ERROR'),
                    'warning'
                ];
    ...
    public function userInfoVerify($params)
    {
        ...
        $username = v2_decrypt_js_rsa($params['username']);//收件人用户名-已解密
        $flag = false;
        ...
        if ($userType == 2) {
            ...
            $email = v2_decrypt_js_rsa($params['email']);
            if (empty($data)) {
                $info = [
                    'result' => 2,  //用户名不存在
                ...
                if ($data[0]['email'] == null) {
                    ...
                    if (strcasecmp($email, $userEmail) == 0) {  //发送邮件
                        ...
                        $message = str_replace('backupServerHost', (new \app\v2\common\logic\Report())->getMasterNodeIpLink(), $message);
                        $message = str_replace(
                            'supportEmailHref',
    ...
    public function resetPassWord($params)
    {
        ...
        $token = v2_decrypt($_COOKIE['resetPwdToken']);
...
from bd_user_group bug left join mt_user_group_tenant mugt on bug.user_group_uuid = mugt.user_group_uuid 
    ...
    public function editSelfInfo($params)
    {
        ...
        if (!empty($password)) {
            $setClauses[] = "password = ?";
            $decrypt_password = v2_decrypt_js_rsa($password);
            $MD5_password = md5($decrypt_password);
            $params[] = $MD5_password;
        }
        if (!empty($customePassword)) {
            $setClauses[] = "custome_password = ?";
            $decrypt_custome_password = v2_decrypt_js_rsa($customePassword);
    ...
    public function getUserSelfInfo()
    {
        ...
        if ($data) {
            ...
            $info = array(
                ...
                'user_type_des' => v2_get_user_type_des($data[0]['user_type']),
    ...
    public function getUserPermission()
    {
        ...
        if (!empty(v2_auth_is_admin()['is_admin'])) {
            // 这里判断下是否有全局观察者权限
    ...
    public function editCustomepass($params)
    {
        ...
        $decrypt_custome_password = v2_decrypt_js_rsa($custome_password);
        $MD5_custome_password = md5($decrypt_custome_password);
