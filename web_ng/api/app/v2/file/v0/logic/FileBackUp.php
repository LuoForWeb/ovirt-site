...
use app\v2\s3\v0\logic\ObsBackup;
...
class FileBackUp extends Base
{
    ...
    {
        $result = array();
        $sub_module_type = $params['subModuleType'];
    ...
    {
        $nodes = $group = $agent = [];
        $search = v2_escape_wildcard($params['content']);
        ...
        if (v2_auth_need_check_look()) {
            ...
            $clientUuidSql = v2_auth_get_source_by_type($allResourceType['CLIENT'], 'ba.agent_uuid');
            $sql .= " and $clientUuidSql";
        }
        if(!empty($search)) {
            ...
            foreach ($data as $d) {
                ...
                if (!in_array($d['group_uuid'], $group)) {
                    ...
                    $nodes[] = array(
                        ...
                        'uuid' => $d['group_uuid'],
                        'eventType' => "group",
                        'agentList' => $dataagent[0]['agent_uuid'],
                ...
                if (!in_array($d['agent_uuid'], $agent)) {
                    ...
                    $nodes[] = array(
                        ...
                        'osType' => $d['os_type'],
                        "chkDisabled" => $chk,
                        'netModel' => intval($d['net_model']),
                        'isCheckFlag' => $chk,   //是否可以被选中
    ...
    private function getFileDir(array $params): array
    {
     ...
     /**
     * 文件备份-检测对象上有没有对应的路径（用于应用到其他对象和修改任务）
     * @return json
     */
    public function checkOtherPath($params)
    {
        $groupUuid = $params['group_uuid'];
        $agentUuid = $params['object_uuid'];
        ...
        if (!empty($applyPathList)) {
            ...
            $sql = "select path_name, path_type, code_type from fs_path_list where agent_uuid = ? and task_uuid = ? ";
        ...
        $item_list = array();
        $agentInfo = $this->getAgentName($agentUuid)[0];
        foreach ($data as $d) {
            $item_list[] = array(
                "item_type" => $d['path_type'],
                "item_path" => $d['path_name'],
                "code_type" => $d['code_type']
            );
        }
        $msg = array(
            ...
            "submodule_type" => "1",
            "object_name" => $agentInfo['agent_name'],
            "object_ip" => $agentInfo['agent_ip'],
            "item_list" => $item_list
        );
        $opName = 'FS_OP_TYPE_BACKUP_SEARCH_FILE_STAT';
        $nodeuuid = (new Node())->getLocalNodeUUID();
        ...
        if ($mbResult['result']) {
            ...
            if (!empty($mbResult['msg']['item_list'])) {
                foreach ($mbResult['msg']['item_list'] as $item) {
                    if ($item['item_type'] > 0) {
                        $info['file_nodes'][] = array(
                            "path_name" => $item['item_path'],
                            "path_type" => $item['item_type'],
                            "code_type" => $item['item_code_type'],
                            'file_size' => $item['file_size'],
                            'unix_create_time' => $item['unix_create_time'],
                            'unix_modify_time' => $item['unix_modify_time'],
                            'group_uuid' => $groupUuid,
                            'uuid' => $agentUuid,
                        ); 
                    } else {//有跳过的
                        $info['pass_flag'] = true;
                        $info['pass_nodes'][] = array(
                            "path_name" => $item['item_path'],
                            "path_type" => $item['item_type'],
                            "code_type" => $item['item_code_type'],
                            'file_size' => $item['file_size'],
            ...
            //失败
            exit($this->muOpResult(false, xphp_get_lang('UI_FILE_SRC_CHECK'), '', 'warning', $mbResult['msg']['error_code']));
        }
    }
     ...
     */
    public function getAgentName($agent_uuid)
    {
        $agentInfo = array();
        $sql = "select agent_name, ip, hostname from bd_agent where agent_uuid = ?";
        $data = $this->dbSelect($sql, array($agent_uuid));
        if (!empty($data[0]['agent_name']) && $data[0]['agent_name'] != $data[0]['ip']) {
     ...
     * @return array
     */
    public function createBackupSearchJob(array $params): array
    {
        $opName = 'FS_OP_TYPE_BACKUP_SEARCH_THREAD_CREATE';
        $msg = $params['info'];
        $nodeuuid = (new Node())->getLocalNodeUUID();
        $mbResult = $this->service()->mbFSMsgs($nodeuuid, $opName, json_encode($msg), true, true);
        if ($mbResult['result']) {
            if ($mbResult['msg']["result_type"] == 1) {//绝对路径访问，直接返回文件列表
                $filenode = array();
                if (!empty($mbResult['msg']['item_list'])) {
                    foreach ($mbResult['msg']['item_list'] as $file) {
                        if ($file['item_type'] == 1) {
                            // 文件
                            $titleDes = xphp_get_lang('WEB_FILE_FILE_SIZE') . v1_calsize($file['file_size'], true)
                                . PHP_EOL . xphp_get_lang('WEB_FILE_MODIFY_TIME') . $this->parseDate($file['unix_modify_time']);
                        } else {
                            $titleDes = xphp_get_lang('WEB_FILE_MODIFY_TIME') . $this->parseDate($file['unix_modify_time']);
                        }
                        $filenode[] = array(
                            'pid' => 0,
                            'id' => $file['item_path'],
                            ...
                            'modify_time' => $file['unix_modify_time'],
                            'create_time' => $file['unix_create_time'],
                            'file_offset' => $file['offset'],
                            'icon' => $file['item_type'] == 2 ? './img/fs/wenjianjia.png' : './img/fs/wenjian.png',
                            'iconOpen' => $file['item_type'] == 2 ? './img/fs/wenjianjiaopen.png' : './img/fs/wenjian.png',
                            'iconClose' => $file['item_type'] == 2 ? './img/fs/wenjianjia.png' : './img/fs/wenjian.png',
                            'is_parent' => $file['item_type'] == 2 || $file['item_type'] == 3, //1文件  2文件夹
                            'isParent' => $file['item_type'] == 2 || $file['item_type'] == 3, //1文件  2文件夹
                            'path' => $file['item_path'],
                            'search_node' => true,
                            'uuid' => $params['info']['object_uuid'],
                            'group_uuid' => $params['group_uuid'],
                            'file_path' => $file['item_path'],
                            'groupuuid' => $params['group_uuid'],
                            'code_type' => $file['item_code_type'],
                        );
                    }
                }
                $info = [
                    ...
                    'msg' => array(
                        'file_nodes' => $filenode,
                        'result_type' => $mbResult['msg']["result_type"]
                    ),
                ];
            } else { //result_type为0，搜索
                $info = [
                    'result' => true,
                    'msg' => array(
     ...
     * @return array
     */
    public function stopBackupSearchJob(array $params): array
    {
        $opName = 'FS_OP_TYPE_BACKUP_SEARCH_STOP';
        $nodeuuid = (new Node())->getLocalNodeUUID();
        $mbResult = $this->service()->mbFSMsgs($nodeuuid, $opName, json_encode($params['info']));
        if ($mbResult['result']) {
            $info = [
                'result' => true,
                'msg' => xphp_get_lang('WEB_FS_OP_TYPE_SEARCH_THREAD_CREATE'),
     ...
     * @return array|string
     */
    public function getBackupSearchInfo(array $params)
    {
        $opName = 'FS_OP_TYPE_BACKUP_SEARCH_RESULT_GET';
        $nodeuuid = (new Node())->getLocalNodeUUID();
        $mbResult = $this->service()->mbFSMsgs($nodeuuid, $opName, json_encode($params['info']), true, false);
        $operate = xphp_get_lang('WEB_FS_OP_TYPE_SEARCH_RESULT_GET');
        $result = $mbResult['result'];
        //返回结果到UI
        if (!$result) {
            //失败
            return $this->muOpResult($result, $operate, '', '', $mbResult['error_code']);
        }
        $result = $mbResult['msg'];
        $filenode = array();
        if (intval($result['c_take_num']) > 0) {
        ...
        return [
            ...
            'success_num' => $result['success_num'],//搜索数据中匹配成功的个数
            'current_dir_num' => $result['dir_num'],
            'current_file_num' => $result['file_num'],
            'offset' => $result['offset'],
            'filenode' => $filenode,
            'search_num' => $result['search_num'],
        ];
