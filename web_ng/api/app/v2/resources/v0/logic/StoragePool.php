...
class StoragePool extends Base
{
    ...
    public function getStoragePoolListByStorageUuid(array $storageUuidList): array
    {
        ...
        // 资源拥有过滤
        $includeStoragePoolUuidList = $this->getIncludeStoragePoolUuidListByResource();
        if (is_array($includeStoragePoolUuidList)) {
            // 这里通过存储查资源池，需要用IN操作查询
            $includeStoragePoolUuids = "'" . implode("', '", $includeStoragePoolUuidList) . "'";
            $sql .= " AND bsrp.storage_pool_uuid IN ($includeStoragePoolUuids) ";
        }
    ...
    private function getExcludeStoragePoolUuidListByResource(): array
    {
         ...
         * 修改方案:
         * 1、存储管理通过授权，不让用户查看
         * 2、创建备份任务的备份目标仅允许查看当前用户拥有的资源
         *   a.如果资源池里面的资源全是当前用户拥有的，那么就显示改资源池;但如果存在一个资源是当前用户没有拥有的，那么就不显示
         *   b.当前仅修改备份存储和备份节点，代理和网络后面有需求再修改
         */
        $excludeStoragePoolUuidList = [];
        if (v2_auth_need_check_look()) {
            // 获取拥有的存储资源
            $resourceUuidSql = v2_auth_get_source_by_type(xphp_get_config('resource', 'RESOURCE_TYPE')['STORAGE']);
            // 查询不包含这些存储的资源池
    ...
    /**
     * 根据资源分配获取包含的存储资源池列表
     * @function 用于存储查看存储池时使用
     * @return array|boolean
     */
    private function getIncludeStoragePoolUuidListByResource()
    {
        $includeStoragePoolUuidList = true;
        if (v2_auth_need_check_look()) {
            // 获取拥有的存储资源
            $resourceUuidSql = v2_auth_get_source_by_type(xphp_get_config('resource', 'RESOURCE_TYPE')['STORAGE']);
            // 查询包含这些存储的资源池
            $sql = "SELECT storage_pool_uuid FROM bd_storage_resource_pool_list WHERE storage_uuid IN ($resourceUuidSql) GROUP BY storage_pool_uuid ";
            $includeData = $this->dbSelect($sql);
            if (!is_array($includeData)) {
                $includeData = [];
            }
            $includeStoragePoolUuidList = array_values(array_unique(array_column($includeData, 'storage_pool_uuid')));
        }
        return $includeStoragePoolUuidList;
    }
            ...
            foreach ($storageData as $storageInfo) {
                if ($storageInfo['storage_pool_uuid'] == $storagePoolItem['storage_pool_uuid']) {
                    ...
                    if ($storageInfo['storage_type'] == $allStorageType['REMOTE']) {
                        $nodeStatus = v2_parse_flag_to_bool(intval($storageConfig['remote_status']));
                    }
                    ...
                    if ($allStorageWormType['SIZE'] == $storageInfo['worm_allocate_type']) {
                        //如果是按照大小来告警
                        $wormAllocateValue = v2_calsize($storageInfo['worm_allocate_value']);
                    }
                    $rows[$storagePoolItem['storage_pool_uuid']]['storage_list'][] = [
                        ...
                        'worm' => [
                            'worm_flag' => v2_parse_flag_to_bool($storageInfo['worm_flag']),
