...
namespace app\v2\resources\v0\logic;

use app\v2\backupData\v0\logic\DataManage;
use app\v2\cluster\v0\logic\Cluster;
use app\v2\common\logic\Base;
use app\v2\opcode\NodeOpcode;
use app\v2\opcode\OrchestrationOpcode;
...
class Node extends Base
{
        ...
        foreach ($data as $row) {
            if (!isset($nodeResourceLimitMap[$row['node_uuid']])) {
                $nodeResourceLimitMap[$row['node_uuid']] = [
                    'node_config_flag' => v2_parse_flag_to_bool($row['node_config_flag']),
            ...
            $tmpTime = [
                'start_timestamp' => v2_time_to_sec($row['prohibit_start_time']),
                'end_timestamp' => v2_time_to_sec($row['prohibit_end_time']),
    ...
    private function queryNodeInfo(array $params): array
    {
        ...
        if (v2_auth_need_check_look()) {
            $resourceUuidSql = v2_auth_get_source_by_type(xphp_get_config('resource', 'RESOURCE_TYPE')['NODE'], 'bn.node_uuid');
        ...
        if (v2_auth_need_check_look()) {
            ...
            $resourceUuidSql = v2_auth_get_source_by_type(
    ...
    public function buildNodeStorageInfo(array &$storageList, array $storageInfo)
    {
        if ($storageInfo['storage_uuid']) {
            $storageList[$storageInfo['storage_uuid']] = [
                ...
                'lan_free_flag' => v2_parse_flag_to_bool($storageInfo['lan_free_flag']),
            ];
        }
    }
    ...
    private function getNodeSourceUuidByType(int $type = 1): array
    {
        ...
        $resourceInfo = (new \app\v2\resources\v0\logic\Index())->pGetUserAllResource(
            $user['userUuid'],
            $resourceType['NODE']
        );
     ...
     * 获取节点的状态
     * @param string $nodeUuid 节点uuid
     * @link \app\v2\cluster\v0\logic\Cluster::getClusterConfig()
    ...
    public function addNode($params = [])
    {
        ...
        $addResult = $this->service()->nodeOperateService(
            ...
            v2_parse_bool_to_flag($params['force_flag'] ?? false),
            '',
    ...
    /**
     * 得到本地节点UUID
     * @see \app\v2\common\service\Base::getLocalNodeUuid() 用于获取后台服务使用
     * @return string
     */
    ...
    public function getAutoFindNode()
    {
        ...
        $allUsedNodeFitler = v2_array_sort($allUsedNodeFitler, 'num', 'asc', 0, -1);
    ...
    public function getAddStorageNodeSelect()
    {
        ...
        $softwareType = (new \app\v2\system\v0\logic\Index())->getSoftwareType();
        $list = array();
        $resourceList = array();
    ...
    public function getBackupStorageList(array $params): array
    {
        ...
        if (v2_auth_need_check_look()) {
            $resourceUuidSql = v2_auth_get_source_by_type(xphp_get_config('resource', 'RESOURCE_TYPE')['STORAGE'], 'storage_uuid');
            $sql .= " AND ($resourceUuidSql) ";
        }
        if ($copyFlag) {
            // 副本目的存储
        ...
        foreach ($data as $d) {
            ...
            if (empty($loginUser['tenantuuid'])) {
                $storageDes .= '(' . $this->getStorageTypeDes($d['storage_type']) . ', ' .
                    xphp_get_lang('UI_STORAGE_TOTAL_SIZE') . ':' . v2_calsize($d['total_size'], true)
                    . ', ' . xphp_get_lang('WEB_PLATFORM_DC_AVAILABLE_SPACE')
                    . ':' . v2_calsize($d['free_size'], true) . ')';
            }
            $storageDes .= $name;
            ...
            $info[] = array(
                ...
                'worm_flag' => v2_parse_flag_to_bool($d['worm_flag']),
            );
        }
    ...
    public function getNodeCache(string $nodesUuid): array
    {
        ...
        if (is_array($nodeCacheInfo) && $nodeCacheInfo) {
            $storageList = $this->getStorageInfoByNodeUuid($nodesUuid, [$allStorageType['CLOUD'], $allStorageType['TAPE']]);
            $storageUuidList = array_column($storageList, 'storage_uuid');
            $storageHandler = new Storage();
            $storageStatusList = $storageHandler->batchGetStorageStatus($storageUuidList);
            $storageStatusMap = [];
            foreach ($storageStatusList as $storageStatusInfo) {
                $storageStatusMap[$storageStatusInfo['storage_uuid']] = $storageStatusInfo;
            }
            foreach ($storageList as $index => $storageInfo) {
                if (isset($storageStatusMap[$storageInfo['storage_uuid']])) {
                    $storageList[$index]['storage_online_flag'] = $storageStatusMap[$storageInfo['storage_uuid']]['storage_online_flag'];
                } else {
                    $storageList[$index]['storage_online_flag'] = false;
                }
            }
            $retCacheInfo = [
                ...
                'warning_flag' => v2_parse_flag_to_bool($nodeCacheInfo[0]['warning_flag']),
                'warning_value' => intval($nodeCacheInfo[0]['warning_value']),
                'disk_storage_resource' => $storageList,
            ];
        }
     ...
     * @return array
     */
    public function batchGetStorageInfoByNodeUuidList(array $nodeUuidList): array
    {
        $nodeUuidList = array_values(array_unique($nodeUuidList));
        $nodeUuids = "'" . implode("', '", $nodeUuidList) . "'";
        $nodeStorageMap = [];
        ...
        $sql = "SELECT node_uuid, storage_uuid, storage_nickname, mount_point, status AS storage_status,
                    total_size, free_size, storage_type, lan_free_flag
                FROM bd_storage_resource 
                WHERE node_uuid IN ($nodeUuids) AND storage_type NOT IN ($notUseType)
                    AND storage_type != ? AND storage_type != ? AND lan_free_flag = ?
                    AND use_mode != ? AND source_type = ? ";
        $storageList = $this->dbSelect(
            $sql,
            [$allStorageType['NFS'], $allStorageType['CIFS'], $flag['UNSET'], $allUseMode['READ_ONLY'], $allStorageCategory['BACKUP']]
        );
        if (!is_array($storageList)) {
            $storageList = [];
        }
        foreach ($storageList as $storageInfo) {
            if (!isset($nodeStorageMap[$storageInfo['node_uuid']])) {
                $nodeStorageMap[$storageInfo['node_uuid']] = [];
            }
            $this->buildNodeStorageInfo($nodeStorageMap[$storageInfo['node_uuid']], $storageInfo);
        }

        // 获取节点的共享存储信息
        $sql = "SELECT bssnl.node_uuid, bsr.storage_uuid, bsr.storage_nickname, bssnl.mount_point, bssnl.status AS storage_status,
                    bsr.total_size, bsr.free_size, bsr.storage_type, bsr.lan_free_flag
                FROM bd_shared_storage_node_layout bssnl
                    INNER JOIN bd_storage_resource bsr ON bssnl.storage_uuid = bsr.storage_uuid
                WHERE bssnl.node_uuid IN ($nodeUuids) AND bsr.lan_free_flag = ?
                    AND bsr.use_mode != ? AND bsr.storage_type NOT IN ($notUseType) AND source_type = ? ";
        $shareStorageList = $this->dbSelect($sql, [$flag['UNSET'], $allUseMode['READ_ONLY'], $allStorageCategory['BACKUP']]);
        if (!is_array($shareStorageList)) {
            $shareStorageList = [];
        }
        foreach ($shareStorageList as $shareStorageInfo) {
            if (!isset($nodeStorageMap[$shareStorageInfo['node_uuid']])) {
                $nodeStorageMap[$shareStorageInfo['node_uuid']] = [];
            }
            $this->buildNodeStorageInfo($nodeStorageMap[$shareStorageInfo['node_uuid']], $shareStorageInfo);
        }
    ...
    /**
     * 检查备份节点列表
     * @link \app\v2\resources\v0\logic\NodePool::addNodePool()
     * @link \app\v2\resources\v0\logic\NodePool::editNodePool()
     * @link \app\v2\resources\v0\logic\NodePool::batchDeleteNodePool()
    ...
    {
        $allProhibitTimeType = xphp_get_config('node', 'PROHIBIT_TIME_TYPE', 'resources');
        $nodeConfigFlag = v2_parse_bool_to_flag($params['node_config_flag']);
        ...
        foreach ($params['prohibit_time_info'] ?? [] as $prohibitTime) {
            ...
            foreach ($prohibitTime['time_list'] as $time) {
                $startTime = v2_sec_to_time($time['start_time']);
                $endTime = v2_sec_to_time($time['end_time']);
    ...
    private function buildResourcesLimit(string $nodeUuid, array $resourcesLimitList, array $nodeData): array
    {
        ...
        $resourceLimit['node_config_flag'] = v2_parse_flag_to_bool($currentNodeResourcesLimitData[0]['node_config_flag']);
        ...
        foreach ($currentNodeResourcesLimitData as $item) {
            ...
            $tmpTime = [
                'start_timestamp' => v2_time_to_sec($item['prohibit_start_time']),
                'end_timestamp' => v2_time_to_sec($item['prohibit_end_time']),
    ...
    /**
     * 检查节点网络列表
     * @link \app\v2\resources\v0\logic\NetworkPool::addNetworkPool()
