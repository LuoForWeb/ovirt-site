...
use app\v2\recovery\v0\logic\InstantaneousData;
use app\v2\resources\v0\logic\Node;
use app\v2\user\v0\logic\User as UserHandler;
use app\v2\tenant\v0\logic\Index as TenantHandler;
use app\v2\job\v0\logic\JobInfo as JobInfoHandler;
use app\v2\resources\v0\logic\Storage as StorageHandler;
    ...
    public function getRestoreDataVmTree(array $params)
    {
        ...
        foreach ($data as $d) {
            ...
            if (!in_array($hypervisorType . $taskUuid, $task) && ($_rows && !$jobUuid || !$_rows)) {
                ...
                $thisTaskData = v2_array_sort($thisTaskData, 'task_create_time', 'desc', 0, -1);
        ...
        if (!empty($params['instant_flag'])) {
            ...
            $node = v2_unique_by_two_keys_manual($node, 'id', 'pId');
        }
        // 通过数组分页实现加载更多
...
//        $vmCheck = $params['vm_check'];
        ...
        if (v2_auth_need_check_look()) {
            ...
            $resourceUuidSql = v2_auth_get_users(VmPlatform::instance()->getUserAuthKeyBySubModule($subModule));
        ...
        if ($tableFlag) {
            ...
            foreach ($pointData as $d) {
                $mark = $this->pGetTimepointMark(
                    v2_parse_flag_to_bool($d['weekly_flag']),
                    v2_parse_flag_to_bool($d['monthly_flag']),
                    v2_parse_flag_to_bool($d['yearly_flag']),
                    v2_parse_flag_to_bool($d['importance_flag'])
                );
                $remark = '';   //备注
                $op = array(1, 2, 3);
                //公有云的时间点在云存储无法删除
...
//                    . $i++ . '</span>';
                $records['rows'][] = array(
                    ...
                    'data_size' => v2_calsize($d['total_size'], true),
                    'write_size' => v2_calsize($d['write_size'], true),
                    ...
                    'detail' => array(
                        ...
                        'vm_uuid' => $vmUuid,
                        'task_uuid' => $taskUuid,
                        'remote_flag' => !v2_parse_flag_to_bool(intval($d['data_local_flag'])),
                        'weekly_flag' => v2_parse_flag_to_bool($d['weekly_flag']),
                        'monthly_flag' => v2_parse_flag_to_bool($d['monthly_flag']),
                        'yearly_flag' => v2_parse_flag_to_bool($d['yearly_flag']),
                        'importance_flag' => v2_parse_flag_to_bool($d['importance_flag']),
                        'remark' => $d['remarks'],
...
//                }
            ...
            if ($manageFlag) {
                $mark = $this->pGetTimepointMark(
                    v2_parse_flag_to_bool($point['weekly_flag']),
                    v2_parse_flag_to_bool($point['monthly_flag']),
                    v2_parse_flag_to_bool($point['yearly_flag']),
                    v2_parse_flag_to_bool($point['importance_flag'])
                );
            } else {
                $mark = '';
            }
            ...
            //标记列表
            $markList = array(
                'weekly_flag' => v2_parse_flag_to_bool($point['weekly_flag']),
                'monthly_flag' => v2_parse_flag_to_bool($point['monthly_flag']),
                'yearly_flag' => v2_parse_flag_to_bool($point['yearly_flag']),
                'importance_flag' => v2_parse_flag_to_bool($point['importance_flag']),
            );
...
//                }
            ...
            if ($point['backup_mode'] == $backupModes['FULL']) {
                if (!in_array($timepointUuid, $timepoint)) {
                    ...
                    $node[] = array(
                        ...
                        'integrity_check_flag' => v2_parse_flag_to_bool($point['integrity_check_flag']),
    ...
    public function deleteRestoreData(array $params): string
    {
        ...
        $timepointList = v2_array_sort($timepointList, 'node_uuid', '', 0, -1);
    ...
    public function addRestorePointsRemark(array $params): array
    {
        ...
        $remark = v2_remove_escape($params['remark']);
     ...
     * @return string
     */
    public function getBackupTimepointTreeTitle($dirPath, $remarks): string
    {
        $title = xphp_get_lang('WEB_PLATFORM_DC_VM_SRC_DIR_PATH') . ': ' . $dirPath;
        if (!empty($remarks)) {
