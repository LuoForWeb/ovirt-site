 ...
 * @param bool   $flag  是否读取老版本的权限配置
 * @return array
 */
function xphp_get_menu($name = '', $force = false, $flag = false)
{
    // 强制读取老版本的page配置
    $oldpath = xphp_get_oldpath();
    if ($oldpath) {
        // 读取老框架的配置
        return getConfig($oldpath . 'api/xphp/conf/page.php');
    }
...
function xphp_get_lang($key = '', $module = '', $force = false)
{
    ...
    if (empty($cache) || $force) {
        ...
        if (empty($language) || $force) {
            ...
            if ($oldpath) {
                // 读取老框架的配置
                $language = getConfig($oldpath . 'lang/' . $langtype . '.php');
            } else {
                // 读取出系统公共的语言包
