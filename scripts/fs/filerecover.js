var FileRecover = function () {
	...
	let _agentName, _agentIP, _pointTaskName, _pointName, _path = '', _pathArr = [];
	...
	const searchTimeRange = function () {
		...
		let p = JSON.stringify({storage_uuid: storage_uuid});
		$.post(CONF.AJAXPATH, {m:CONF.M.FILE,f:'getFileDataTree',p: p}, function (d) {
			...
			Metronic.blockUI({target:'#agent_point_tree',animate: true});
			$.post(CONF.AJAXPATH, {m:CONF.M.FILE,f:'searchFsTimepoint',p:data}, function(d) {
				Metronic.unblockUI('#agent_point_tree');
				let pointNode = JSON.parse(d);
				let showNode = [];
				if(pointNode) {
...
//			console.log('New date range selected: ' + start.format('YYYY-MM-DD HH:mm:ss') + ' to ' + end.format('YYYY-MM-DD HH:mm:ss') + ' predefined range: ' + label);
	...
	const setBakSonTree = function(treeId,treeNode,expendFlag) {
		...
		let div = "#recoverFileTree";
		Metronic.blockUI({target: div,animate: true});
		$.ajax({ 
			type: "post", 
	        url: CONF.AJAXPATH, 
	        async:true, 
	        data:{m:CONF.M.FILE,f:'getBackupFileDir',p:params},
	...
	const setBackupFileTree = function(d, treeNode) {
		...
		arr.filelist.push({
			"pId": 0,
			"name": treeNode.agent_name + '('+ treeNode.agent_ip +')',
			"title": treeNode.agent_name + '('+ treeNode.agent_ip +')',
			"isParent": true,
			"open": true,
			"icon": treeNode.ostype == "Windows" ? "./img/os/Windows.png" : "./img/os/Linux.png",
			"type": "0",
		});
		...
		zTreeFile = $.fn.zTree.init($("#recoverFileTree"), setting, arr['filelist']);
	...
	const getRecoverDir = function (treeNode) {
		...
		let div = "#recoverFileTree";
		Metronic.blockUI({target: div,animate: true});
		$.post(CONF.AJAXPATH, {m:CONF.M.FILE,f:'getBackupFileDir',p:params}, function(data){
			$('#step1tips').hide();
			$('#backupfilediv').show();
			$('#filelistdiv').show();
	...
	const setPointTree = function(zNodes) {
		...
		timepointTree = $.fn.zTree.init($("#agent_point_tree"), setting, JSON.parse(zNodes));
	...
	const initPointTree = function(type) {
		...
		data.storage_uuid = $('#storageSelect').val();
		data = JSON.stringify(data);
		$.post(CONF.AJAXPATH, {m:CONF.M.FILE,f:'getFileDataTree',p:data}, setPointTree);
	};
	...
	const startSearch = function () {
		...
		if(inputname == "start") {
			...
			params.info.module_type = 3;//文件模块类型
			params = JSON.stringify(params);
	...
	const showStep1 = function(){
		//设置任务名
		if($.trim($('#jobname').val()) == "") {
			$.post(CONF.AJAXPATH, {m:CONF.M.FILE,f:'getFileRecoverTaskName',p:{}}, function(data){
				$('#jobname').val(data);
			});
		}
		...
		$('#filelist').text(showStr).css('white-space', 'pre-line');
		if (storage_type == 10) {
			//磁带只支持单线程，不显示线程配置
	...
	const hostOnCheck = function(e, treeId, treeNode){
		if(treeNode.chkDisabled) return;
		if (treeNode.event_type === 'agent' && treeNode.os_type === ajaxData.pointInfo.ostype) {// 两者文件客户端操作系统类型一致时才显示恢复到原路径
	 ...
	 * @param {*} keyword 搜索关键词
	 */
	const initHostTree = function(keyword) {
	...
	const step2Valid = function(){
		...
		// 3.表单确认回显
		$('.permission-handle-li').show();//显示权限
		$('.transferLi').hide();
        switch (deviceClient) {
            ...
            case STORAGE_DEVICE_TYPE.NAS: // 恢复到NAS
                ...
                $('.permission-handle-li').hide();//隐藏权限
		...
		$('.recover2agent').text(agentShowInfo);
        //恢复路径
        $('.recover2path').text(pathShowInfo);
		...
		$('#completeConfig').completeStrategyCovery(CONF.MODULE_TYPE.FS, 0 , !integrity_check_flag );
	...
	const getEachStrategy = function(strategy){
		let desEach = '';
	...
	const getStrategyDays = function(days){
		let desDays = '';
	...
	const getStrategyWeek = function(days){
		let desDays = '';
		$.each(days, function(i,d){
			if(1 == d){
				desDays += CONF.WEEK[i] + ", ";
	...
	const getStrategyDes = function(strategy){
		//每天12:12:12开始,不滚动
	...
	const confirmRecoverForm = () => {
		...
		if(CONF.FUNCTIONS.includes('multithread')){
			transferHtml += $('.threadnumlabel').html() + ": " + ajaxData.thread_num + '<br>';
		}
		$('.transferDiv').hide();
		//重试策略
	...
	const step3Valid = function(){
		...
		ajaxData.typeInfo.type = $('#recovertype').val();//恢复方式
	...
	const submit = function(){
		...
		Metronic.blockUI({target: '#filerecovercontent',animate: true, cenrerY: true,});
		...
		pAjaxRequest(ajaxData, api, 'POST', (result) => {
            ...
            Metronic.unblockUI('#filerecovercontent');
        })
	}
	...
	const wizardInit = function(){
        ...
        let handleTitle = function(tab, navigation, index) {
            ...
            let current = index + 1;
            jQuery('li', $('#filerecovercontent')).removeClass("done");
            ...
            if (current == 1) {
                $('#filerecovercontent').find('.button-previous').css('visibility', 'hidden');
                $('#filerecovercontent').find('.button-next').addClass('next-btn-margin-left');
            } else {
                $('#filerecovercontent').find('.button-previous').css('visibility', 'visible');
                $('#filerecovercontent').find('.button-next').removeClass('next-btn-margin-left');
            }

            if (current >= total) {
                $('#filerecovercontent').find('.button-next').hide();
                $('#filerecovercontent').find('.button-submit').css('visibility', 'visible');
            } else {
                $('#filerecovercontent').find('.button-next').show();
                $('#filerecovercontent').find('.button-submit').css('visibility', 'hidden');
            }
            Metronic.scrollTo($('.page-title'));
        }

        // default form wizard
        $('#filerecovercontent').bootstrapWizard({
            ...
            onTabShow: function (tab, navigation, index) {
                ...
                $('#filerecovercontent').find('.progress-bar').css({
                    width: $percent + '%'
                });
            }
        });
        ...
        $('#filerecovercontent').find('.button-previous').css('visibility', 'hidden');
        $('#filerecovercontent .button-submit').click(submit).css('visibility', 'hidden');
	};
	...
	const initListener = function(){
		...
		initFileClientTree(); // 初始化文件客户端树

		// 选择恢复客户端 select change
