var FileCDPEdit = function () {
	...
	var SETTINGS, setFileTypeGridOldDataFlag = false;	//旧的配置信息
...
//        					onCheck: pathOnCheck,
	...
	//检查之前选择的节点是否是根节点
	var oldNodeIfRootNode = function(eachFileInfo, allNode){
		var data = {flag:false, node:null};
		for(var i=0; i<allNode.length; i++){
			if(allNode[i].dir == eachFileInfo.path){
				data.flag = true;
				data.node = allNode[i];
				break;
			}
	...
	var groupOldTree = function(){
		var allNode = fileTree.getNodes();		//所有根节点
		var rootNode = [];
		for(var i=0; i<allNode.length; i++){
			var node = {};
			node.dir = allNode[i].dir;
			node.id = allNode[i].id;
			rootNode.push(node);
		}
		var fileInfo = SETTINGS.producthostInfo.fileInfo;	//之前所有选择的节点
		
		Metronic.blockUI({target: '#file_tree',animate: true});
		//遍历之前所有的节点,一层一层展开树(如果没有找到?继续加载),选择指定节点
		for(var i=0; i<fileInfo.length; i++){
			//如果是根节点,直接选择,如果不是根节点,继续查找
			var thisData = oldNodeIfRootNode(fileInfo[i], allNode)
			if(thisData.flag){
				//如果是根节点,直接选择
				fileTree.checkNode(thisData.node, true, false, true);
			}else{
				loadEachCheckNode(SETTINGS.producthostInfo.hostuuid, rootNode, fileInfo[i].path);
			}
		}
		Metronic.unblockUI('#file_tree');
	}
	
	//加载每一个node,每次请求的是从根到所选节点
	var loadEachCheckNode = function(hostuuid, rootNodes, checkPath){
//		debugger;
		var p = {hostuuid:hostuuid, rootNodes:rootNodes, checkPath:checkPath, limit:20};
		p = JSON.stringify(p);
		$.ajax({ 
			type: "post", 
	        url: CONF.AJAXPATH, 
	        async:false, 	//同步获取
	        data:{m:CONF.M.FILECDP,f:'getEachCheckNodeTreeChain',p:p},
	        success: function(data){ 
	        	var result = JSON.parse(data);
//	        	debugger;
	        	for(var i=0; i<result.nodes.length; i++){
	        		var fnode = fileTree.getNodeByParam("id", result.nodes[i].id, null);
	        		if(null == fnode){
	        			//如果这个节点不存在,添加这个节点到父节点下面
	        			var node = fileTree.getNodeByParam("id", result.nodes[i].pid, null);
	        			fileTree.addNodes(node, result.nodes[i], true);
	        			//如果这个节点checked为true,表示这个节点也应该被选中
	        			if(result.nodes[i].checked){
	        				var checkNode = fileTree.getNodeByParam("id", result.nodes[i].id, null);
		        			fileTree.checkNode(checkNode, true, true, true);
	        			}
	        		}else{
	        			//如果这个节点已经加载了
	        			if(fnode.more){
	        				//如果找到的这个节点是更多,就移除这个节点,避免更多出现在中间,移除后再重新插入
	        				fileTree.removeNode(fnode);
	        				var node = fileTree.getNodeByParam("id", result.nodes[i].pid, null);
		        			fileTree.addNodes(node, result.nodes[i], true);
	        			}else{
	        				//如果这个节点不是更多,并且这个节点checked为true,表示这个节点也应该被选中
	        				if(result.nodes[i].checked){
	        					fileTree.checkNode(fnode, true, true, true);
	        				}
	        			}
...
//			
	...
	var initRansomwareTable = function(){
		...
		if(!initFileTypeGridFlag){
			...
			fileTypeGrid.init({src: $("#ransomwaretable"),  dataTable:dataTableOpt, onDataLoad:setFileTypeGridOldData});
	...
	var setFileTypeGridOldData = function(){
		if(setFileTypeGridOldDataFlag) return true;
		var ransomware = SETTINGS.highInfo.ransomware;
		if("2" == ransomware.ransomwaretype){
			//如果是指定文件类型备份,设置上次选择的内容
			var selectTypes = ransomware.selectTypes;
			var tabledata = fileTypeGrid.getDataTable().data();
			for(var i=0; i<tabledata.length; i++){
				for(var j=0; j<selectTypes.length; j++){
					if(tabledata[i][1] == selectTypes[j].extname){
						var inputStr = "input[data-name=" + tabledata[i][4] + "]";
						var thisInput = $('#ransomwaretable').find(inputStr);
		...
		setFileTypeGridOldDataFlag = true;
	}
...
//            $('.step-title', $('#vmbackupcontent')).text('Step ' + (index + 1) + ' of ' + total);
	...
	var submit = function(){
    	...
    	$.post(CONF.AJAXPATH, {m:CONF.M.FILECDP,f:'modifyBackupJob',p:jsonData}, function(data){
...
//		}
	...
	var initOldSettings = function(){
		...
		data = JSON.stringify(data);
		$.post(CONF.AJAXPATH, {m:CONF.M.FILECDP,f:'getBackupTaskAllInfo',p:data}, function(d){
	...
	var initStep3Settings = function(){
		//常规配置
		var general = SETTINGS.highInfo.general;
		$('#multithreading').bootstrapSwitch('state', general.multithreading);
		$('#mirrorimage').bootstrapSwitch('state', general.mirrorimage);
		$('#emptydir').bootstrapSwitch('state', general.emptydir);
		
		//防勒索
		var ransomware = SETTINGS.highInfo.ransomware;
		$('#ransomwaretype').val(ransomware.ransomwaretype);
		ransomwaretypeChange();
		//表格选择在表格初始化后回调设置,不在这里设置
		...
		//文件过滤
		//----时间过滤
		var filter = SETTINGS.highInfo.filter;
		$('#timefilter').bootstrapSwitch('state', filter.timefilter);
		$('#timefiltertype').val(filter.timefiltertype);
		$('#timefiltervalue').val(filter.timefiltervalue);
		//----名称过滤
		$('#namefilter').bootstrapSwitch('state', filter.namefilter);
		$('#namefilterlist').val(filter.namefilterlist);
		
		//时间策略
		var strategy = SETTINGS.highInfo.strategy;
		$('#strategytype').val(strategy.strategytype);
		strategytypeChange();
		$('.starttime').val(strategy.starttime);
		$('.endtime').val(strategy.endtime);
		
		//历史数据
		var history = SETTINGS.highInfo.history;
		$('#historytime').val(history.historytime);
		$('#historydel').val(history.historydel);
		$('#historymod').val(history.historymod);
		$('#historytimeinterval').val(history.historytimeinterval);
