...
//--------------------------
        ...
        var initListeners = function() {
            ...
            $("#"+settings.class+'_script_method').change(function() {
                ...
                if(privateConfig[settings.class][clickedValue]){
                    privateConfig[settings.class][clickedValue]['script_type'] = 0;
                }
                //清空脚本内容
        ...
        var initScriptContent = function() {
            ...
            for(let key in privateConfig[settings.class]){
                ...
                scriptDiv.children().last().before(tab);
            }
            ...
            tabs.each(function(index,element){
                //获取当前tab的对象
                var thisTab = $(element);
                thisTab.append('<i class="viconfont vicon-a-Check-onexiaoyan1 save_tab"></i>');
            })
        ...
        var initZoomUpload = function() {
           ...
           function checkThisData(script_name,scriptType, scriptContent, clickedValue, scriptmethod){
                let result = {
                    "title" : '',
                    "check"  : true,
                };
                //脚本类型不能为空---------------
                if((scriptmethod == 0) && (scriptType == 0 || scriptType == null || scriptType == undefined||  scriptType == "")){
                    result.check = false;
                    result.title = LANG.UI_SCRIPT_TYPE_IS_NULL;
                    return result;
                }
                //脚本类型不能为空---------------
                if((scriptmethod == 1) && (scriptType == 0 || scriptType == null || scriptType == undefined||  scriptType == "")){
                    result.check = false;
                    result.title = LANG.UI_SCRIPT_LIST_IS_NULL;
                    return result;
                }
                //脚本名称不能为空----------------
                if(script_name == ''){
            ...
            $("#"+settings.class+'_save_script').on('click', function(e) {
                ...
                let resultCheck = checkThisData(script_name,scriptType, scriptContent, clickedValue, scriptmethod);
            ...
            $("."+settings.class+'_script_tab').on('click', 'i.vicon-a-Close-oneguanbi', function(e) {
             ...
             $("."+settings.class+'_script_tab').on('click','label', function(e) {
                ...
                $("#"+settings.class+'_script_name').val(data['script_name'] ?? "");
                //脚本库
                $("#"+settings.class+'_script_list').val(data['script_uuid'] ?? 0);
                //填写脚本内容
                aceEditor[settings.class].setValue(data['script_content'] ?? "");
                //间隔
                $("#"+settings.class+'_exec_interval_spinner').spinner('value', parseInt(data['exec_interval'])?? 30);
