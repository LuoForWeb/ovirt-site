var AppGroup = function(){
	...
	let initListeners = function(){
		...
		$('#addAppGroup').on('click', function(){
			let url = "./content/platform/dataverification/add_appgroup.php";
			LOCATION(url)
		});
		...
		$('#editAppGroup').on('click', function(){
			...
			//组合修改的的url链接
			let url = "./content/platform/dataverification/add_appgroup.php?uuid=" + ID;
			LOCATION(url);
		...
		//点击搜索图标搜索
		$('#vin_appgroup_toolbar').on('click', '.search-btn', function () {
			queryParams.search = $('.appGroupSearch').val();
			$("#appgroup_table").bootstrapTable('refresh',{query:queryParams});
		});
	}
	...
	let addObjectList = function(data){
		...
		info +=
			...
			'</div>' +
			'</div>' +
			'<div class="row static-info display-hide">' +
