var VirtualLab = function(){
	...
	const initListeners = function(){
		...
		$('#addLab').on('click', function(){
			let url = "./content/platform/dataverification/add_virtual_lab.php";
			LOCATION(url)
		});
		...
		$('#editLab').on('click', function(){
			...
			if(select.length > 1){
				...
				//组合修改的的url链接
				let url = "./content/platform/dataverification/add_virtual_lab.php?uuid=" + select[0]['lab_uuid'];
				LOCATION(url);
		...
		$('#refreshLab').on('click', function(){
			...
			let ids = [];
			let hypervisor = select[0].hypervisor_type;
			for (let i = 0; i < select.length; i++) {
				ids.push(select[i].lab_uuid);
				if(i>0 && hypervisor != select[i].hypervisor_type){
				...
				bootbox.confirm({
					...
					callback: function(r) {
						if(!r) return;
						submitRefresh(ids, hypervisor);
					}
				});
			});

		//点击搜索图标搜索
		$('#vin_lab_toolbar').on('click', '.search-btn', function () {
			queryParams.search = $('.labSearch').val();
			$("#lab_table").bootstrapTable('refresh',{query:queryParams});
		});
