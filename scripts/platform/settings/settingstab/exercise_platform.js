//容灾演练平台
var Settings_System_Recovery = function(){

	var lastIp = '';//最终传参的ip

	var addListeners = function(){
		var systemrecoveryform = $('#systemrecoveryform');
		var error2 = $('.alert-danger', systemrecoveryform);
		var success2 = $('.alert-success', systemrecoveryform);
		systemrecoveryform.validate({
		...
		$("#recoverysubmit").click(function(){
			if (!$('input[name=ipaddr]').val()) {
				// 允许保存空的ip或者域名
				submit();
				return;
			}
			if (systemrecoveryform.validate().form()) {
				submit();
			}
		});
	}
	...
	var submit = function(){
		Metronic.blockUI({target: '#systemrecoveryform',animate: true});
		var data = {};
		data.httpType = $('#httpType').val();
		data.ipaddr = $('input[name=ipaddr]').val();
		
		var p = JSON.stringify(data);
		$.post(CONF.AJAXPATH, {m:CONF.M.SYSTEM, f:'setRecoveryInfo', p:p}, function(d){
			Metronic.unblockUI('#systemrecoveryform');
	    	if(OPREL(d)){
	    	}
		});
	}
	...
	var initOldInfo = function(){
		$.post(CONF.AJAXPATH, {m:CONF.M.SYSTEM, f:'getDefaultRecoveryInfo', p:{}}, function(d){
			var data  = JSON.parse(d);
			if(data.length == 0) return;
			//可视化配置初始化
			$('#httpType').val(data.httpType);
