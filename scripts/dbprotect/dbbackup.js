var DbBackup = function () {
    ...
    var zTreeAgent, zTreeDB = [], nodeParamList;
    ...
    let oracleCurrentIsMultiTaskFlag = false;  // 正在创建的是否为归档日志备份任务
    ...
    var initData = function () {
        ...
        data.highInfo.reserve = {
            type: 1,
            value: 0,
            strategyMode: CONF.RESERVE_STRATEGY_MODE.CHIAN,  // 默认按链
            enable_flag: false,
        };
    ...
    var initListener = function () {
        ...
        //后退
        $('#searchdb').on('propertychange', searchVM).on('input', searchVM);
        ...
        // 加密密码输入清除placeholder
        $('#password, #repassword').on('input propertychange', function () {
    ...
    const archivelogBackupDaysChange = function () {
        ...
        $('#archivelogBackupDaysSpinner').spinner("value", value);
    };
    ...
    var searchAgent = function () {
        ...
        var checkNode = zTreeAgent.getCheckedNodes().filter(v => v.eventtype !== 'group');
            ...
            case 4:  // 日志
                setTimeout(() => {
                    if (sqlserverDbSimpleModeFlag === true && checked) {  // SQL Server数据库简单模式不能选择日志备份
                        UIToastr.showWarning(LANG.UI_DB_BACKUP_CREATE_TITLE, LANG.UI_DB_BACKUP_SQLSERVER_SIMPLE_MODE_NONSUPPORT_LOG_BACKUP);
    ...
    var searchVM = function () {
        ...
        var checkNode = zTreeDB.getCheckedNodes();
        var allNode = zTreeDB.transformToArray(zTreeDB.getNodes());
        nodeParamList = zTreeDB.getNodesByParamFuzzy('name', value);
        if (nodeParamList.length) {
            zTreeDB.hideNodes(allNode);
        }
        //连接搜索的和所勾选的
        nodeParamList = nodeParamList.concat(checkNode);
        var nodeParamList1 = zTreeDB.transformToArray(nodeParamList);
        for (var n in nodeParamList1) {
            findParent(zTreeDB, nodeParamList1[n]);
        }
        zTreeDB.showNodes(nodeParamList);
    }
    ...
    var initSpinner = function () {
        ...
        $('#channelCountSpinner').spinner({value: 1, step: 1, min: 1, max: 32});
        ...
        $('#mysqlParallelSpinner').spinner({value: 1, step: 1, min: 1, max: 16});
     ...
     * @return {Object}
     */
    const getTimeStrategySuggestTimeSync = () => {
        let suggestTime = {
            start_time: '23:00:00',
            end_time: '23:30:00',
            roll_end_time: '23:30:00',
        };
        pAjaxRequest({}, `/api/v1/jobs/time/crow/list`, 'GET', res => {
            if (res.success) {
                suggestTime = res.data.suggest_time;
            }
        }, false);
        return suggestTime;
    };
     ...
     * @param {*} dbType
     */
    const getDefaultTimeStrategy = dbType => {
        switch (dbType) {
            case CONF.DB_TYPE.SQLSERVER:
                let suggestTime = getTimeStrategySuggestTimeSync();
                let timeStrategy = {
                    time: {
                        type: 'strategy',
                        timeInfo: {
                            fullInfo: {
                                ...
                                days: [0, 0, 0, 0, 1, 0, 0],
                                frequency: '',
                                startTime: suggestTime.start_time,
                                endTime: suggestTime.end_time,
                                rollFlag: false,
                                rollInterval: '01:00:00',
                                roll_end_time: suggestTime.roll_end_time,
                                dbtype: dbType,
                            },
                        }
                    }
                ...
                if (sqlserverDbSimpleModeFlag === false) {
                    timeStrategy.time.timeInfo.logInfo = {
                        mode: 4,
                        type: 1,
                        days: [0, 0, 0, 0, 0, 0, 0],
                        frequency: '',
     ...
     * 重置备份目标
     */
    const resetBackupTarget = (backupNodeTree, backupStorageTree) => {
        // 取消之前的禁用
    ...
    const setStep2MultiTask = () => {
        ...
        if (!oracleCurrentIsMultiTaskFlag) {  // 二次备份才填充
            resetBackupTarget(backupNodeTree, backupStorageTree);
            return;
        }
        let dbType = parseInt($('#dbtype').val());
    ...
    const initOracleSkipDatafileBadBlock = () => {
        ...
        let options = ``;
        let checkedSourceNodes = zTreeAgent.getCheckedNodes(true);
                ...
                onCheck: (ev, treeId, treeNode) => {
                    ...
                    if (treeNode.checked) {
                        let html = `<div class="skip-datafile-bad-block-configure_item" data-table-space-index="${treeNode.table_space_index}" title="${treeNode.table_space_name}">
                            ...
                            <div class="table-space-result display-none">
    ...
    const buildBackupSourceData = () => {
        ...
        for (let i = 0; i < agentList.length; i++) {
            ...
            $.each(nodes, function (i, d) {
                ...
                switch (dbType) {
                    case CONF.DB_TYPE.ORACLE:
                        if (d.db_backup_info.length) {
                            if (d.db_backup_info[0].multi_task_flag) {
                                oracleCurrentIsMultiTaskFlag = false;  // 当前存在归档日志任务，因此当前新建的是完备任务
                            } else {
                                oracleCurrentIsMultiTaskFlag = true;
                            }
                        } else {
                            oracleCurrentIsMultiTaskFlag = false;  // 当前不存在任务，当前新建的是完备任务
                        }
    ...
    const initAssociatedTaskData = () => {
        return new Promise(resolve => {
            ...
            existsTaskUuid = nodes[0].db_backup_info[0].job_uuid;
            Metronic.blockUI({target: '#dbbackupcontent', animate: true});
            let reqData = {
    ...
    var step1Valid = function (showTitleCallback) {
        ...
        // 初始化备份策略
        DBSubType = dbType;
        $('#tab_common').backupStrategy(CONF.MODULE_TYPE.DB, false, getDefaultTimeStrategy(dbType), dbType, afterFirstInitBackupStrategy);
        ...
        switch (dbType) {
            ...
            case CONF.DB_TYPE.ORACLE:
                ...
                $('.oracleChannelDiv').show();	// 通道数
                $('#delarchivelog').bootstrapSwitch('state', true);         // 默认开启删除归档日志
                $('#oracleFilesPerset').bootstrapSwitch('state', false);    // 默认关闭设置fileperset配置
                $('#oracleFilesPersetDatafileDiv').hide();                  // 数据文件filesperset个数
                $('#oracleFilesPersetArchivelogDiv').hide();                // 归档日志文件filesperset个数
                $('#oracleFilesPersetDatafile').val(64);                // 数据文件filesperset个数
                $('#oracleFilesPersetArchivelog').val(64);              // 归档日志文件filesperset个数
                $('#oracleCheckArchivelog').bootstrapSwitch('state', true); // 默认开启检查归档日志文件
                $('#archivelogBackupTimesCheck').bootstrapSwitch('state', false);  // 默认关闭备份次数
                $('#archivelogBackupDaysCheck').bootstrapSwitch('state', false);   // 默认关闭备份天数
                $('.archivelogBackupTimesDiv').hide();   // 隐藏备份次数输入框
                $('.archivelogBackupDaysDiv').hide();    // 隐藏备份天数输入框
                $('#enableBctFlag').bootstrapSwitch('state', false);   // BCT默认关闭
                $('.db-tips').show();  // 时间策略提示
                $('#setSectionSizeFlag').bootstrapSwitch('state', false);   // 默认关闭多段传输
                $('.sectionSizeDiv').hide();  // Oracle分块大小
            ...
            case CONF.DB_TYPE.MYSQL:
                ...
                $('#mysqlSrcCompressed').bootstrapSwitch('state', false);   // 默认关闭MySQL源端压缩
            ...
            case CONF.DB_TYPE.DM:
                ...
                $('.diff').show();			        // 显示差异备份
                $('.dmDiv').show();			        // 显示dm功能选项
                $('#dmcompress').bootstrapSwitch('state', false);        // 默认关闭DM压缩
                $('#deldmarchivelog').bootstrapSwitch('state', false);   // 默认关闭删除归档日志
                $('.db-tips').show();               // 时间策略提示
            ...
            case CONF.DB_TYPE.VASTBASE:
                ...
                $('#archiveAlarmCheck').bootstrapSwitch('state', false);  // 归档存储告警默认关闭
                $('.warnningdiv').hide();          // 隐藏归档日志告警配置
            ...
            case CONF.DB_TYPE.TIDB:
                ...
                $('#tidbEncrypt').bootstrapSwitch('state', true);   // 默认开启TiDB加密
                $('#tidbCompress').bootstrapSwitch('state', true);  // 默认开启TiDB加密
                $('#tidbCompressLevelSpinner').spinner("value", 1);
                $('#tidbCompressMethod').val(TIDB_COMPRESS_METHOD_ENUM.lz4).trigger('change');

                $('#tabAdvancedPerformanceTab').addClass('active').show();  // 性能优化
                $('#tab_advanced_performance').addClass('active').addClass('in').removeClass('fade');
                $('.advancedPerformanceShowDiv').show();
                // $('.safeStrategyLi').hide();  // 隐藏安全策略
                // $('#wormConfig_check').bootstrapSwitch('state', false);
            ...
            case CONF.DB_TYPE.SAPHANA:
                ...
                $('#sapHanaCompress').bootstrapSwitch('state', false);  // 默认开启SAP HANA加密
                $('#channelCount').val(1);        // 通道数默认设置为1
            ...
            $.each(checkNodes, (index, checkNode) => {
                if (checkNode.eventtype === 'cluster') {
                    ...
                    if (dbType === CONF.DB_TYPE.MONGODB) {
                        // MongoDB有多级
                        let hostCheckNodes = zTreeAgent.getCheckedNodes(true);
    ...
    var showStep1 = function () {
        //设置任务名
        $.post(CONF.AJAXPATH, {m: CONF.M.DBPROTECT, f: 'getDBBackupTaskName', p: {}}, function (data) {
            $('#jobname').val(data);
            $('#tidbFullJobName').val(data);
            $('#tidbLogJobName').val(data + '_' + LANG.UI_STRATEGY_LOG);
        });
        //设置代理端
    ...
    const setMultiTaskStorageStrategy = () => {
        ...
        let dbType = parseInt($('#dbtype').val());
        if (dbType === CONF.DB_TYPE.ORACLE) {
            if (oracleCurrentIsMultiTaskFlag && associatedTaskData) {  // 归档日志备份任务才填充
    ...
    const setMultiTaskSafetyStrategy = () => {
        ...
        $('#integrityCheck_check').bootstrapSwitch('state', true).trigger('switchChange.bootstrapSwitch').bootstrapSwitch('disabled', false);
        $('#integrityCheck input[name="checkDay"]').prop('disabled', false).prop('checked', false);
        $('#check_every').prop('checked', true).trigger('change');
        if (dbType === CONF.DB_TYPE.ORACLE) {
            if (oracleCurrentIsMultiTaskFlag) {
    ...
    const initWormStrategy = () => {
        ...
        // Oracle归档日志任务的WORM设置
        if (dbType === CONF.DB_TYPE.ORACLE) {
            if (oracleCurrentIsMultiTaskFlag && associatedTaskData) {  // 归档日志备份任务需要与主任务同步
                let safeConfigStrategy = associatedTaskData.safe_config_strategy;
    ...
    var step2Valid = function () {
        ...
        // 触发脚本开关事件
        $('#scriptConfigSwitch').trigger('switchChange.bootstrapSwitch');
        //是否需要显示传输网络标记
    ...
    var showStep2 = function () {
        ...
        if (CONF.BD_STORAGE_TYPE.TAPE !== data.highInfo.node.storage_type) {
            ...
            if (dbType === CONF.DB_TYPE.ORACLE) {
                $('#oracleChannelNum').val(4).prop('disabled', false);  // 设置默认值
    ...
    var setStep4StoreStrategyDes = function () {
        ...
        // 压缩存储
        storeInfo += $('.compressLabel').html() + ": " + getSwitchDes(data.highInfo.store.compress) + '<br>';
        // 压缩等级
    ...
    var setStep4TransferStrategyDes = function () {
        ...
        if (CONF.FUNCTIONS.includes('multithread')) {
            // 传输线程
            if (data.srcInfo.dbType === CONF.DB_TYPE.MONGODB) {
                if (CONF.BD_STORAGE_TYPE.TAPE !== data.highInfo.node.storage_type) {  // 磁带不显示通道数
        ...
        switch (data.srcInfo.dbType) {
            ...
            case CONF.DB_TYPE.ORACLE:
                ...
                if (CONF.BD_STORAGE_TYPE.TAPE !== data.highInfo.node.storage_type) {  // 非磁带才有通道数
                    ...
                    data.agentInfo.channel_count = data.highInfo.transfer.threadnum;
                }
                // BCT
    ...
    var wizardInit = function () {
        ...
        var handleTitle = function (tab, navigation, index) {
            ...
            if (current >= total) {
                $('#dbbackupcontent').find('.button-next').hide();
                $('#dbbackupcontent').find('.button-submit').css('visibility', 'visible');
            } else {
                $('#dbbackupcontent').find('.button-next').show();
                $('#dbbackupcontent').find('.button-submit').css('visibility', 'hidden');
            }
        ...
        $('#dbbackupcontent .button-submit').click(() => {
            getDbCurrentUseLicense().then(showOracleSyncConfigDialog).then(submit);
    ...
    const getDbCurrentUseLicense = () => {
        return new Promise((resolve) => {
            judgeLicenseExpireTime().then(() => {
                let dbType = parseInt($('#dbtype').val());
                let backedUpAgentUuidSet = getBackedUpAgentUuidSet();
                // 按客户端数量授权
                let currentAgentUuidSet = new Set();
                let checkedSources = zTreeAgent.getCheckedNodes(true);
                ...
                switch (data.backupInfo.type) {
                    case 'strategy':
                        if ($('#logBackup').prop('checked')) {  // 勾选了日志备份
                            ...
                            data.subTaskName = tidbLogTaskName;
                        }
                        break;
                }
                $('.jobnametip').hide();
    ...
    const createDBBackupJob = data => {
        ...
        let dependTaskUuid = '';
        if (dbType === CONF.DB_TYPE.ORACLE) {
            if (oracleCurrentIsMultiTaskFlag) {  // 当前创建的是归档日志任务
                dependTaskUuid = associatedTaskData.job_uuid;
            }
        }
        let reqData = {
            task_name: data.taskName,
            sub_task_name: data.subTaskName,
            ...
            storage_strategy: {
                ...
                compress_flag: data.highInfo.store.compress,
        ...
        pAjaxRequest(reqData, `/api/v1/db/jobs/backup`, 'POST', res => {
            Metronic.unblockUI('#dbbackupcontent');
            if (!res.success) {
                UIToastr.showWarning(LANG.UI_DB_BACKUP_CREATE_TITLE, res.message);
                return;
            }
            UIToastr.showSuccess(LANG.UI_DB_BACKUP_CREATE_TITLE, res.message);
        ...
        switch (dbType) {
            case CONF.DB_TYPE.SQLSERVER:
                ...
                if (mixedRecoveryModeFlag) {
                    ...
                    UIToastr.showWarning(LANG.UI_DB_BACKUP_CREATE_TITLE, LANG.UI_DB_BACKUP_SQLSERVER_MIXED_SIMPLE_AND_OTHER_MODE);
    ...
    const checkBackupNode = async (ev, treeId, treeNode) => {
        if (
            ...
            if (CONF.DB_TYPE.ORACLE === treeNode.db_type) {
                 ...
                 * 4. 如果仅有归档日志备份任务，那么需删除后才能创建普通备份任务(bug#22034 去除此规则)
                 * 5. 如果存在备份任务，且备份任务的状态不是停止状态，那么不能新建备份任务
                ...
                if (treeNode.checked) {
                    ...
                    if (mixedBackupFlag) {
                        ...
                        UIToastr.showWarning(LANG.UI_DB_BACKUP_CREATE_TITLE, LANG.UI_DB_BACKUP_ORACLE_MIXED_LOG_BACKUP);
                        return;
                    }
                    if (diffBackupFlag) {
                        zTreeAgent.checkNode(treeNode, false);
                        UIToastr.showWarning(LANG.UI_DB_BACKUP_CREATE_TITLE, LANG.UI_DB_BACKUP_ORACLE_DIFF_LOG_BACKUP);
                        return;
                    }
                    if (logBackupTaskFlag && backupTaskFlag) {
                        zTreeAgent.checkNode(treeNode, false);
                        let taskName = '';
                        let logTaskName = '';
                        ...
                        UIToastr.showWarning(LANG.UI_DB_BACKUP_CREATE_TITLE, message);
                        return;
                    }
                    // if (logBackupTaskFlag) {bug#22034 去除此规则
                    //     zTreeAgent.checkNode(treeNode, false);
                    //     UIToastr.showWarning(LANG.UI_DB_BACKUP_CREATE_TITLE, LANG.UI_DB_BACKUP_ORACLE_ONLY_BACKUP_TIPS);
                    //     return;
                    // }
                    // 存在的备份任务不处于停止状态
                    if (backupTaskFlag || logBackupTaskFlag) {  // 存在完备任务或归档日志任务
                        if (treeNode.db_backup_info[0].job_status !== CONF.TASK_STATUS.STOPPED) {
                            zTreeAgent.checkNode(treeNode, false);
                            let message = LANG.UI_DB_BACKUP_ORACLE_EXISTS_TASK_NOT_STOPPED.replace('%1s', treeNode.name).replace('%2s', treeNode.db_backup_info[0].job_name);
                            UIToastr.showWarning(LANG.UI_DB_BACKUP_CREATE_TITLE, message);
                            return;
                        }
                    }
            ...
            if (treeNode.checked) {
                ...
                if (CONF.DB_TYPE.TIDB === treeNode.db_type) {
                    let checkNodes = zTreeAgent.getCheckedNodes(true);
                    for (const checkNode of checkNodes) {
    ...
    var initDbTypeSelect = function () {
        ...
        pAjaxRequest(reqData, `/api/v1/db/type`, 'GET', res => {
            ...
            $dbType.html(options).on('change', dbtypeHandler);
            initBackupTree();
        });
    }
    // ---------策略选择---------
    ...
    var strategyHandler = function () {
        ...
        if (DBSubType === CONF.DB_TYPE.MONGODB) {
            // MongoDB增量和差异同时存在两个都取消
            if (typeof strategy.time !== 'undefined' && typeof strategy.time.timeInfo !== 'undefined') {
                if (
                    typeof strategy.time.timeInfo.incrInfo !== 'undefined' &&
                    typeof strategy.time.timeInfo.diffInfo !== 'undefined' &&
                    Object.values(strategy.time.timeInfo.incrInfo).length &&
                    Object.values(strategy.time.timeInfo.diffInfo).length
                ) {
                    delete strategy.time.timeInfo.incrInfo;
                    delete strategy.time.timeInfo.diffInfo;
                }
            }
        }
        // 初始化全局策略数据--调用公共的方法
    ...
    const initSafeStrategy = () => {
        // 完整性校验
        $('#integrityCheck').completeDetectionBackup('col-md-3', true, CONF.MODULE_TYPE.DB);
    };

    return {
        //main function to initiate the module
        init: function () {
            console.log('db-backup');  // 这里打印是为了方便debug
            wizardInit();
            initDbTypeSelect();
            initListener();
            initRetryStrategy();
