(function($){
  $.fn.vmConfigOne = function(options){
	  var defaults = {
		  //多维数组,每一项表示每一个虚拟机的配置
		  'config' : [],
		  'hide' : [],	//是否影藏某项
	  ...
	  //得到隐藏的类 type:(string)   storage, network,...
	  ...
	  var getVMDiv = function(config, num){
		  var div = '';
		  //左边
		  div += 	'<div class="portlet-body bd1de5 configsdiv">' + 
						'<div class="row">' + 
						...
						'<div class="tab-pane active in" id="tab_' + num + '_1">' + 
							'<div class="form-group col-md-12">' + 
								...
								'<div class="input-icon right">' + 
									'<i style="top:-4px;" class="fa"></i>' + 
									'<input type="text" maxlength="60" class="form-control input-sm" value="' + config['vmname'] + '" name="vmname"/> ' + 
								'</div> ' + 
							'</div>' + 
							'<div class="form-group col-md-12">' + 
								...
								'* </span></label>' + 
								'<div>' + 
									'<input type="checkbox" name="vmpower" class="make-switch" data-on-color="primary" data-off-color="info" data-size="small"' + 
...
//		  }
		  ...
		  for(var i=0; i<config.storage.length; i++){
			  ...
			  div += '<tr id="' + config.storage[i]['size'] + '">' + 
			  		'<input type="checkbox" class="display-none" checked="checed" id="' + config.storage[i]['vdi_uuid'] +'" value="'+ config.storage[i]['vdi_name'] + '">'+
						...
						'<td class="vmstorage" id="' + config.storage[i]['vdi_uuid'] + '"><select class="form-control select2me input-sm" name="hoststorage" '+disableTips+'></select></td>' + 
					'</tr>';
		  }
	  ...
	  var getStorageDiv = function(config, num){
		  ...
		  for(var i=0; i<config.storage.length; i++){
			  div += '<tr id="' + config.storage[i]['size'] + '">' +
						...
						'<td class="highlight">' + config.storage[i]['vdi_name'] + '</td>' + 
						'<td class="hidden-xs">' + config.storage[i]['virtual_size'] + '</td>' + 
						'<td class="vmstorage storage-active" id="' + config.storage[i]['vdi_uuid'] + '"><select class="form-control select2me input-sm" name="hoststorage"></select></td>' + 
						'<td class="vmdisktype display-none" id="' + config.storage[i]['vdi_uuid'] + '"><select class="form-control select2me input-sm" name="disktype"></select></td>' +
				  	 '</tr>';
		  }
	  ...
	  var getNetworkDiv = function(config, num){
		  ...
		  for(var i=0; i<config.network.length; i++){
			  div += '<tr class="vmnetwork">' + 
						...
						'<td><select class="form-control input-sm" name="hostnetwork"></select></td>' +
				  	 '</tr>';
		  }
  ...
  $.fn.getvmConfigOne = function(options){
	  //获取最终配置信息
