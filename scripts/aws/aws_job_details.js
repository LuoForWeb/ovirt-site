var AWSJobDetails = function () {
	...
	var initInstanceTableFlag = false; //实例列表初始化标志
	...
	const BACKUP_OTHER_POINT_ABNORMAL_DESC_MAP = {
		0: LANG.UI_SAFE_STRATEGY_INTEGRITY_CHECK_INC_ERROR_POLICY_OPTION_TERMINAL_BACKUP,
		1: LANG.UI_SAFE_STRATEGY_INTEGRITY_CHECK_INC_ERROR_POLICY_OPTION_REDO_FULL,
		2: LANG.UI_SAFE_STRATEGY_INTEGRITY_CHECK_INC_ERROR_POLICY_OPTION_REDO_INCR
	}; // 验证策略 - 其他备份点异常描述

	const RECOVERY_TIMEPOINT_SCAN_TYPE_DESC_MAP = {
		2: LANG.UI_BACKUP_DATA_POINT_VIRUS_STATUS_NOT_SCAN,
		3: LANG.UI_BACKUP_DATA_POINT_VIRUS_STATUS_NORMAL,
		4: LANG.UI_BACKUP_DATA_POINT_VIRUS_STATUS_ABNORMAL,
	}; // 验证策略 - 备份点病毒扫描类型描述
	const RECOVERY_TIMEPOINT_HANDLE_METHOD_DESC_MAP = {
		1: LANG.UI_RECOVERY_ZHIJIE,
		2: LANG.UI_RECOVERY_VIRUS_COVERY,
		3: LANG.UI_RECOVERY_RESCAN,
	}; // 验证策略 - 备份点处理方式描述
	const RECOVERY_TIMEPOINT_SCAN_ABNORMAL_OP_DESC_MAP = {
		1: LANG.UI_SAFE_SCAN_ABNORMAL_STOP,
		2: LANG.UI_SAFE_SCAN_ABNORMAL_RECOVER_NO_WEB,
		3: LANG.UI_SAFE_SCAN_ABNORMAL_VIRUS_STOP,
	}; // 验证策略 - 备份点扫描异常后操作描述
	const RECOVERY_TIMEPOINT_SCAN_VIRUS_OP_DESC_MAP = {
		1: LANG.UI_SAFE_SCAN_VIRUS_STOP_RECOVER,
		2: LANG.UI_SAFE_SCAN_VIRUS_RECOVER_NO_INTER,
		3: LANG.UI_SAFE_SCAN_VIRUS_VIRUS_RECOVER,
	}; // 验证策略 - 备份点检测到病毒后操作描述
	...
	var initBasicInfo = function(){
		...
		var init = function(){
			if(0 == $('#task_uuid').size()){
        		clearTimeout(timerTask.AWSJobDetails_taskRunningInfo);
        		return;
        	}
			var data = {};
			data.job_uuid = $("#task_uuid").val();
			pAjaxRequest(data, "/api/v1/cloud/jobs/basic_info", "GET", function (d) {
				setBasicInfo(d.data, timerTask.AWSJobDetails_taskRunningInfo);
			}, false);
			timerTask.AWSJobDetails_taskRunningInfo = setTimeout(init, updateInterval);
		}
		init();
	}
	...
	var setBasicInfo = function(data, timeoutID){
		...
		//存储信息
		if(1 == data.taskTypeFlag && data.storageInfo.flag){
		...
		$('#highLevel').html(getFlagLevelInfo(data.modeStrategy.highLevel));
		if (CONF.VM_TYPE.HUAWEICLOUD === parseInt(data.hypervisor)) {
			// 华为云高速模式的描述为“保留快照”
			$(`.highLevelLabel`).text(LANG.UI_TASK_AWS_KEEP_SNAPSHOT);
			$('#highLevel').html(getFlagLevelInfo(data.modeStrategy.keepSnapshot));
		}
		// $('#cbtMode').html(getFlagLevelInfo(data.modeStrategy.cbtMode));
		// $('#resetcbt').html(getFlagLevelInfo(data.modeStrategy.resetCbt));
		$('#prisnapshot').html(getFlagLevelInfo(data.snapshotPriority));
		$('#incMode').html(data.modeStrategy.incMode);
		// $('#quiesceSnapshot').html(getFlagLevelInfo(data.modeStrategy.quiesceSnapshot));
		...
		$('#appliance').html(data.applianceDes);
		$('#applianceRec').html(data.applianceDes);
		$('#presnapshot').html(getFlagLevelInfo(data.modeStrategy.presnapshot));
		$('#snapshottimeout').html(data.modeStrategy.snapshotTimeout + LANG.UI_PUBLIC_SECOND);
		...
		if(1 == data.taskTypeFlag){
			...
			$("#transportAgent").html(data.transportStrategy.agent_type);
			...
			// 快照超时时间
			if (CONF.VM_TYPE.AWS == data.hypervisor) {
			...
			if (!CONF.FUNCTIONS.includes('worm') && !CONF.FUNCTIONS.includes('integrity') && !CONF.FUNCTIONS.includes('virusKill')
				|| data.storageInfo.storage.storage_type == CONF.BD_STORAGE_TYPE.TAPE) {
				// 都没，隐藏安全策略
				$('.safemodeDiv').hide();
			}
			// <------  END SAFE STRATEGY  ------>
...
//			$('#tab_1_4').hide();
			...
			$("#recoveryEncrypt").html(getFlagLevelInfo(data.transportStrategy.encrypt));
			// 传输加密算法
			if (data.transportStrategy.encrypt) {
				let encryptMethod = data.transportStrategy.encrypt_method;
				let method = LANG.UI_GLOBAL_STRATEGY_TRANSFER_ENCRYPT_RSA;
		...
		if (1 == data.taskTypeFlag) {
			if(!initInstanceTableFlag){
				initBackupInstanceTable();
			}
		} else {
			if(!initInstanceTableFlag){
				initRecoveryInstanceTable();
			}
		}
	...
	var showSelectConditions = function (selectConditions, autoJoinFlag) {
		...
		switch (parseInt(selectConditions.select_type)) {
			case 0:
				//按前缀排除
				rules += LANG.UI_BACKUP_INSTANCE_NAME_PREFIX_FILTER;
				break;
			case 1:
				//按前缀匹配
				rules += LANG.UI_BACKUP_INSTANCE_NAME_PREFIX_MATCH;
				break;
			case 2:
				// 按关键词排除
				rules += LANG.UI_BACKUP_INSTANCE_NAME_KEYWORD_FILTER;
				break;
			case 3:
				// 按关键词匹配
				rules += LANG.UI_BACKUP_INSTANCE_NAME_KEYWORD_MATCH;
				break;
			default:
				break;
		}
    ...
    var initSpeed = function () {
        ...
        function update(){
        	if(0 == $('#speedchart').size()){
        		clearTimeout(timerTask.AWSJobDetails_speed);
        		return;
        	}
			...
			timerTask.AWSJobDetails_speed = setTimeout(update, updateInterval);
        }
        update();
...
//	}
    ...
    //字符串超过某长度的省略为‘...’
	var omitStr = function (string, reserved_length) {
		return (string.length > reserved_length) ? string.substring(0, reserved_length) + '...' : string;
	}

	//初始化备份任务实例表格
	var initBackupInstanceTable = function () {
		if (!initInstanceTableFlag) {
			let options = {
				tableArea: '#vms',
				vin_url: '/api/v1/cloud/jobs/' + _jobUuid + '/instances',
				vin_method: 'GET',
				queryParamsType: '',
				queryParams: {},
				pagination: false,
				...
				detailFormatter: function (index, row) {
					return LANG.UI_TASK_AWS_INSTANCE_PATH + ': ' + row.detail.spath;
				},
				onRefresh: function (params) {
					$("#instance-table").bootstrapTable('hideLoading');
				},
				onCheck: function (row) {
					instanceData.push(row.vm_uuid)
					var index = instanceData.indexOf(row.vm_uuid); // 查找元素的索引
					if (index == -1) {
						instanceData.push(row.vm_uuid)
					}
				},
				onCheckAll: function (row) {
					for (var i = 0; i < row.length; i++) {
						var index = instanceData.indexOf(row[i].vm_uuid); // 查找元素的索引
						if (index == -1) {
							instanceData.push(row[i].vm_uuid)
						}
					}
				},
				onUncheckAll: function (a, row) {
					for (var i = 0; i < row.length; i++) {
						var index = instanceData.indexOf(row[i].vm_uuid); // 查找元素的索引
						if (index != -1) {
							instanceData.splice(index, 1); // 从数组中删除一个元素
						}
					}
				},
				...
				columns: [ //列定义
					...
					{
						field: 'name',
						title: LANG.UI_TASK_AWS_INSTANCE_NAME_NEW,
						// width: '300px',
					},
					{
						field: 'task_type',
					...
					{
						field: 'size',
						title: LANG.UI_TASK_AWS_INSTANCE_SIZE,
						// width: '100px',
					},
					{
						field: 'valid_size',
		...
		timerTask.AWSJobDetails_vmGrid = setTimeout(initBackupInstanceTable, 5000);
	}

	//初始化恢复任务实例表格
	var initRecoveryInstanceTable = function () {
		if (!initInstanceTableFlag) {
			let options = {
				vin_url: '/api/v1/cloud/jobs/' + _jobUuid + '/instances',
				vin_method: 'GET',
				...
				detailView: true,
				detailFormatter: function (index, row) {
					let str = '<table class="instance-detail-table">';
					str += '<tr><td style="width: 200px">' + LANG.UI_RECOVERY_TIMEPOINT + ': </td><td>' + row.detail.timepoint_des + '</td></tr>';
					str += '<tr><td style="width: 200px">' + LANG.UI_TASK_AWS_SRC_INSTANCE_PATH + ': </td><td>' + row.detail.spath + '</td></tr>';
					if (1 == row.level) {
						//实例恢复显示恢复目标实例路径
						str += '<tr><td style="width: 200px">' + LANG.UI_RECOVERY_GOAL + ': </td><td>' + row.detail.tpath + '</td></tr>';
					}
					str += '</table>';
					return str;
				},
		...
		timerTask.AWSJobDetails_vmGrid = setTimeout(initRecoveryInstanceTable, 5000);
	}
	...
	var initHistoryTable = function () {
			...
			let options = {
				...
				detailFormatter: function (index, row) {
					...
					if (1 == row.job_type_value) {
						str += '<tr>' +
							'<th width="20%">' + LANG.UI_TASK_AWS_INSTANCE_NAME + '</th>' +
							'<th>' + LANG.UI_SEARCH_TASK_TYPE + '</th>' +
							'<th width="20%">' + LANG.UI_TASK_AWS_INSTANCE_PATH + '</th>' +
							'<th>' + LANG.UI_TASK_AWS_INSTANCE_SIZE + '</th>' +
						...
						$.each(vms_details, function (i, v) {
							...
							if (0 == i && row.detail.list.vms_changed_status) {
								...
								if (addvms.length) {
									str += LANG.UI_VM_ADD_INSTANCE + ': <br>';
									$.each(addvms, function(i, v) {
										str += '&nbsp;&nbsp;' + v + '<br>';
									});
								}
								if (removevms.length) {
									str += LANG.UI_VM_REMOVE_INSTANCE + ': <br>';
		...
		timerTask.AWSJobDetails_historyGrid = setTimeout(initHistoryTable, 10000);
	}
  ...
  //版本差异处理,主要是处理标准版本功能限制
	...
	var cleanTime = function (){
		clearTimeout(timerTask.AWSJobDetails_taskRunningInfo);
		clearTimeout(timerTask.AWSJobDetails_speed);
		clearTimeout(timerTask.AWSJobDetails_logGrid);
		clearTimeout(timerTask.AWSJobDetails_historyGrid);
	}
    ...
    var initListener = function(){
		...
		var startVmJob = function(mode){
			var selectRows = $('#instance-table').bootstrapTable('getSelections');
			var data = {};
			data.vm_uuids = [];
		...
		$('#searchInput').on('blur', function () {
			$(this).prop('placeholder', LANG.UI_TASK_INSTANCE_SEARCH_TIPS);
		});
	...
	const searchInstanceTable = () => {
		$("#instance-table").bootstrapTable('refreshOptions', {
			queryParams: function (queryParams) {
				return $.extend(queryParams, {search_name: $.trim($('#searchInput').val())});
			}
		});
	}
	...
	var stopJob = function(){
		...
		if (CONF.TASK_STATUS.STOPPING == jobParams.status) {
			//公有云任务强制停止需二次确认
			bootbox.confirm({
	...
	/**
	 * 构造权限校验所需参数，type:1-任务操作，2-虚拟机操作
	 */
	const checkAuth = (type) => {
		if (type === 1) {
			// 非分配资源的权限
			return {type: 1, user_uuid: _userUuid, auth: 'awsprotect'};
		} else if (type === 2) {
			let select = $('#instance-table').bootstrapTable('getSelections');
			if (!select.length) {
				UIToastr.showInfo(LANG.UI_JOB_START_INSTANCE_BACKUP_JOB, LANG.UI_JOB_START_INSTANCE_BACKUP_JOB_TIPS);
				return false;
			}
			let source_uuids = select.map(item => item.vm_uuid).join(',');
			// 分配资源的权限
			return {type: 2, source_uuid: source_uuids, source_type: 58};
		} else {
			return false;
		}
	}
