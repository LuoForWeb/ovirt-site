var VcenterDetails = function () {
	...
	var setNoXenServerTree = function(zNodes){
		var setting = {
			check: {
				enable: true,
				chkStyle: 'radio',
		...
		zTreeVT = $.fn.zTree.init($("#vm_tree_vt"), setting, zNodes);
	}
	...
	var setThirdTree = function(zNodes){
		...
		// var nodes = JSON.parse(zNodes);
		zTree = $.fn.zTree.init($("#vm_tree"), setting, zNodes);
	}
...
//	};
	...
	var initTree = function() {
		let platform_uuid = $('#vcuuid').val();
		pAjaxRequest({display_mode: 1, platform_uuid: platform_uuid}, "/api/v1/vm/platforms/tree", "GET", function (data) {
			setTree(data.data.info);
		}, true);
		pAjaxRequest({display_mode: 2, platform_uuid: platform_uuid}, "/api/v1/vm/platforms/tree", "GET", function (data) {
			setNoXenServerTree(data.data.info);
		}, true);
		pAjaxRequest({display_mode: 3, platform_uuid: platform_uuid}, "/api/v1/vm/platforms/tree", "GET", function (data) {
	...
	var createNewBackupTask = function (row) {
		...
		let url = './content/vm/vmbackup.php?showtype=' + vmshowtype + '&vmuuid=' + vmuuid + '&vcenteruuid=' + vcenteruuid + '&hypervisor=' + hypervisor;
		LOCATION(url, 'vmprotect');
	}
				...
				columns: [
					...
					},
					{
						...
						formatter: function (value, data, index) {
							if (!value.length || !CONF.PERMISSION_ARR.includes('p_vm_overview_vmoperate')) {
								return '';
							}
...
//			console.log('New date range selected: ' + start.format('YYYY-MM-DD HH:mm:ss') + ' to ' + end.format('YYYY-MM-DD HH:mm:ss') + ' predefined range: ' + label);
	...
	const checkAuth = (type, user_uuid = '') => {
		...
		if (type === 'addMulti') {
			if (!select.length) {
				UIToastr.showInfo(LANG.UI_VCENTER_VM_ADD_TO_TASK, LANG.UI_VM_REPORT_NO_ADD_TO_JOB_VM_TIPS);
				return false;
			}
			...
			if (!select.length) {
				UIToastr.showInfo(LANG.UI_VM_REPORT_CREATE_NEW_TASK, LANG.UI_VM_REPORT_NO_CREATE_JOB_VM_TIPS);
				return false;
			}
			let user_uuids = select.map(item => item.user_uuid).join(',');
			return {type: 1, user_uuid: user_uuids, auth: authKey};
