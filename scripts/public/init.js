var VinchinInit = (() => {
	...
	const initCompanyCss = () => {
		var width = $('.page-sidebar-menu ').width();
    	var spanWidth = $('.scopyright span').width();
    	var marginWidth = (width - spanWidth) / 2;
    	if(marginWidth < 0 ){
    		marginWidth = 0;
    	}
    	$('.scopyright span').css('margin-left', marginWidth + 'px');
    	$('.scopyright').show();
	}
	...
	const initConfig = () => {
		pAjaxRequest({}, '/api/v1/system/config/base_info', "GET", function (result) {
			CONF.SYSTEM_INIT.SYSTEM_CONFIG_FLAG = true;
	...
	const initSystemAuthInfo = () => {
		pAjaxRequest({type: 'a', module: 'vm'}, '/api/v1/system/auth/base_info', 'GET', (res) => {
			try {
				if (res.success) {
