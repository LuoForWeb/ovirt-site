var add_cluster =  function(){
    ...
    var initListeners =  function(){
        ...
        //点击确定按钮
        $('#clustersubmit').on('click',submitAddCluster);
        //点击取消按钮
        $('#clutercancel').on('click',cancelAddCluster);
        //ssl勾选框
    ...
    //获取集群名称
    var getClusterName =  function(){
        pAjaxRequest({}, "/api/v1/hadoop/cluster/name", "GET", function(result){
            if(result.success){
                $('#clustername').val(result.data);
            }
        });
    }
    //传输代理开关
    ...
    var verifyChange =  function(){
        ...
        //验证方式切换 所有信息都清空
        // $('.addnodeform ').find($('input[name=hostip]')).val('');
        // $('.addnodeform ').find($('input[name=username]')).val('');
        // $('.addnodeform ').find($('input[name=restapi]')).val('');
    ...
    var submitAddCluster =  function(){
        ...
        for (let i = 0; i < nodelist.length; i++) {
            if(nodelist[i].krb5_keytab == undefined  && nodelist[i].verification_type ==  2){
                notselectfile =  true;
                break;
            }
        }
        ...
        Metronic.blockUI({target: $("#AddClusterDiv"),animate: true});
        var  requestdata =  {
            "hadoop_cluster_name":String(cluster_name),
            "namenodes":nodelist,
            "applianceuuid":applianceuuid
        };
        //向后端发送数据
        pAjaxRequest(requestdata, "/api/v1/hadoop/cluster", "POST", function(result){
            Metronic.unblockUI($("#AddClusterDiv"));
            if(result.success){
                UIToastr.showSuccess(LANG.UI_HADOOP_ADD_CLUSTER,result.message);
                LOCATION('./content/hadoop/hadoop_cluster.php','infrastructure');
            }else{
                UIToastr.showWarning(LANG.UI_HADOOP_ADD_CLUSTER,result.message);
            }
        });
    }
    //获取节点信息
    var getNodeList = function(){
        ...
        for (let index = 0; index < forms.length; index++) {
            ...
            if(formitem.attr("id") != null){
                ...
                var restapi  = parseInt( formitem.find("#restapi").val());
                ...
                var nodeitem = {
                    ...
                    "udp_preference_limit":udpprincipal,
