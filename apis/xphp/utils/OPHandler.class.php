...
class OPHandler{
     ...
     * @return string           json
     */
    public function muOpResult($result, $operate, $msg = '', $level = '', $errorCode = 0, $extInfo = ''){
        $oldMsg = $msg;
        ...
        if($errorCode > 0){
            ...
            if($errorCode == 20 && $operate == Xphp::$_lang['WEB_NODE_APPLIANCE_OP_ADD']){
                $msg .= Xphp::$_lang['WEB_ERROR_PF_NETWORK_CONNECT_ERROR'];
            }else{
     ...
     * @param bool $command 是否为命令模式(消息发送成功后立即返回),默认FALSE
     */
    protected function mbOSMsg($opName, $msg, $sync = FALSE, $command = FALSE){
        return $this->mbMsg($opName, $msg, Xphp::$_config['MODULE_TYPE']['OS'], $sync, $command);
    }
    ...
    public function writeLog($msg, $type = 1){
        $file = Xphp::$_config['LOG_INFO']['PATH']. "_" .date("Y-m-d");
    ...
    public function route($params){
        $this->version = Xphp::$_apiVersion;
        $this->params = $params;
        $function = Xphp::$_apiFunction;
        $requestMethod = Xphp::$_apiRequestMethod;
        if(!$this->routeTable[$function][$requestMethod]){
            //模块错误
            return http_response_code(405);
        }
        $moduleFunction = $this->routeTable[$function][$requestMethod];
        return $this->$moduleFunction();
    }
    
    /**
     * 检查用户请求的版本是否有对应该的版本
     * @param array $functionVersion    类方法内部定义的版本列表
     * @param string $requestVersion    用户请求的版本
     */
    protected function checkVersion($functionVersion, $requestVersion){
        if(empty($functionVersion[$requestVersion])){
     ...
     * @param array $extra
     */
    public function apiResponse($success, $codeStr = '', $data = array(), $mbResult = array()){
        $apiError = include CONF_PATH . "api_error.php";
        $code = array_search($codeStr, $apiError['codes']);
        $description = $apiError['description'][$codeStr];
        if(false === $code){
            if($success){
                $codeStr = 'API_CODE_SUCCESS';
            }else{
                $codeStr = 'API_CODE_ERROR';
            }
            $code = array_search($codeStr, $apiError['codes']);
            $description = $apiError['description'][$codeStr];
        }
        $extra = array();
        if(!empty($mbResult)){
            if(false === $mbResult['result']){
                //如果后台返回错误，组合后台错误到附加字段
                $errorCode = $mbResult['errorCode'];
                $extra['error_code'] = $errorCode;
                $error = include ROOT_PATH . "api/xphp/conf/error.php";
     ...
     * 要求检测的参数不能是空,如果检测的参数有空则直接返回参数错误,程序强制退出
     */
    protected function apiParamsCheck(){
        $nums = func_num_args();
        $args = func_get_args();
        $flag = false;
        for($i=0; $i<$nums; $i++){
        ...
        if($flag){
            exit($this->apiResponse(false, "API_CODE_PARAMS_ERROR"));
        }
    }
