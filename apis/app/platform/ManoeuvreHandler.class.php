...
class ManoeuvreHandler extends BLLHandler{
    ...
    public function getOrchProxyVMConfig($params){
        ...
        $info['vmgateway'] = '';
        //备份服务器网卡
        $networkCard = Xphp::instance('SystemHandler', 'getNetworkCardList');
...
//         return;
    ...
    public function getTaskMonitor($params){
        ...
        $sql = "select  bri.task_uuid, bri.total_size, bri.current_total_size, bri.current_file_size, bri.current_completed_size, bri.current_vm_uuid,
        ...
        //演练总进度
        $info['totalprogress'] = round($data[0]['current_total_size'] / $data[0]['total_size'] * 100, 2);
        //饼图状态统计
...
//         $info['hosts'] = array();
    ...
    private function getTaskMonitorAllVMInfo($data, $proxyuuids){
        ...
        foreach ($data as $k => $d){
            ...
            $vms[] = array(
                ...
                'progress' => $this->getEachVMProgress($d['task_status'], $d['current_completed_size'], $d['current_file_size']), //进度
    ...
    public function getPlanVMTimepointTree($params){
        ...
        $sql = "select op.plan_name, op.create_time as pt, opg.group_name, opg.create_time as pgt, 
                ...
                opv.vm_name, opv.vm_uuid, opv.vcenter_uuid, opv.dir_path, opv.timepoint_type, 
        ...
        foreach ($data as $d){
            ...
            //插入时间点
            $node = array_merge($node, $this->getRecoveryVMTimepoint($nodeuuid, $d['child_uuid'], $d['vcenter_uuid'], $d['vm_uuid'], $d['plan_uuid']));
        }
     ...
     * @param string $vmuuid
     */
    private function getRecoveryVMTimepoint($nodeuuid, $childuuid, $vcenteruuid, $vmuuid, $planuuid){
        $node = array();
        $sql = "select bbt.timepoint_uuid, unix_timestamp(bbt.timepoint) timepoint, bbt.backup_mode, bbt.task_name, bbt.depend_point_uuid, bbt.task_create_time, bbt.task_uuid,
        ...
        foreach ($data as $d){
            if(!in_array($d['hypervisor_type'], $hyperviosr)){
                ...
                $tree[] = array(
                    ...
                    "nocheck" => true,
                    "type" => 0,
                    "icon" => $vcenter->getHypervisorIcon($d['hypervisor_type'], false),
                );
            }
...
//TODO这里要把预案对应的演练宿主机默认选择
                ...
                "type" => 1,
                "icon" => $vcenter->getHypervisorIcon($d['hypervisor_type'], true),
