...
class CopyHandler extends BLLHandler{
    
    /**
     * 创建副本任务,备份数据到异地
     * @param unknown $params
     */
    public function creatCopytoRemote($params){
        ...
        $vmsInfo = $this->getBackupCopyVMsInfo($params['srcInfo']['vminfo']);
        ...
        $pfMsg['task_type'] = Xphp::$_config['TASKTYPE']['BACKUP_COPY'];
     ...
     * @param unknown $params
     */
    public function editCopyJob($params){
...
//         $nodeuuid = $this->getCopyNodeuuid($params['highInfo']['node']);
     ...
     * @param unknown $params
     */
    public function creatCopyfromRemote($params){
        $task_name = $params['taskName'];
        $this->paramsCheck($task_name);
        ...
        $vmsInfo = $this->getBackupCopyVMsInfo($params['vmInfo']['points']);
        ...
        //返回结果到UI
        if($result){
        	$startResult = $this->startCopyBackJob($task_name);
        	$startResult = true;
        	if($startResult){
        		//启动任务成功,直接返回创建任务成功
        		return $this->muOpResult($result, $operate, $msg);
     ...
     */
    private function getBackupCopyVMsInfo($vminfo){
    ...
    public function getCopyStorageList($params){
        $sql = "select storage_nickname, storage_uuid, total_size, free_size from bd_storage_resource where storage_type = ?";
        $data = $this->dbSelect($sql, array(Xphp::$_config['BD_STORAGE_TYPE']['REMOTE']));
        $info = array(
            array(
                'value' => 0,
                'text' => Xphp::$_lang['UI_COPY_DATA_SELECT_STORAGE'],
            )
        );
        $utils = Xphp::instance("Utils");
        ...
        foreach ($data as $d){
            $info[] = array(
                ...
                'text' => $d['storage_nickname'] . "(" .   $storageHandler->getStorageTypeDes(Xphp::$_config['BD_STORAGE_TYPE']['REMOTE']) .
    ...
    //TODO,得到异地的存储列表,暂用,后面根据需要删除或修改
    public function getCopyStorageInfo($params){
        $storageuuid = $params['storageuuid'];
        $this->paramsCheck($storageuuid);
        
        
        $nodeuuid = Xphp::instance('NodeHandler', 'getLocalNodeUUID');  //获取本地节点uuid
        $module_type = Xphp::$_config['MODULE_TYPE']['VM'];
        //获取存储信息
        $opName = 'NODE_SR_OP_SCAN_REMOTE_SYSTEM';
        $msg = json_encode(array('storage_uuid'=>$storageuuid));
        ...
        if($mbResult['result']){
            ...
            foreach ($storageList as $list){
                $storage[] = array(
                    ...
                    'name' => $list['name'],
                    'type' => $list['storage_type'],
                    'text' => $list['name'] . "(" . $storageHandler->getStorageTypeDes($list['storage_type']).
                    ", " . Xphp::$_lang['UI_STORAGE_TOTAL_SIZE'] . ":" . $utils->calSize($list['size']) .
                    ", " . Xphp::$_lang['WEB_PLATFORM_DC_AVAILABLE_SPACE'] . ":" . $utils->calSize($list['free_size']) . ")",
                	'total_size' => $list['size'],
                	'free_size' => $list['free_size']
                );
            }
            $platformHandler = Xphp::instance('PlatformHandler');
            $sortStorage = $platformHandler->_array_column($storage,'free_size');
            array_multisort($sortStorage ,SORT_DESC, $storage);
        }
     ...
     * 获取虚拟机副本任务信息(修改任务用)
     */
    public function getCopyTaskAllInfo($params){
    	$taskUUID = $params['taskuuid'];
    	...
    	if($data){
    		$info = array(
    				...
    				//保留策略
    				'brs' => $this->getCopyReserve($data[0]['strategy_id']),
    				//节点
    				'node' => array(
    						...
    						'type' => $this->getCopyStorageType(intval($data[0]['target_storage_type']))
    				),
    				//传输策略
    				'bts' => array(
    				...
    				//虚拟机信息
    				'vm_info' => $this->getCopyEditInfo($taskUUID),
    				//时间策略
     ...
     * @return string
     */
    public function getCopyEditInfo($taskuuid){
    	$info = array();
     ...
     * @return $type
     */
    private function getCopyStorageType($storageType){
    	$type = 1;  //本地存储
...
//     					$n['chkDisabled'] = true;
     ...
     * @param unknown $params
     */
    public function getCopyStorage($params){
    	$copybackFlag = $params['copybackflag'];
    	$sql = "select storage_nickname, storage_uuid, storage_type, storage_config from bd_storage_resource where use_mode in (2, 3)
        ...
        foreach ($data as $d){
        	...
        	if($d['storage_type'] == Xphp::$_config['BD_STORAGE_TYPE']['REMOTE']){
        		...
        		$text = $d['storage_nickname'].'('.$config['remote_ip'].')';
        	}
            $info[] = array(
                'uuid' => $d['storage_uuid'],
                'text' => $text,
     ...
     * @param unknown $params
     */
    public function getCopyTaskTree($params){
    	$storageuuid = $params['storageuuid'];
    	...
    	$sql = "select bbt.task_name,unix_timestamp(bbt.task_create_time) task_create_time, bbt.data_local_flag,
                ...
                where  bbt.timepoint_uuid = vbt.timepoint_uuid and
    				   bbt.storage_uuid = bsr.storage_uuid and
                       bbt.deleted_flag = ? and bbt.available_flag = ?
	                   and bbt.import_flag = ? and bbt.module_type = ? and bbt.copy_flag = ? and bbt.remote_status = ? and
                       bbt.user_uuid = ? ";
    	$flag = Xphp::$_config['FLAG'];
    	$sqlParams = array($flag['UNSET'], $flag['SET'], $flag['UNSET'], Xphp::$_config['MODULE_TYPE']['BACKUP_COPY_CLIENT'], $flag['SET'], Xphp::$_config['COPY_POINT_STATUS']['AVAILABLE'],
    			Xphp::$_user['useruuid']);
    	...
    	//得到当前任务所有副本任务uuid
    	$currentTaskUUID = $this->getAllCopyTaskUUID();
     ...
     * @param unknown $params
     */
    public function getCopyPointVmTree($params){
    	$taskuuid = $params['taskuuid'];
    	$sql = "select vbt.vcenter_uuid,vbt.vm_uuid, vbt.vm_name, vbt.dir_path, vbt.hypervisor_type, unix_timestamp(bbt.task_create_time) task_create_time, bbt.task_name  from bd_backup_timepoint bbt, vm_backup_timepoint vbt where bbt.timepoint_uuid = vbt.timepoint_uuid and bbt.task_uuid = ? and bbt.copy_flag = ?";
    	$data = $this->dbSelect($sql,array($taskuuid, Xphp::$_config['FLAG']['SET']));
    	$vm = array();
    	$node = array();
    	$timepoint = array();
    	...
    	foreach ($data as $d){
    		...
    		$sqlPoint = "select bbt.timepoint_uuid, unix_timestamp(bbt.timepoint) timepoint, bbt.task_name, bbt.depend_point_uuid, bbt.backup_mode,
                ...
                where bbt.timepoint_uuid = vbt.timepoint_uuid and
                       bbt.storage_uuid = bsr.storage_uuid and
                       bbt.deleted_flag = ? and bbt.available_flag = ? and bbt.remote_status = ?
	                   and bbt.import_flag = ? and bbt.module_type = ? and
                       bbt.task_uuid = ?  and
                       vbt.vm_uuid = ? and bbt.copy_flag = ? order by vbt.vm_uuid,  bbt.timepoint";
    		 
    		$flag = Xphp::$_config['FLAG'];
    		$sqlParams = array($flag['UNSET'], $flag['SET'], Xphp::$_config['COPY_POINT_STATUS']['AVAILABLE'], $flag['UNSET'], Xphp::$_config['MODULE_TYPE']['BACKUP_COPY_CLIENT'],$taskuuid,  $vmuuid, $flag['SET']);
     ...
     * @param unknown $params
     */
    public function getCopyPointTree($params){
    		...
    		if($dataflag){
    			...
    			if(!in_array($localFlag, $storage)){
    				...
    				if($localFlag == Xphp::$_config['FLAG']['SET']){
    					...
    					$name = Xphp::$_lang['UI_COPY_ALLOPATRIC_STORAGE'];
    				}
    				$node[] = array(
    						"id" => $localFlag,
    						"pId" => 0,
     ...
     * @param unknown $params
     */
    public function getSyncCopyPoint($params){
    	$taskuuid = $params['id'];
    	...
    	$sqlPoint = "select bbt.timepoint_uuid, unix_timestamp(bbt.timepoint) timepoint, bbt.task_name, bbt.depend_point_uuid, bbt.backup_mode,bbt.data_local_flag,
                ...
                where bbt.timepoint_uuid = vbt.timepoint_uuid and
	                   ...
	                   and bbt.import_flag = ? and bbt.module_type = ? and
                       bbt.task_uuid = ?  and
                       vbt.vm_uuid = ? and vbt.vcenter_uuid = ? and bbt.copy_flag = ? ";
    	 
    	$flag = Xphp::$_config['FLAG'];
    	$sqlParams = array($flag['UNSET'], $flag['SET'], Xphp::$_config['COPY_POINT_STATUS']['AVAILABLE'], $flag['UNSET'], Xphp::$_config['MODULE_TYPE']['BACKUP_COPY_CLIENT'],$taskuuid,  $vmuuid, $vcenteruuid, $flag['SET']);
    	if(!empty($storageuuid)){
    		$sqlPoint .= " and bbt.storage_uuid = ?";
    		$sqlParams = array($flag['UNSET'], $flag['SET'], Xphp::$_config['COPY_POINT_STATUS']['AVAILABLE'], $flag['UNSET'], Xphp::$_config['MODULE_TYPE']['BACKUP_COPY_CLIENT'],$taskuuid,  $vmuuid, $vcenteruuid, $flag['SET'], $storageuuid);
     ...
     * return array
     */
    public function getAllCopyTaskUUID(){
    	$taskuuid = array();
    	$sql = "select task_uuid from bd_task where module_type = ? and delete_flag = ? and task_type in (".Xphp::$_config['TASKTYPE']['BACKUP_COPY'].", ".Xphp::$_config['TASKTYPE']['BACKUP_COPY_FETCH'].")";
     ...
     * @param unknown $params
     */
    public function getVMCopyTaskName($params){
    	$taskName = Xphp::$_lang['UI_PLATFORM_COPY_JOB'];
    	$vmHandler = Xphp::instance('Vmhandler');
     ...
     * @param unknown $params
     */
    public function getVMCopyBackName($params){
    	$taskName = Xphp::$_lang['UI_PLATFORM_COPY_BACK_JOB'];
    	$vmHandler = Xphp::instance('Vmhandler');
    	return $vmHandler->getValidTaskName($taskName);
    }
     ...
     * @param unknown $params
     */
    public function deleteCopypoint($params){
    	...
    	$opName = 'BD_BACKUP_COPY_POINT_OP_DELETE';
    	$msg = array($pointUUID);
    	...
    	if($result){
    		$sqlCount = "select count(bbt.id) as total from bd_backup_timepoint bbt, vm_backup_timepoint vbt
                    ...
                    bbt.available_flag = ? and vbt.vm_uuid = ? and vbt.vcenter_uuid = ? and bbt.task_uuid = ? and bbt.copy_flag = ? and bbt.timepoint_uuid = ?
                    ";
    		$flag = Xphp::$_config['FLAG'];
    		$dataCount = $this->dbSelect($sqlCount, array($flag['UNSET'], $flag['SET'], $vmuuid, $vcenteruuid, $taskuuid, $flag['SET'],$pointUUID));
