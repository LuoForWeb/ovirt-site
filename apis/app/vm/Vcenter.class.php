...
//             $this->systemLog('VM_SYSTEMLOG_DESC_KEY_ADD_VCENTER_SUCCESS', $descriptionParam, Xphp::$_config['LOGLEVEL']['ERROR']);
    ...
    public function getBackupTree($params){
        ...
        $sql = "select vv.vcenter_id, vv.vcenter_uuid, vv.vcenter_ip, vv.nickname, vv.vcenter_name, vv.hypervisor_type, vv.vcenter_flag, vv.detail, vv.username, vh.online_flag
    ...
    public function getBackupTreeSpeed($params){
        ...
        $sql = "select vcenter_flag, hypervisor_type from vm_vcenter where vcenter_uuid = ?";
...
//            $name = $vcenterip . "(" . $vcentetname . ")";
    ...
    public function getBackupTreeOldInfo($params){
        ...
        $topNode = $utils->object_array(json_decode($topNode));
        //获取该任务备份对象
        $sql = "select vol.object_uuid, vol.type, vol.vcenter_uuid, vv.vcenter_flag, vol.vm_config, vol.exclude_vm_uuid_list 
        ...
        if (empty($data)) {
            //获取该任务所备份的虚拟机
            $sql = "select vml.vm_uuid as object_uuid, 7 as type, vml.vcenter_uuid, vv.vcenter_flag , vml.vm_config, '' as exclude_vm_uuid_list 
        ...
        if(!empty($data)){
            ...
            foreach ($data as $d){
                ...
                if (Xphp::$_config['VM_TREE_TYPE']['VM'] != $d['type'] && !$params['auto_join_flag']) {
                    $notInTaskVms = $this->getObjectVmsNotInTask($params['displaymode'], $d['object_uuid'], $params['taskuuid']);
                    if ($notInTaskVms) {
...
//                }
     ...
     * @return array
     */
    public function getObjectVmsNotInTask($display_mode, $object_uuid, $task_uuid): array
    {
        $vms = [];
        $sql = "select vt.name, vt.uuid, vt.type, vt.dir_path  
            from vm_tree vt where vt.display_mode = ? and vt.parent_uuid = ? and vt.uuid != vt.parent_uuid 
            and vt.uuid not in (select vm_uuid from vm_machine_list where task_uuid = ?)";
        $data = $this->dbSelect($sql, [$display_mode, $object_uuid, $task_uuid]);
        foreach ($data as $item) {
            if (Xphp::$_config['VM_TREE_TYPE']['VM'] == $item['type']) {
                $vms[] = $item;
            } else {
                $vms = array_merge($vms, $this->getObjectVmsNotInTask($display_mode, $item['uuid'], $task_uuid));
            }
        }
        return $vms;
    }
    ...
    public function getBackupSyncVcenter($params){
        ...
        return $this->getTreeFromVMTree($hypervisor, $vcflag, $uuid, $showtype, TRUE, $openFlag, $modifyflag, $backupFlag, $taskuuid);
    }
     ...
     * @param $hypervisor   虚拟化类型
     * @param $vcenterFlag  虚拟化中心标志
     * @param string $vcenterUUID
     * @param int $showType
     ...
     * @return string
     */
    private function getTreeFromVMTree($hypervisor, $vcenterFlag, $vcenterUUID, $showType, $showVMFlag, $openFlag = FALSE, $modifyFlag = FALSE, $backupFlag = false, $taskuuid = ''){
        ...
        $sql = "select vv.user_uuid, vt.tree_id, vt.type, vt.name, vt.uuid, vt.parent_uuid, vt.dir_path, vt.conn_state, vt.power_state,
...
//            }
            ...
            $node[] = array(
                ...
                "nocheck" => !$hascheck,
                "path" => htmlspecialchars($d['dir_path']),
                "eventtype" => $vmFlag ? 'vm' : '',
                "version" =>$d['version'],
    ...
    public function getBackupTreeNodeTitle($type, $name, $vmuuid, $vcenteruuid, $backupVMInfo, $detail){
        ...
        return $name . "('" . $taskname . "'". Xphp::$_lang['WEB_BACKUP_TASK_PROTECTED'] . ")";
    }
