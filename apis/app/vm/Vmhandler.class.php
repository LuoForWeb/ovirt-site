...
class Vmhandler extends BLLHandler{
    ...
    public function createBackupJob($params){
        ...
        $pfMsg['backup_level'] = $this->getBackupSnapshotMode($params['srcInfo']['vmType'],
            $params['highInfo']['mode']['snapshotcheck'], $params['highInfo']['mode']['incmode']);
        //静默快照
        ...
        $pfMsg['reset_cbt_flag'] = (string)$utils->parseBoolToFlag($params['highInfo']['mode']['resetcbtcheck']);
        //V-CBT
...
//         }
    ...
    public function editBackupJob($params){
        ...
        //租户内检查虚拟机个数是否超过授权个数
        if(!empty($_SESSION['tenantuuid'])){
            $this->checkTenantVmAuth(count($params['srcInfo']['vminfo']), $taskuuid);
        }
        //如果是租户内部的用户操作，检查设置速度是否超出租户配置限制速度
//         if(!empty($_SESSION['tenantuuid'])){
//             $this->pCheckSpeedLimit($params['speedInfo'], $operate);
//         }
        ...
        $sql = "update vm_task set level = ?, quiesce_snapshot = ?, transport_priority = ?, valid_data_backup = ?, serial_snapshot_flag = ?, 
                   ...
                   agent_uuid = ?, display_mode = ?, select_conditions = ?, pre_create_snap_flag = ?, storage_snapshot_enable_flag = ?, detail = ? where task_uuid = ?";
        //         $level = $this->getVMTaskLevel($params['highInfo']['mode']['snapshotcheck']);
        $level = $this->getBackupSnapshotMode($params['srcInfo']['vmType'],
            $params['highInfo']['mode']['snapshotcheck'], $params['highInfo']['mode']['incmode']);
        ...
        $detailArr['reset_cbt_flag'] = (string)$utils->parseBoolToFlag($params['highInfo']['mode']['resetcbtcheck']);
        $detailArr['auto_join_flag'] = (string)$utils->parseBoolToFlag($params['srcInfo']['auto_join_flag']);
        $result = $result && $this->dbExec($sql, array($level, $quiesceSnapshot, $transportMode, $cbtMode, $snapshotMode,
                $parsefsMode, $swapMode, $deletefileMode, $gapMode, $agentUuid, $newDisplayMode, $globalSelectJson, $presnapshot,
                $storagesnapshot, json_encode($detailArr), $taskuuid));
        ...
        foreach ($vmInfo as $vm){
            ...
            if (Xphp::$_config['VM_TREE_TYPE']['VM'] == $vm['type']) {
                ...
                $sqlParams = array($taskuuid, $vm['vcuuid'], $vm['vmuuid'], $vm['vmname'], $vm['version'],
                    $vm['hostuuid'], $vm['path'], json_encode($vmConfig, JSON_UNESCAPED_UNICODE));
                $result = $result && $this->dbExec($sql, $sqlParams);
            } else {
                //其他对象递归获取下面的虚拟机并更新vm_machine_list
                $vms = $this->getObjectVms($displayMode, $vm['vmuuid'], $taskuuid);
                //遍历插入
                foreach ($vms as $vm2) {
                    //在排除列表的不插入
                    ...
                    $sqlParams = array($taskuuid, $vm2['vcuuid'], $vm2['vmuuid'], $vm2['vmname'], $vm2['version'],
                        $vm2['hostuuid'], $vm2['path'], json_encode($vmConfig, JSON_UNESCAPED_UNICODE));
            ...
            $sql2Params = array($taskuuid, $vm['vmuuid'], $vm['vcuuid'], $vm['type'], $exclude_vm_uuids, $vm['vmname'], json_encode($vmConfig, JSON_UNESCAPED_UNICODE), $vm['path']);
...
//        $result = $result && $this->dbExec($sql, $sqlParams);
     ...
     * @return array
     */
    public function getObjectVms($display_mode, $object_uuid, $task_uuid): array
    {
        $vms = [];
        $sql = "select distinct vt.type, vt.name, vt.uuid, vt.dir_path path, 
            vm.vcenter_uuid vcuuid, vm.vm_uuid vmuuid, vm.vm_name vmname, vm.version, vm.host_uuid hostuuid 
            from vm_tree vt left join vm_machine vm on vt.uuid = vm.vm_uuid and vt.vcenter_uuid = vm.vcenter_uuid where vt.display_mode = ? and vt.parent_uuid = ? and vt.uuid != vt.parent_uuid 
        ...
        $data = $this->dbSelect($sql, [$display_mode, $object_uuid, Xphp::$_config['TASKTYPE']['BACKUP'], $task_uuid]);
        foreach ($data as $item) {
            if (Xphp::$_config['VM_TREE_TYPE']['VM'] == $item['type']) {
                $vms[] = $item;
            } else {
                $vms = array_merge($vms, $this->getObjectVms($display_mode, $item['uuid'], $task_uuid));
            }
        }
        return $vms;
    }
    ...
    public function createRecoverJob($params){
        ...
        $pfMSg['recovery_vm_uuids'] = $this->groupRebuildVMInfo($recovery_position,
            $params['pointInfo']['points'], $params['recoverInfo']);
...
//         }
    ...
    private function crossHypervisorRecoveryCheck($oldHypervisor, $newHypervisor){
        ...
        //如果跨平台恢复未授权,不能恢复,ics/ics-vvdk不限制
        if(!in_array($oldHypervisor, $icsTypes) && !in_array($newHypervisor, $icsTypes) && true !== $license['authfun']['crossHypervisorRecovery']){
            $msg = Xphp::$_lang['UI_RECOVERY_CHECK_CROSS_HYPERVISOR_LICENSE_TIPS1'];
            exit($this->muOpResult(false, Xphp::$_lang['UI_RECOVERY_VM_DESCRIPTION'], $msg, 'warning'));
        }
        //如果跨平台恢复授权数量不足,不能恢复
        if(!in_array($oldHypervisor, $icsTypes) && !in_array($newHypervisor, $icsTypes) && $license['v2v']['valid'] <= 0 && $license['v2v']['total'] != -1){
    ...
    public function groupTransportStrategy($transport,$strategygroupuuid = ""){
        $strArr = array(
            ...
            'network_pool_uuid' => !empty($transport['network_pool_uuid'])? $transport['network_pool_uuid']: "", //传输网络
        ...
        if($mbResult['result']){
            ...
            foreach ($storageList as $list){
                ...
                if(!empty($_SESSION['tenantuuid']) && !$allStorageFlag){
                    if($list['storage_uuid'] != $tenantDesStorage) continue;
                }
        ...
        if($mbResult['result']){
            ...
            foreach ($networkList as $list){
                ...
                if(!empty($_SESSION['tenantuuid']) && !$allNetworkFlag){
                    if($list['network_uuid'] != $tenantNetwork) continue;
                }
                $network[] = array(
                    'uuid' => $list['network_uuid'],
...
//             }
    ...
    public function getSyncTimepoint($params){
        ...
        $sqlPoint = "select bbt.detail, bbt.deleted_flag, bbt.timepoint_uuid, unix_timestamp(bbt.timepoint) timepoint, bbt.backup_mode, bbt.task_name, bbt.depend_point_uuid,
		              ...
		              vbt.vm_uuid, vbt.vm_name, vbt.dir_path, vbt.vcenter_uuid, vbt.version, vbt.hypervisor_type, bsr.node_uuid, bsr.status
                from bd_backup_timepoint bbt, vm_backup_timepoint vbt, bd_storage_resource bsr
                where bbt.timepoint_uuid = vbt.timepoint_uuid and
...
//             }
            ...
            if($point['backup_mode'] == Xphp::$_config['BACKUP_MODE']['FULL']){
                if(!in_array($timepointuuid, $timepoint)){
                    ...
                    $node[] = array(
                        ...
                        "type" => 3,
                        "vmuuid" => $point['vm_uuid'],
                        "vmname" => htmlspecialchars($point['vm_name']),
                        "pointname" => $this->parseDate($point['timepoint']),
                        "vcenteruuid" => $point['vcenter_uuid'],
                        "timepointuuid" => $timepointuuid,
                        "createtime" => $taskCreateTimeIn,
                        ...
                        "version" => $point['version'],
                        "icon" => $this->getTimepointIcon($point['backup_mode']),
                        "title" => htmlspecialchars($this->getBackupTimepointTreeTitle($point['dir_path'], $point['remarks'])),
                        "hypervisor" => $hypervisor,
    ...
    public function getRecoverHost($params){
        ...
        foreach ($data as $d){
            ...
            if($instantFlag && $hypersior!= intval($d['hypervisor_type']) && (
                    intval($d['hypervisor_type']) == Xphp::$_config['VMHYPERVISORTYPE']['VM_HYPERVISOR_TYPE_SMARTX_KVM'] ||
...
//                 "name" => $d['vcenter_ip'] == $d['nickname'] ? $d['vcenter_ip'] : $d['nickname'] . "(" . $d['vcenter_ip'] . ")",
    ...
    private function getPlanVMCheckDisabled($vms, $vmuuid, $vcenteruuid){
        $chkDisabled = false;
        if(empty($vms)) return $chkDisabled;
        foreach ($vms as $vm){
...
//        $dataexport = $this->dbSelect($sqlexport, array());
    ...
    public function getMotionVMConfigs($params){
        ...
        if($hypervisor == Xphp::$_config['VMHYPERVISORTYPE']['VM_HYPERVISOR_TYPE_RHV_KVM'] || $hypervisor == Xphp::$_config['VMHYPERVISORTYPE']['VM_HYPERVISOR_TYPE_ZSTACK']
                ...
                foreach ($data as $d){
                    ...
                    $strategyData[] = array(
                        ...
                        'mode' => !in_array(Xphp::$_config['BACKUP_MODE']['FULL'], $allBackupMode) && Xphp::$_config['BACKUP_MODE']['INCREMENTAL'] == $d['mode']
                            ? Xphp::$_config['BACKUP_MODE']['PINCREMENTAL'] : $d['mode'],  // 没有完全备份就是永久增量
    ...
    public function getBackupTaskAllInfo($params){
        ...
        if($data){
            ...
            $detailArr = (!$detail || 'null' == $detail) ? [] : json_decode($detail, true);
            $reset_cbt_flag = $detailArr['reset_cbt_flag'] ?? Xphp::$_config['FLAG']['UNSET'];
        ...
        if(!empty($data)){
            ...
            foreach ($data as $d){
                ...
                if(!empty($d['vm_detail'])){
                    ...
                    if(in_array(intval($d['hypervisor_type']), Xphp::$_config['VMHYPERVISORGROUP']['openstack']) ||
    ...
    public function getVMReportLastBackupInfo($vmList){
        ...
        $sqlParams = array(Xphp::$_config['FLAG']['SET'], Xphp::$_config['TASKTYPE']['BACKUP'], Xphp::$_config['MODULE_TYPE']['VM']
        );
        $data = $this->dbSelect($sql, $sqlParams);
        $i = 0;
        $backupStorage = 0;
...
//             $sizeInfo = $this->getVmStorageSize($d['vm_uuid'], $d['vcenter_uuid']);
    ...
    private function getGrainFileClassName($filetype, $filename, $size){
        ...
        if(intval($filetype) == Xphp::$_config['GUEST_FILE_ITEM_TYPE']['DIR']){
            ...
            return $class;
        }elseif(intval($filetype) == Xphp::$_config['GUEST_FILE_ITEM_TYPE']['SLINK'] ||
    ...
    private function getVmGrainRecoveryFileDirFirst($taskuuid, $sclass, $showtype, $searchName){
        ...
        foreach ($rootList as $d){
            //按名称模糊搜索
            if(!empty($searchName)){
                if(!in_array($searchName, $d)) continue;
            }
    ...
    public function downloadGrainRecoveryDirCheck($params){
        ...
        if($mbResult['result']){
            ...
            //失败,返回后台失败信息
            return $this->muOpResult($mbResult['result'], Xphp::$_lang['VM_PRIVATE_TASK_OP_GRAIN_START_DOWNLOAD_DIR'],
                Xphp::$_lang['VM_PRIVATE_TASK_OP_GRAIN_START_DOWNLOAD_DIR'], '', $mbResult['errorCode']);
        }
    }
    ...
    public function getVMEditInfo($taskuuid, $display_mode, $auto_join_flag){
        ...
        $sql =  "select vol.object_uuid, vol.vcenter_uuid, vol.type, vol.exclude_vm_uuid_list, vol.object_name, vol.vm_config, 
            ...
            from vm_object_list vol left join vm_tree vt on vol.object_uuid = vt.uuid and vol.type = vt.type and vt.display_mode = ?
            where vol.task_uuid = ?";
        ...
        foreach ($data as $d){
            ...
            if (Xphp::$_config['VM_TREE_TYPE']['VM'] != $d['type'] && !$auto_join_flag) {
                $notInTaskVms = $Vcenter->getObjectVmsNotInTask($display_mode, $d['object_uuid'], $taskuuid);
    ...
    public function groupTaskSpeedGlobalList($speedList) {
        ...
        if ($speedList['type'] == 1) {
            ...
            $speedList['speed'] = $speedList['speed'] ?: [];
            // 自定义
            $info['strategy_uuid'] = '';
            $info['strategy_name'] = '';
            $info['is_global'] = 0;
            $info['extra_info'] = json_encode($speedList['speed']);
            // 查询出type和组装下策略
            if (isset($speedList['speed']) && count($speedList['speed'])) {
                $utils = Xphp::instance('Utils');
                $strategy_type = $speedList['speed'][0]['type'];
                $info['strategy_type'] = $strategy_type;
                if (in_array($strategy_type, [1, 4, 5])) {
                    // 这三个类型的没有days的值
                    $timelist = [];
                    foreach ($speedList['speed'] as $item) {
                        $timelist[] = [
    ...
    public function getVirtualLabHost($params){
        ...
        foreach ($data as $d){
            ...
            if($editFlag){
                ...
                if($vcenteruuid != $d['vcenter_uuid']) continue;
                $openFlag =true;
            }
            $node = array(
                "id" => $d['vcenter_uuid'],
...
//                     $pid = $timepointuuid;
    ...
    public function getBackupTimepointTree($params){
        ...
        // $this->paramsCheck($showtype);
        $sql = "select bbt.storage_uuid, bbt.task_name, bbt.task_type, bbt.module_type,
    			bbt.task_uuid, bbt.user_uuid, bbt.user_name, vbt.vcenter_uuid, vbt.vm_uuid, vbt.vm_config, vbt.vm_name, vbt.dir_path, vbt.hypervisor_type, bsr.node_uuid, bsr.storage_uuid,bsr.storage_type, vbt.version
                from bd_backup_timepoint bbt, vm_backup_timepoint vbt, bd_storage_resource bsr
                where bbt.timepoint_uuid = vbt.timepoint_uuid and
                      bbt.storage_uuid = bsr.storage_uuid and 
                      bbt.available_flag = ?
        ...
        if(empty($storageuuid)){
            ...
            //如果是瞬时恢复或者细粒度恢复 所有存储还要排除云存储
            if($instantflag || $grainflag){
                $sql .= " and bsr.storage_type not in (". Xphp::$_config['BD_STORAGE_TYPE']['HUAWEI_CBR'] . "," . Xphp::$_config['BD_STORAGE_TYPE']['CLOUD'] . ") ";
            }else{
