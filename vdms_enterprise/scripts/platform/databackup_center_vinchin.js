var DataBackupCenter = function () {
    ...
    var setAlarmInfo = function (d) {
        ...
        var badgeType = "badge-mark-warning";
        var tips = "";
        if (errorTotal > 0) {
            badgeType = "badge-mark-danger";
        }
        if (total > 0) {
            var tips = '<span class="badge-mark navigation ' + badgeType + ' badgemark">' + total + '</span>';
        }
        ...
        if (d.storage) {
            //管理备份存储
            var tips = '<span class="badge-mark menu badge-mark-warning badgemark">' + d.storage + '</span>';
        ...
        if (d.lisence) {
            //系统授权
            var tips = '<span class="badge-mark menu badge-mark-danger badgemark">' + d.lisence + '</span>';
    ...
    const initAwaitingApprovalReport = () => {
        return new Promise((resolve, reject) => {
            pAjaxRequest({ offset: 0, type: 3, limit: 5 }, '/api/v1/industry/report', 'GET', function (res) {
                if (res.success) {
                    let awaitingApprovalList = res.data.rows.length > 0 ? res.data.rows.map(i => { return { ...i,approval_list: JSON.parse(i.approval_list) }}) : [];

                    resolve(awaitingApprovalList);
                } else {
                    reject([]);
                }
            });
    ...
    const initAwaitingInitiateReport = () => {
        return new Promise((resolve, reject) => {
            pAjaxRequest({ offset: 0, type: 4, limit: 5 }, '/api/v1/industry/report', 'GET', function (res) {
                if (res.success) {
                    let awaitingInitiateList = res.data.rows;
                    
                    resolve(awaitingInitiateList);
                } else {
                    reject([]);
                }
            });
    ...
    const initAwaitingApprovalPlan = () => {
        return new Promise((resolve, reject) => {
            pAjaxRequest({ offset: 0, type: 4, limit: 5 }, '/api/v1/industry/plan', 'GET', function (res) {
                if (res.success) {
    ...
    const initAwaitingInitiatePlan = () => {
        return new Promise((resolve, reject) => {
            pAjaxRequest({ offset: 0, type: 5, limit: 5 }, '/api/v1/industry/plan', 'GET', function (res) {
                if (res.success) {
                    let awaitingApprovalList = res.data.rows;
    ...
    const renderTodoList = (arraylist) => {
        if (!Array.isArray(arraylist) || arraylist.length === 0) return '暂无数据';

        let resultHtml = '';
        arraylist.forEach(item => {
            let badge = '';
            switch (item.reportType) {
                case 'awaiting-approval-report':
                    badge = `<span class="badge badge-warning ${item.routeBtnClass}" data-uuid="${item.uuid}">待审批</span>`;
                    break;
                case 'awaiting-initiate-report':
                    badge = `<span class="badge badge-secondary ${item.routeBtnClass}" data-uuid="${item.uuid}">待发起</span>`;
                    break;
                case 'awaiting-approval-plan':
                    badge = `<span class="badge badge-warning ${item.routeBtnClass}" data-uuid="${item.uuid}">待审批</span>`;
                    break;
                case 'awaiting-initiate-plan':
                    badge = `<span class="badge badge-secondary ${item.routeBtnClass}" data-uuid="${item.uuid}">待发起</span>`;
                    break;
                default:
                    break;
            ...
            resultHtml += `<div class="item mt8">
                            <div class="item-info"><div>${item.reportName}${item.typeDes}</div><span class="approve-time" style="color:#666666;margin-left:auto;white-space:nowrap">${item.time}</span></div>
                            ${badge}
                        </div>`;
        });
    ...
    const initTodoList = async () => {
        let awaittingApprovalReports = await initAwaitingApprovalReport();
        if (awaittingApprovalReports.length > 0) {
            awaittingApprovalReports = awaittingApprovalReports.map(item => {
                ...
                return {
                    ...
                    reportType: 'awaiting-approval-report',
        ...
        let awaittingInitiateReports = await initAwaitingInitiateReport();
        if (awaittingInitiateReports.length > 0) {
            awaittingInitiateReports = awaittingInitiateReports.map(item => {
                return {
                    uuid: item.job_uuid,
                    reportName: item.name,
                    reportType: 'awaiting-initiate-report',
        ...
        let awaittingApprovalPlans = await initAwaitingApprovalPlan();
        if (awaittingApprovalPlans.length > 0) {
            awaittingApprovalPlans = awaittingApprovalPlans.map(item => {
                return {
                    uuid: item.uuid,
                    reportName: item.name,
                    reportType: 'awaiting-approval-plan',
        ...
        let awaittingInitiatePlans = await initAwaitingInitiatePlan();
        if (awaittingInitiatePlans.length > 0) {
            awaittingInitiatePlans = awaittingInitiatePlans.map(item => {
                return {
                    uuid: item.uuid,
                    reportName: item.name,
                    reportType: 'awaiting-initiate-plan',
                    time: item.update_time,
                    typeDes: '待发起方案',
                    moveDes: '待发起',
        ...
        let finalTodoList = [...awaittingApprovalReports, ...awaittingInitiateReports, ...awaittingApprovalPlans, ...awaittingInitiatePlans];

        // 按时间排序，将最近的代办项放在最前面
        finalTodoList.sort((a, b) => new Date(b.time) - new Date(a.time));
        finalTodoList = finalTodoList.slice(0, 5);

        let html = renderTodoList(finalTodoList);

        $('.message-content').html(html);

        $('.awaiting-approval-report-btn').on('click', () => {
            LOCATION('./content/platform/industry/report_list.php', 'verification_job');
        });

        $('.awaiting-initiate-report-btn').on('click', function() {
            let uuid = $(this).attr('data-uuid');
            LOCATION(`./content/platform/dataverification/verification_job_details.php?type=37&uuid=${uuid}`, 'industry_report');
        });
