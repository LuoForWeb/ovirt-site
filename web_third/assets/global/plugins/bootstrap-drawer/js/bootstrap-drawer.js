...
+function ($) {
    ...
    Drawer.prototype.show = function(_relatedTarget){
        ...
        if(!this.$backdrop){
            const _backdrop = this.$element.data('backdrop');
            this.$backdrop = $(`<div class="drawer-backdrop" data-backdrop="${_backdrop}"></div>`);
            this.$body.append(this.$backdrop);
            this.$backdrop.data('bs.drawer', this);
            const self = this;
            this.$backdrop.click(function(e){
                if ('static' !== $(this).data('backdrop')) {
                    $(this).data('bs.drawer').hide(e);
                } else {
                    self.$element.addClass('drawer-static');
                    setTimeout(() => {
                        self.$element.removeClass('drawer-static');
                    }, 300);
                }
            });
        }
