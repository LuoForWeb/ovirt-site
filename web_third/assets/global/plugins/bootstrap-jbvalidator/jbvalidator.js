(function ($) {
    ...
    const EXCLUDE_VALIDATION_TOOLBAR_CLASS = 'toolbar-search-wrapper';
    const EXCLUDE_VALIDATION_PAGINATION_CLASS = 'fixed-table-pagination';
    ...
    $.fn.jbvalidator = function (options) {
        ...
        let errorMessages = {
            maxValue: "值必须小于或等于 %s.",
            minValue: "值必须大于或等于 %s.",
            maxLength: "最大字符数不能超过 %s 个",
            minLength: "最小字符数不能少于 %s 个",
            minSelectOption: "至少选择 %s 项",
            maxSelectOption: "至多选择 %s 项",
            groupCheckBox: "至少选择 %s 项",
            equal: "此字段与 %s 字段不匹配",
            fileMinSize: "文件大小不能小于 %s 字节",
            fileMaxSize: "文件大小不能大于 %s 字节",
            number: "请输入数字",
            HTML5: {
                valueMissing: {
                    INPUT: {
        ...
        /**
         * Check whether the element is within the container that needs to be excluded
         * @param {HTMLElement} el 
         * @returns {boolean}
         */
        function isInExcludedContainer(el) {
            return $(el).closest('.' + EXCLUDE_VALIDATION_TOOLBAR_CLASS).length > 0 || 
                   $(el).closest('.' + EXCLUDE_VALIDATION_PAGINATION_CLASS).length > 0;
        }

        /**
         * Obtain the form elements that need to be verified
         * @param {HTMLElement} form 
         * @returns {jQuery}
         */
        function getValidatableElements(form) {
            let formElement = form ? form : FORM;
            return $(formElement).find(selector).filter(function() {
                return !isInExcludedContainer(this);
            });
        }
        ...
        let checkAll = function (form, event) {
            ...
            getValidatableElements(thisForm).each((i, el) => {
         ...
         */
        let run = function () {
            getValidatableElements(FORM).each((i, el) => {
        ...
        let validationRun = function (el, event) {
            ...
            let isAncestorHidden = $(el).parents().is(':hidden');
        ...
        let validator = {
            ...
            multiSelectMin: function (el) {
                ...
                if ($(el).prop("tagName") === "SELECT" && $(el).prop('multiple')) {
                    ...
                    if (selectedCount < mustSelectedCount && ($(el).prop('required') || selectedCount > 0)) {
            ...
            groupCheckRadio: function (el, event) {
                if (hasAttr(el, 'type', 'radio')) {
                    
                    let checkGroup = $(el).closest('[data-checkradio-group]');
                    
                    if (checkGroup.length && isInExcludedContainer(checkGroup[0])) {
                        return '';
                    }
                    
                    let checkedCount = checkGroup.find('input[type=radio]:checked').length;
                    ...
                    if (checkGroup) {
                        ...
                        if (typeof event.originalEvent !== "undefined" && event.originalEvent.type === 'input') {
                            $(checkGroup).find('input[type=radio]').each((i, item) => {
                                $(item).trigger('input');
                            })
                        }

                        if (groupRequired && checkedCount === 0) {
                            return errorMessages.HTML5.valueMissing.INPUT.radio.sprintf(checkedCount);
                        }
            ...
            customMin: function (el) {
                ...
                if (hasAttr(el, 'data-v-min')) {
                    ...
                    if (isNaN(value) && ($(el).prop('required') || value.length > 0)) {

                        return errorMessages.number;
                    }

                    if (value < mustMin && ($(el).prop('required') || value.length > 0)) {
            ...
            customMax: function (el) {
                ...
                if (hasAttr(el, 'data-v-max')) {
                    ...
                    if (value > mustMax && ($(el).prop('required') || value.length > 0)) {
            ...
            customMinLength: function (el) {
                ...
                if (hasAttr(el, 'data-v-min-length')) {
                    ...
                    if (value < mustMin && ($(el).prop('required') || value > 0)) {
            ...
            customMaxLength: function (el) {
                ...
                if (hasAttr(el, 'data-v-max-length')) {
                    ...
                    if (value > mustMax && ($(el).prop('required') || value > 0)) {
