...
class Upload {
	...
	private $params; //参数
	private $defaultMaxSize = 1024; //上传文件默认最大值
// 	private $defaultAllowFileType = array('gif','jpeg','jpg','png','bmp','swf', 'txt');
	private $defaultAllowFileType = array('key');
	...
	private function checkUpload($error) {
		if ($error == Upload::UPLOAD_ERR_INI_SIZE) { //上传是否超过ini设置
			return 'upload_err_ini_size';
		} elseif ($error == Upload::INPUT_MAX_FILE_SIZE) { //上传是否超过表单设置
			return 'input_max_file_size';
		} elseif ($error == Upload::UPLOAD_HALF) { //上传一半
			return 'upload_half';
		} elseif ($error == Upload::UPLOAD_ERR_NO_TMP_DIR) { //上传临时目录创建错误
