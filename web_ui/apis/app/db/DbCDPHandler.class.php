...
class DbCDPHandler extends OPHandler{
    ...
    public function createBackupJob($params){
        ...
        if($data[0]['total'] > 0){
            return $this->muOpResult(false, $operate,"对应业务主机和备份主机已经有备份任务存在!", "warning");
        }        
...
//         return true;
    ...
    private function settingTakeoverConfig($productHostInfo, $standbyhostInfo, $params){
        ...
        if(!$takeoverInfo['check']) return true; //如果接管关闭,直接返回
...
//         }
    ...
    public function createRecoveryJob($params){
        ...
        return $this->muOpResult($result, '创建数据库恢复任务');
    }
    ...
    public function getDbHostInfo($params){
        ...
        $sql = "select cdh.host_uuid, cdh.host_name, cdh.ip, cdh.os_version, 
                cdh.host_type, cdh.status, cdh.register_time, cdh.detail, bu.user_name from 
                cdp_db_host cdh, bd_user bu where cdh.user_uuid = bu.user_uuid ";
        $sqlCount = "select count(ip) as total from cdp_db_host ";
    ...
    public function addHost($params){
        ...
        $data = $this->checkHostInfo($hosttype, $hostip);
     ...
     * @param unknown $hostusername
     * @param unknown $hostpassword
     */
    private function checkHostInfo($hosttype, $hostip){
        $rpc = Xphp::instance('DbRPCHandler');
        ...
        $data = array(
            'licport' => "7999",
            'licipstr' => Xphp::$_config['DB_CDP_LICENSE_IP'],
            'licuser' => "admin",
            'licpass' => "admin"
        );
...
//         $this->checkHostInSystem($operate, $hosttype, $hostip);
    ...
    public function deleteHost($params){
        ...
        if(Xphp::$_config['DB_CDP_HOST_STATUS']['ONLINE'] == intval($hostInfo['status'])){
            //如果机器在线,清除授权
            //配置网络授权机器
            $data = array();
        ...
        if($hostInfo['host_type'] == Xphp::$_config['DB_CDP_HOST_TYPE']['product']){
            ...
            $result = $rpc->freeLicenseCenterAuth(Xphp::$_config['DB_CDP_LICENSE_IP'], $data);
        }else{
            //如果是备份主机
            $data = array(
                'syscode' => Xphp::$_config['DB_CDP_SYSCODE']['standbyhost'],
    ...
    private function checkHostInSystem($operate, $hostType, $hostIP){
        $sql = "select count(ip) as total from cdp_db_host where ip = ? and host_type = ?";
        $data = $this->dbSelect($sql, array($hostIP, $hostType));
        if($data[0]['total'] > 0){
    ...
    public function gettHostInfoWithType($params){
        ...
        $sql = "select host_uuid, host_name, ip from cdp_db_host where status = 1 and host_type = ?";
        $data = $this->dbSelect($sql, array($hosttype));
        $info = array();
        foreach ($data as $d){
            $info[] = array(
                'uuid' => $d['host_uuid'],
    ...
    public function gettRecoveryHostInfo($params){
        $sql = "select host_uuid, host_name, ip from cdp_db_host where status = 1";
    ...
    public function getHostInstance($params){
        ...
        if(!empty($standbyhostuuid)  && in_array($params['dbtype'], Xphp::$_config['DB_CDP_VENDOR'])){
            ...
            if($standbyhostInfo['ip'] == $desthostInfo['ip']){
                //如果本机恢复
                $syscode = Xphp::$_config['DB_CDP_SYSCODE']['standbyhost'];
            }else{
                //如果异机恢复
    ...
    public function getHostService($params){
        ...
        foreach ($service as $d){
            $data = array(
                '<input type="checkbox" name="id[]" value="'. $d['serviceName'] .'">',
                $d['displayName'],
                $d['note'],
...
//             $dbData = array_reverse($dbData);
            ...
            //如果是接管任务,获取接管信息
            if($taskStatus == Xphp::$_config['TASKSTATUS']['TAKEOVER'] ||
...
//         $data = array_reverse($data);
        ...
        foreach ($data as $key => $value){
            ...
            if(0 > $bkstatus){
                //错误
                $info[] = array($value['btime_err'],'错误','数据库"' . $dbTxt . '"备份错误,请检查! 错误码: "'. $bkstatus .'",
                                                错误描述: "' . $this->getErrorCodeDes($bkstatus) . '"',array('level'=>3));
                $dbFailureCount++;
            }
    ...
    private function getTakeoverRunningLog($taskStatus, $productHost, $standbyHost){
        ...
        if(in_array($takeroverid, $startStatusArr)){
            ...
            foreach ($stepInfo as $key => $value){
                if(0 == $key) continue;        //第一项未总状态,跳过,标准流程0,1,4,5,6,7,9,10,11,12
                switch ($value['step']){
                    case 0://0
...
//                         $info[] = array($value['btime'], '正常', "停用接管服务器服务", array('level'=>1));
        ...
        if(in_array($takeroverid, $stopStatusArr)){
            ...
            $stepInfo = $data[0]['steps'];
            foreach ($stepInfo as $key => $value){
                if(0 == $key) continue;        //第一项未总状态,跳过,标准流程0,101,105,106,107,113
                switch ($value['step']){
                    case 0://0
    ...
    public function getDetailsDatabase($params){
        ...
        foreach ($data as $d){
            ...
            $speedAndProcess = $this->calSyncDbSpeedAndProcess($dbStatus, $syncStartTime, 
                $syncEndTime, $currentTime, $filesize, $recvsize);
    ...
    private function getDBconfigInfo($config, $dbtype, $dbsrv, $dbname){
        ...
        foreach ($config as $conf){
            if($conf['dbtype'] == $dbtype && $conf['dbsrv'] == $dbsrv && $conf['dbname'] == $dbname){
                $conf['memory_target'] = $utils->calSize($conf['memory_target']);
                $conf['pga_aggregate'] = $utils->calSize($conf['pga_aggregate']);
                $conf['sga_max_size'] = $utils->calSize($conf['sga_max_size']);
                $conf['sga_target'] = $utils->calSize($conf['sga_target']);
                return $conf;
            }
        }
    }
...
//         return;
        ...
        $info = array(
            ...
            'log' => $this->getRecoveryRunningLog($dbData, $standbyHost['ip'], $msgData)
        );
        return json_encode($info);
    }
     ...
     * @param unknown $params
     */
    public function getRecoveryRunningLog($dbData, $standbyHostIP, $msgData){
        $startTime = $dbData['btime'];
        ...
        if($dbData['workid'] ==  $workidConf['UNWRITE']){
            //workid 4 导入
            $info[] = array($dbData['segtime3'], '正常', "开始数据恢复,传输恢复文件", array('level'=>1));
            $info[] = array($dbData['segtime4'], '正常', "文件传输完成,正在恢复数据库", array('level'=>1));
        }
        if($dbData['workid'] ==  $workidConf['FINISHED'] && 0 == $dbData['msgcode']){
            ...
            $rpc = Xphp::instance('DbRPCHandler');
            $result = $rpc->scanBackupFiles($standbyHostIP, $msgData);
            if($result['result']){
                //如果请求成功,添加恢复路径
    ...
    public function getCDPDBTree($params){
        ...
        $sql = "select bt.task_uuid, bt.task_name, cdt.product_host_uuid, cdt.standby_host_uuid, cdt.config, 
                ...
                where bt.task_uuid = cdt.task_uuid and cdt.standby_host_uuid = cdh.host_uuid and bt.task_type = ?";
        $data = $this->dbSelect($sql, array(Xphp::$_config['TASKTYPE']['DB_CDP_BACKUP']));
    ...
    public function recoveryInstanceTestCon($params){
        ...
        foreach ($data as $d){
            if($d['dbtype'] == $instance['dbtype'] && $d['dbsrv'] == $instance['instance'] && 
     ...
     */
    private function checkHostServiceStatus($syscode, $hostip){
        $rpc = Xphp::instance('DbRPCHandler');
        $data = array(
            'syscode' => $syscode
        );
...
//         $this->checkRPCMsg($operate, $result);
    ...
    private function checkHostBackupJobInRunning($hostIP){
        ...
        $sqlParams = array(Xphp::$_config['TASKTYPE']['DB_CDP_BACKUP'], 
            Xphp::$_config['TASKSTATUS']['RUNNING'], $hostInfo['host_uuid']);
        $data = $this->dbSelect($sql, $sqlParams);
        if($data[0]['total'] > 0){
            return true;
        }
    ...
    private function checkHostRecoveryJobInRunning($hostIP){
        ...
        $sql = "select count(bt.task_uuid) as total from bd_task bt, cdp_db_task as cdt where
                bt.task_uuid = cdt.task_uuid and  bt.task_type = ? and bt.task_status = ? and 
                product_host_uuid = ?";
        $sqlParams = array($taskuuid, Xphp::$_config['TASKTYPE']['DB_CDP_RECOVERY'], 
            Xphp::$_config['TASKSTATUS']['RUNNING'], $hostInfo['host_uuid']);
        $data = $this->dbSelect($sql, $sqlParams);
        if($data[0]['total'] > 0){
            return true;
...
//                         'endTime' => $dbs['endtime'],
     ...
     * @param string $hostip
     */
    private function startBakSystem($syscode, $hostip){
        //先获取系统状态
     ...
     * @param unknown $hostip
     */
    private function stopBakSystem($syscode, $hostip){
        //先获取系统状态
        $serviceStatus = $this->checkHostServiceStatus($syscode, $hostip);
        $bakStatus = $serviceStatus['bakStatus'];
        $resStatus = $serviceStatus['resStatus'];
    ...
    public function checkHostLog($params){
        ...
        $result = $rpc->getRunLogs($hostip, $msgData);
        $data = $this->checkRPCMsg($operate, $result);
        //如果能够读取到记录,返回成功
        if(count($data) > 0){
            return $this->muOpResult(true, $operate);
    ...
    public function getHostLogFile($params){
        ...
        Header("Content-Disposition: attachment; filename=" . $hostip . ".txt");
        do{
            $msgData = array(
                'fno' => 0,
                'bfoff' => $bfoff,
     ...
     * @param unknown $params
     */
    public function getEnvStr($params){
     ...
     * @param unknown $params
     */
    public function getLicenseCenterAuthorInfo($params){
        $hostip = Xphp::$_config['DB_CDP_LICENSE_IP'];
        $operate = "获取网络获取授权配置";
        $rpc = Xphp::instance('DbRPCHandler');
        $data = array();
...
//         kill $check
    ...
    private function writeTaskLog($taskuuid, $taskName, $task_type, $desKey){
        ...
        $sqlParams = array(
            ...
            Xphp::$_config['MODULE_TYPE']['OEM_DBCDP'], 0, date("Y-m-d H:i:s"), $desKey, $desParam,
