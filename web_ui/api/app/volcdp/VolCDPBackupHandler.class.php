 ...
 ********************************************/
class VolCDPBackupHandler extends BLLHandler
{
    /**
     * 创建VolCDP备份任务
     * @param unknown $params
     */
    public function createVolCDPBackupJob($params)
    {
        $task_name = htmlspecialchars_decode($params['taskName']);
        //         $this->paramsCheck($task_name);
        return Xphp::$_lang['UI_VOL_CDP_BACKUP_CREATE_TASK'] . ":" . $task_name;
    }
     ...
     * @return string
     */
    public function getVolCdpBackupAgentTree($params)
    {
        ...
        if ($utils->v1_auth_need_operation()) {
            ...
            $resourceUuidSql = $utils->v1_auth_get_source_by_type(10, 'agent_uuid');
            $sql .= " and ({$resourceUuidSql}) ";
        }

        $sql .= " order by online_flag ";
        $sqlParams = array(Xphp::$_config['AGENT_TYPE']['NORMAL']);
        ...
        foreach ($data as $d) {
            ...
            if (count($isCreateTask) > 0) {
                ...
                $inTask = true;
                $title = $hostInfo . "( " . $task_name . Xphp::$_lang['WEB_BACKUP_TASK_PROTECTED'] . ")";
            }
            ...
            if ($onlineFlag == Xphp::$_config['FLAG']['UNSET']) {
                $onlineStr = Xphp::$_lang['WEB_AGENT_STATUS_OFFLINE'];
     ...
     * @param unknown $params
     */
    public function updateAgentVolInfo($params)
    {
        $agentuuid = $params['agent_uuid'];
        $refreshAgentSet = array();
        $refreshAgentSet['agent_uuid'] = $agentuuid;
        ...
        $nodeHandler = Xphp::instance('NodeHandler');
        $nodeuuid = $nodeHandler->getvCenterUUID();  //获取主节点UUID
        $opName = 'VOL_CDP_TASK_SCANNING_DISK_INFO';  //更新应用控制码
    ...
    private function getVolRelationDosVol($appVolInfo, $agentuuid)
    {
        ...
        foreach ($appVolInfo as $d) {
            ...
            if ($systemVol) {
                ...
                $bootData = $this->dbSelect($bootSql, array($agentuuid, Xphp::$_config['FLAG']['SET']));
                if (!empty($bootData)) {
     ...
     * 删除数组中指定键的对应的元素
     */
    public function byKeyReitem($array, $key)
    {
    ...
    public function getHostVolinfo($params)
    {
        ...
        $sqlCount = $sql;
        $data = $this->dbSelect($sqlCount, array($agentuuid));
        ...
        if (!empty($data)) {
            foreach ($data as $vol_info) {
                ...
                $systemVol = false;
                if ($volTypeValue & $VolCdpDes['VOL_TYPE']['BD_SYSTEM_VOLUME']) {
                    $systemVol = true;
                }
                ...
                if ($volTypeValue & $VolCdpDes['VOL_TYPE']['BD_BOOT_VOLUME'] && $volTypeValue & $VolCdpDes['VOL_TYPE']['BD_SYSTEM_VOLUME']) {
                    $isBootOrSysVolume = true;
                }
                if (($volTypeValue & $VolCdpDes['VOL_TYPE']['BD_EFI_VOLUME'] || $volTypeValue & $VolCdpDes['VOL_TYPE']['BD_BOOT_VOLUME']) && $isBootOrSysVolume != true) {
                    $bootPart = true;
                ...
                $uefi = false;
                if($volTypeValue & $VolCdpDes['VOL_TYPE']['BD_EFI_VOLUME']){
                    $uefi = true;
                }
                ...
                $displayVolName =  $volIcon . $vol_info['display_name'];
                if($volTypeValue & $VolCdpDes['VOL_TYPE']['BD_REMOVABLE_VOLUME']){
                    $displayVolName = $displayVolName." (".Xphp::$_lang['WEB_OS_REMOVE_DEVICE'].")";
                }
                $volumesInfo[] = array(
                    '<input type="checkbox" id = "' . $bootPartStr . '" class="editor-active" ' . $disabled . ' name="id[]" value="' . $vol_info['vol_uuid'] . '" "' . $volIsChecked . '">',
                    $displayVolName,
                    $mountPoint,
                    $utils->calSize($vol_info['capacity'], true),
                    $utils->calSize($vol_info['free_space'], true),
                    $vol_info['vol_uuid'],
                    $vol_info['capacity'],
                    $vol_info['display_name'],
                    $vol_info['os_type'],
                    $volTypeValue,
                    $systemVol,
                    $bootPart,
    ...
    private function getAppVolumeContainsSystemVol($selectvol, $volData)
    {
        ...
        for ($i = 0; $i < count($selectvol); $i++) {
            ...
            $volTypeValue = $detail->vol_type;
            if (($volTypeValue & $VolCdpDes['VOL_TYPE']['BD_SYSTEM_VOLUME']) && $selectvol[$i]['vol_uuid'] == $volData['vol_uuid']) {
                $isCheck = true;
                break;
            } else {
    ...
    public function getStandbyHostInfo($params)
    {
        ...
        if (!empty($masterAgentUuid)) {
            $sql .= " and agent_uuid != ?";
            $sqlParams = array(Xphp::$_config['AGENT_TYPE']['NORMAL'], Xphp::$_config['AGENT_TYPE']['MEMORY_OS'], $masterAgentUuid);
        }
        ...
        foreach ($data as $host_info) {
            ...
            if ($host_info['agent_type'] == Xphp::$_config['AGENT_TYPE']['NORMAL'] && $masterOsType != $host_info['os_type']) {
                continue;
            }
            //如果节点状态正常
            ...
            if ($agentType == Xphp::$_config['AGENT_TYPE']['MEMORY_OS']) {
            ...
            $inTask = false;
            if (count($isCreateTask) > 0) {
                $taskName = $isCreateTask['task_name'];
                $inTask = true;
            ...
            if ($isUsedTakeoverStandbyInfo['is_enabled']) {
                $inTask = true;
                $taskName = $isUsedTakeoverStandbyInfo['task_name'];
            }

            if($isUsedHaStandbyInfo['is_enabled']){
                $inTask = true;
                $taskName = $isUsedHaStandbyInfo['task_name'];
            }

            if($isUsedFbTargetInfo['is_enabled']){
                $inTask = true;
                $taskName = $isUsedFbTargetInfo['task_name'];
            }

            if($isUsedRecoverTargetInfo['is_enabled']){
                $inTask = true;
                $taskName = $isUsedRecoverTargetInfo['task_name'];
            }
...
//            }
            ...
            $systemVol = false;
            if (
                ($volTypeValue & $VolCdpDes['VOL_TYPE']['BD_BOOT_VOLUME']
            ...
            if ($agentType != Xphp::$_config['AGENT_TYPE']['MEMORY_OS'] && $v['is_boot'] == Xphp::$_config['FLAG']['SET']) {
                continue;
            }
    ...
    public function getHostConfiguredAppTree($params)
    {
        ...
        return json_encode($appTreeInfo);
    }
    ...
    public function createBackupJob($params)
    {
        ...
        $nodeInfo = $this->groupBackupNodeInfo($params['highInfo']['node'],Xphp::$_config['MODULE_TYPE']['VOL_CDP']);
        ...
        $pfMsg['task_type'] = Xphp::$_config['TASKTYPE']['VOL_CDP_BACKUP'];
        $utils = Xphp::instance('Utils');
    ...
    private function groupBackupTimeList($params, $strategygroupuuid)
    {
        ...
        if (!empty($params['tagInfo'])) {  //标签策略
            $msg = $this->groupEachTagstrategy(Xphp::$_config['BACKUP_MODE']['TAG'], $params['tagInfo'], $strategygroupuuid);
        }
        return $msg;
    }
    /**
    ...
    {
        $this->paramsCheck($mode, $strategyArray);
        $strArr = array();
        $utils = Xphp::instance('Utils');
        foreach ($strategyArray as $strategy) {
            ...
            if ($strategy['rollFlag']) {  //滚动执行
                $rollFlagInfo = Xphp::$_config['STRATEGY_ROLL_TYPE']['ON'];
                $rollInterval = $this->getRollInterval($strategy['rollInterval']);
                $rollEndTime = $strategy['endTime'];
            } else {  //不滚动
                $rollFlagInfo = Xphp::$_config['STRATEGY_ROLL_TYPE']['OFF'];
                $rollInterval = 0;
                $rollEndTime = '';
            }
            $strArr[] = array(
     ...
     * @param array $hostInfo     本次备份的客户端
     * @param string $taskuuid  任务UUID(修改的时候用)
     */
    private function checkTaskLegal($operate, $dbInfo, $taskuuid)
    {
...
//         }
    ...
    public function verifyingVolCdpAuthNum()
    {
        ...
        $productUsed = $homepageHandler->getVolcdpProtectHost($module);
    ...
    public function verifyVolCdpTakeoverAuthNum()
    {
        $module = Xphp::$_config['MODULE_TYPE']['VOL_CDP'];
        $homepageHandler = Xphp::instance('HomepageHandler');
        $productUsed = $homepageHandler->getVolcdpTakeoverNum();
        $licenseInfo = $this->getBdLicenseAboutVolcdp();
     ...
     * @param unknown $params
     */
    public function getSysAuthFunc($params)
    {
        $systemHandler = Xphp::instance('SystemHandler');
        $extension = $systemHandler->getExtensionLicense();
        $authFunction = $extension['f'];
        $authPage = $extension['p'];
        $info = array(
    ...
    public function getBackupAgentNetworkInfo($params)
    {
        ...
        $sql = "select cvba.id, cvba.master_agent_detail from cdp_vol_backup_agent cvba, bd_backup_timepoint bbt where master_agent_uuid = ? and bbt.task_uuid = ? 
            and bbt.timepoint_uuid =  cvba.timepoint_uuid order by cvba.id limit 0,1";
    ...
    public function getAgentCachePath($params)
    {
        ...
        return json_encode($agentPathInfo);
    }
}