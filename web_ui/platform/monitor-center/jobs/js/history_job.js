var HistoryJob = function () {
        ...
        },
        {
            ...
            value: [
                ...
                },
                {
                    ...
                    value: '50', // 原本应该是54，虚拟机整机迁移任务类型都是54，但因为历史原因后台存的50，且改动麻烦，所以先传50
                        ...
                        case 26: //文件复制
                            ...
                            for (var i = 0; i< data.list.copy_list.length; i++) {
                                ...
                                tbody += `<tr>` +
                                            ...
                                            `<td class="path-no-border"> `+ path +`</td>`+
    ...
    const getDBJobDetails = function (data) {
        ...
        var getRecoveryDetails = function (data) {
            ...
            thead += `<th width="6%">${LANG.UI_PUBLIC_STATUS}</th>`;
            thead += `<th width="10%">${LANG.UI_PUBLIC_DESCRIPTION}</th>`;
            thead += `</thead>`;
    ...
    const addListeners = function () {
        ...
        window.$on('history_job_datepicker-updateDateRangeEvent', (data) => {
            ...
            $('#history_table').bootstrapTable('refresh', { query: { start_time: startTime, end_time: endTime } });
        });
    ...
    const initHistoryJobTableFilter = () => {
        ...
        const filteredOptions = HISTORY_JOB_TABLE_FILTER_OPTIONS.slice(0, 3).filter(item => {
            // 对每个 item 的 value 进行过滤
            const filteredValues = item.value.filter(val => permissions.includes(val.id.split('history-')[1]));
        ...
        // 将前三项替换为过滤后的结果
        HISTORY_JOB_TABLE_FILTER_OPTIONS.splice(0, 3, ...filteredOptions);
