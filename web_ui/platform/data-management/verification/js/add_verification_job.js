var VerificationJob = function(){
	...
	let initListeners = function(){
		...
		$('.select2').select2({
			...
			selectOnClose: true,
			width: 'resolve',
			dropdownAutoWidth: true, // 下拉菜单自动宽度
		...
		$('#verifyMode').on('change', function(e, oldValue, newValud) {
			if(newValud == 1){
				...
				$('.virtualLabDiv').hide();
				$('.verifyTypeDiv').hide();
				$('#verifyType').find('input[data-mode=1]').attr("checked", false);
				$('#verifyType').find('input[data-mode=0]').attr("checked", true);
				...
				if ($('#oem_version').val() != "vdms"){
					...
					if($.inArray("dataVerificationDR", CONF.FUNCTIONS) == -1){
						$('.labTypeDiv').hide();
						$('#labType').find('input[data-mode=2]').attr("checked", true);
						$('#labType').find('input[data-mode=1]').attr("checked", false);
						$('.virtualLabDiv').show();
					}else{
						$('.labTypeDiv').show();
						$('#labType').find('input[data-mode=1]').attr("checked", true);
						$('#labType').find('input[data-mode=2]').attr("checked", false);
						$('.virtualLabDiv').hide();
					}
				}
		...
		//验证类型
		$('#verifyType').find('.form-check-input').on('click', function(){

			let mode = $(this).data('mode');
			if (0 == mode) {
				//自动验证
				$('#verifyType').find('input[data-mode=1]').prop("checked",false);
				$('#startType').attr('disabled', false);
			} else {
				//手动验证
				$('#verifyType').find('input[data-mode=0]').prop("checked", false);
		...
		//虚拟演练室构建类型
		$('#labType').find('.form-check-input').on('click', function(){
			let mode = $(this).data('mode');
			if (2 == mode) {
				$('.autolabTips').hide();
				$('.virtualLabDiv').css('display','flex');
				$('#labType').find('input[data-mode=1]').prop("checked",false);
			} else {
				$('.autolabTips').show();
				$('.virtualLabDiv').hide();
				$('#labType').find('input[data-mode=2]').prop("checked",false);
			}
		});
	...
	let wizardInit = function(){
		$('#submit_form').smartWizard({
			...
			toolbar: {
				...
				extraHtml: '<button class="btn sw-btn-submit sw-btn" type="submit" id="submit">'+LANG.UI_PUBLIC_SUBMIT+'</button>' // Extra html to show on toolbar
			},
			lang: {
				next: LANG.UI_PUBLIC_NEXT_STEP,
				previous: LANG.UI_PUBLIC_PREV_STEP
			}
		});

		// 初始化表单验证
		// backupFormValidator = $('#backup_form').jbvalidator({
		// 	errorMessage: true,
		// 	successClass: true
		// });
		...
		$('#submit_form').on('leaveStep', function(e, anchorObject, currentStepIndex, nextStepIndex, stepDirection, callback) {
			...
			//如果第一步 上一步按钮隐藏
			if (current == 1) {
				$('#verificationContent').find('.sw-btn-prev').css('visibility', 'hidden');
				$('#verificationContent').find('.sw-btn-next').addClass('next-btn-margin-left');
			} else {
				$('#verificationContent').find('.sw-btn-prev').css('visibility', 'visible');
				$('#verificationContent').find('.sw-btn-next').removeClass('next-btn-margin-left');
			}

			//处理最后一步
			if (current >= total) {
				$('#verificationContent').find('.sw-btn-next').hide();
				$('#verificationContent').find('.sw-btn-submit').css('visibility', 'visible');
			} else {
				$('#verificationContent').find('.sw-btn-next').show();
				if (editFlag){
					$('#verificationContent .sw-btn-submit').css('visibility', 'visible');
				}else{
					$('#verificationContent .sw-btn-submit').css('visibility', 'hidden');
				}
			}
		...
		if (editFlag){
			$('#verificationContent .sw-btn-submit').click(submit).css('visibility', 'visible');
		}else{
			$('#verificationContent .sw-btn-submit').click(submit).css('visibility', 'hidden');
		}
	}
	...
	let step1Valid = function(){
		...
		//完全可恢复性验证-手动验证 屏蔽最大验证数量
		if(data.basic_info.automatic_verifitied_flag == 1 || $('#oem_version').val() == "vdms"){
			$('.threadNumDiv').hide();
		}else{
			$('.threadNumDiv').show();
		}
	...
	let step2Valid = function(){
		...
		if(data.basic_info.appgroup_uuid != ""){
			...
			for(var i = 0;i<appgroupList.length;i++){
				...
				//文件对比验证配置
				if ($('#oem_version').val() == "vdms" && appgroupList[i].general_config.module_type == 5){
	...
	let getNetworkConf = function(liID,div){
		...
		for(let i=0; i< network.length; i++){
			let info = {};
			info.network_name = $(div + liID + " #network_" + networkList[liID][i] + " input[name=networkname]").val();
			info.ip = $(div + liID + " #network_" + networkList[liID][i] + " input[name=ipaddress]").val();
			info.gateway = $(div + liID + " #network_" + networkList[liID][i] + " input[name=gateway]").val();
			info.netmask =$(div + liID + " #network_" + networkList[liID][i] + " input[name=netmask]").val();
			if($(div + liID + " #network_" + networkList[liID][i] + " select[name=mactype]").val() == 1){
				info.mac_address =$(div + liID + " #network_" + networkList[liID][i] + " input[name=macaddress]").val();
			}else{
				info.mac_address = "";
			}
			list.push(info);
	...
	let step3Valid = function(){
		...
		let getHighStr = function(){
			...
			data.high_strategy.limit_boot_vm_num = $('#vmnum').val();
			data.high_strategy.doc_compare_thread = $('#fileThreadNum').val();
	...
	let showStep3 = function(){
		...
		//自动验证
		if(data.basic_info.automatic_verifitied_flag == 0 && $('#oem_version').val() != "vdms"){
			//同时处理机器数量
			...
			pAjaxRequest(data, "/api/v1/verification", "PUT", function (result) {
				...
				if (result.success) {
					...
					if ($('#oem_version').val() == "vdms" ){
						...
						LOCATION('/platform/jobs/html/jobs.php', 'task');
					}
				} else {
	...
	let initJobReport = function(taskuuid){
		...
		let params = {
			...
			module_type:  $('select[name=module]').val(),
			job_uuid: taskuuid,
		...
		pAjaxRequest(params, "/api/v1/industry/report", "POST", function (result) {
			...
			if ($('#oem_version').val() == "vdms" ){
				LOCATION('/platform/jobs/html/verify_jobs.php', 'verification_job');
			}else{
				LOCATION('/platform/jobs/html/jobs.php', 'task');
			}
		});
	...
	let editVerifyJob = function(){
		...
		if ($('#oem_version').val() != "vdms"){
			if($.inArray("dataVerificationDR", CONF.FUNCTIONS) == -1){
				$('.labTypeDiv').hide();
				$('#labType').find('input[data-mode=2]').prop("checked", true);
				$('#labType').find('input[data-mode=1]').prop("checked", false);
				$('.virtualLabDiv').show();
			}else{
				$('.labTypeDiv').show();
				$('#labType').find('input[data-mode=1]').prop("checked", true);
				$('#labType').find('input[data-mode=2]').prop
				("checked", false);
				$('.virtualLabDiv').hide();
			}
		}
	...
	let initOldData = function(settings){
		...
		for (let i=0;i<settings.item_list.length;i++){
			...
			//文件对比验证配置
			if ($('#oem_version').val() == "vdms" && settings.item_list[i].general_config.module_type == 5){
	...
	let initStep1Settings = function(settings){
		...
		if(mode == 1){
			...
			$('.verifyTypeDiv').hide();
			$('#verifyType').find('input[data-mode=1]').prop("checked", false);
			$('#verifyType').find('input[data-mode=0]').prop("checked", true);
		}else{
			$('.appgroupDiv').show();
			if ($('#oem_version').val() != "vdms"){
				if($.inArray("dataVerificationDR", CONF.FUNCTIONS) == -1){
		...
		if(settings.basic_info.automatic_verifitied_flag == 1){
			$('#verifyType').find('input[data-mode=1]').prop("checked", true);
			$('#verifyType').find('input[data-mode=0]').prop("checked", false);
	...
	let initStep3Settings = function(settings){
		...
		//过载保护
		$('#ignore_resource_limit').prop('checked', settings.high_strategy.ignore_resource_limiting_flag);
	}
	...
	let initAppGroup = function(){
		...
		pAjaxRequest(params, "/api/v1/verification/app_group", "GET", function (result) {
			...
			//切换应用组
			$('select[name=appgroup]').on('change', function(){
				initAppGroupDetails();
			});
		});
	...
	let initLabSelect = function(){
		...
		pAjaxRequest(params, "/api/v1/verification/lab", "GET", function (result) {
			...
			for(let i=0; i<list.length; i++){
				let option = $("<option>").text(list[i].lab_name + "(" +list[i].hypervisor_type_des+ ")").val(list[i].lab_uuid).attr('data-type', list[i].hypervisor_type);
				labselect.append(option);
			}
			if (editFlag){
	...
	let initLabConfig = function(){
		...
		pAjaxRequest({}, "/api/v1/verification/lab/"+  labuuid, "GET", function (result) {
			...
			$('#show_labdetail_drawer').offcanvas('show');
		});
	...
	let initObjectTree = function(nodes){
		if(nodes.length == 0){
			$('#noobjecttips').show();
			$('.three_tree').hide();
			$('.src-wrap__content__ztree').unblock();
			return;
		}else{
			$('#noobjecttips').hide();
			$('.three_tree').show();
		...
		for(let i=0; i<nodes.length; i++){
			...
			let info2 = {
				...
				task_uuid: nodes[i].task_uuid,
				task_name: nodes[i].task_name + '(' + moduleDes +')',
				title: nodes[i].title,
		...
		for(let i=0;i<zNodes.length;i++){
			if(zNodes[i].checked && zNodes[i].eventtype == "object"){
				for (let j=0;j<data.item_list.length;j++){
					if(data.item_list[j].general_config.item_uuid == zNodes[i].object_uuid && data.item_list[j].general_config.backup_task_uuid == zNodes[i].task_uuid){
	...
	var getCpuMode = function(oldnum){
		var div = '';
		var list = [LANG.UI_PUBLIC_DEFAULT, "custom","host-passthrough", "host-model", 'EPYC'];
		...
		pAjaxRequest(p, "/api/v1/verification/get_timepoint", "GET", function (d) {
			...
			if (timepointInfo.length !=0){
				...
				if (node.module_type == 5){
					...
					let p = {};
					p.agent_uuid = node.object_uuid;
					pAjaxRequest(p, "/api/v1/verification/get_client_info", "GET", function (d) {
						//加载默认选项
						let settings = {};
						settings.general_config = {
							'cpu_arch' : d.data['cpu_arch'],
	...
	let addObjectList = function(id,node,checkFlag, config, appFlag = false){
		...
		if(checkFlag){
			...
			let max_screen_wait_time = config.verify_config.max_screen_wait_time ? config.verify_config.max_screen_wait_time : "00:01:00";
			//获取验证对象时间点信息
			...
			//head
			info +=
				'<li class="accordion-item mb10" id="'+domID+liID+'">' +
				'<h4 class="accordion-header" id="header'+domID+liID+'">' +
				'<a title="'+nodeName+'" class="accordion-button fs-4 fw-bold collapsed"  data-bs-placement="top"  data-bs-toggle="collapse" data-bs-target="#config'+liID+'" aria-expanded="false" aria-controls="config'+liID+'" style="white-space: nowrap;overflow: hidden;text-overflow: ellipsis;">'+
				'<span class="font-green-seagreen"><i class="viconfont vicon-zhuji2 mr5"></i>'+nodeName+'</span>' +
				'</a>' +
				'</h4>';
			//content
			info +=
				'<div class="accordion-collapse collapse" aria-labelledby="header'+domID+liID+'" data-bs-parent="#'+domID+liID+'" id="config'+liID+'">' +
				'<div class="accordion-body" style="padding: 16px"><div class="tabs ">' +
				'<ul class="nav nav-tabs nav-line-tabs">';

			//整机支持基本信息修改
			if ($.inArray(node.module_type, [3,4,11,14,28]) == -1){
				info += '<li class="nav-item ">' +
					'<a class="nav-link" href="#common_tab'+liID+'" data-bs-toggle="tab" aria-expanded="false">'+LANG.UI_VM_SETTING_V2_GENERAL+'</a>' +
					'</li>';
			}
			info +=	'<li class="nav-item">' +
				'<a class="nav-link active" href="#verify_tab'+liID+'" data-bs-toggle="tab" aria-expanded="false">'+ LANG.UI_VERIFY_VERIFY_CONFIG +'</a>' +
				'</li>';
			if (data.basic_info.verify_mode == 1){

			}else{
				info += '<li class="nav-item ">' +
					'<a class="nav-link" href="#network_tab'+liID+'" data-bs-toggle="tab" aria-expanded="false">'+ LANG.UI_VERIFY_NETWORK_CONFIG +'</a>' +
					'</li>';
			}
			...
			//通用配置
			info +=
				'<div class="tab-pane fade" id="common_tab'+liID+'" role="tabpanel"><div class="row mt15">';

			if ($.inArray(node.module_type, [3,4,11,14,28]) == -1){
				info += '<div class="form-group row">' +
			...
			if (data.basic_info.verify_mode != 1 ){
				info +='<div class="form-group row"><label class="control-label col-md-4">'+ LANG.UI_VERIFY_CPU_MODE +'</label><div class="col-md-6"><select class="form-select form-control  input-sm" name="cpumode">' + getCpuMode(config.general_config.cpu_mode) + '</select>' +
					'</div>' +
					'<a style="position:absolute;left:0;top:5px;" class="configTips ml10" data-bs-toggle="tooltip" data-bs-html="true" data-bs-placement="right" title="'+proper+'">' +
					'<i class="viconfont vicon-tishi"></i>' +
					'</a>' +
					'</div>' +
					'<div class="form-group row"><label class="control-label col-md-4" style="word-break: normal;">'+ LANG.UI_VM_SETTING_CPU_ARCH +'</label><div class="col-md-6"><select class="form-select form-control  input-sm" name="cpuArch">' + getCpuArchOption(config.general_config.cpu_arch) + '</select></div></div>' +
					'<div class="form-group row "><label class="control-label col-md-4">'+LANG.UI_VERIFY_CPU_NUM+'</label><div class="col-md-6"><select class="form-select form-control  input-sm" name="cpunum">' + getSocketAndCoresOption(config.general_config.cpu_num, hostSettings.cpu_num) + '</select></div></div>' +
					'<div class="form-group row"><label class="control-label col-md-4" style="word-break: normal;">'+LANG.UI_VERIFY_CPU_EVERY_CORE+'</label><div class="col-md-6"><select class="form-select form-control  input-sm" name="corenum">' + getSocketAndCoresOption(config.general_config.core_num,hostSettings.core_num) + '</select></div></div>' +
					'<div class="form-group row"><label class="control-label col-md-4">'+LANG.UI_VERIFY_MEMORY_SIZE+'</label><div class="col-md-6"><div style="display:inline-flex;width:100%;"><select class="form-select form-control  input-sm" name="memory">' + getSocketAndCoresOption(config.general_config.memory_size_int, hostSettings.memory_size_int, true) + '</select><select class="form-control  input-sm" name="unit" style="margin-left:5px;width:65px;">' + getMomoryUnit(config.general_config.memory_size_unit) + '</select></div></div></div>' +
					'<div class="form-group row"><label class="control-label col-md-4" style="word-break: normal;">'+ LANG.UI_VM_OS_TYPE +'</label><div class="col-md-6"><select class="form-select form-control  input-sm" name="ostype">' + getOsOption(config.general_config.os_type, config.general_config.cpu_arch) + '</select></div></div>' +
					'<div class="form-group row"><label class="control-label col-md-4" style="word-break: normal;">'+ LANG.UI_VM_OS_VERSION +'</label><div class="col-md-6"><select class="form-select form-control  input-sm" name="osversion">' + getOSVersionOption(config.general_config.os_version, config.general_config.os_type, config.general_config.cpu_arch) + '</select></div></div>' +
					'<div class="form-group row"><label class="control-label col-md-4" style="word-break: normal;">'+ LANG.UI_VERIFY_OBJECT_DISK_TARGET_BUS +'</label><div class="col-md-6"><select class="form-select form-control  input-sm" name="disk_target_bus">' + getDiskTypeOption(config.general_config.disk_target_bus) + '</select></div></div>' +
					'<div class="form-group row"><label class="control-label col-md-4" style="word-break: normal;">'+ LANG.UI_VERIFY_OBJECT_NETCARD_TARGET_BUS +'</label><div class="col-md-6"><select class="form-select form-control  input-sm" name="netcard_target_bus">' + getNetworkTypeOption(config.general_config.netcard_target_bus) + '</select></div></div>';
			}else{
				info +='<div class="form-group row"><label class="control-label col-md-4" style="word-break: normal;">'+ LANG.UI_VM_OS_TYPE +'</label><div class="col-md-6"><select class="form-select form-control  input-sm" name="ostype">' + getOsOption(config.general_config.os_type, config.general_config.cpu_arch) + '</select></div></div>'+
					'<div class="form-group row"><label class="control-label col-md-4" style="word-break: normal;">'+ LANG.UI_VM_OS_VERSION +'</label><div class="col-md-6"><select class="form-select form-control  input-sm" name="osversion">' + getOSVersionOption(config.general_config.os_version, config.general_config.os_type, config.general_config.cpu_arch) + '</select></div></div>';
			}
			...
			if(data.basic_info.automatic_verifitied_flag == 1){
				let checked = "checked";
				if(!config.general_config.start_vm_flag){
					checked = "";
				}
				// 手动验证 加上自动开机选项
				info += '<div class="form-group row">' +
					'<label class="control-label col-md-4">'+LANG.UI_VERIFY_AUTO_START+'</label>' +
					'<div class="col-md-6 form-group-top4-labe form-check form-switch">' +
					'<input type="checkbox" class="make-switch configCheck autostartCheck form-check-input" '+checked+'>' +
					'<a class="ml10 configTips" data-bs-toggle="tooltip" data-bs-placement="right" html="'+LANG.UI_VERIFY_AUTO_START_TIPS+'">' +
					'<i class="viconfont vicon-tishi"></i>' +
					'</a>' +
					'</div>' +
					'</div>';
			...
			//验证配置
			info +=
				'<div class="tab-pane fade show active" id="verify_tab'+liID+'" role="tabpanel">' +
				'<div class="row mt15">';
			if ($.inArray(node.module_type, [3,4,11,14,28]) != -1){
				info += '<div class="form-group row">' +
					'<label class="control-label col-md-4">'+LANG.UI_VERIFY_SELECT_TIMEPOINT+'</label>' +
			...
			info +='<div class="form-group row '+displayHide+gmpDisplay+ '">' +
				'<label class="control-label col-md-4">'+LANG.UI_VERIFY_PING_TEST+'</label>' +
				'<div class="col-md-8"><div class="form-check form-switch">' +
				'<input type="checkbox" class="make-switch form-check-input configCheck pingCheck " '+ping_test_flag+' >' +
				'<a class="ml10 configTips"  data-bs-toggle="tooltip" data-bs-placement="right" title="'+LANG.UI_VERIFY_PING_TIPS+'">' +
				'<i class="viconfont vicon-tishi"></i>' +
				'</a></div>' +
				'</div>' +
				'</div>' +
				'<div class="form-group row '+ displayHide + gmpDisplay +'">' +
				'<label class="control-label col-md-4">'+LANG.UI_VERIFY_HEARTBEAT+'</label>' +
				'<div class="col-md-8"><div class="form-check form-switch">' +
				'<input type="checkbox" class="make-switch form-check-input configCheck heartCheck" '+heartbeat_flag+' >' +
				'<a class="ml10 configTips" data-bs-toggle="tooltip" data-bs-placement="right" title="'+LANG.UI_VERIFY_HEARTBEAT_TIPS+'">' +
				'<i class="viconfont vicon-tishi"></i>' +
				'</a></div>' +
				'</div>' +
				'</div>' +
				'<div class="form-group row '+displayHide+'">' +
				'<label class="control-label col-md-4">'+LANG.UI_VERIFY_SCREEN+'</label>' +
				'<div class="col-md-8"><div class="form-check form-switch">' +
				'<input type="checkbox" class="make-switch form-check-input configCheck screenCheck" '+print_screen_flag+' >' +
				'<a class="ml10 configTips" data-bs-toggle="tooltip" data-bs-placement="right" title="'+LANG.UI_VERIFY_SCREEN_TIPS+'">' +
				'<i class="viconfont vicon-tishi"></i>' +
				'</a></div>' +
				'</div>' +
				'</div>' +
				...
				'<div class="form-group row display-hide'+ hostflag + gmpDisplay + '" id="virusConfig_'+ liID +'" >' +
				'</div>';

				if(node.integrity_check_flag == 1){
					info+='<div class="form-group row '+ gmpDisplay + '">' +
					'<label class="control-label col-md-4">'+LANG.UI_SAFE_STRATEGY_INTEGRITY_CHECK+'</label>' +
					'<div class="col-md-8"><div class="form-check form-switch">' +
					'<input type="checkbox" class="make-switch form-check-input configCheck integrity_check_flag" '+integrity_check_flag+'>' +
					'<a class="ml10 configTips" data-bs-toggle="tooltip" data-bs-placement="right" title="'+LANG.UI_SAFE_STRATEGY_BACKUP_POINT_CHECK+'">' +
					'<i class="viconfont vicon-tishi"></i>' +
					'</a></div>' +
					'</div>' +
					'</div>';
					...
					info+='<div class="form-group row '+ gmpDisplay + '">' +
						'<label class="control-label col-md-4">'+LANG.UI_SAFE_STRATEGY_INTEGRITY_CHECK+'</label>' +
						'<div class="col-md-8"><div class="form-check form-switch">' +
						'<input type="checkbox" class="make-switch form-check-input configCheck integrity_check_flag" disabled >' +
						'<a class="ml10 configTips" data-bs-toggle="tooltip" data-bs-placement="right" title="'+LANG.UI_SAFE_STRATEGY_BACKUP_POINT_CHECK+'">' +
						'<i class="viconfont vicon-tishi"></i>' +
						'</a></div>' +
						'</div>' +
				...
				//开机上线检测时间
				info += '<div class="form-group row '+displayHide+'">' +
					'<label class="control-label col-md-4">'+LANG.UI_VERIFY_OBJECT_PING_WAIT_TIME+'</label>' +
					'<div class="col-md-6 " >' +
					'<div class="input-group">' +
					'<input type="text" value="'+max_ping_wait_time+'" name="max_ping_wait_time"  class="form-control flatpickr-timepicker">'+
					'</div>' +
					'<div>' +
				...
				info += '<div class="form-group row '+displayHide+'">' +
					...
					'<div class="col-md-6 " >' +
					'<div class="input-group">' +
					'<input type="text" value="'+max_boot_time+'" name="boottime"  class="form-control flatpickr-timepicker">'+
					'</div>' +
					'<div>' +
			...
			if ($('#oem_version').val() == "vdms" && node.module_type == 5){
				...
				info +=
					...
					'<select class="form-control  input-sm compare_mode" ><option value="1">MD5</option></select>' +
					'</div>' +
					'</div>' +
					'<div class="form-group row fileTreeDiv">' +
			...
			//网络配置
			info +=
				'<div class="tab-pane fade pd10" id="network_tab'+liID+'" role="tabpanel">' +
				'<div class="mt15">' +
				'<button type="button"  class="btn btn-primary btn-sm data-resource2 mb10 addNetwork"><i class="viconfont vicon-ge_add_task"></i>'+LANG.UI_VERIFY_ADD_NETWORK+'</button>' +
			...
			$('#' + domID + escapeJquery(liID) + ' .select_point_' + liID).on('click', function(){
				...
				if(appFlag || node.module_type == CONF.MODULE_TYPE.VOL_CDP || $('#oem_version').val() == "vdms" || data.basic_info.automatic_verifitied_flag == 1){
					option = ` <option value="2">`+name +`</option>
                                <option value="3">`+LANG.UI_VERIFY_SELECT_TIMEPOINT_SELECT+`</option>`;
				}
				$('#pointType').empty().html(option);
				$('#select_point_drawer').offcanvas('show');
				let params = {};
			...
			$('#' + domID + escapeJquery(liID) + ' .vir_det_kill_flag').on('change', function(){
				if(this.checked){
			...
			$('#' + domID + escapeJquery(liID) + " .configTips").tooltip();	   //初始化tips
			$('#' + domID + escapeJquery(liID) + " .keyupInput").off().keyup(function(){
				let value = $(this).val().replace(/[^\d]/g,'');
				$(this).val(value);
			});

			$('#' + domID + escapeJquery(liID) + " .pingCheck").on('change', function(){
				if(this.checked){
			...
			$('#' + domID + escapeJquery(liID) + " .heartCheck").on('change', function(){
				if(this.checked){
					$('#' + domID + escapeJquery(liID) + " .pingCheck").prop('disabled', false);
				}else{
					$('#' + domID + escapeJquery(liID) + " .pingCheck").prop('checked', true);
					$('#' + domID + escapeJquery(liID) + " .pingCheck").prop('disabled', true);
				}
			});
			$('#' + domID + escapeJquery(liID) + " .pingCheck").on('change', function(){
				if(this.checked){
					$('#' + domID + escapeJquery(liID) + " .heartCheck").prop('disabled', false);
				}else{
					$('#' + domID + escapeJquery(liID) + " .heartCheck").prop('checked', true);
					$('#' + domID + escapeJquery(liID) + " .heartCheck").prop('disabled', true);
	...
	let initNetwork = function(info, liID, domID){
		...
		//头部
		network += '<li  class="accordion-item mb10" style="position: relative;width:90%;" id="network_'+uuid+'"><h4 class="accordion-header" id="network_header_"'+uuid+'><a class="accordion-button fs-4 fw-bold '+collapsed+'"'+
			'data-bs-toggle="collapse" data-bs-target="#network_content_'+uuid+'" aria-expanded="false" aria-controls="network_content_'+uuid+'"  ><span class="font-green-seagreen">'+network_name+'</span></a></h4>' +
			'<div class="accordion-collapse collapse '+collapsedIn+'" id="network_content_'+uuid+'" aria-labelledby="network_header_"'+uuid+' data-bs-parent="#'+domID+liID+' .addNetworkDiv"><div class="accordion-body"><div class="form-group col-md-12 ">';

		//网络配置部分
		network += `<div class="form-group row display-hide">
		...
		network += `<div class="form-group row">
					<label class="control-label col-md-4 iplabel">`+LANG.UI_PUBLIC_IP_ADDRESS+`</label>
					<div class="col-md-6">
						<div class="input-icon right">
		...
		network += `<div class="form-group row">
					<label class="control-label col-md-4 iplabel">`+LANG.UI_PUBLIC_IP_NETMASK+`</label>
					<div class="col-md-6">
						<div class="input-icon right">
		...
		network += `<div class="form-group row">
				...
				<div class="col-md-6">
					<select class="form-select form-control  input-sm" name="mactype" `+disabled+`>
		...
		network += `<div class="form-group macdiv row display-hide">
			...
			switch (pointType) {
				...
				case TIMEPOINT_TYPE_ALL:
					...
					if (appgroupList[i].verify_config.newest_timepoint) {
						source_list.push({
							name: name,
							timepoint_uuid: appgroupList[i].verify_config.newest_timepoint.timepoint_uuid,
							module_type: moduleType,
							os_type: ostype,
							os_arch: cpuArch,
							os_version: osversion,
				...
				case TIMEPOINT_TYPE_SELECT:
					// 指定时间点传选择的第一个
					source_list.push({
						name: appgroupList[i].verify_config.newest_timepoint.timepoint.timepoint + ' (' + appgroupList[i].verify_config.newest_timepoint.timepoint.backup_mode_des + ')',
						timepoint_uuid: appgroupList[i].verify_config.newest_timepoint.timepoint.timepoint_uuid,
						module_type: moduleType,
						os_type: ostype,
						os_version: osversion,
						os_arch: cpuArch,
	...
	let getObjectInfo = function(){
		...
		$('.src-wrap__content__ztree').block();
