var VerifyJobDetails = function () {
    ...
    let initLogGrid = function(){
		let getLiInfo = function (res) {
			...
			for (let i = 0; i < res.data.length; i++) {
				info +=
					...
					'<div class="cont-col1">' + getIcon(res.data[i].level_value) + '</div>' +
					'<div class="cont-col2">' +
					'<div class="desc">' + res.data[i].description + '</div>' +
					'</div>' +
					'</div>' +
					'</div>' +
					'<div class="col2 logtimecol"><div class="date">' + res.data[i].op_time + '</div></div></li>';
			}
		...
		// 获取任务运行日志
		let getLog = function () {
			let jobs_uuid = $("#task_uuid").val();
			pAjaxRequest({ jobs_uuid: jobs_uuid }, "/api/v1/logs/jobs/running/logs", "GET", getLiInfo)
		}
		getLog();
    }
	...
	let initObjectDetails = function(row){
		...
		pAjaxRequest(params, '/api/v1/verification/object_info', 'GET', (result) => {
			if (result.success){
				...
				//主机配置
				var cpuModeDes = [LANG.UI_PUBLIC_DEFAULT, "custom","host-passthrough", "host-model", 'EPYC'];
    ...
    var initHistoryGrid = function(){
		...
		if($('#oem_version').val() != "vdms"){
			columns.push({
				...
				sortable: false, //默认可排序，禁用排序才写此项
				formatter:function (value, row, index, field){
					let nameStr = '<a class="historyDetails" id="' + row.job_id + '">' + LANG.UI_JOB_VERTIFY_REPORT + '</a>';
					return nameStr;
				},
				events:operates,
			});
