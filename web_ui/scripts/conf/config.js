var CONF = {
	...
	// <----------------------------------------------- BEGIN PUBLIC CONFIG ----------------------------------------------->
	/**
	 * 判断系统是否完成初始化关键信息[系统配置信息](init.js)
	 * @author JackC
	 * @email 1366294101@qq.com
	 */
	SYSTEM_INIT: {
		SYSTEM_CONFIG_FLAG: false,
	},
	...
	// 模块类型对应的任务类型（用于过滤器中 任务类型列 进行权限过滤）键名和键值数组中的值都是取得过滤器中配置好的id数据
	MODULE_TYPE_TO_TASK_TYPE_MAP: {
		'vmprotect': [
			'task_type_backup',
			'task_type_recover',
			'task_type_copy',
			'task_type_copy_fetch',
			'task_type_archive',
			'task_type_archive_fetch',
			'task_type_data_verify',
			'task_type_motion',
			'task_type_instance_recover',
			'task_type_grain_recover',
			'task_type_cross_platform_recover'
		],
		'prcloud_protect': [
			'task_type_backup',
			'task_type_recover',
	...
	VMTYPE_GROUP : {
		...
		PUBLICCLOUD : [100, 101],
		PRIVATECLOUD: [14, 15, 22, 26, 30, 31, 34, 35, 36, 37, 38, 39, 54, 60],
	},
	...
	CDP_TASK_RUNNING_STAGE:{
		...
		FAILBACK_WAIT_CONVERT_TO_CDP:83,//等待恢复回切实时保护
		FAILBACK_IN_SERVER_DATA_CONSISTENCY_CHECK:90, //回切数据一致性校验
		FAILBACK_IN_STANDBY_DATA_CONSISTENCY_CHECK:91, //回切机数据一致性校验
		FAILBACK_IN_SERVER_REALTIME_CONSISTENCY_CHECK:92//回切实时数据一致性校验
	},
