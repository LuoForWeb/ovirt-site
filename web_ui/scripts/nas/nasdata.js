var NasData = function () {
	...
	var initListener = function(){
		if (CONF.PERMISSION_ARR.includes('p_nasdata_detele')) {
			$('#allDelete').show();
		...
		$('#serach_submit').on('click', function(){
			...
			var data = {m:CONF.M.NAS,f:'getNasTimepointGrid',p:params};
...
//			console.log('New date range selected: ' + start.format('YYYY-MM-DD HH:mm:ss') + ' to ' + end.format('YYYY-MM-DD HH:mm:ss') + ' predefined range: ' + label);
	...
	var submitSelectDelete = function(){
		...
		//发送到自己模块
    	$.post(CONF.AJAXPATH, {m:CONF.M.NAS,f:'deleteSelectTimepoint',p:params}, function(data){
	...
	var getSyncVcenterInfo = function(treeId, treeNode, refreshFlag, expendFlag){
		...
		var data = JSON.stringify({taskuuid:treeNode.taskuuid, id: treeNode.id, agentuuid: treeNode.agentuuid,
			refresh:refreshFlag, storage_uuid: treeNode.storage_uuid, dataFlag: true,recoverflag:true});
	...
	var initTree = function() {
		var storage_uuid = $('#storageSelect').val();
		var p = JSON.stringify({storage_uuid: storage_uuid, dataflag:true });
		$.post(CONF.AJAXPATH, {m:CONF.M.NAS,f:'getNasDataTree',p: p}, setTree);
	};
    //初始化时间点展示方式和事件
	...
	var addOpButton = function(){
		...
		for(var i=0; i<data.length; i++){
			opButton(opDiv[i], data[i][6], i,data[i][8]);
		}
		$('.remarktips').popover();	   //初始化tips
	}
	...
	var opButton = function(div, opCode, rowNum,otherMsg){
		...
		if(mode ==1){
			$.each(opCode, function(i, d){
				switch(d){
					case 1:
						if (CONF.PERMISSION_ARR.includes('p_nasdata_remark')) {
					...
					case 2:
						...
						break;
					case 3:
						if (CONF.PERMISSION_ARR.includes('p_nasdata_star')) {
                ...
                bootbox.prompt({ 
				    ...
				    callback: function (result) {
				        ...
				        if(hex_md5(result) == _UserPassword){
							...
							// 检测要删除的是否有归档数据
							var archiveFlag = data[row][8].src_data_deleted_flag==1 ? true: false;
							//如果是归档数据，三次弹出确认框再确认
							if(archiveFlag) {
								bootbox.confirm({
									title: LANG.UI_DATA_DELETE_TIMEPOINT,
	...
	var submitDelete = function(button){
		var data = grid.getDataTable().data();
		var row = $(button).parents('tr').get(0)._DT_RowIndex;
		var params = {uuid:[data[row][8].uuid], agentuuid: [data[row][8].agentuuid], taskuuid: [data[row][8].taskuuid]};
		params = JSON.stringify(params);
		Metronic.blockUI({target: '#timepointdiv',animate: true});
		//发送到自己模块
    	$.post(CONF.AJAXPATH, {m:CONF.M.NAS,f:'deleteTimepoint',p:params}, function(data){
    		Metronic.unblockUI('#timepointdiv');
    		if(OPREL(data)){
    			grid.getRefresh({});
    			var d = JSON.parse(data);
	...
	var fileDataTable = function (agentuuid, taskuuid,storage_uuid) {
		...
		if(!gridInitFlag){
			...
			grid = new Datatable();
			var data = {m:CONF.M.NAS,f:'getNasTimepointGrid',p:{agentuuid:agentuuid, taskuuid:taskuuid, storage_uuid: storage_uuid}};
			grid.setAjaxParam(data);
