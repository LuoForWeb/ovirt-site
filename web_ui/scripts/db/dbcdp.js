var DbCDP = function () {
	...
	var networkInfo, takeovertd, currentNetwork;
...
//        					beforeExpand: nodeExpand
	...
	var opButton = function(div, data){
		var btn = '<button type="button" class="btn green-haze btn-sm takeovernwbt" name="' + data[3].adaptername + '">' + 
	...
	var initServiceGrid = function(){
		...
		if(!initServiceGridFlag){
			...
			serviceGrid.init({src: $("#takeoverservicetable"),  dataTable:dataTableOpt});
...
//            $('.step-title', $('#vmbackupcontent')).text('Step ' + (index + 1) + ' of ' + total);
	...
	var submit = function(){
    	...
    	$.post(CONF.AJAXPATH, {m:CONF.M.DBCDP,f:'createBackupJob',p:jsonData}, function(data){
	...
	var initStorageSelect = function(){
		...
		$.post(CONF.AJAXPATH, {m:CONF.M.STORAGE,f:'getBackupStorageList',p:jsonData}, function(d){
    		...
    		softselect.empty();
			for(var i=0; i<data.length; i++){
				if(data[i].type == CONF.BD_STORAGE_TYPE.TAPE){
                    continue;
                }
