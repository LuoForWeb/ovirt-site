var FSJobDetails = function () {
	...
	var initBasicInfo = function(){
		...
		var init = function(){
			if(0 == $('#task_uuid').size()){
        		clearTimeout(timerTask.FSJobDetails_basicInfo);
        		return;
        	}
			var data = {};
			data.uuid = $("#task_uuid").val();
			data = JSON.stringify(data);
			$.post(CONF.AJAXPATH, {m:CONF.M.JOB,f:'getFsBasicInfo',p:data}, function(d){setBasicInfo(d, timerTask.FSJobDetails_basicInfo)});
			timerTask.FSJobDetails_basicInfo = setTimeout(init, updateInterval);
		}
		init();
	}
	...
	var setBasicInfo = function(data, timeoutID){
		...
		if(data.taskTypeFlag!=2) {//备份
			...
			$('.fsStartDiv').show();
			$('#compress_storage').html(getFlagLevelInfo(data.storageInfo.high.compressed));
			// 压缩等级
			...
			// 归档
			if (CONF.FUNCTIONS.includes("fileArchiveMode")) {
				data.archiveflag != 2 ? archiveflag=true : archiveflag = false;
				$('#archivecheck').html(getFlagLevelInfo(archiveflag));
				if(archiveflag) {
					$(".archive-select-form-item").show();
					$('#archiveSelect').html(data.archiveflag == 1 ? LANG.UI_FILE_FILE_DIRECTORY : LANG.UI_FILE_FILE);
				} else {
					$(".archive-select-form-item").hide();
				}
			}
			// 扫描线程
			$('#scan_thread_num').html(data.scan_thread_num);
			if(data.scan_thread_num == 1) {
				// 扫描文件速度
				$('#scan_file_speed').html(getScanSpeed(data.scan_file_num));
			} else {
				$('.scan-file-speed-form').hide();
			}
			// 文件权限备份
			$('#permission').html(getFlagLevelInfo(data.permission_operate_flag));
			//跳过文件告警智能判断
		...
		if(1 != data.taskTypeFlag){//恢复
			...
			if(data.new_root_path == LANG.UI_FILE_RECOVERY_ORIGINAL_PATH) {
				$('.file-skip-dir-form-item').hide();
			} else {
				$('.file-skip-dir-form-item').show();
				$('#file_skip_dir').html(getFlagLevelInfo(data.dir_tree_recovery_flag));
			}
			// 同名文件处理
			...
			if (data.src_module_type && data.src_module_type != "OBS" && data.src_module_type != "HADOOP" ) {
				// 无效快捷方式清理
				$('#clear_shortcut').html(getFlagLevelInfo(data.link_file_pass_flag));
			} else {
				$('.clear-shortcut-form-item').hide();
			}
			// 跨平台任务不显示文件权限恢复
		...
		if(data.reservedStrategy){
			if (data.storageInfo.storage && data.storageInfo.storage.typenum == CONF.BD_STORAGE_TYPE.TAPE) {
				...
				$('.reserve-strategy-form-item #reserve_strategy').html(`${LANG.UI_TAPE_SELECT_GENERATE_STRATEGY}:${data.tape_strategy.backup_set_strategy_des} <br> ${LANG.UI_TAPE_RESERVE_STRATEGY}:${data.tape_strategy.reserve_strategy_des}`);
			} else {
				$('#reserve_strategy').html(getReservedStrategy(data.reservedStrategy, data.archiveflag));
			}
		}else{
			//恢复,隐藏保留策略
			$('.reserve-div').hide();
		...
		if(data.storageInfo.flag){
			...
			$('#backup_node').html(data.storageInfo.node_pool_nickname ?
								   data.storageInfo.node_pool_nickname +":<br/>" + node.name + "<br>" + node.ip :
								   node.name + "<br>" + node.ip);
			$('#storage_device').html(data.storageInfo.storage_pool_nickname ?
		...
		if(parseInt(data.transportStrategy.reconnect_times) == 0){
			...
			$("#reconnectTimes_2").html(LANG.UI_GLOBAL_STRATEGY_RECONNECT_TIMES_INFINITE);
		}else{
            $('#reconnectTimes').html(reconnect_times);
            $('#reconnectTimes_2').html(reconnect_times);
		}
		// 重连时间间隔
		$('#reconnectInterval').html(reconnect_interval);
		$('#reconnectInterval_2').html(reconnect_interval);
	...
	var initOpButton = function(data) {
		...
		$(".search-btn").unbind().on('click', function(){
			if ($('.file-detail-search').val().trim() == "") {
				return;
			}
    		fsgrid.getRefresh(getParams());
    	});
		//清除搜索
		$(".clear").unbind().on('click', function(){
    		$('.file-detail-search').val("");
			fsgrid.getRefresh(getParams());
    	});
		// 输入框 keyup 和 input 事件监听输入内容变化
		$('.file-detail-search').on('input focus', function () {
		...
		$('.file-detail-search').on('blur', function () {
		    if ($(this).val().trim() === '') {
		        $('.clear').addClass('hide');
		    }
		});
	...
	//得到保留策略描述信息
	var getReservedStrategy = function(msg,archiveflag){
		var reservedStr = '';
		if(!msg){
	...
	var setControlBtn = function(id, available){
		if (available){
            $("." + id).find('.btn').attr('disabled', false);
        } else {
            $("." + id).find('.btn').attr('disabled', true);
        }
	}
	...
	var fsGridLoad = function(){
		//多客户端进度条处理
		var fstabledata = fsgrid.getDataTable().data();
		if(fstabledata.length > 1) {//客户端数量大于1
			$(".progressDiv").hide();
			...
			for(var i=0;i<fstabledata.length;i++) {
				if(fstabledata[i].taskStatus== 1 || fstabledata[i].taskStatus== 4){//整个任务是等待或停止状态
					$(".agentName").text("--");
				}else {
					if(fstabledata[i][11].popover== LANG.UI_FILE_DETAILS_JOB_SCANNING) {//扫描中
						$(".agentName").text(fstabledata[i][2] + " ; " + LANG.UI_FILE_DETAILS_JOB_CALCULATING);
					} else if (fstabledata[i][11].popover== LANG.UI_FILE_RUNNING) {//传输中
						$(".agentName").text(fstabledata[i][2] + " ; " + LANG.UI_FILE_DETAILS_JOB_TRANSFERING);
					}
				}

			}
		}else if(fstabledata.length == 1) {
			$(".multiProgressDiv").hide();
			$(".progressDiv").show();
		}
		if(detailsInfo){
			var openTr = $('#filestable').find('tbody > tr')[detailsIndex];
			$(openTr).after(detailsInfo);
			$(openTr).find('.row-details-close').addClass("row-details-open").removeClass("row-details-close");
		}
    }
	//初始化主机列表表格
	var initFSGrid = function(){
		var updateInterval = 5000;
    	var initFlag = false;
    	fsgrid = new Datatable();
    	var dataTableOpt = {
    			"ordering": false,
                "paging":false,
                "info":false
    	...
    	var init = function(){
			if(0 == $('#task_uuid').size()){
        		clearTimeout(timerTask.FILEJobDetails_fsGrid);
        		return;
        	}
    		if(!initFlag){
    			let data = {m:CONF.M.JOB,f:'getDetailsFs',p:getParams()};
    			fsgrid.setAjaxParam(data);
    			fsgrid.init({src: $("#filestable"), showDetail:true, onDataLoad:fsGridLoad, dataTable:dataTableOpt});
    	    	initFlag = true;
    		}else{
				var select = fsgrid.getSelectedRows();
    			fsSelect = select;
    			var saveCheck = function(){
    				for(var i=0;i<fsSelect.length;i++){
    					$("input[name=id"+fsSelect[i]+"]").attr('checked', true);
        				$("input[name=id"+fsSelect[i]+"]").parent().addClass('checked');
        			}
    			}
    			fsgrid.getRefresh(getParams(), saveCheck, false);
    		}
    		timerTask.FILEJobDetails_fsGrid = setTimeout(init, updateInterval);
    	}
    	init();
    	$('#filestable').on('click', ' tbody td .row-details', function () {
        	var data = fsgrid.getDataTable().data();
            ...
            if($(this).hasClass('row-details-open')){
            	...
            	//如果是收起的
            	$('#filestable').find('tr .details').parent().remove();
            	$('#filestable').find('.row-details-open').addClass("row-details-close").removeClass("row-details-open");
            	$(this).addClass("row-details-open").removeClass("row-details-close");
            	var row = $(this).parent().parent().prevAll().length;
            	addDetails(nTr, data[row]);
            	detailsIndex = $(this).parents('tr')[0].rowIndex - 1;
            	detailsInfo = $('#filestable').find('tbody tr .details').parents('tr')[0];
            }
            return;
        });
	//添加详情信息
	var addDetails = function(nTr, data){
		...
		sOut += '<table>';
		sOut += getFSDetails(data);
		sOut += '</table></td></tr>';
		$(nTr).after(sOut);
	}

	var getFSDetails = function(data){
		if(LANG.UI_NAS_BACKUP == data.tasktype){
			//文件备份
			return getBackupFSDetails(data);
		}else if(LANG.UI_NAS_RESTORE == data.tasktype){
			//文件恢复
			return getRecoveryFSDetails(data);
		}
	}

	//备份数据库详情
	var getBackupFSDetails = function(data){
		var allpath = [];
		var wildcardmode,wildcard = [];
		allpath = data[10].join('\n');
		if(data[12].wildcardmode != null && data[12].wildcard != null) {
			wildcard = data[12].wildcard.join('<br>');
			data[12].wildcardmode==1?wildcardmode = LANG.UI_FILE_WILDCARD_BAK_FILTER:wildcardmode = LANG.UI_FILE_WILDCARD_BAK_SELECT;
		}else {
		...
		if(Object.keys($('#archiveflag')).length != 0) {
			...
			contentTh = '<th style="width: 10%;">'+LANG.UI_ARCHIVE+'</th>'
			contentTb = '<td>'+ file_archive +'</td>'
		}
		var details = '<thead><th style="width: 18%;">'+LANG.UI_FILE_COUNT_WILDCARD_MODE+'</th><th style="width: 18%;">'+LANG.UI_FILE_WILDCARD+'</th>'+ contentTh +'<th style="width: 18%;">'+LANG.UI_FILE_BAK_PATH+'</th><th style="width: 42%;"></th></thead><tbody><tr><td>'
		+ wildcardmode +'</td><td>'+ wildcard +'</td>'+ contentTb +'<td><textarea style="outline: none;" cols="60" rows="3">' + allpath + '</textarea></td></tr></tbody>';
		return details;
	}

	//恢复数据库详情
	var getRecoveryFSDetails = function(data){
		var allpath = [];
		allpath = data[10].join('\n');
		//判断是否是跨平台恢复
		...
		+  data[2] +'</td><td>'
		+ data.des_agent +'</td><td class="'+ className +'">'
	...
	$('.deletefs').unbind().on('click', function(){
		if($(this).find('.btn').attr('disabled')){
			return true;
		}
		deleteFs();
	...
	var startFsJob = function(mode){
		var select = fsgrid.getSelectedRows();
		if(!select.length){
			return UIToastr.showInfo(LANG.UI_FILE_DETAILS_JOB_SELECT_HOST, LANG.UI_FILE_DETAILS_JOB_SELECT_HOST_TOBAK);
		}
		checkOperateAuth({
			type: 1,
			user_uuid: task_user_uuid,
			...
			var datatable = fsgrid.getDataTable().data();
			var data = {};
			data.fs_uuids = select;
			data.taskuuid = $("#task_uuid").val();
			data.backup_mode = mode;
			var p = JSON.stringify(data);
			$.post(CONF.AJAXPATH, {m:CONF.M.JOB,f:'startFSJob',p: p}, function(d){
				if(OPREL(d)){
					fsgrid.getRefresh({});
				}
			});
	...
	var deleteFs = function(){
		var select = fsgrid.getSelectedRows();
		//获取有多少条数据
		var dataList = fsgrid.getDataTable().rows();
		...
		bootbox.confirm({
			...
			callback: debounce(function(r) {
				if(!r) return;
				submitDeleteTable(select, fsgrid);
			}, 300),
		});
	}

	var submitDeleteTable = function(select, fsgrid){
		var data = {};
		data.fs_uuids = select;
		data.taskuuid = $('#task_uuid').val();
		var p = JSON.stringify(data);
		$.post(CONF.AJAXPATH, {m:CONF.M.JOB, f:'deleteSelectFs', p: p}, function(d){
			if(OPREL(d)){
				fsgrid.getRefresh({});
			}
		})
	...
	var initHistoryGrid = function(){
		...
		var init = function(){
			if(!initFlag){
				var data = {};
				data.uuid = $("#task_uuid").val();
				data = {m:CONF.M.JOB,f:'getFSDetailsHistory',p:data};
	...
	var addHisDetails = function(nTr, data){
		...
		$("a.downloadPassFile").off().click(function(){
			...
			if(data[9].taskType == 1){//备份
				...
				//备份文件总数
				var agent_uuid;
				data[9].info.agent_info_list.forEach(item=> {
					agent_uuid = $(this).attr('value');
					if(item.agent_uuid == $(this).attr('value')) {
						...
						+ data[9].info.passfile_number_limit +'</td><td>'
						+ data[9].info.passfile_ratio_limit +'%</td>');
				...
				$('#downloadTxt').off().click(function() {
					downloadPassFun(data.history_uuid,agent_uuid);
				});
			} else {//恢复
    ...
    var getBackupHisDetails = function(data){
		if(!data) return LANG.UI_PUBLIC_NOTHING;
		var dataSet = [],passFile = [];
		var tbodyContent = "",details = "",agent_uuid="";
		if(data.agent_info_list == undefined || data.agent_info_list == "") {
		...
		if(data.agent_info_list.length != 0) {
			data.agent_info_list.forEach(item=> {
				...
				if(item.wildcard_list == "") {
					...
					if(wildcard_list.wildcard_mode != null && wildcard_list.wildcard !=null) {
				...
				var file_archive = data.file_archive_flag !=2 ? LANG.UI_FILE_ENABLE:LANG.UI_FILE_DISABLE;
				dataSet.push([item.src_agent_name+'('+item.src_agent_ip+')',
			...
			dataSet.forEach(item => {
				...
				if((parseInt(item[1]) + parseInt(item[9])) != 0) {
				...
				//判断是否显示归档
				if(Object.keys($('#archiveflag')).length != 0) {
					contentTh = '<th style="width: 5%;">'+LANG.UI_ARCHIVE+'</th>'
					contentTb = '<td style="padding-top: 25px;">'+ item[10]+'</td>'
				}
				tbodyContent += '<tr>'+
									'<td style="padding-top: 25px;">'+ item[0]+'</td>'+
									'<td style="padding-top: 25px;">'+ item[11]+'</td>'+
									'<td style="padding-top: 25px;">'+ item[6]+'</td>'+
									'<td style="padding-top: 25px;">'+ item[7]+'</td>'+ contentTb +
									'<td style="padding-top: 25px;">'+ item[1]+'</td>'+
									'<td style="padding-top: 25px;">'+ item[9]+'</td>'+
									'<td style="padding-top: 25px;">' + '<a value ="'+ item[12] +'" class="downloadPassFile" '+ style +'>'+ elecontent +'<span class="display-none">'+ item[8] +'</span></a>' + '</td>' +//跳过文件下载
			...
			var details = '<div class="table-container" style="max-height: 250px;overflow: auto;"><table style="width: 100%;" class="table" id="fileHistoryDetailtable"><thead><tr role="row"class="heading"><th width="15%">'+LANG.UI_FILE_AGENT_NAME+'/IP</th><th width="8%">'+LANG.UI_SEARCH_TASK_TYPE+'</th><th style="width: 10%;">'+LANG.UI_FILE_COUNT_WILDCARD_MODE+'</th><th style="width: 10%;">'+LANG.UI_FILE_WILDCARD+'</th>'+ contentTh +'<th width="10%">'+LANG.UI_FILE_COUNT_PASS+'</th><th width="10%">'+LANG.UI_FILE_COUNT_PASS_DIR+'</th><th width="10%">' + LANG.UI_FILE_PASSFILE_LISTS + '</th><th width="7%">'+LANG.UI_OS_PLUG_TOTAL_SIZE+'</th><th width="10%">'+LANG.UI_FILE_PROCESSED_CAPACITY+'</th><th width="5%">' + LANG.UI_PUBLIC_DESCRIPTION + '</th></tr></thead><tbody>'+tbodyContent+'</tbody></table></div>';
		}else {
			return LANG.UI_PUBLIC_NOTHING;
		}
		return details;
 	 ...
 	 function pow1024(num) {
    	...
    	var getRecoveryHisDetails = function(data){
    		...
    		for(var i=0; i<data.file_list.length; i++){
    			list += data.file_list[i] + "\n";
    		}
			tbodyContent += '<tr>'+
									'<td style="padding-top: 25px;">'+ data.src_agent +'</td>'+
									'<td style="padding-top: 25px;">'+ data.des_agent_name+'('+data.des_agent_ip+')'+'</td>'+
									'<td style="padding-top: 25px;"><textarea style="outline: none;" cols="40" rows="2">'+ list +'</textarea></td>'+
									...
									'<td style="padding-top: 25px;">'+ data.agent_info_list[0].description +'</td>'+
								'</tr>';
			details = '<div class="table-container" style="max-height: 250px;overflow: auto;"><table style="width: 100%;" class="table" id="fileRecoveryDetailtable"><thead><tr role="row"class="heading"><th width="15%">'+ LANG.UI_FILE_DETAIL_SRC_NAME +'</th><th width="15%">'+ LANG.UI_FILE_DETAIL_DES_NAME +'</th><th width="15%">'+ LANG.UI_FILE_RECOVER_PATH +'</th><th width="10%">'+ LANG.UI_FILE_COUNT_TOTAL +'</th><th width="10%">'+ LANG.UI_FILE_COUNT_PASS +'</th><th width="10%">'+LANG.UI_FILE_COUNT_PASS_DIR+'</th><th width="15%">'+ LANG.UI_FILE_PASSFILE_LISTS +'</th><th width="15%">' + LANG.UI_PUBLIC_DESCRIPTION + '</th></tr></thead><tbody>'+tbodyContent+'</tbody></table></div>';
	...
	var getParams = function() {
		let data = {};
		data.uuid = $("#task_uuid").val();
		data.keyword = $(".file-detail-search").val().trim();
		return data;
	}
