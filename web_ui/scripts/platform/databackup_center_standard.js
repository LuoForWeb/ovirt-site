var DataBackupCenter = function () {
    ...
    const initProtectEcharts = (data) => {
        const weekdata = data.allmodule.data.slice(0, 7).reverse();
        const monthdata = data.allmodule.data.slice(0, 30).reverse();
        const yeardata = data.allmodule.data.slice(0, 365).reverse();

        // 统一渲染函数
        const renderChart = (chartId, chartData, dateNum, key, showClass, nodataClass) => {
            const isZero = chartData.every(num => num === 0);
            ...
            const interval = maxNum / 5;
            const option = getoption(chartData, interval, dateNum, maxNum, data, key);

            // 延迟渲染 + 两次resize
            setTimeout(() => {
                myChart.resize(); // 第一次resize是因为浏览器虽然渲染了DOM，但是echarts实例记录到的可能是初始化的尺寸（可能为0）
                myChart.setOption(option, true);
                myChart.resize(); // 第二次resize是为了强制基于最新的配置和容器尺寸重新渲染图表
            }, 0);
        };

        // 单调用三处图表渲染
        renderChart('data_week_line', weekdata, 7, 'week', '.data_week_line', '.week_line_nodata');
        renderChart('data_month_line', monthdata, 30, 'month', '.data_month_line', '.month_line_nodata');
        renderChart('data_year_line', yeardata, 365, 'year', '.data_year_line', '.year_line_nodata');
    };
    ...
    const initStorageEcharts = (data) => {
        ...
        // 定义内部渲染函数（关键修复点）
        const renderStorageChart = (chartId, chartData, dateNum, key, showClass, nodataClass) => {
            const isZero = chartData.every(num => num == 0);
            ...
            const option = getstorageoption(chartData, interval, dateNum, maxNum, data, key);

            // 【关键修复】使用 setTimeout 确保 DOM 布局完成
            setTimeout(() => {
                // 1. 先 resize 确保容器尺寸正确
                myChart.resize();
                // 2. setOption (带 true) 强制重绘
                myChart.setOption(option, true);
                // 3. 再 resize 修正 dataZoom 位置
                myChart.resize();
            }, 0);
        };

        // 调用渲染函数（传递完整参数）
        renderStorageChart('storage_week_bar', weekdata, 7, 'week', '.storage_week_bar', '.week_bar_nodata');
        renderStorageChart('storage_month_bar', monthdata, 30, 'month', '.storage_month_bar', '.month_bar_nodata');
        renderStorageChart('storage_year_bar', yeardata, 365, 'year', '.storage_year_bar', '.year_bar_nodata');
    };
