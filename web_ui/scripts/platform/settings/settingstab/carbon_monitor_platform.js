...
var Settings_System_Recovery = function(){
	...
	var addListeners = function(){
		var carbonMonitorForm = $('#carbonMonitorForm');
		var error2 = $('.alert-danger', carbonMonitorForm);
		var success2 = $('.alert-success', carbonMonitorForm);
		carbonMonitorForm.validate({
		...
		$("#carbonSubmit").click(function(){
			if (!$('input[name=ipaddr]').val()) {
				// 允许保存空的ip或者域名
				submit();
				return;
			}
			if (carbonMonitorForm.validate().form()) {
				submit();
			}
		});
		$('#configCarbon').on('switchChange.bootstrapSwitch', configChange);
	}
	...
	$("#carbonCancel").click(function(){
		LOCATION('./content/platform/settings/setting_manager.php?tab=7', 'setting_manager');
	});
	
	var submit = function(){
		Metronic.blockUI({target: '#carbonMonitorForm',animate: true});
		var data = {};
		data.httpType = $('#httpType').val();
		data.ipaddr = $('input[name=ipaddr]').val();
		data.config_carbon_flag = $('#configCarbon').get(0).checked;
		var p = JSON.stringify(data);
		$.post(CONF.AJAXPATH, {m:CONF.M.SYSTEM, f:'setCarbonMonitorConf', p:p}, function(d){
			Metronic.unblockUI('#carbonMonitorForm');
	    	if(OPREL(d)){
	    	}
		});
	}
	...
	var initOldInfo = function(){
		$.post(CONF.AJAXPATH, {m:CONF.M.SYSTEM, f:'getCarbonMonitorInfo', p:{}}, function(d){
			...
			//可视化配置初始化
			$('#configCarbon').bootstrapSwitch('state', data.config_carbon_flag);
			if (data.config_carbon_flag) {
