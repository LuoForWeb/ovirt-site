...
//                error2.hide();
	...
	var initListeners = function(){
		...
		$("#monetary_unit").on('change',function(){
			$(".update_money").text($("#monetary_unit").find("option:selected").text())
		});
		//根据策略类型选择不同显示不同时间单位
	...
	var submitBnt = function(){
		...
		$.post(CONF.AJAXPATH, {m:CONF.M.BILLING, f:"editBilling", p:data}, function(d){
			if(OPREL(d)){
	...
	var initData = function(){
		...
		$.post(CONF.AJAXPATH, {m:CONF.M.BILLING, f:"getEditBillingList", p:data}, function(d){
			...
			$("#billing_description").val(Data['description']);
			$("#billing_type").val(Data['billing_type']);
			$('#monetary_unit').selectpicker('val', Data['monetary_unit']);
			$('#monetary_unit').selectpicker('refresh');
			$('#billing_num_type').val(Data['billing_mode']);
			$('#num_selected').selectpicker('val',JSON.parse(Data['num_type']))
			$('#num_selected').selectpicker('refresh');
			
			//单价json格式，需要处理
			var unit_cost = JSON.parse(Data['unit_cost']);
			$("#billing_vm").val(unit_cost['billing_vm']);
			$("#billing_fs").val(unit_cost['billing_fs']);
			$("#billing_db").val(unit_cost['billing_db']);
			$("#billing_capacity").val(unit_cost['billing_capacity']);
			//按钮switch
			$int_inform = Data['inform'];
			if($int_inform==1){
				$("#inform").bootstrapSwitch('state',true);
				$("display_inform").css("display","block");
			}else{
				$("#inform").bootstrapSwitch('state',false);
			}
			
			$("#inform_period").val(Data['inform_period']);
			$(".update_money").text($("#monetary_unit").find("option:selected").text())
	...
	//数据加载后根据加载数据不同展示不同的输入框
	var initDisplay = function(data_list){
		if(data_list['billing_mode']==1){
			$("#display_capacity").css("display","block");
		}else{
			$("#display_num_type").css("display","block");
		}
