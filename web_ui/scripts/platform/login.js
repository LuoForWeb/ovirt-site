var Login = function () {
	...
	var submit = function(){
		...
		var remember=false;
		if($('.form-actions .remember-true').css('display')!='none'){
			remember=true;
		}
		//  如果有验证码的情况先比对验证码
	...
	// 和旧的version作比较
	var checkAndCleanOlderStorage = function (){
		var old_version = localStorage.getItem('pre_web_version');
		var new_version = $('.version').val();
		...
		localStorage.setItem('pre_web_version',new_version);
	}
	var loginResult = function(d){
		//退出登录，移除exchange用于恢复页面验证身份的session
		sessionStorage.removeItem('exchange_recovery_pass');
		var loginResult = d.data;
		var loginErrorCount=loginResult.faliCount;//获取登录错误次数
		var tips = '';
		//用户名密码正确
		if(1 == loginResult.result) {
				if ($.trim($('input[name=username]').val()) == localStorage.getItem('username')) {
			...
			localStorage.setItem('count',loginErrorCount);
			...
			if(localStorage.getItem('count') >= 3) {  //有验证码的情况
				 refresh_code();  //刷新验证码
				_CodeShow = true;
				$('.input-val').val('');
				$(".login-code").show();
	...
	var rememberToggle=function () {
		if($('.form-actions .checkbox .remember-true').attr('data-checked')){
			$('.checkbox .remember-true').show()
			$('.checkbox .remember-false').hide();
		}
		//默认是不记住密码
		$('.checkbox .remember-false').click(function () {
			$('.checkbox .remember-false').hide();
			$('.checkbox .remember-true').show();
		})
		$('.checkbox .remember-true').click(function () {
			$('.checkbox .remember-true').hide();
			$('.checkbox .remember-false').show();
		})
		$('.checkbox .remember-pwd').click(function () {
			$('.checkbox .remember-true').toggle();
			$('.checkbox .remember-false').toggle();
		})
	}
