...
//                    Metronic.scrollTo(error2, -200);
	...
	var initOldInfo = function(){
		...
		$.post(CONF.AJAXPATH, {m: CONF.M.TENANT, f: "getTenantEditInfo", p: p }, function(d){
			var jsonData = JSON.parse(d);
			$('#tenantname').val(jsonData.tenantname).prop('disabled', true);
			$('#nickname').val(jsonData.nickname);
			$('#adminname').val(jsonData.adminname);
			$('#password').val(jsonData.password);
			$('#rpassword').val(jsonData.password);
			oldPass = jsonData.password;
			$('#email').val(jsonData.email);
			
			adminUUID = jsonData.adminuuid;
