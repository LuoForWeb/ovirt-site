...
(function($){
                ...
                columns: [ //列定义
                    ...
                    },
                    {
                        ...
                        formatter: function (value, data) {
                            let new_name =  data.vm_name + '_' + getDatetime(data.timepoint);
                            // 恢复到华为云时原有的非法字符转为下划线
                            if (CONF.VM_TYPE.HUAWEICLOUD == target_hypervisor) {
                                new_name = new_name.replace(/[^\u4e00-\u9fa5a-zA-Z0-9._-]/g, '_');
                            }
                            return new_name;
                        }
                    },
    ...
    const initEditInstanceRecoverInfo = function (rowData) {
        ...
        // 恢复到华为云时原有的非法字符转为下划线
        if (CONF.VM_TYPE.HUAWEICLOUD == target_hypervisor) {
            _currentInstanceConfig.instance_name = _currentInstanceConfig.instance_name.replace(/[^\u4e00-\u9fa5a-zA-Z0-9._-]/g, '_');
        }
        ...
        _IOS.val(_currentInstanceConfig.os_type);
    ...
    const saveVolRowData = function (rowData) {
        ...
        // 已排除的不添加，注意初始化时checked=undefined
        let checked = $('#vol-table').find('tr[data-uniqueid=' + rowData.uuid + ']').find(`input[name=btSelectItem]`).prop('checked');
        if (-1 < $.inArray(rowData.timepoint_uuid, timepoint_uuids) && false !== checked) {
    ...
    const initListener = function(){
        ...
        _IRegion.on('change', function () {
            ...
            switch (target_hypervisor) {
                case CONF.VM_TYPE.AWS:
                    getTypeConfigs($(this).find('option:selected').val(), true, _IAZ.val() ?? '', _IOS.val() ?? '');
                    break;
                case CONF.VM_TYPE.HUAWEICLOUD:
                    getAllConfigsByRegion($(this).find('option:selected').val(), _IAZ.find('option:selected').val(), _IOS.find('option:selected').val());
                    break;
                default:
                    break;
            }
        ...
        _IAZ.on('change', function () {
            switch (target_hypervisor) {
                case CONF.VM_TYPE.HUAWEICLOUD:
                    getTypeConfigs(_IRegion.find('option:selected').val(), false, $(this).find('option:selected').val(), _IOS.find('option:selected').val());
                    break;
                default:
                    break;
            }
        ...
        _IType.next('.bootstrap-select').find('.dropdown-toggle').on('click', function () {
            getTypeConfigs(_IRegion.val(), false, _IAZ.val() ?? '', _IOS.val() ?? '');
        });
        ...
        _ISG.on('click', function () {
            getTypeConfigs(_IRegion.val(), true, _IAZ.val());
        });
    ...
    const getTypeConfigs = function (region_uuid, loadNetwork = true, available_zone = '', os_type = '') {
        switch (target_hypervisor) {
            case CONF.VM_TYPE.AWS:
        ...
        let archVal = parseInt(_ICPU.val());
        let architecture = _ICPU.find(`option:selected`).attr('data-des') ?? DEFAULT_ARCHITECTURE;
        // AWS传值只有x86_64/arm64
        // 华为云传值只有x86/arm
        if (archVal === CPU_ARCH.x86 || archVal === CPU_ARCH.x86_64) {
            switch (target_hypervisor) {
                case CONF.VM_TYPE.AWS:
                    architecture = 'x86_64';
                    break;
        ...
        if (archVal === CPU_ARCH.ARM || archVal === CPU_ARCH.ARM64) {
            switch (target_hypervisor) {
                case CONF.VM_TYPE.AWS:
                    architecture = 'arm64';
                    break;
    ...
    //异步获取实例类型等并初始化，华为云使用
    const getAllConfigsByRegion = function (region_uuid, available_zone, os_type = '') {
        let archVal = parseInt(_ICPU.val());
        let architecture = _ICPU.find(`option:selected`).attr('data-des') ?? DEFAULT_ARCHITECTURE;
        // 华为云传值只有x86/arm
        if (archVal === CPU_ARCH.x86 || archVal === CPU_ARCH.x86_64) {
    ...
    const saveInstanceConfigTemp = function () {
        ...
        let insConfig = {
            ...
            cpu_arch: _ICPU.val(),
            boot_mode: _currentInstanceConfig.boot_mode,
        }
    ...
    const saveVolConfigTemp = function () {
        ...
        if (2 == $('#recovertype-select').val()) {
            ...
            _tr.find('td').eq(1).text(volConfig.vol_new_name);
            _tr.find('td').eq(4).text(volConfig.volume_disk_type);
            _tr.find('td').eq(5).text(volConfig.disk_iops != 0 ? volConfig.disk_iops : '--');
            _tr.find('td').eq(8).text(region);
            _tr.find('td').eq(9).text(zone);
            _tr.find('td').eq(10).text(kms);
        } else {
            ...
            if (v.instance_uuid == volConfig.instance_uuid && v.volume_uuid != volConfig.volume_uuid) {
                if (2 == $('#recovertype-select').val()) {
                    ...
                    if (CONF.VM_TYPE.AWS == target_hypervisor) {
                        ...
                        let _tr = $('#vol-table').find('tr[data-uniqueid=' + v.volume_uuid + ']');
                        _tr.find('td').eq(8).text(region);
                        _tr.find('td').eq(9).text(zone);
                    }
                } else {
        ...
        $.each(os_version_list, function (i, v) {
            ...
            if (v.os_type == osType) {
                if (v.os_version == osVersion) {
                    //默认选中
                    option += `<option value="${v.os_version}" selected="selected">${v.os_description}(${interfacesbak})</option>`;
                } else {
                    option += `<option value="${v.os_version}">${v.os_description}</option>`;
                }
            }
        });
        return option;
