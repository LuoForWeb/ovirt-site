...
(() => {
    ...
    const getOptions = options => {
        return {
            ...
            remote_network_ip: typeof options.remote_network_ip === 'undefined' ? null : options.remote_network_ip,
            remote_network_port: typeof options.remote_network_port === 'undefined' ? null : parseInt(options.remote_network_port),
            ...
            pAjaxRequest(reqData, `/api/v1/copy/nodes/net/remote`, 'GET', res => {
                ...
                for (const index in res.data.msg) {
                    ...
                    nodes.push({
                        ...
                        network_pool_uuid: '',
                        network_ip: row.ip,
                        network_port: parseInt(row.port),
                        node_uuid: '',
                        network_type: 1,
                        eventtype: 'network',
    ...
    const initData = id => {
        ...
        initNetworkPoolTree(id).then(() => {
            initNetworkTree(id).then(() => {
                ...
                // 设置默认值
                if (cache[id].options.remote_flag) {
                    if (cache[id].options.remote_network_ip !== null && cache[id].options.remote_network_port !== null) {
                        let remoteNetworkNodes = cache[id].network_tree.getNodesByParam('network_ip', cache[id].options.remote_network_ip);
                        if (remoteNetworkNodes.length) {
                            for (const remoteNetworkNode of remoteNetworkNodes) {
                                if (remoteNetworkNode.network_port === cache[id].options.remote_network_port) {
                                    cache[id].network_tree.checkNode(remoteNetworkNode, true, true, true);
