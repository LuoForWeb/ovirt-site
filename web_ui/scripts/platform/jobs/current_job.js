...
var CurrentJob = function () {
    ...
    let TEMP_ADVANCED_SEARCH_PARAMS = {}; // 临时高级搜索参数
    let openedDropdownId = ''; // 记录当前打开的操作下拉菜单的ID
    let isMouseOnDropdown = false; // 标记鼠标当前是否悬停在下拉菜单上
    let hasMouseEverBeenOnDropdown = false; // 标记鼠标是否曾经进入过下拉菜单
    ...
    const PAGE_RECORD = JSON.parse(sessionStorage.getItem('current_table_pageRecord')) || { offset: 0, limit: 10 };
    ...
    const getParams = function () {
        ...
        let sessionStorageParams = JSON.parse(sessionStorage.getItem('current_job_filter_params')) || {};
    ...
    /**
     * @description: 记录当前点击的操作按钮，并处理下拉菜单的显示/隐藏
     * @param {object} target - 当前点击的按钮元素
     */
    const recordAndToggleDropdown = function (target) {
        let $target = $(target);
        let $parentGroup = $target.parent('.btn-group');
        let dropdownId = $target.attr('id');

        // 如果点击的是同一个已展开的按钮，则关闭它并清除所有状态
        if ($parentGroup.hasClass('open') && openedDropdownId === dropdownId) {
            $parentGroup.removeClass('open');
            openedDropdownId = null;
            isMouseOnDropdown = false;
            hasMouseEverBeenOnDropdown = false;
        } else {
            // 如果点击的是新按钮，先关闭其他可能已展开的菜单
            if (openedDropdownId) {
                $('#' + openedDropdownId).parent('.btn-group').removeClass('open');
            }
            // 展开当前点击的菜单，并重置状态
            $parentGroup.addClass('open');
            openedDropdownId = dropdownId;
            isMouseOnDropdown = false; // 刚打开时，鼠标肯定不在菜单上
            hasMouseEverBeenOnDropdown = false; // 重置“进入过”状态
        }
    }
    ...
    var startJobFunc = function (row, taskType, type) {
        ...
        pAjaxRequest(params, "/api/v1/jobs/start/" + row.job_uuid + "", 'POST', function (data) {
            ...
            if(type == 0){
                op  = LANG.UI_COPY_SEND_SCHEDULE_JOB_MESSAGE; 
            }
            if (operateResponseList(data, op)) {
                $("#current_table").bootstrapTable('refresh');
                $("#current_table").bootstrapTable('hideLoading');
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            switch (status) {
                ...
                case CONF.TASK_STATUS.STARTING: //启动中
                    ...
                    if (taskType == CONF.TASK_TYPE.INSTANT_RECOVERY_MOTION) { // 虚拟机迁移任务，迁移状态为完成时可启用 完成迁移 操作，且禁用停止迁移操作，反之
                ...
                if (value != [] && value != '') {
                    switch (key) {
                        ...
                        case 'transport_strategy':
                            // 归档、归档回传、副本回传（18/19/20）→ 都不显示 传输数据压缩配置
                            const hideCompressTypes = [
                                CONF.TASK_TYPE.BACKUP_COPY_FETCH,          // 18
                                CONF.TASK_TYPE.ARCHIVE,  // 19
                                CONF.TASK_TYPE.ARCHIVE_FETCH      // 20
                            ];
                            const needHideCompress = hideCompressTypes.includes(row.job_type_value);

                            if (!needHideCompress) {
                                if (row.job_type_value === CONF.TASK_TYPE.BACKUP_COPY) {
                                    if (row.storage_type === 8) { // 副本到异地存储 的场景则显示加密传输 和 传输数据压缩
                                        let encryptText = value.encrypt_flag_value ? `${LANG.UI_COPY_BACK_ENCRYPT}：${LANG.UI_PUBLIC_ON_ONE}` : `${LANG.UI_COPY_BACK_ENCRYPT}：${LANG.UI_PUBLIC_OFF_ONE}`;
                                        let compressText = value.compress_flag_value ? `${LANG.UI_COPY_BACK_COMPRESS}：${LANG.UI_PUBLIC_ON_ONE}` : `${LANG.UI_COPY_BACK_COMPRESS}：${LANG.UI_PUBLIC_OFF_ONE}`;

                                        $(element).append(
                                            `<p><b>`+ LANG.UI_STRATEGY_TRANSFER +`：</b> `+ encryptText + `， `+ compressText + ` </p>`
                                        );
                                    } else { // 副本到非异地存储的场景只显示 加密传输
                                        let encryptText = value.encrypt_flag_value ? `${LANG.UI_COPY_BACK_ENCRYPT}：${LANG.UI_PUBLIC_ON_ONE}` : `${LANG.UI_COPY_BACK_ENCRYPT}：${LANG.UI_PUBLIC_OFF_ONE}`;

                                        $(element).append(
                                            `<p><b>`+ LANG.UI_STRATEGY_TRANSFER +`：</b> `+ encryptText + ` </p>`
                                        );
                                    }
                                }
                            }
    ...
    var tableInit = function () {
        //任务的操作事件
        var operates = {
            'click .btn-dropdown-operate': function (event, value, row, index) {
                recordAndToggleDropdown(event.currentTarget);
                event.stopPropagation(); // 阻止事件冒泡，避免触发下面的document点击事件
            },
        ...
        var options = {
            ...
            resizable: true,
            pageSize: PAGE_RECORD ? PAGE_RECORD.limit : 10,
            pageNumber: PAGE_RECORD ? PAGE_RECORD.offset + 1 : 1,
            ...
            onCheck: function (row, $element) {
                ...
                $.each(selectedRow, function (index) {
                    ...
                    if (selectedRow[index].job_status_value === 19) { // 挂起状态的任务只能停止，不能批量启动，也不能删除
                        $('.batch-start').removeClass('batch_start_active');
                        $('.batch-start i').removeClass('icon-start');
                        $('.batch-start span').removeClass('bgb');
                        $('.batch-delete').removeClass('batch_delete_active');
            ...
            PostBody: function () {
                ...
                // 如果展开的操作下拉菜单ID存在，就保持展开
                if (openedDropdownId) {
                    let $dropdown = $('#' + openedDropdownId);
                    if ($dropdown.length) {
                        $dropdown.parent('.btn-group').addClass('open');
                    } else {
                        // 如果刷新后按钮不存在了（例如，任务已完成被移除），则清除ID
                        openedDropdownId = null;
                    }
                }
            ...
            onPageChange: function (number, size) {
                ...
                sessionStorage.setItem('current_table_pageRecord', JSON.stringify(pageRecord));
            },
            customTool: { //自定义工具栏
        ...
        // 为下拉菜单绑定鼠标悬停事件
        $('#current_table').on('mouseenter', '.dropdown-menu', function() {
            // 确保只有当有菜单被记录为打开时，才设置isMouseOnDropdown
            if (openedDropdownId && $(this).parent().find('#' + openedDropdownId).length) {
                isMouseOnDropdown = true;
                hasMouseEverBeenOnDropdown = true; // 标记鼠标已经进入过
            }
        ...
        $(document).on('click', function (e) {
            // 如果点击的目标不在一个打开的btn-group内部
            if (openedDropdownId && !$(e.target).closest('.btn-group.open').length) {
                $('#' + openedDropdownId).parent('.btn-group').removeClass('open');
                openedDropdownId = null;
                isMouseOnDropdown = false;
                hasMouseEverBeenOnDropdown = false;
    ...
    const hrefFormatter = function (value, row, index, field) {
        ...
        const baseHref = `${url}?type=${encodeURIComponent(row.job_type_value)}&uuid=${encodeURIComponent(row.job_uuid)}`;
        let href = baseHref;
    ...
    const initCurrentJobTimer = function () {
        ...
        // 只有当菜单曾被打开、鼠标曾进入过、且现在已离开时，才在刷新前将其标记为关闭
        if (openedDropdownId && !isMouseOnDropdown && hasMouseEverBeenOnDropdown) {
            openedDropdownId = null;
        }
            ...
            selectFn: (val) => {
                ...
                switch (len) {
                    case 2: // 模块类型选的所有
                        ...
                        TEMP_ADVANCED_SEARCH_PARAMS = Object.assign(TEMP_ADVANCED_SEARCH_PARAMS, taskTypeParams);
                        break;
                    case 3: // 模块类型非所有
    ...
    const addListeners = function () {
        ...
        $('#current_job_advanced_search_submit').on('click', () => {
            ...
            // 临时高级搜索参数合并给高级搜索参数
            ADVANCED_SEARCH_PARAMS = $.extend(true, {}, TEMP_ADVANCED_SEARCH_PARAMS);
            ...
            $('#current_table').bootstrapTable('refresh', { pageNumber: 1, query: { ...ADVANCED_SEARCH_PARAMS } });
        ...
        window.$on('current_job_filter_btn-updateFilterEvent', (filterData) => {
            ...
            sessionStorage.setItem('current_job_filter_params', JSON.stringify(sessionStorageParams));

            FILTER_PARAMS = Object.assign(ADVANCED_SEARCH_PARAMS, FILTER_PARAMS);

            $('#current_table').bootstrapTable('refresh', { pageNumber: 1, query: { ...FILTER_PARAMS } });
        });
        ...
        window.$on('current_job_datepicker-updateDateRangeEvent', (data) => {
            ...
            $('#current_table').bootstrapTable('refresh', { pageNumber: 1, query: { start_time: startTime, end_time: endTime } });
        });
    ...
    const initCurrentJobTableFilter = () => {
        ...
        const filteredJobTableOptions = CURRENT_JOB_TABLE_FILTER_OPTIONS.reduce((acc, item) => {
            // 如果是需要按对象权限过滤的列
            if (objectPermissionFields.includes(item.field)) {
                // 对每个 item 的 value 进行过滤
                const filteredOptions = item.value.filter(option => permissions.includes(option.id));
        ...
        // 用过滤后的新数组完全替换旧数组的内容
        CURRENT_JOB_TABLE_FILTER_OPTIONS.length = 0;
        CURRENT_JOB_TABLE_FILTER_OPTIONS.push(...filteredJobTableOptions);
        ...
        for (const moduleKey in moduleTypeToTaskTypeMap) {
            ...
            if (permissions.includes(moduleKey)) {
                moduleTypeToTaskTypeMap[moduleKey].forEach(taskTypeId => {
                    allowedTaskTypeSet.add(taskTypeId);
                });
            }
        }

        // 2. 找到“任务类型”过滤器这一项（最后一项）
        const taskTypeFilterIndex = CURRENT_JOB_TABLE_FILTER_OPTIONS.findIndex(item => item.field === 'task_type');
        if (taskTypeFilterIndex > -1) {
            const taskTypeFilterItem = CURRENT_JOB_TABLE_FILTER_OPTIONS[taskTypeFilterIndex];

            // 3. 过滤 value 数组
            taskTypeFilterItem.value = taskTypeFilterItem.value.filter(item => {
                // GMP 剔除数据验证
                if (CONF.VENDOR === CONF.VENDOR_LIST.gmp && item.id === 'task_type_data_verify') {
                    return false;
                }
                // 只保留并集中存在的任务类型
            ...
            taskTypeFilterItem.value = taskTypeFilterItem.value.filter(item => {
                ...
                if ((!permissions.includes('copy_protect') && item.id === 'task_type_copy') || (!permissions.includes('copy_protect') && item.id === 'task_type_copy_fetch')) {
                    return false;
                }

                // permissions中有 ‘archive_new’ 表示有归档权限，没有则要移除taskTypeFilterItem中id为task_type_archive和task_type_archive_fetch
                if ((!permissions.includes('archive_new') && item.id === 'task_type_archive') || (!permissions.includes('archive_new') && item.id === 'task_type_archive_fetch')) {
                    return false;
                }
            ...
            if (taskTypeFilterItem.value.length === 0) {
                CURRENT_JOB_TABLE_FILTER_OPTIONS.splice(taskTypeFilterIndex, 1);
            }

            // 浅拷贝一份，避免直接修改原数组
            CURRENT_JOB_TABLE_FILTER_OPTIONS[taskTypeFilterIndex] = {...taskTypeFilterItem};
        }
        ...
        // 获取 sesseionStorage中保存的过滤条件，需要回显到过滤器上，给CURRENT_JOB_TABLE_FILTER_OPTIONS中勾选的项加上 checked: true
        let sessionStorageParams = JSON.parse(sessionStorage.getItem('current_job_filter_params')) || {};
        if (sessionStorageParams && sessionStorageParams.originalData && sessionStorageParams.originalData.length > 0) {
            ...
            CURRENT_JOB_TABLE_FILTER_OPTIONS.forEach(group => {
