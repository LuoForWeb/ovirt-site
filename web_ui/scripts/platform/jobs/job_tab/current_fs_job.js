var CurrentFsJob = function () {
	...
	var addOpButton = function(d){
		...
		var opDiv = $('#fsJobTable tbody > tr').find('td:eq(10)');
		var nameDiv = $('#fsJobTable tbody > tr').find('td:eq(1)');
		var levelDiv = $('#fsJobTable tbody > tr').find('td:eq(6)');
		var agentDiv = $('#fsJobTable tbody > tr').find('td:eq(3)');
		//有几列 添加 几次
		for(var i=0; i<opDiv.length; i++){
			...
			setAgentTitle(agentDiv[i], data[i]);
		}
		//opButton 对应点击操作
		addOpButtonListener();
		for(var i=0; i<opDiv.length; i++){
			...
			switch(status){
				...
				case 5:
					...
					//停止中状态，变为强制停止
					$('#fsJobTable #' +uuid + ' .stop').html('<a href="javascript:;"><i class="viconfont vicon-ge_suspend-copy"></i> ' + LANG.UI_JOB_FORCE_STOP + '</a>');
					break;
				case 6:
					addForbidButton(uuid, "pause");
					addForbidButton(uuid, "delete");
...
//					addForbidButton(vmuuid, "takeover");
		...
		addDetailsInfo();
		//切换页数保存到cookie
		$('select[name=fsJobTable_length]').on('change', function(){
			pageLength.fs = this.value;
			var data = JSON.stringify(pageLength);
			$.cookie("pageLengthFs", data);
		});
	}
	
	//添加禁止点击的按钮样式
	var addForbidButton = function(uuid, option){
		$('#fsJobTable #' + uuid + ' .' +  option).unbind();
		$('#fsJobTable #' + uuid + ' .' +  option + ' a').css("opacity",".4");
		$('#fsJobTable #' + uuid + ' .' +  option + ' a').css("cursor","default");
		$('#fsJobTable #' + uuid).on("click", "." +  option + " a", function(e) {
	        e.stopPropagation();
	    });
	}
	...
	var addDetailsInfo = function(){
		var currentPage = parseInt($('#current_fs_job .pagination-panel-input').val());
		if(currentPage != pageIndex){
			return true;
		}
		if(detailsInfo){
			var tr = $('#fsJobTable tbody tr');
			var data = grid.getDataTable().data();
        	addDetails(tr[detailsIndex], data[detailsIndex]);
			var openTr = $('#fsJobTable tbody > tr')[detailsIndex];
	...
	var setAgentTitle = function (div, data) { 
		var agentlist = "";
		var titledes = data[2].join('\n');
		if(data[2].length > 3) {
			agentlist = '<div title="'+ titledes +'">' 
			+ data[2][0] + '\n' + data[2][1] + '\n' + data[2][3] + '\n' 
			+ "<div>...</div>" + '</div>'
		}else {
			agentlist = titledes;
	...
	var opJob = function(button, funName){
		...
		Metronic.blockUI({target: '#current_fs_job',animate: true});
    	$.post(CONF.AJAXPATH, {m:CONF.M.JOB,f:funName,p:params}, function(data){
    		Metronic.unblockUI('#current_fs_job');
    		if(OPREL(data)){
	...
	var addOpButtonListener = function(){
		$('#current_fs_job .start').unbind().on('click', startJob); //启动完备任务
		$('#current_fs_job .pause').unbind().on('click', pauseJob); //暂停任务
		$('#current_fs_job .stop').unbind().on('click', stopJob);   //终止任务
		$('#current_fs_job .edit').unbind().on('click', editJob);   //编辑任务
		$('#current_fs_job .delete').unbind().on('click', deleteJob);//删除任务
		$('#current_fs_job .startDiff').unbind().on('click', startDiff);//差异
		$('#current_fs_job .startIncr').unbind().on('click', startIncr);//增量
		$('#current_fs_job .startLog').unbind().on('click', startLog);//日志
		$('#current_fs_job .startStra').unbind().on('click', startStra);//启动策略
	}
	
	var searchCurrentJob = function(){
		//清除高级筛选显示内容
		$('#fs_searchDiv .searchContent').text('');
		$('#fs_searchDiv').hide();
		var name = $.trim($('.fsjob_search').val());
		var p = {start:0, length:10, search:{name:name}};
		var data = {m:CONF.M.JOB,f:'getCurrentFsJobs',p:p};
	...
	var addListeners = function(){
		...
		$('#fsjob_searchbtn').on('click', searchCurrentJob);
		
		$('.fsjob_search').keypress(function (e) {
            if (e.which == 13) {
            	searchCurrentJob();
            }
        });
		...
		//弹出高级搜索模态框
		$('#fsjob_searchAll').on('click', function(){
			$('#fsJobModal').modal({'width':'851px', 'height':'400px'});
		});
		
		//高级搜索发送请求到服务端
		$('#fs_serach_submit').on('click', function(){
			$('.fsjob_search').val('');
			var p = {};
			p.startTime = _daterangepicker_starttime; //任务开始时间查询范围开头
			p.endTime = _daterangepicker_endtime;		//任务开始时间查询范围结尾
			p.fs_tasktype = $('#fsJobModal #fs_tasktype').val();
			
			p.fs_status = $('#fsJobModal #fs_status').val();
			p.fs_taskName = $('#fsJobModal #fs_taskName').val();
			p.fs_node = $('#fsJobModal #fs_node').val();
			searchParams = p;
			var params = {start:0, length:10, search: p, accurateFlag: true};
			var data = {m:CONF.M.JOB,f:'getCurrentFsJobs',p:params};
			grid.setAjaxParam(data);
			//添加搜索条件显示
			addSearchContent(p);
			$('#fsJobModal').modal('hide');
			grid.getRefresh(params, undefined, true);
			clearTimeout(timerTask.CurrentFSJob_data);
		});
	...
	var addSearchContent = function(p){
		var info = "";
		$('#fs_searchDiv .searchContent').text('');
		var fs_taskName = xssEncode($('#fsJobModal #fs_taskName').val());
		if(p.startTime && p.endTime){
			info += '<span id="time" title="' + p.startTime + "~" + p.endTime + '"> ' + LANG.UI_SEARCH_TIME_RANGE +': <i>' + p.startTime + "~" + p.endTime + '</i><em>X</em></span>';
		}
		if(p.fs_tasktype !="0"){
			info += '<span id="fs_tasktype" title="' + $('#fsJobModal #fs_tasktype').find("option:selected").text() + '"> '+ LANG.UI_SEARCH_TASK_TYPE +': <i>' + $('#fsJobModal #fs_tasktype').find("option:selected").text() + '</i><em>X</em></span>';
		}
		if(p.fs_status != "0"){
			info += '<span id="fs_status" title="' + $('#fsJobModal #fs_status').find("option:selected").text() + '"> '+ LANG.UI_SEARCH_TASK_STATUS +': <i>' + $('#fsJobModal #fs_status').find("option:selected").text() + '</i><em>X</em></span>';
		}
		if(p.fs_taskName){
			info += '<span id="fs_taskName" style="position: relative"><span id="fs_taskNameDetail" style="display: none;position: absolute;bottom: -40px;left: 0px;background: white"> ' + fs_taskName + '</span> '+ LANG.UI_SEARCH_TASK_NAME +': <i>' + fs_taskName + '</i><em>X</em></span>&nbsp;';		}
		if(p.fs_node != "0"){
			info += '<span id="fs_node" title="' + $('#fsJobModal #fs_node').find("option:selected").text() + '"> '+ LANG.UI_SEARCH_SELET_NODE +': <i>' + $('#fsJobModal #fs_node').find("option:selected").text() + '</i><em>X</em></span>';
		}
		
		$('#fs_searchDiv .searchContent').append(info);
		$('#fs_searchDiv').show();
		$('#fs_searchDiv .searchContent #fs_taskName').mouseenter(function(e){
			$('#fs_taskNameDetail').show()
		}).mouseleave(function(){
			$('#fs_taskNameDetail').hide()
		});
		$('#fs_searchDiv .searchContent em').on('click', function(){
			$(this).parent().remove();
			var searchContent = $('#fs_searchDiv .searchContent');
		...
		$('#fs_searchDiv .clearSearch').on('click',function(){
			$('#fs_searchDiv .searchContent').text('');
			$('#fs_searchDiv').hide();
...
//			timeStr += getEachStrategy(msg.timeStrategy[i]);
    ...
    var handleRecords = function () {
    	//读取cookie里的保存列表状态
    	var length = "";
    	if($.cookie('pageLengthFs')){
			var pageList = JSON.parse($.cookie('pageLengthFs'));
			if(pageList.fs){
	    		length = pageList.fs;
	    	}
    	...
    	var initGrid = function(){
    		...
    		if(!gridInitFlag){
    			//加载显示页数分页条
    			grid = new Datatable();
        		var data = {m:CONF.M.JOB,f:'getCurrentFsJobs',p:getParams()};
        		grid.setAjaxParam(data);
            	grid.init({src: $("#fsJobTable"), showDetail:true, dataTable:dataTableOpt, onDataLoad:addOpButton});
            	gridInitFlag = true;
    		...
    		timerTask.CurrentFSJob_data = setTimeout(initGrid, updateInterval);
    	}
    	initGrid();
    	
    	
    	$('#fsJobTable').on('click', ' tbody td .row-details', function () {
        	var data = grid.getDataTable().data();
    ...
    var inintDatatimePicker = function(){
		//初始化日期时间选择控件
		$('#daterangepickerCurrentFsJob').daterangepicker({
...
//			console.log('New date range selected: ' + start.format('YYYY-MM-DD HH:mm:ss') + ' to ' + end.format('YYYY-MM-DD HH:mm:ss') + ' predefined range: ' + label);
		...
		$('#daterangepickerCurrentFsJob').on('apply.daterangepicker', function(ev, picker) {
			//给全局变量赋值,然后设置input
		...
		$('#daterangepickerCurrentFsJob').on('cancel.daterangepicker', function(ev, picker) {
			//清除全局变量,然后设置input
    ...
    var initNodeSelect = function(){
    	$.post(CONF.AJAXPATH, {m:CONF.M.NODE,f:'getNodeList',p:{}}, function(d){
			...
			var nodeSelect = $('#fsJobModal #fs_node');
			nodeSelect.empty();
	...
	var moduleHandler = function(){
		...
		$.post(CONF.AJAXPATH, {m:CONF.M.ROLE,f:'getTaskTypeByPermission',p:datalist}, function(d){
			...
			$('#fs_tasktype').empty().html(select_html);
		});
	}
