var PublicCloudDetail = (function() {
    let PUBLIC_CLOUD_REPORT_TEMPLATE_UUID = '';
    let CURRENT_RUNNING_TENDENCY_QUERY_PARAMS = {
        type: CONF.REPORT_TYPE.PUBLIC_CLOUD,
        timeInterval: CONF.RUNNING_TIME_TYPE.LAST_MONTH,
    }; // 当前任务运行趋势查询参数对象
    let customField = {};
    let PUBLIC_REPORT_TABLE_FILTER_OPTIONS = [
        {
            label: LANG.UI_PLATFORM_VM_CLOUD_PLATFORM_TYPE,
            field: 'vm_type',
            value: []
        },
        {
     ...
     * @param {*} fields 
     */
    const initAllPublicCloudType = (fields) => {
        let params = { cloud_flag: true, cloud_type: 'public' };
        ...
        pAjaxRequest(params, "/api/v1/vm/platforms/hypervisors", "GET", (res) => {
            if (res.success) {
                ...
                if (vmList.length > 0) {
                    PUBLIC_REPORT_TABLE_FILTER_OPTIONS[0].value = vmList.map(item => {
                        return {
                ...
                let filterData = PUBLIC_REPORT_TABLE_FILTER_OPTIONS.filter(option => {
    ...
    const initOverviewData = () => {
        pAjaxRequest({}, '/api/v1/report/template/overview/8', 'get', function (res) {
            try {
                if (res.success) {
    ...
    const getReportDetail = () => {
        pAjaxRequest({}, '/api/v1/report/template/detail/' + PUBLIC_CLOUD_REPORT_TEMPLATE_UUID, 'get', (res) => {
            try {
                ...
                if (res.success) {
                    ...
                    if (running_tendency.history) {
                        ...
                        getPublicCloudReportTaskRunningData(CURRENT_RUNNING_TENDENCY_QUERY_PARAMS);
                    } else {
                    ...
                    // 渲染数据明细表格
                    initPublicReportDetailsTable();

                } else {
                    UIToastr.showWarning(LANG.UI_GET_PUBLIC_CLOUD_REPORT_DATA_ERROR);
                }
            } catch (error) {
                UIToastr.showWarning(LANG.UI_GET_PUBLIC_CLOUD_REPORT_DATA_ERROR);
            } finally {
    ...
    const initPublicCloudReportEchart = (xData, yData, onlyData, echartId, unit) => {
    ...
    const getPublicCloudReportTaskRunningData = (params) => {
            ...
            try {
                if (res.success) {
                    ...
                    if (res.data.every(i => i.value === 0)) {
                        $(`#public_cloud_task_running_chart`).addClass('display-none');
                        $(`#public_cloud_task_running_chart`).siblings().removeClass('display-none');
                    } else {
                        ...
                        $(`#public_cloud_task_running_chart`).removeClass('display-none');
                        $(`#public_cloud_task_running_chart`).siblings().addClass('display-none');

                        initPublicCloudReportEchart(xData, yData, onlyData, 'public_cloud_task_running_chart', unit);
                    }
                } else {
    ...
    const watchEchartSizeChange = () => {
        window.onresize = function() {
            ...
            $('#public_cloud_task_running_chart').css({'width': chartWidth + 'px', 'height': chartHeight + 'px'});
            taskRunningChart.resize();
        }
    ...
    const initVmTableFilter = (filterData) => {
        if ($('#public_report_filter_btn').length === 0) {
            $('#public_report_filter_wrapper').initFilter({
                filterSlotId: 'public_report_filter_wrapper',
                filterBtnId: 'public_report_filter_btn',
                filters: filterData
            });
        } else {
            $('#public_report_filter_wrapper').resetFilter({
                filterBtnId: 'public_report_filter_btn',
                filters: filterData
            });
        }
    ...
    const initPublicReportDetailsTable = () => {
        if ($('#public_cloud_report_table').children().length === 0) { // 表格为渲染
            let options = {
                vin_url: "/api/v1/report/template/public_data_details",
                vin_method: "GET",
                ...
                showColumns: false,
                onPostBody: function() {
                    let tableData = $('#public_cloud_report_table').bootstrapTable('getData');
                ...
                columns: [
                    ...
                    {
                        field: 'ip',
                        title: LANG.UI_PLATFORM_VM_CLOUD_PLATFORM,
                        sortable: true,
                        align: 'center',
                    ...
                    {
                        field: 'vm_type',
                        title: LANG.UI_PLATFORM_VM_CLOUD_PLATFORM_TYPE,
                        sortable: true,
                        align: 'center',
            ...
            $('#public_cloud_report_table').baseTableConfig().init(options);
        } else { // 表格已渲染直接刷新
            $('#public_cloud_report_table').bootstrapTable('refresh');
        }
    }
    ...
    const initRouteParams = () => {
        ...
        PUBLIC_CLOUD_REPORT_TEMPLATE_UUID = !!route.data.url.split('?')[1] ? uuidStr.split('=')[1] : ''; // 保存报表uuid

        if (PUBLIC_CLOUD_REPORT_TEMPLATE_UUID) {
            getReportDetail();
        }
    }
    ...
    const reportOverviewExport = () => {
        $('#public_cloud_export').attr('data-bs-indicator', 'on');

        // 使用按钮防抖1.5秒执行一次导出
        debounce(captureScrollAndGeneratePDF($('.report-detail__content').get(0), 1500, 'public-cloud-report.pdf',  LANG.UI_VINCHIN_REPORT_WATER_NAME, function() {
            $('#public_cloud_export').removeAttr('data-bs-indicator');
        }), 1500);
    }
    ...
    const initListener = () => {
        ...
        $('#report_search').off().on('click', () => {
            ...
            if (filterParams.search) {
                $('#public_cloud_report_table').bootstrapTable('refresh', {query: {...filterParams}})
            }
        });
        ...
        // 监听导出
        $('#public_cloud_export').on('click', reportOverviewExport);

        window.$off('public_report_filter_btn-updateFilterEvent');

        // 监听数据明细 - 过滤器组件派发的数据，以更新表格
        window.$on('public_report_filter_btn-updateFilterEvent', (filterData) => {
