/*
 * @Author: ChengJiaFu
 * @Date: 2025-07-18 09:48:28
 * @Description: k8s报表详情
 * @version: 1.0
 */
var K8sDetail = (function () {
    ...
    const STATUS_TYPE_TO_ECHART_ID_MAP = {
        1: 'k8s_report_all_chart',
        2: 'k8s_report_success_chart',
        3: 'k8s_report_failed_chart'
    }; // 任务运行趋势状态对应echart id
    const K8S_FILTER_OPTIONS = [
        {
    ...
    const initRouteParams = () => {
        ...
        K8S_REPORT_TEMPLATE_UUID = !!route.data.url.split('?')[1] ? uuidStr.split('=')[1] : ''; // 保存报表uuid

        if (K8S_REPORT_TEMPLATE_UUID) {
            getReportDetail();
        }
    }

    /**
     * 获取k8s报表概览详情（28是k8s的module_type）
     */
    const initOverviewData = () => {
        pAjaxRequest({}, '/api/v1/report/template/overview/28', 'get', function (res) {
            try {
                if (res.success) {
                    let overviewList = res.data.overviewList;

                    $('#k8s_total').text(overviewList.total);
                    $('#k8s_online').text(overviewList.online);
                    $('#k8s_offline').text(overviewList.offline);
                    $('#k8s_protected').text(overviewList.protectedNum);
                    $('#backup_data_num').text(!overviewList.backupDataSize.value ? 0 : unitConver(Number(overviewList.backupDataSize.value)).size);
                    $('#backup_data_unit').text(!overviewList.backupDataSize.value ? 'B' : unitConver(Number(overviewList.backupDataSize.value)).unit);
                } else {
                    UIToastr.showWarning(LANG.UI_GET_K8S_REPORT_OVERVIEW_DATA_FAILED);
                }
            } catch (error) {
                UIToastr.showWarning(LANG.UI_GET_K8S_REPORT_OVERVIEW_DATA_FAILED);
            }
    ...
    const getReportDetail = () => {
        pAjaxRequest({}, '/api/v1/report/template/detail/' + K8S_REPORT_TEMPLATE_UUID, 'GET', (res) => {
            try {
                ...
                if (res.success) {
                    ...
                    // 渲染概览数据
                    if (!overview.k8s_total && !overview.k8s_online && !overview.k8s_offline && !overview.k8s_protected && !overview.backup_data) {
                        ...
                        overview.k8s_total ? $('.k8s-total-box').removeClass('display-none') : $('.k8s-total-box').hide();
                        overview.k8s_online ? $('.k8s-online-box').removeClass('display-none') : $('.k8s-online-box').hide();
                        overview.k8s_offline ? $('.k8s-offline-box').removeClass('display-none') : $('.k8s-offline-box').hide();
                        overview.k8s_protected ? $('.k8s-protected-box').removeClass('display-none') : $('.k8s-protected-box').hide();
                        overview.backup_data ? $('.backup-data-box').removeClass('display-none') : $('.backup-data-box').hide();
                    ...
                    if (running_tendency.history) {
                        ...
                        getK8sReportTaskRunningData(CURRENT_RUNNING_TENDENCY_QUERY_PARAMS, CURRENT_TASK_RUNNING_ECHART_ID);
                    } else {
                    ...
                    if (customField.task_status) fields.push('task_status');
                     ...
                     if (fields.length > 0) {
                        // 渲染数据明细表格过滤器
                        initk8sTableFilter(K8S_FILTER_OPTIONS);
                    }

                    // 定制数据选了开始时间和完成时间则渲染时间选择器
                    if (customField.start_time) {
                        initK8sTableDaterangePicker();
                    }

                    // 渲染数据明细表格
                   initK8sReportDetailsTable();
                    
                } else {
                    UIToastr.showWarning(LANG.UI_GET_K8S_REPORT_DATA_FAILED);
                }
            } catch (error) {
                UIToastr.showWarning(LANG.UI_GET_K8S_REPORT_DATA_FAILED);
            } finally {
     ...
     * @param {*} unit 
     */
    const initK8sReportEchart = (xData, yData, onlyData, echartId, unit) => {
     ...
     * @param {*} echartId 
     */
    const getK8sReportTaskRunningData = (params, echartId) => {
            ...
            try {
                if (res.success) {
                    ...
                    if (res.data.every(i => i.value === 0)) {
                        ...
                        initK8sReportEchart(xData, yData, onlyData, echartId, unit);
                    }
                } else {
     ...
     * @param {*} filterData 
     */
    const initk8sTableFilter = (filterData) => {
        if ($('#k8s_report_filter_btn').length === 0) {
            $('#k8s_report_filter_wrapper').initFilter({
                filterSlotId: 'k8s_report_filter_wrapper',
                filterBtnId: 'k8s_report_filter_btn',
                filters: filterData
            });
        } else {
            $('#k8s_report_filter_wrapper').resetFilter({
                filterBtnId: 'k8s_report_filter_btn',
                filters: filterData
            });
        }
     ...
     * 初始化数据明细时间选择器
     */
    const initK8sTableDaterangePicker = () => {
        $('#k8s_report_daterangepicker_wrapper').initDateRangePicker({
            slotId: 'k8s_report_daterangepicker_wrapper', // 日期范围组件在父组件插槽位置的id
            dateRangePickerId: 'k8s_datepicker', // 选择器button id
            startTime: '', // 开始时间
            endTime: '', // 结束时间
            maxDate: 'now', // 最大可用时间
    ...
    const initK8sReportDetailsTable = () => {
        if ($('#k8s_report_table').children().length === 0) {
            let options = {
                vin_url: "/api/v1/report/template/k8s_data_details",
                vin_method: "GET",
                ...
                dateRangePickerId: 'k8s_datepicker', // 日期选择器组件button id
                onPostBody: function() {
                    let tableData = $('#k8s_report_table').bootstrapTable('getData');
            ...
            $('#k8s_report_table').baseTableConfig().init(options);
        } else {
            $('#k8s_report_table').bootstrapTable('refresh');
        }
    }
    ...
    const reportOverviewExport = () => {
        $('#k8s_export').attr('data-bs-indicator', 'on');

        // 使用按钮防抖1.5秒执行一次导出
        debounce(captureScrollAndGeneratePDF($('.report-detail__content').get(0), 1500, 'kubernetes-report.pdf',  LANG.UI_VINCHIN_REPORT_WATER_NAME, function() {
            $('#k8s_export').removeAttr('data-bs-indicator');
        }), 1500);
    }
    ...
    const initListener = () => {
        ...
        $('#report_search').off().on('click', () => {
            ...
            if (FILTER_PARAMS.search) {
                $('#k8s_report_table').bootstrapTable('refresh', {query: { ...FILTER_PARAMS }})
            }
        });
        ...
        // 监听导出
        $('#k8s_export').on('click', reportOverviewExport);

        window.$off('k8s_report_filter_btn-updateFilterEvent');

        // 监听数据明细 - 过滤器组件派发的数据，以更新表格
        window.$on('k8s_report_filter_btn-updateFilterEvent', (filterData) => {
        ...
        window.$off('k8s_datepicker-updateDateRangeEvent');

        // 监听k8s表格 - 日期范围选择组件派发的数据，以更新表格
        window.$on('k8s_datepicker-updateDateRangeEvent', (data) => {
            // 记录选择的开始时间和结束时间，用于过滤搜索的联动
            startTime = data.startTime;
            endTime = data.endTime;

            $('#k8s_report_table').bootstrapTable('refresh', { query: { start_time: startTime, end_time: endTime } });
        });
    }
    ...
    const watchEchartSizeChange = () => {
        window.onresize = function() {
            switch (CURRENT_TASK_RUNNING_ECHART_ID) {
                case 'filecopy_report_all_chart':
                    // 有数据时，导航栏折叠/展开时，echart图要重绘
                    if (!$('#filecopy_report_all_chart').hasClass('display-none')) {
                        let chartWidth = $('#all_tab').width();
                        let chartHeight = $('#all_tab').height();
                        $('#filecopy_report_all_chart').css({'width': chartWidth + 'px', 'height': chartHeight + 'px'});

                        taskRunningChart.resize();
                    }
                    break;
                case 'filecopy_report_success_chart':
                    // 有数据时，导航栏折叠/展开时，echart图要重绘
                    if (!$('#filecopy_report_success_chart').hasClass('display-none')) {
                        let chartWidth = $('#success_tab').width();
                        let chartHeight = $('#success_tab').height();
                        $('#filecopy_report_success_chart').css({'width': chartWidth + 'px', 'height': chartHeight + 'px'});

                        taskRunningChart.resize();
                    }
                    break;
                case 'filecopy_report_failed_chart':
                    // 有数据时，导航栏折叠/展开时，echart图要重绘
                    if (!$('#filecopy_report_failed_chart').hasClass('display-none')) {
                        let chartWidth = $('#failed_tab').width();
                        let chartHeight = $('#failed_tab').height();
                        $('#filecopy_report_failed_chart').css({'width': chartWidth + 'px', 'height': chartHeight + 'px'});
        ...
        $(window).on('echart-resize', function() {
            ...
            setTimeout(() => {
                switch (CURRENT_TASK_RUNNING_ECHART_ID) {
                    case 'k8s_report_all_chart':
                        // 有数据时，导航栏折叠/展开时，echart图要重绘
                        if (!$('#k8s_report_all_chart').hasClass('display-none')) {
                            let chartWidth = $('#all_tab').width();
                            let chartHeight = $('#all_tab').height();
                            $('#k8s_report_all_chart').css({'width': chartWidth + 'px', 'height': chartHeight + 'px'});

                            taskRunningChart.resize();
                        }
                        break;
                    case 'k8s_report_success_chart':
                        // 有数据时，导航栏折叠/展开时，echart图要重绘
                        if (!$('#k8s_report_success_chart').hasClass('display-none')) {
                            let chartWidth = $('#success_tab').width();
                            let chartHeight = $('#success_tab').height();
                            $('#k8s_report_success_chart').css({'width': chartWidth + 'px', 'height': chartHeight + 'px'});

                            taskRunningChart.resize();
                        }
                        break;
                    case 'k8s_report_failed_chart':
                        // 有数据时，导航栏折叠/展开时，echart图要重绘
                        if (!$('#k8s_report_failed_chart').hasClass('display-none')) {
                            let chartWidth = $('#failed_tab').width();
                            let chartHeight = $('#failed_tab').height();
                            $('#k8s_report_failed_chart').css({'width': chartWidth + 'px', 'height': chartHeight + 'px'});
