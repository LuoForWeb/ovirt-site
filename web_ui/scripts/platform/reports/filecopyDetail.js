var FilecopyDetail = (function () {
    ...
    const STATUS_TYPE_TO_ECHART_ID_MAP = {
        1: 'filecopy_report_all_chart',
        2: 'filecopy_report_success_chart',
        3: 'filecopy_report_failed_chart'
    }; // 任务运行趋势状态对应echart id
    const FILECOPY_FILTER_OPTIONS = [
        {
    ...
    let FILECOPY_REPORT_TEMPLATE_UUID = '';
    let CURRENT_TASK_RUNNING_ECHART_ID = 'filecopy_report_all_chart';
    let CURRENT_RUNNING_TENDENCY_QUERY_PARAMS = {
        type: CONF.REPORT_TYPE.FILECOPY,
    ...
    const initRouteParams = () => {
        ...
        FILECOPY_REPORT_TEMPLATE_UUID = !!route.data.url.split('?')[1] ? uuidStr.split('=')[1] : ''; // 保存报表uuid

        if (FILECOPY_REPORT_TEMPLATE_UUID) {
            getReportDetail();
        }
    }

    /**
     * 获取文件复制报表概览数据 26是文件复制的module_type
     */
    const initOverviewData = () => {
        pAjaxRequest({}, '/api/v1/report/template/overview/26', 'get', function (res) {
            try {
                if (res.success) {
                    let overviewList = res.data.overviewList;

                    $('#source_object').text(overviewList.totalClient);
                    $('#target_object').text(overviewList.totalClient);
                    $('#protected_object').text(overviewList.protectedNum);
                    $('#task_numbers').text(overviewList.taskNum);
                    $('#copy_data_num').text(!overviewList.totalCopyData ? 0 : unitConver(Number(overviewList.totalCopyData)).size);
                    $('#copy_data_unit').text(!overviewList.totalCopyData ? 'B' : unitConver(Number(overviewList.totalCopyData)).unit);
                } else {
                    UIToastr.showWarning(LANG.UI_GET_FILE_COPY_REPORT_OVERVIEW_DATA_FAILED);
                }
            } catch (error) {
                UIToastr.showWarning(LANG.UI_GET_FILE_COPY_REPORT_OVERVIEW_DATA_FAILED);
            }
    ...
    const getReportDetail = () => {
        pAjaxRequest({}, '/api/v1/report/template/detail/' + FILECOPY_REPORT_TEMPLATE_UUID, 'GET', (res) => {
            try {
                ...
                if (res.success) {
                    ...
                    // 渲染概览数据
                    if (!overview.file_copy_object && !overview.file_copy_target && !overview.file_copy_protected && !overview.task_total && !overview.copy_data) {
                        ...
                        overview.file_copy_object ? $('.source-object-box').removeClass('display-none') : $('.source-object-box').hide();
                        overview.file_copy_target ? $('.target-object-box').removeClass('display-none') : $('.target-object-box').hide();
                        overview.file_copy_protected ? $('.protected-object-box').removeClass('display-none') : $('.protected-object-box').hide();
                        overview.task_total ? $('.task-numbers-box').removeClass('display-none') : $('.task-numbers-box').hide();
                        overview.copy_data ? $('.copy-data-box').removeClass('display-none') : $('.copy-data-box').hide();
                    ...
                    if (running_tendency.history) {
                        ...
                        getFilecopyReportTaskRunningData(CURRENT_RUNNING_TENDENCY_QUERY_PARAMS, CURRENT_TASK_RUNNING_ECHART_ID);
                    } else {
                    ...
                    if (customField.task_status) fields.push('task_status');
                     ...
                     if (fields.length > 0) {
                        // 渲染数据明细表格过滤器
                        initFilecopyTableFilter(FILECOPY_FILTER_OPTIONS);
                    }

                    // 定制数据选了开始时间和完成时间则渲染时间选择器
                    if (customField.start_time) {
                        initFileCopyTableDaterangePicker();
                    }

                    // 渲染数据明细表格
                   initFilecopyReportDetailsTable();
                } else {
                    UIToastr.showWarning(LANG.UI_GET_FILE_COPY_DATA_FAILED);
                }
            } catch (error) {
                UIToastr.showWarning(LANG.UI_GET_FILE_COPY_DATA_FAILED);
            } finally {
     ...
     * @param {*} unit 
     */
    const initFilecopyReportEchart = (xData, yData, onlyData, echartId, unit) => {
            ...
            tooltip: {
                ...
                formatter: function(params) {
                    ...
                    for (let i = 0, l = params.length; i < l; i++) {
                        ...
                        relVal += '<br/>' + ' <span style="display:inline-block;margin-right:5px;width:12px;height:12px;border-radius: 2px!important;background-color:#13C4B6;"></span>' + ' <span style="color: #2D3748!important">' + LANG.UI_COPY_DATA  + pvalue + '</span> ' ;
                    }
                    return relVal;
                }
            },
     ...
     */
    const getFilecopyReportTaskRunningData = (params, echartId) => {
            ...
            try {
                if (res.success) {
                    ...
                    if (res.data.every(i => i.value === 0)) {
                        ...
                        initFilecopyReportEchart(xData, yData, onlyData, echartId, unit);
                    }
                } else {
     ...
     * @param {*} filterData 
     */
    const initFilecopyTableFilter = (filterData) => {
        if ($('#filecopy_report_filter_btn').length === 0) {
            $('#filecopy_report_filter_wrapper').initFilter({
                filterSlotId: 'filecopy_report_filter_wrapper',
                filterBtnId: 'filecopy_report_filter_btn',
                filters: filterData
            });
        } else {
            $('#filecopy_report_filter_wrapper').resetFilter({
                filterBtnId: 'filecopy_report_filter_btn',
                filters: filterData
            });
        }
     ...
     * 初始化数据明细时间选择器
     */
    const initFileCopyTableDaterangePicker = () => {
        $('#filecopy_report_daterangepicker_wrapper').initDateRangePicker({
            slotId: 'filecopy_report_daterangepicker_wrapper', // 日期范围组件在父组件插槽位置的id
            dateRangePickerId: 'filecopy_datepicker', // 选择器button id
            startTime: '', // 开始时间
            endTime: '', // 结束时间
            maxDate: 'now', // 最大可用时间
    ...
    const initFilecopyReportDetailsTable = function () {
        if ($('#filecopy_report_table').children().length === 0) { // 表格为渲染
            let options = {
                vin_url: "/api/v1/report/template/filecopy_data_details",
                vin_method: "GET",
                ...
                onPostBody: function() {
                    let tableData = $('#filecopy_report_table').bootstrapTable('getData');
                ...
                columns: [
                    ...
                    },
                    {
                        ...
                        sortable: false,
                        align: 'center',
                        visible: customField.source_object
                    },
                    {
                        field: 'target_object',
                        ...
                        sortable: false,
                        align: 'center',
                        visible: customField.target_object
                    },
                    {
                        field: 'start_time',
            ...
            $('#filecopy_report_table').baseTableConfig().init(options);
        } else { // 表格已渲染直接刷新
            $('#filecopy_report_table').bootstrapTable('refresh');
        }
    };
    ...
    const reportOverviewExport = () => {
        $('#filecopy_export').attr('data-bs-indicator', 'on');

        // 使用按钮防抖1.5秒执行一次导出
        debounce(captureScrollAndGeneratePDF($('.report-detail__content').get(0), 1500, 'filecopy-report.pdf',  LANG.UI_VINCHIN_REPORT_WATER_NAME, function() {
            $('#filecopy_export').removeAttr('data-bs-indicator');
        }), 1500);
    }
    ...
    const initListener = () => {
        ...
        $('#report_search').off().on('click', () => {
            ...
            if (FILTER_PARAMS.search) {
                $('#filecopy_report_table').bootstrapTable('refresh', {query: { ...FILTER_PARAMS }})
            }
        });
        ...
        // 监听导出
        $('#filecopy_export').on('click', reportOverviewExport);

        window.$off('filecopy_report_filter_btn-updateFilterEvent');

        // 监听数据明细 - 过滤器组件派发的数据，以更新表格
        window.$on('filecopy_report_filter_btn-updateFilterEvent', (filterData) => {
        ...
        window.$off('filecopy_datepicker-updateDateRangeEvent');

        // 监听文件复制表格 - 日期范围选择组件派发的数据，以更新表格
        window.$on('filecopy_datepicker-updateDateRangeEvent', (data) => {
