var ObsDetail = (function() {
    const VENDOR_TYPE_DES = {
        ...
        8: LANG.UI_OBS_VENDOR_OTHER
    };
    let OBS_REPORT_TEMPLATE_UUID = '';
    let CURRENT_RUNNING_TENDENCY_QUERY_PARAMS = {
        type: CONF.REPORT_TYPE.OBS,
        timeInterval: CONF.RUNNING_TIME_TYPE.LAST_MONTH,
    }; // 当前任务运行趋势查询参数对象
    let customField = {};
    let normalTag = '<span class="label label-success">' + LANG.UI_JOB_CROWD_NORMAL + '</span>';
    let unnormalTag = '<span class="label label-default">' + LANG.UI_CLOUD_PLATFORM_OFFLINE + '</span>';
    let filterParams = { search: '' };
    const OBS_REPORT_TABLE_FILTER_OPTIONS = [
        {
            label: LANG.UI_PUBLIC_STATUS,
            field: 'status',
            value: [
                {
                    id: 'obs_status_online',
                    value: CONF.OBS_STATUS_TYPE.ONLINE,
                    text: LANG.UI_CLOUD_PLATFORM_ONLINE,
                    tag: true,
                    type: 'success'
                },
                {
                    id: 'obs_status_offline',
                    value: CONF.OBS_STATUS_TYPE.OFFLINE,
    ...
    const initOverviewData = () => {
        pAjaxRequest({}, '/api/v1/report/template/overview/34', 'get', function (res) {
            try {
                if (res.success) {
                    let overviewList = res.data.overviewList;

                    $('#obs_total_num').text(overviewList.obs_total);
                    $('#obs_normal_num').text(overviewList.obs_normal);
                    $('#obs_offline_num').text(overviewList.obs_offline);
                } else {
                    UIToastr.showWarning(LANG.UI_GET_OBS_REPORT_OVERVIEW_DATA_ERROR);
                }
            } catch (error) {
                UIToastr.showWarning(LANG.UI_GET_OBS_REPORT_OVERVIEW_DATA_ERROR);
            }
    ...
    const getReportDetail = () => {
        pAjaxRequest({}, '/api/v1/report/template/detail/' + OBS_REPORT_TEMPLATE_UUID, 'get', (res) => {
            try {
                ...
                if (res.success) {
                    ...
                    // 渲染概览数据
                    if (!overview.ob_total && !overview.ob_normal && !overview.ob_offline) {
                        ...
                        overview.ob_total ? $('.obs-total-box').removeClass('display-none') : $('.obs-total-box').hide();
                        overview.ob_normal ? $('.obs-normal-box').removeClass('display-none') : $('.obs-normal-box').hide();
                        overview.ob_offline ?  $('.obs-offline-box').removeClass('display-none') : $('.obs-offline-box').hide();
                    ...
                    if (running_tendency.history) {
                        ...
                        getObsReportTaskRunningData(CURRENT_RUNNING_TENDENCY_QUERY_PARAMS);
                    } else {
                    ...
                    if (fields.length > 0) {
                        let filterData = OBS_REPORT_TABLE_FILTER_OPTIONS.filter(option => {
                            return fields.includes(option.field);
                        });
        
                        // 渲染数据明细表格过滤器
                        initObsTableFilter(filterData);
                    }

                    // 渲染数据明细表格
                    initObsReportDetailsTable();

                } else {
                    UIToastr.showWarning(LANG.UI_GET_OBS_REPORT_DATA_ERROR);
                }
            } catch (error) {
                UIToastr.showWarning(LANG.UI_GET_OBS_REPORT_DATA_ERROR);
            } finally {
    ...
    const initObsReportEchart = (xData, yData, onlyData, echartId, unit) => {
    ...
    const getObsReportTaskRunningData = (params) => {
            ...
            try {
                if (res.success) {
                    ...
                    if (res.data.every(i => i.value === 0)) {
                        $(`#obs_task_running_chart`).addClass('display-none');
                        $(`#obs_task_running_chart`).siblings().removeClass('display-none');
                    } else {
                        ...
                        $(`#obs_task_running_chart`).removeClass('display-none');
                        $(`#obs_task_running_chart`).siblings().addClass('display-none');

                        initObsReportEchart(xData, yData, onlyData, 'obs_task_running_chart', unit);
                    }
                } else {
    ...
    const watchEchartSizeChange = () => {
        window.onresize = function() {
            ...
            $('#obs_task_running_chart').css({'width': chartWidth + 'px', 'height': chartHeight + 'px'});
            taskRunningChart.resize();
        }
     ...
     * @param {*} filterData 
     */
    const initObsTableFilter = (filterData) => {
        if ($('#nas_report_filter_btn').length === 0) {
            $('#obs_report_filter_wrapper').initFilter({
                filterSlotId: 'obs_report_filter_wrapper',
                filterBtnId: 'obs_report_filter_btn',
                filters: filterData
            });
        } else {
            $('#obs_report_filter_wrapper').resetFilter({
                filterBtnId: 'obs_report_filter_btn',
                filters: filterData
            });
        }
    ...
    const initObsReportDetailsTable = () => {
        if ($('#obs_report_table').children().length === 0) { // 表格为渲染
            let options = {
                vin_url: "/api/v1/report/template/ob_data_details",
                vin_method: "GET",
                ...
                showColumns: false,
                onPostBody: function() {
                    let tableData = $('#obs_report_table').bootstrapTable('getData');
                ...
                columns: [
                    {
                        field: 'vendor',
                        title: LANG.UI_STORAGE_CLOUD_VENDOR,
                        sortable: true,
                        align: 'center',
                        visible: customField.vendor,
                        formatter: (val) => {
                    ...
                    },
                    {
                        ...
                        sortable: false,
                        visible: customField.endpoint_override,
                        align: 'center'
                    },
                    {
                        field: 'nickname',
                    ...
                    },
                    {
                        ...
                        sortable: true,
                        visible: customField.obs_create_time,
                        align: 'center',
                    },
                    {
                        field: 'status',
                    ...
                    },
                    {
                        ...
                        sortable: false,
                        align: 'center',
                        visible: customField.owner,
                    },
                ],
            }
            $('#obs_report_table').baseTableConfig().init(options);
        } else { // 表格已渲染直接刷新
            $('#obs_report_table').bootstrapTable('refresh');
        }
    }
    ...
    const reportOverviewExport = () => {
        $('#obs_export').attr('data-bs-indicator', 'on');

        // 使用按钮防抖1.5秒执行一次导出
        debounce(captureScrollAndGeneratePDF($('.report-detail__content').get(0), 1500, 'obs-report.pdf',  LANG.UI_VINCHIN_REPORT_WATER_NAME, function() {
            $('#obs_export').removeAttr('data-bs-indicator');
        }), 1500);
    }
    ...
    const initRouteParams = () => {
        ...
        OBS_REPORT_TEMPLATE_UUID = !!route.data.url.split('?')[1] ? uuidStr.split('=')[1] : ''; // 保存报表uuid

        if (OBS_REPORT_TEMPLATE_UUID) {
            getReportDetail();
        }
    }
    ...
    const initListener = () => {
        ...
        $('#report_search').off().on('click', () => {
            ...
            if (searchVal) {
                $('#obs_report_table').bootstrapTable('refresh', {query: {...filterParams}})
            }
        });
        ...
        // 监听导出
        $('#obs_export').on('click', reportOverviewExport);

        window.$off('obs_report_filter_btn-updateFilterEvent');

        // 监听数据明细 - 过滤器组件派发的数据，以更新表格
        window.$on('obs_report_filter_btn-updateFilterEvent', (filterData) => {
            if (filterData.length > 0) {
                
                let obsStatusList = getTableFilterParams('status', filterData);

                if (obsStatusList.value.length > 0) {
                    filterParams.status = obsStatusList.value;
                } else {
                    filterParams.status = '';
                }
            } else {
