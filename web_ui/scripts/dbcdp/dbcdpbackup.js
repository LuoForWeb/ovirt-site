var Dbcdpbackup = function () {
    ...
    // 扫描目标用户
    let targetScanUserFlag = true;
    // 扫描目标用户错误信息
    let targetScanUserTip = '';
    //----------------STEP2----------------
    // 源映射树
    ...
    let net_model = '';
    let default_source_file_cache_path = '';
    ...
    const initListeners = () => {
        ...
        // 自定义源机文件缓存路径
        $('#source_customFileCachePath').off().on('click', clicksourceCustomFileCachePath);
        // 自定义备机文件缓存路径
        $('#backup_customFileCachePath').off().on('click', clickbackupCustomFileCachePath);

        // 默认源机文件缓存路径
        $('#source_defaultFileCachePath').off().on('click', clicksourcefaultFileCachePath);
        // 默认备机文件缓存路径
        $('#backup_defaultFileCachePath').off().on('click', clickbackupfaultFileCachePath);
    ...
    const initSpinner = function () {
        ...
        $('.numberFailuresDiv').spinner({value: 3, step: 3, min: 1, max: 100});
    ...
    const clicksourcefaultFileCachePath = () => {
        ...
        $('#source_fileCachePath').attr("readonly", "readonly");
        $('#source_fileCachePath').val(default_source_file_cache_path);
    }
    ...
    const initInstanceTree = function () {
        ...
        pAjaxRequest(param, '/api/v1/db/instances', 'GET', res => {
            setOracleBackupTree(res.data.rows,'#agent_tree',checkOracleBackupNode);
        });
    };
    ...
    const setOracleBackupTree = (instanceList,id,checkedFn) => {
        ...
        for (const standaloneNode of standaloneNodes) {
            ...
            }else if(standaloneNode.db_cdp_backup_info.length>0){
    ...
    const getBackupTreeSetting = (checkedFn) => {
        return {
            ...
            view: {
                ...
                nameIsHTML: true,  // 必须设置为true才能解析HTML标签
            }
        };
    };
    ...
    const checkOracleBackupNode = (ev, treeId, treeNode) => {
        // 单节点
        if(!treeNode.cluster_flag && treeNode.db_cdp_backup_info.length > 0){
            treeNode.checked = false;
            let message = treeNode.title + LANG.UI_DB_CDP_HOST_DATABASE_IN + treeNode.db_cdp_backup_info[0].job_name + LANG.UI_DB_CDP_TASK_PROTECTING;
            UIToastr.showWarning(LANG.UI_DB_BACKUP_CREATE_TITLE, message);
            return;
        }
        // 集群
        if(treeNode.cluster_flag){
            let childrenNode = treeNode.children;
            for(let i=0;i<childrenNode.length;i++){
                if(childrenNode[i].db_cdp_backup_info.length > 0){
                    treeNode.checked = false;
                    let message = childrenNode[i].title + LANG.UI_DB_CDP_HOST_DATABASE_IN + childrenNode[i].db_cdp_backup_info[0].job_name + LANG.UI_DB_CDP_TASK_PROTECTING;
                    UIToastr.showWarning(LANG.UI_DB_BACKUP_CREATE_TITLE, message);
                    return;
                }
            }
    ...
    /**
     * 选中节点及其所有后代节点
     * @param {Object} zTreeObj - zTree实例对象
     * @param {Object} node - 当前节点
     */
    function checkNodeAndDescendants(zTreeObj, node, isCheckable) {
        if (isCheckable) {
            // 选中当前节点及后代
            zTreeObj.checkNode(node, true, false); // 选中当前节点（不触发事件）
            node.nocheck = false;
        } else {
            zTreeObj.checkNode(node, false, false); // 取消选中当前节点
            node.nocheck = true;
        }
        ...
        if (node.children) {
            node.children.forEach(child => {
                checkNodeAndDescendants(zTreeObj, child, isCheckable);
            });
        }
        // 刷新节点样式（确保置灰/选中状态立即生效）
        ...
        return {
            ...
            callback: {
                ...
                onCheck:function (event, treeId, treeNode){
                    ...
                    // 检查当前操作的是否是根节点，并且是取消勾选操作
                    if (treeNode.checked === false && treeNode.level === 0) {
                        // 阻止取消勾选，强制重新勾选
    ...
    const expandInstanceUserTreeNode = (treeId, treeNode) => {
        ...
        pAjaxRequest(param, `/api/v1/dbcdp/databases`, 'POST', res => {
            ...
            if (!res.success) {
                targetScanUserFlag = false;
                targetScanUserTip = res.message;
            ...
            targetScanUserFlag = true;
            let nodes = [];
            let rows = res.data.rows;
            const allEmpty = rows.every(row => row.user_list.length === 0);
            ...
            // 全选所有节点
            if(treeNode.type == 'sourceNode'){
                var zTreeObj = $.fn.zTree.getZTreeObj(treeId);
                // 获取所有根节点（若需要处理所有节点的children，可先获取全部节点）
                var allNodes = zTreeObj.getNodes(); // 获取根节点数组
                // 遍历所有节点，处理它们的children
                allNodes.forEach(node => {
                    // 若当前节点有子节点，遍历子节点
                    if (node.children) {
                        node.children.forEach(child => {
                            if (child.open_mode  === 1) {
                                checkNodeAndDescendants(zTreeObj, child,true);
                            }else {
                                checkNodeAndDescendants(zTreeObj, child, false);
                            }
                        });
                    }
                });
            }else if(treeNode.type == 'targetNode'){
    ...
    const expandOracleSelectedBackupNode = (treeId, treeNode) => {
        ...
        pAjaxRequest(param, `/api/v1/dbcdp/databases`, 'POST', res => {
            ...
            default_source_file_cache_path = res.data.agent_install_dir;
            ip_scope_info = res.data.ip_scope_info;
            $('#source_fileCachePath').val(default_source_file_cache_path);
            $('#source_fileCachePath').attr('title',default_source_file_cache_path);
            let rows = res.data.rows;
            const allEmpty = rows.every(row => row.user_list.length === 0);
            if(allEmpty){
                sourceScanUserFlag = false;
    ...
    const initStandBySelect =  () => {
        ...
        pAjaxRequest(param, '/api/v1/db/instances', 'GET', res => {
            ...
            for(row of rows){
                ...
                if(!clusterFlag){
                    if(row.db_cdp_backup_info.length == 0 && row.agent_info.online_flag && row.agent_info.agent_uuid != data.sync_object.source_agent_uuid){
                        // 无任务、在线、不是源机
                        backupAgentArr.push(row);
                    }
                }else{
                    if(row.db_cdp_backup_info.length == 0 && row.cluster_info.cluster_uuid != data.sync_object.source_cluster_uuid){
                        // 集群(不能是和源相同的集群且无任务)
                        backupAgentArr.push(row);
                    }
                }
    ...
    const contrastApp =  (sourceApp, targetApp) => {
        ...
        if (sourceApp.app_type !== targetApp.app_type) {
            appValidFlag = false;
            UIToastr.showWarning(LANG.UI_VISUAL_DBCDP_SYNC, LANG.UI_DB_CDP_BACKUP_SOURCE_APP_SELECT_TIP1);
            return false;
        } else {
            // 提取版本号中的数字部分
            ...
            for (var i = 0; i < Math.max(v1Parts.length, v2Parts.length); i++) {
                ...
                if (v1Num > v2Num) {
                    UIToastr.showWarning(LANG.UI_VISUAL_DBCDP_SYNC, LANG.UI_DB_CDP_BACKUP_APP_SELECT_TIPS+'<br>'+LANG.UI_DB_CDP_BACKUP_SOURCE_VERSION+':'+sourceApp.app_version+'<br>'+LANG.UI_DB_CDP_BACKUP_END_VERSION+':'+targetApp.app_version);
                    appValidFlag = false;
                    return false;
                } else if (v1Num < v2Num) {
                    appValidFlag = true;
    ...
    const step1Valid = () => {
        ...
        if(!sourceScanUserFlag){
            UIToastr.showWarning(LANG.UI_DB_CDP_SCAN_APP_INFO,sourceScanUserTip);
            return false;
        }
        ...
        if(targetArr.length ==0 ){
            UIToastr.showWarning(LANG.UI_VISUAL_DBCDP_SYNC, LANG.UI_DB_CDP_BACKUP_NO_BACKUP_MACHINES_AVAILABLE);
            return false;
        }
     ...
     */
    const step2Valid = () => {
        if(!targetScanUserFlag){
            UIToastr.showWarning(LANG.UI_DB_CDP_SCAN_APP_INFO,targetScanUserTip);
            return false;
        }
        // 验证备机
        ...
        if(data.auto_takeover_flag){
            // 自动接管
            let failbackTargetIp = $('#failbackTargetIp').val();
            ...
            if(serviceIpDriftFlag && !data.sync_object.source_cluster_flag){
                data.takeover_object.failback_target_ip = failbackTargetIp;
            }

            // 验证回切通讯ip
            if (isIpShow && (failbackTargetIp === '' || !ipCheckFlag)) {
    ...
    const preStep4 = () => {
        ...
        if(autoTakeoverFlag){
            // 服务IP漂移
            let serviceIpShiftStr = isIpShow ? LANG.UI_PUBLIC_ON : LANG.UI_PUBLIC_OFF;
            if(isIpShow){
        ...
        // 延迟加载时间
        let times =  $('.days').val() + LANG.UI_PUBLIC_UNIT_DAY + $('.hours').val() + LANG.UI_DB_CDP_HOURS + $('.minutes').val() + LANG.UI_PUBLIC_MINUTE + $('.seconds').val() + LANG.UI_PUBLIC_SECOND;
        if(CONF.LANGUAGE != "zh-cn" && CONF.LANGUAGE != "zh-tw"){
            //英文版需要加上空格
            times =  $('.days').val() + LANG.UI_PUBLIC_UNIT_DAY + ' ' + $('.hours').val() + LANG.UI_DB_CDP_HOURS + ' ' + $('.minutes').val() + LANG.UI_PUBLIC_MINUTE + ' ' + $('.seconds').val() + LANG.UI_PUBLIC_SECOND;
        }
        ...
        // 忽略节点资源限制
        let node_resource_limit_ignore_str = $('')
    ...
    const getTaskName = () => {
        // 默认任务名
        let param = {'job_name': LANG.UI_DB_CDP_COPY_TASK};
    ...
    const submit = () => {
        ...
        pAjaxRequest(data, '/api/v1/dbcdp/jobs/sync', 'POST', function (d) {
            Metronic.unblockUI('#dbcdpbackupcontent');
