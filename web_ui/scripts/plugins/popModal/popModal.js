...
(function($) {
	$.fn.popModal = function(method) {
				...
				if (elem.next('div').hasClass(elemClass)) {
					...
					if ($.isFunction(_options.html)) {
						...
						if ($.type(_options.html) == 'object') {
							_options.html.css('display','block');
					...
					elemObj = $('.' + elemClass);
					//添加css样式
					elemObj.css('max-width',_options.maxWidth);
					elemObj.css('max-height',_options.maxHeight);
					...
					elemObj.find('.close').unbind().bind('click', function() {
						popModalClose();
					});
					
					elemObj.find('[data-popModalBut="close"]').unbind().bind('click', function() {
						popModalClose();
					});

					elemObj.find('[data-popModalBut="ok"]').unbind().bind('click', function(event) {
						var ok;
					...
					elemObj.find('[data-popModalBut="cancel"]').unbind().bind('click', function() {
		...
		function popModalClose() {
			...
			_options.html.css('display','none');
			reverseEffect();
			getAnimTime();
		}
	...
	$('* [data-popModalBind]').unbind().bind('click', function() {
...
(function($) {
	...
	$('* [data-notifyModalBind]').unbind().bind('click', function() {
...
(function($) {
		...
		var methods = {
			init : function(params) {
				...
				function bindFooterButtons() {
					elemObj.find('[data-dialogModalBut="close"]').unbind().bind('click', function() {
						dialogModalClose();
					});

					elemObj.find('[data-dialogModalBut="ok"]').unbind().bind('click', function(event) {
						var ok;
						if (_options.onOkBut && $.isFunction(_options.onOkBut)) {
							ok = _options.onOkBut(event);
						}
					...
					elemObj.find('[data-dialogModalBut="cancel"]').unbind().bind('click', function() {
						if (_options.onCancelBut && $.isFunction(_options.onCancelBut)) {
							_options.onCancelBut();
						}
						dialogModalClose();
				...
				elemObj.find('.' + prevBut).unbind().bind('click', function() {
					if (currentDialog > 0) {
						--currentDialog;
				...
				elemObj.find('.' + nextBut).unbind().bind('click', function() {
					if (currentDialog < maxDialog) {
						++currentDialog;
						if (currentDialog > 0) {
	...
	$('* [data-dialogModalBind]').unbind().bind('click', function() {
