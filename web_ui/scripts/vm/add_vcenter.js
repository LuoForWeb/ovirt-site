var AddVcenter = function(){
	var authFun = [];
	var applianceuuid = "";
	var cloudStorageUuids = []; //存放云存储uuid
...
//                    error2.hide();
          ...
          //测试连接
    		$('#testConnect').on('click', function(){
				...
				var password = btoa($('#epassword').val());
    ...
    var vmTypeChangeTips = function(type){
    	...
    	switch(type){
    		case 1:
    		case 21:
    			//VMware vSphere、Cloudview SVM
    			usernamePlaceholder.prop('placeholder', "root / administrator@vsphere.local");
    			break;
    		case 3:
    		case 8:
    		case 9:
    		...
    		case 25:
    		case 28:
    			//Citrix XenServer、Inspur InCloud Sphere、Haisige vGate、WinHong Cnware、D-Server、V-Server
    			usernamePlaceholder.prop('placeholder', "root");
    			break;
    		case 10:
			case 41:
    			//Neokylin
    			usernamePlaceholder.prop('placeholder', "admin@internal");
    			break;
			case 24:
			case 40:
			case 57:
			...
			case 50:
			case 52:
				//RedHat RHV/Ovirt  Oracle Linux Virtualization Manager(OLVM)
				usernamePlaceholder.prop('placeholder', "admin@internal/admin@ovirt@internalsso");
				break;
    		case 11:
    		case 51:
    			//H3C CAS
    		...
    		case 12:
    		case 47:
    			//SANGFOR HCI、SANGFOR VVDK
    			usernamePlaceholder.prop('placeholder', "admin");
				if (CONF.VM_TYPE.SANGFORVVDK == type) {
					usernamePlaceholder.prop('placeholder', "");
					$('input[name=ipaddr]').prop('placeholder', "192.168.1.110:4430");
				}
    			break;
    		case 16:
    			//Huawei FusionSphere Kvm
    			$('input[name=ipaddr]').prop('placeholder', "192.168.1.110:7443");
    			usernamePlaceholder.prop('placeholder', "gesysman");
    			break
    		case 17:
    		...
    		case 2:
    			//Hyper-v
    			$('#hypervType').val(1);
    			usernamePlaceholder.prop('placeholder', "domain\\administrator");
    			break;
    		case 26:
    		case 56:
    		case 59:
    		...
    		case 48:
    			...
    			break
			case 42:
				$('input[name=ipaddr]').prop('placeholder', "192.168.1.110:8006");
				break
			case 43:
				$('input[name=ipaddr]').prop('placeholder', "192.168.1.110:7443");
    			break;
    ...
    $('#hypervType').on('change', function(){
    	...
    	switch(select){
			...
			case 2:
				//Hyper-V Manager
				usernamePlaceholder.prop('placeholder', "administrator");
				break;
		}
    })
    ...
    $('#vmtype').on('change', function(){
		...
		if (CONF.VM_TYPE.INSPURVVDK == select
			|| CONF.VM_TYPE.SMARTX == select
			|| CONF.VM_TYPE.SANGFORVVDK == select
			|| CONF.VM_TYPE.VOLC == select
			|| CONF.VM_TYPE.KSPHERE == select
			|| CONF.VM_TYPE.ARCFRA == select
		) {
			$('.ip-port').hide();
			$('#domainDiv').hide();
			$('.applianceDiv').show();
    	...
    	if("" == select || "1" == select || "21" == select || "17" == select || "33" == select || "44" == select
		|| CONF.VM_TYPE.SANGFORVVDK == select
		|| CONF.VM_TYPE.XFUSIONKVM == select
		|| CONF.VM_TYPE.H3CCASCVD == select
		|| CONF.VM_TYPE.VOLC == select
		|| CONF.VM_TYPE.KSPHERE == select
		|| CONF.VM_TYPE.ARCFRA == select
		|| CONF.VM_TYPE.LENOVOAIO == select
		){
    		$('#agentdownload').hide();
    		$('#agentInit').hide();
    		$('#vmtypediv').addClass('pt70');
    	}else{
    		var selectText = $(this).find('option:selected').text();
    		var html = '<i class="fa fa-warning "></i> ';
    		html += '<strong>' + selectText + '</strong>';
    		if(select == "32"){
				html += LANG.UI_VCENTER_ADD_HYPERVISOR_WIN_TIPS ;
			} else if (CONF.VM_TYPE.INSPURVVDK == select) {
				html += LANG.UI_VCENTER_ADD_HYPERVISOR_ICSVVDK_TIPS;
			}else if (CONF.VM_TYPE.FUSIONKVM == select) {
				html += LANG.UI_VCENTER_ADD_HYPERVISOR_FUSIONKVM_TIPS;
			}else{
				html += LANG.UI_VCENTER_ADD_HYPERVISOR_TIPS;
			}
    		$('#agentdownloadtips').html(html);
    		$('#agentdownload').show();
    		$('#vmtypediv').removeClass('pt70');
    		if (CONF.VM_TYPE.INSPURVVDK == select) {
				$('#agentInit').hide();
			} else {
				$('#agentInit').show();
    	...
    	if(CONF.VMTYPE_GROUP.REDHAT.includes(select)){
    		$('.highsafeCheckDiv').hide();
    		$('.highsafeDiv').hide();
		...
		if (select == "33") {
			...
			$('.arcfratips').hide();
			$('.iptips').hide();
		} else if (select == CONF.VM_TYPE.ARCFRA) {
			$('.loginDiv').hide();
			$('.smartxtips').hide();
			$('.arcfratips').show();
			$('.iptips').hide();
    	...
    	//ICS/Inspur VVDK
    	if(select == "24" || CONF.VM_TYPE.INSPURVVDK == select || CONF.VM_TYPE.KSPHERE == select){
			$('.username').find('.required').hide();
			$('.password').find('.required').hide();
    	}else{
    		$('.icsDiv').hide();
			$('.username').find('.required').show();
			$('.password').find('.required').show();
    	}

		// sangfor vddk
		if (CONF.VM_TYPE.SANGFORVVDK == select) {
			$('.sangforvvdkLoginDiv').show();
			$('#sangforvvdkLoginType').val('ak/sk');
			$('.username-label').hide();
			...
			$('#sangforvvdkLoginType').val('token');
			$('.username-label').show();
		...
		//ics/ics vvdk
		if (CONF.VM_TYPE.INCLOUDKVM == select || CONF.VM_TYPE.INSPURVVDK == select || CONF.VM_TYPE.KSPHERE == select) {
			$('.icsLoginDiv').show();
		} else {
			// 重置
			$('.icsLoginDiv').hide();
    	...
    	vmTypeChangeTips(parseInt(select));
		...
		if (CONF.VMTYPE_GROUP.REDHAT.includes(select)) {
			...
			let storageOptions = $('#selectstorage option');
			$.each(storageOptions, function (i, v) {
				if (-1 != $.inArray(v.value, cloudStorageUuids)) {
					storageOptions.eq(i).remove();
				}
			});
		}

		// h3c UIS/CAS
		if (CONF.VM_TYPE.H3C == select || CONF.VM_TYPE.H3CCASCVD == select) {
			$('#httpTypeDiv').show();
		...
		// huawei kvm
		if (CONF.VM_TYPE.FUSIONKVM == select || CONF.VM_TYPE.XFUSIONKVM == select) {
    ...
    var initAgentVersion = function(){
		...
		if(name == 'FusionOne Compute' || name == 'Huawei FusionCompute KVM'){
			$('.version_pick_wrapper').hide();
		}else{
			$('.version_pick_wrapper').show();
		}
    }
    ...
    var submit = function(){
    	...
    	Metronic.blockUI({target: '#addcontent',animate: true});
		pAjaxRequest(data, "/api/v1/vm/platforms", "POST", function (d) {
			registerResult(d);
		}, true);
    };
    ...
    var registerResult = function(data){
		Metronic.unblockUI('#addcontent');
		if (data.success) {
			UIToastr.showSuccess(LANG.UI_VCENTER_ADD, LANG.UI_VCENTER_ADD_SUCCESS_TIPS);
			DataBackupCenter.updateTopAlarmTips();
			//如果是深信服,同步虚拟化中心
			let hypervisor = parseInt($('#vmtype').val());
			if (hypervisor == 12 || hypervisor == 47) {
				doSyncVcenter();
			}
			href2VcenterManager();
		} else {
			UIToastr.showWarning(LANG.UI_VCENTER_ADD, data.message);
		}
    }
    ...
    var doSyncVcenter = function(id){
		var data = {};
		data.ip = $('input[name=ipaddr]').val();
		pAjaxRequest(data, "/api/v1/vm/platforms/synchronization", "POST", function (data) {
			if (data.success) {
				UIToastr.showSuccess(LANG.UI_VCENTER_SYNC_TIPS, LANG.UI_VCENTER_SYNC_SUCCESS_TIPS);
			} else {
				UIToastr.showWarning(LANG.UI_VCENTER_SYNC_TIPS, LANG.UI_VCENTER_SYNC_FAILED_TIPS);
			}
		}, true);
	}
	...
	$.validator.addMethod("valueRequire", function (value, ele) {
		...
		if (CONF.VM_TYPE.INCLOUDKVM == hypervisor || CONF.VM_TYPE.INSPURVVDK == hypervisor || CONF.VM_TYPE.INSPURVVDK == hypervisor) {
			return true;
		}
		return '' != $.trim(value);
		...
		pAjaxRequest(data, "/api/v1/storages/backup", "GET", function (d) {
			...
			for(var i=0; i<data.length; i++){
				if (CONF.VMTYPE_GROUP.REDHAT.includes(parseInt($('#vmtype').val())) && CONF.BD_STORAGE_TYPE.CLOUD == data[i].storage_type) {
					//ovirt自动平台备份暂时不支持云存储，屏蔽
					cloudStorageUuids.push(data[i].storage_uuid);
					continue;
				}
				if (CONF.BD_STORAGE_TYPE.CLOUD == data[i].storage_type) {
					cloudStorageUuids.push(data[i].storage_uuid);
				}
