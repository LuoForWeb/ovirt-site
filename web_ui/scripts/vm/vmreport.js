var VmReport = function () {
	...
	var addMultToBackupTask = function () {
		...
		if (selectedRow.length == 0) {
			return UIToastr.showInfo(LANG.UI_VCENTER_VM_ADD_TO_TASK, LANG.UI_VM_REPORT_NO_ADD_TO_JOB_VM_TIPS);
		}
		var vcenteruuid;
		var vmshowtype;
		var vmnamelist = [];
		...
		for (var i = 0; i < selectedRow.length; i++) {
			if (vcenteruuid && selectedRow[i].details.platform_uuid != vcenteruuid) {
				return UIToastr.showWarning(LANG.UI_VCENTER_VM_ADD_TO_TASK, LANG.UI_VM_REPORT_NOT_SAME_VCENTER_VM_TIPS);
			}
	...
	var createNewBackupTask = function (row) {
		...
		if (2 == sub_module_type) {
			...
			LOCATION(url, 'prcloud_backup');
		} else {
			url = './content/vm/vmbackup.php?showtype=' + vmshowtype + '&vmuuid=' + vmuuid + '&vcenteruuid=' + vcenteruuid + '&hypervisor=' + hypervisor;
			LOCATION(url, 'vmbackup');
		}
	}
	...
	var createMultNewBackupTask = function(){
		var selectedRow = grid.bootstrapTable('getSelections');
		if (selectedRow.length == 0) {
			return UIToastr.showInfo(LANG.UI_VM_REPORT_CREATE_NEW_TASK, LANG.UI_VM_REPORT_NO_CREATE_JOB_VM_TIPS);
		}
		var vmuuid = [];
		var vcenteruuid;
		var vmshowtype;
		...
		for (var i = 0; i < selectedRow.length; i++) {
			if (vcenteruuid && selectedRow[i].details.platform_uuid != vcenteruuid) {
				return UIToastr.showWarning(LANG.UI_VM_REPORT_CREATE_NEW_TASK, LANG.UI_VM_REPORT_NOT_SAME_VCENTER_VM_TIPS);
			}
			if (!vcenteruuid) {
				vcenteruuid = selectedRow[i].details.platform_uuid;
				vmshowtype = selectedRow[i].details.display_mode;
	...
	var handleRecords = function () {
		...
		let options = {
			...
			exportOptions: {
				fileName: LANG.UI_VM_REPORT_EXPORT_FILE_NAME,
	...
	var addListeners = function(){
		...
		$('#search').on('blur', function () {
			$(this).prop('placeholder', LANG.UI_VM_REPORT_SEARCH_TIPS);
		});
		...
		$('#serach_submit').on('click', function(){
			...
			p.vmCluster = $('#searchmodal #vmCluster').val();
			p.vcenteruuid = $('#searchmodal #vcenterSelect').val();
			p.hostName = $('#searchmodal #hostName').val();
			//添加搜索条件显示
			addSearchContent(p);
	...
	var addSearchContent = function(p){
		...
		if(p.vmName){
			info += '<span id="vmName" style="position: relative"><span id="vmNameDetail" style="display: none;position: absolute;bottom: -40px;left: 10px;background: white"> ' + vmName + '</span> '+LANG.UI_JOB_VM_NAME+': <i>' + vmName + '</i><em>X</em></span>';		}
		if(p.vmIP){
			info += '<span id="vmIP" style="position: relative"><span id="vmIPDetail" style="display: none;position: absolute;bottom: -40px;left: 10px;background: white"> ' + vmIP + '</span> '+LANG.UI_SEARCH_VM_IP+': <i>' + vmIP + '</i><em>X</em></span>';		}
		if(p.hostName){
			info += '<span id="hostName" style="position: relative"><span id="hostNameDetail" style="display: none;position: absolute;bottom: -40px;left: 10px;background: white"> ' + hostName + '</span> '+LANG.UI_SEARCH_HOST_TENANT+': <i>' + hostName + '</i><em>X</em></span>';		}
		if(p.vmCluster){
			info += '<span id="vmCluster" style="position: relative"><span id="vmClusterDetail" style="display: none;position: absolute;bottom: -40px;left: 10px;background: white"> ' + vmCluster + '</span> '+LANG.UI_SEARCH_CLUSTER+': <i>' + vmCluster + '</i><em>X</em></span>';		}
		if(p.backupFlag != "0"){
		...
		if(p.vcenteruuid != "0"){
			info += '<span id="vcenterSelect" title="' + $('#vcenterSelect').find("option:selected").text() + '"> '+LANG.UI_VCENTER_VCENTER+': <i>' + $('#vcenterSelect').find("option:selected").text() + '</i><em>X</em></span>';
		}
		...
		$('#searchDiv .searchContent #vmCluster').mouseenter(function(e){
			$('#vmClusterDetail').show()
		}).mouseleave(function(){
			$('#vmClusterDetail').hide()
		});
  ...
  //初始化虚拟化中心列表
    var initVcenterSelect = function(){
		pAjaxRequest({sub_module_type: sub_module_type}, "/api/v1/vm/overview/platforms", "GET", function (d) {
			...
			vcenterSelect.empty();
			var option = $("<option>").text(LANG.UI_SEARCH_ALL_VCENTERS).val('0');
