    ...
    $.fn.initDateRangePicker = function(options) {
        ...
        const initDateRangePicker = () => {
            ...
            $(`#${settings.dateRangePickerId}`).daterangepicker({
                ...
                timePickerSeconds: settings.timePickerSeconds === true ? true : false,
                timePicker: settings.timePicker === true ? true : false, // 是否显示时间,时分
                timePicker24Hour: settings.timePicker24Hour === true ? true : false, // 是否是24小时制
                alwaysShowCalendars: settings.alwaysShowCalendars === true ? true : false, // 是否总是显示日期选择
                ranges: typeof settings.ranges !== 'undefined' ? settings.ranges : DateRangePickerLocales.getRangesConfig(CONF.LANGUAGE),  // 是否显示日期范围（今天、昨天、本月、上月、自定义等）
                locale: DateRangePickerLocales.getLocalConfig(CONF.LANGUAGE),
                parentEl: `#${settings.slotId}` || 'body',
                opens: typeof settings.opens !== 'undefined' ? settings.opens : 'right',  // 打开的位置(left、right、center)
                ...
                if (typeof settings.opens !== 'undefined' && settings.opens === 'center') {  // 将位置设置在中间
                    let $dateRangePicker = $(`#${settings.slotId} .daterangepicker.toolbar-daterangepicker`);
                    let dateRangePickerShowWidth = Math.ceil($dateRangePicker.outerWidth(true));
                    let dateRangePickerBtnWidth = Math.ceil($(`#${settings.dateRangePickerId}`).outerWidth(true));
                    $dateRangePicker.css('left', Math.ceil(dateRangePickerBtnWidth / 2) - Math.ceil(dateRangePickerShowWidth / 2));
                }
