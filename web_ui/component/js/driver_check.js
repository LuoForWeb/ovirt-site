...
(($, vinchinUI, UIToastr, LANG) => {
    ...
    const sendDriverCheckRequest = (driverCache, reqData) => {
        return new Promise((resolve, reject) => {
            vinchinUI.blockUI({target: `#${driverCache.options.element_set.check_list_id}`, animate: true});
            pAjaxRequest(reqData, `/api/v1/drivers/check`, 'POST', result /** @param {DriverResponse} result */ => {
                vinchinUI.unblockUI(`#${driverCache.options.element_set.check_list_id}`);
                if (!result.success) {
...
})(jQuery, vinchinUI, UIToastr, LANG);