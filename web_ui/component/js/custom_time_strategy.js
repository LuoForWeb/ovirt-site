...
(() => {
    ...
    const getTimePicker = ({id, tips = '', popover = ''}) => {
        ...
        <div id="${id}__wrapper" class="col-md-7">
            ...
            <a class="popover-abs-position ${popover ? '' : 'display-none'}" data-toggle="tooltip" data-placement="right" data-html="true"
                title="${popover}">
    ...
    const initRender = prefixId => {
        ...
        // 提示信息
        $(`#${prefixId} .popover__wrapper [data-toggle="tooltip"]`).tooltip();
        $(`#${prefixId} .popover-abs-position`).tooltip();
    ...
    const getShowBack = (des, id) => {
        return `
        <li class="list-group-item time-strategy-show-item" data-toggle="tooltip" data-placement="top" data-html="true"
            title="${des}" id="${id}">
            <div class="col1">
                <div class="cont">
    ...
    const showConfig = (prefixId, configItem, key) => {
        ...
        $(`#${configItem.idx}`).tooltip().find('div.delete-list a.del-item').on('click', () => {
            ...
            $(`#${configItem.idx}`).tooltip('hide').remove();
