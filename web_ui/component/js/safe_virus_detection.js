(function ($) {
					...
					<div class="panel panel-default strategy-panel">
						<div class="panel-heading">
							<h4 class="panel-title">
								<a class="accordion-toggle accordion-toggle-styled" data-container="body" data-trigger="hover" data-placement="top"
	...
	const initVirusEngine = (id, scanStrategy) => {
		...
		if (scanStrategy.show_scan_engine_flag) {
			...
			vinchinUI.blockUI({ target: `#${id}`, animate: true });
			pAjaxRequest({ offset: 0, limit: 200 }, `/api/v1/virus`, `GET`, res => {
				vinchinUI.unblockUI(`#${id}`);
				if (!res.success) {
					return;
				}
				let options = ``;
		...
		const getContent = () => {
			...
			<div class="form-group">
				...
				<div class="col-md-3 form-group-content flex-items-center">
					...
					<a class="ml12 mb-5" data-toggle="tooltip" data-placement="top" data-html="true" title="${LANG.UI_SAFE_STRATEGY_BACKUP_POINT_CHECK_VIRUS}">
...
<!--								<div class="help-block">${LANG.UI_SAFE_MAX_NUM_OF_ONE_DETECTION}</div>-->
		...
		const getContent = () => {
			...
			<div class="form-group">
				...
				<div class="col-md-7 form-group-content flex-items-center">
					...
					<a class="ml12 mb-5" data-toggle="tooltip" data-placement="top" data-html="true"
						title=${LANG.UI_SAFE_STRATEGY_BACKUP_POINT_CHECK_VIRUS}>
						<i class="viconfont vicon-tishi"></i>
					</a>
				</div>
			</div>
						...
						<div class="${percent = 'col-md-3' ? 'col-md-9' : 'col-md-8'}">
							<div class="accordion virusTakeoverStrategy">
								<div class="panel panel-default strategy-panel">
									<div class="panel-heading">
										<h4 class="panel-title">
											<a class="accordion-toggle accordion-toggle-styled" data-container="body"
	...
	$.fn.getTakeAppOver = function () {
		let id = $(this).attr('id');
		let takeoverStrategy = parseInt($(`#${id}_takeover_select`).find('.radio-group__item.active').attr("value"));
		let interruptPolicy = parseInt($(`#${id}_scan_strategy`).val() || -1);
