 ...
 * @LastEditTime: 2026-03-27 09:31:17
 * @Version: 2.0
...
class DataManage extends Base
{
    ...
    public function getTaskGrid($params)
    {
        ...
        $sortArr = [
            ...
            "module_type_des" => "bbt.module_type",
        ...
        $sql = " FROM bd_backup_timepoint bbt
                ...
                WHERE bbt.deleted_flag = {$FLAG['UNSET']}  AND bbt.import_flag = {$FLAG['UNSET']} AND (dbt.depend_task_uuid = '' OR dbt.depend_task_uuid IS NULL) AND (agent.storage_location != 0 OR agent.storage_location IS NULL) ";
        ...
        if (!empty($sort) && !empty($order)) {
            $sql .= " ORDER BY {$sortArr[$sort]} {$order} , bbt.task_uuid ASC";
        }
        $sqlTask = $sqlData . $sql;
        if (!empty($limit)) {
        ...
        if (!empty($data)) {
            foreach ($data as $d) {
                ...
                $sub_type_des = $VMHYPERVISORDES[$d['hypervisor_type']] ?: $DB_TYPE_DES[$d['db_type']] ?: '';
                $record['rows'][] = [
                    "id" => $d['task_uuid'],
                    ...
                    "vcenter_name" => $vm_vcenter_arr[$d['vcenter_uuid']] ?? '--',
                    "sub_type" => $d['hypervisor_type'] ?: $d['db_type'] ?: 0,
                    "sub_type_des" => in_array($d['task_type'], [$TASK_TYPE['BACKUP_COPY'], $TASK_TYPE['BACKUP_COPY_FETCH'], $TASK_TYPE['ARCHIVE'], $TASK_TYPE['ARCHIVE_FETCH']]) ? '---' : $sub_type_des,
    ...
    public function getItemGrid($params)
    {
        ...
        $sql = " FROM bd_backup_timepoint bbt  LEFT JOIN vm_backup_timepoint vbt ON bbt.timepoint_uuid = vbt.timepoint_uuid
                ...
                LEFT JOIN bd_agent ba ON ba.agent_uuid = cddbi.source_agent_uuid
                WHERE bbt.deleted_flag = {$FLAG['UNSET']} AND bbt.import_flag = {$FLAG['UNSET']} AND (agent.storage_location != 0 OR agent.storage_location IS NULL)";
        // 数量查询
        $sqlCount = "SELECT COUNT(bbt.timepoint_uuid) AS total ";
        // 增加过滤条件
        $sql = $this->addFilterParams($sql, $params);
        // 模糊搜索备份对象名称
        if (!empty($search)) {
            $sql .= " AND (vbt.vm_name LIKE '%{$search}%' OR fbt.agent_name LIKE '%{$search}%' OR dbt.cluster_name LIKE '%{$search}%' OR obt.os_name LIKE '%{$search}%' OR mbt.organization_name LIKE '%{$search}%' OR kbt.cluster_name LIKE '%{$search}%' OR fbt.agent_ip  LIKE '%{$search}%' OR dbt.agent_ip LIKE '%{$search}%'  OR dbt.dir_path LIKE '%{$search}%' OR obt.agent_ip LIKE '%{$search}%' OR JSON_EXTRACT(agent.master_agent_detail, '$.ip') LIKE '%{$search}%')";
        }
        // 去除重复的对象 同一个对象 属于多个模块显示多次
        $sql .= "  GROUP BY bbt.module_type,bbt.task_type,fbt.agent_uuid,vbt.vcenter_uuid,vbt.vm_uuid,dbt.agent_uuid, dbt.instance_name, dbt.db_name, dbt.cluster_uuid,obt.agent_uuid, JSON_EXTRACT(mbt.organization_info, '$.organization_uuid'), kbt.cluster_uuid,agent.master_agent_uuid,cddbi.source_agent_uuid ";
        if (!empty($sort) && !empty($order)) {
            if ($sort == 'item_name') {
                $sql .= " ORDER BY vbt.vm_name {$order}, fbt.agent_name {$order}, dbt.db_name {$order}, obt.os_name {$order}, mbt.organization_name {$order}, bbt.id {$order}";
            } else {
        ...
        if (!empty($data)) {
            foreach ($data as $d) {
                ...
                $record['rows'][] = array(
                    ...
                    "db_type" => $d['db_type'] ?? 0,
                    "sub_type_des" => $VMHYPERVISORDES[$d['hypervisor_type']] ?: xphp_get_config('db', 'DB_TYPE_DES')[$d['db_type']] ?: '--',
    ...
    private function getSqlWhere($params, $flag = false)
    {
        ...
        if ($module_type == $MODULE['DB']) {
            ...
            //验证需要
            if(!empty($itemId) && $verifyFlag){
                $itemIdArr = implode("','", $itemId);
                 $sql .= " AND dbt.db_uuid IN ('{$itemIdArr}') ";
            }
        ...
        if (!empty($params['vcenter_uuid'])) {
            $sql .= " AND vbt.vcenter_uuid = '{$params['vcenter_uuid']}'";
        }
        // 查询条件-搜索关键字
        if (!empty($search)) {
            $sql .= " AND (bbt.timepoint LIKE '%{$search}%' OR vbt.vm_name LIKE '%{$search}%' OR fbt.agent_name LIKE '%{$search}%' OR dbt.dir_path LIKE '%{$search}%' OR obt.os_name LIKE '%{$search}%' OR mbt.organization_name LIKE '%{$search}%' OR kbt.cluster_name LIKE '%{$search}%' OR fbt.agent_ip LIKE '%{$search}%' OR dbt.agent_ip LIKE '%{$search}%' OR obt.agent_ip LIKE '%{$search}%')";
        }
        // 筛选时间点正常异常状态
        if (!empty($status)) {
            // 正常状态
    ...
    public function getTaskTree($params)
    {
        ...
        $sql .= " GROUP BY bbt.module_type,bbt.task_type,fbt.agent_uuid,vbt.vcenter_uuid,vbt.vm_uuid,dbt.agent_uuid,dbt.instance_name,dbt.db_name,dbt.cluster_uuid,obt.agent_uuid, JSON_EXTRACT(mbt.organization_info, '$.organization_uuid'), kbt.cluster_uuid,vol.id";
        $dataCount = $this->dbSelect($sqlCount . $sql) ?? [];
        if (!empty($sort) && !empty($order)) {
            if ($sort == 'item_name') {
            ...
            foreach ($data as $d) {
                ...
                if (!in_array($task_node_id, $task_arr) && !$params['more_flag']) {
                    ...
                    $node['tree'][] = [
                        ...
                        "pId" => $task_node_pid,
                        "task_uuid" => $d['task_uuid'],
                        "name" => $d['module_type'] == $MODULE_TYPE['VOL_CDP'] ? $d['task_name'] : $newest_data[$d['task_uuid']],
                ...
                if ($d['module_type'] == $MODULE_TYPE['DB']) {
                    ...
                    if (!empty($d['db_cluster_uuid']) && empty($d['depend_task_uuid'])) {
                        ...
                        // 以数据库为备份对象增加实例显示
                        if (in_array($d['db_type'], [$DB_TYPE['SQLSERVER'], $DB_TYPE['SAPHANA']]) && empty($d['db_cluster_uuid']) && empty($d['depend_task_uuid'])) {
                    ...
                    if (in_array($d['module_type'], [2, 17, 22])) {
                        $itemNodeId = $d['task_uuid'] . $item_uuid . $d['vcenter_uuid'];
                    }
                    // 增加对象显示
                    if ($itemNodeId && !in_array($itemNodeId, $item_arr) && empty($d['depend_task_uuid'])) {
                        $total_size = $d['total_size'];
                        array_push($item_arr, $itemNodeId);
                        $node_name = $total_size ? $unifiedField['item_name'] . "(" . v1_calsize($total_size, true) . ")" : $unifiedField['item_name'];
    ...
    private function getTaskAllPoints($params)
    {
        ...
        $sqlCount = "SELECT COUNT({$countChar}) as total
                    ...
                    LEFT JOIN bd_storage_resource bsr ON bbt.storage_uuid = bsr.storage_uuid
                    WHERE bbt.deleted_flag = {$FLAG['UNSET']}  AND (agent.storage_location != 0 OR agent.storage_location IS NULL)";
        // 查询数量
     ...
     * @param mixed $db_name
     */
    private function getItemNewName($module_type, $item_uuid, $agent_uuid = '', $instance_name = '', $db_name = '', $vcenter_uuid = '')
    {
        $MODULE = xphp_get_config('module', 'MODULE_TYPE');
        switch ($module_type) {
            case $MODULE['VM']:
            ...
            case $MODULE['PRIVATE_CLOUD']:
                $sql = "SELECT
                        ...
                        WHERE
                            ...
                            AND vbt.vcenter_uuid = '{$vcenter_uuid}'";
                break;
            case $MODULE['FS']:
            case $MODULE['NAS']:
            case $MODULE['OBS']:
    ...
    private function buildPointNode($d, $storageName, $node_name_array, $pId = '', $children = [], $diff_chain_flag = false)
    {
        ...
        $storage_name = $storageName[$d['storage_uuid']]['storage_name'] . '(' . $node_name_array[$d['node_uuid']] . ')';
        if (!empty($d['real_node_uuid'])) {
            $storage_name = $storageName[$d['storage_uuid']]['storage_name'] . '(' . $node_name_array[$d['real_node_uuid']] . ')';
        }
        return [
            "id" => $d['timepoint_uuid'],
            ...
            "details" => [
                ...
                "worm_info" => [
                    ...
                    "worm_protect_flag" => $worm_info['protect_flag'], // 剩余期限具体时间
                    "show_worm_flag" => true, // 完备点显示配置worm按钮，非完备点仅在未配置worm时显示，已配置不显示
                ],
            ...
            "tape_storage_flag" => $d['storage_type'] == $BD_STORAGE_TYPE['TAPE'],
            "children" => json_encode($children),
            "backup_set_id" => $d['backup_set_id'],
    ...
    private function getDeletePoints($params, $flag = false, $abnormal_flag = false)
    {
        ...
        $storage_uuid = $params['storage_uuid'];
        $sql = "SELECT bbt.timepoint_uuid, bbt.module_type, bbt.task_type, bbt.storage_uuid, bbt.task_uuid, bsr.node_uuid, bbt.real_node_uuid, bsr.storage_type,bbt.user_uuid, vbt.hypervisor_type AS sub_type, dbt.db_type FROM bd_storage_resource bsr, bd_backup_timepoint bbt " . self::POINT_LEFT_JOIN_SQL . " WHERE bbt.storage_uuid = bsr.storage_uuid";
    ...
    public function getPointDetailsInfo($params)
    {
        ...
        if (!empty($data)) {
            ...
            $return['detail'] = [
                ...
                "last_verify_time" => $data['end_time'] ?? '--',
    ...
    public function getRemoteGrid($params)
    {
        ...
        $sortArr = [
            'point_num' => 'timepoint_count',
            'total_size' => 'sum_total_size',
            'write_size' => 'sum_write_size',
            'task_name' => 'tree_name',
            'module_type' => 'module_type',
            'task_create_time' => 'task_create_time',
        ];
        $data = $this->service()->getRemoteTree($node_uuid, 0, $opName, $countMsg) ?? [];
        $taskList = $data['msg'] ?? [];
        $task_uuid_arr = array_column((array)$taskList, 'tree_uuid');
        // 获取所有任务列表
        $all_task_arr = $this->getAllTaskIdArr($task_uuid_arr);
        // 按表格排序完备点
        $List = v1_array_sort($taskList, $sortArr[$params['sort']], $params['order'], $params['offset'], $params['limit']);  //排序
        if (!empty($List)) {
            foreach ($List as $d) {
                // 任务是否已删除
                $taskAvailable = !in_array($d['tree_uuid'], $all_task_arr);
                $records['rows'][] = array(
                    "id" => $d['tree_uuid'],
    ...
    private function getRemotePoint($params)
    {
        ...
        if (!empty($full_List)) {
            foreach ($full_List as $d) {
                ...
                if (!empty($children)) {
                    // 组装非完备点数据
                    $child_list = $this->getChildrenList($children, $storageName, $configCache, $d['timepoint_uuid'], $item_name[0], $item_name, $remote_ip);
                }
                ...
                switch ($d['module_type']) {
                    ...
                    case $MODULE['OS']:
                        ...
                        $dir_path = $this->getPartitionInfo($info['os_config'], $info['os_type']);
                        break;
                    case $MODULE['M365']:
    ...
    private function getChildrenList($list, $storageName, $configCache, $timepoint_uuid, $item_uuid, $item_name, $remote_ip)
    {
        $records = [];
        foreach ($list as $d) {
            $point_status = $this->getPointStatus($d['operation_status'], 0, 0, 0);
            $info = json_decode($d['module_timepoint_info'], true);
            switch ($d['module_type']) {
    ...
    {
        $result = [];
        $sql = "SELECT host_name,ip,node_uuid,node_nickname FROM bd_node";
        $data = $this->dbSelect($sql, []) ?? [];
        foreach ($data as $d) {
            $node_name = $d['host_name'] . '(' . $d['ip'] . ')';
            if ($d['node_nickname'] != $d['host_name'] && !empty($d['node_nickname'])) {
                $node_name = $d['node_nickname'] . '(' . $d['ip'] . ')';
            }
            $result[$d['node_uuid']] = $node_name;
        }
        return $result;
    }
    // 获取标签点列表
