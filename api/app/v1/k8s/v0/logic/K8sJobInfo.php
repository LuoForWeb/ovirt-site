...
class K8sJobInfo extends Base
{
     ...
     * $stage_percent 阶段百分比
     */
    public function getPercentKubeTask($task_type, $task_status, $current_stage,$stage_percent){

        $stage_percent = intval($stage_percent);
        $stageArr = xphp_get_config('task', 'COMMON_STAGE_LIST');
        $statusArr = xphp_get_config('task', 'TASKSTATUS');
        $taskTypeArr = xphp_get_config('task', 'TASKTYPE');
        if($task_status != $statusArr['RUNNING']){
            return 0;
        }
        if($task_type == $taskTypeArr['KUBE_BACKUP']){
            if ($current_stage == 0) {
                if ($task_status == $statusArr['SUCCESSED']){
                   $progress = 100;
                } else {
                  $progress = 0;
                }
             } else if ($current_stage == $stageArr["APP_TASK_STAGE_TYPE_EXEC_PRE_HOOK"]) {
                  $progress = 5 + (5 * ($stage_percent / 100 ));
             } else if ($current_stage == $stageArr["APP_TASK_STAGE_TYPE_BACKUP_RESOURCES"]) {
                $progress = 10 + (15 * ($stage_percent / 100 ));
             } else if ($current_stage == $stageArr["APP_TASK_STAGE_TYPE_CREATE_PVC_SNAPSHOT"]) {
                $progress = 25 + (15 * ($stage_percent / 100 ));
             } else if ($current_stage == $stageArr["APP_TASK_STAGE_TYPE_PREPARE_PVC_INSTANCES"]) {
                $progress = 40 + (15 * ($stage_percent / 100 ));
             } else if ($current_stage == $stageArr["TYPE_DATA_TRANSPORT"]) {
                $progress = 55 + (30 * ($stage_percent / 100 ));
             } else if ($current_stage == $stageArr["APP_TASK_STAGE_TYPE_CLEAN_TMP_RESOURCES"] ) {
                $progress = 85 + (5 * ($stage_percent / 100 ));
             } else if ($current_stage == $stageArr["APP_TASK_STAGE_TYPE_EXEC_POST_HOOK"] || $current_stage == $stageArr["APP_TASK_STAGE_TYPE_EXEC_POST_FAILED_HOOK"]) {
                $progress = 90 + (5 * ($stage_percent / 100 ));
             } else if ($current_stage == $stageArr["APP_TASK_STAGE_TYPE_SAVE_TIMEPOINT"]) {
                $progress = 95 + (3 * ($stage_percent / 100 ));
             } else if ($current_stage == $stageArr["APP_TASK_STAGE_TYPE_DO_TIMEPOINT_RESERVED_STRATEGY"]) {
                $progress = 98 + (2 * ($stage_percent / 100 ));
             }
        }else if($task_type == $taskTypeArr['KUBE_RECOVERY']){
            if ($current_stage == 0) {
                if ($task_status == $statusArr['SUCCESSED']){
                    $progress = 100;
                } else {
             ...
             } else if ($current_stage == $stageArr['TYPE_INTEGRITY_CHECK']) {
                $progress = 1 + (4 * ($stage_percent / 100 ));
             } else if ($current_stage ==  $stageArr['APP_TASK_STAGE_TYPE_EXEC_PRE_HOOK']) {
                $progress = 5 + (5 * ($stage_percent / 100 ));
             } else if ($current_stage == $stageArr['APP_TASK_STAGE_TYPE_PREPARE_PVC_INSTANCES']) {
                $progress = 10 + (25 * ($stage_percent / 100 ));
             } else if ($current_stage == $stageArr['TYPE_DATA_TRANSPORT']) {
                $progress = 35 + (30 * ($stage_percent / 100 ));
             } else if ($current_stage == $stageArr['APP_TASK_STAGE_TYPE_CLEAN_TMP_RESOURCES'] ) {
                $progress = 65 + (5 * ($stage_percent / 100 ));
             } else if ($current_stage == $stageArr['APP_TASK_STAGE_TYPE_RECOVER_RESOURCES'] ) {
                $progress = 70 + (20 * ($stage_percent / 100 ));
             } else if ($current_stage == $stageArr['APP_TASK_STAGE_TYPE_EXEC_POST_HOOK'] || $current_stage == $stageArr['APP_TASK_STAGE_TYPE_EXEC_POST_FAILED_HOOK']) {
                $progress = 90 + (5 * ($stage_percent / 100 ));
             }
