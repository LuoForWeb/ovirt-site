 ...
 * @LastEditTime: 2026-03-20 11:30:31
 * @Version: 1.0
...
class CopyBackUp extends Backup
{
    ...
    public function createCopyJob($params)
    {
        ...
        // 自动添加副本对象
        $pfMsg['copy_ignore_object_list'] = $params['copyItemInfo']['copy_ignore_object_list'];
        $pfMsg['auto_add_flag'] = v1_parse_bool_to_flag($params['copyItemInfo']['auto_add_flag']);
        // 传输网络
    ...
    public function createCopyBackJob($params)
    {
        ...
        $pfMsg['copy_mode'] = $params['copyItemInfo']['copy_mode'];
        // 自动添加副本对象
        $pfMsg['copy_ignore_object_list'] = [];
        $pfMsg['auto_add_flag'] = false;
        // 传输网络
        $pfMsg['transport_ip'] = $params['strategyInfo']['transferStrategy']['transferNet'];
        // 传输网络
        $pfMsg['transport_port'] = $params['strategyInfo']['transferStrategy']['transferPort'];
    ...
    private function getBackupSourceTask($params)
    {
        ...
        $nodes = [];
        $task_uuid_arr = [];
        $archive_flag = $params['archive_flag'] || $params['archive_back_flag'];
        $sql = $this->getBackupTaskSql($params);
        // 查询任务列表
     ...
     * @return {}
     */
    private function getItemInTask($sql, $data_type, $copyItemArr, $no_check_flag, $p_incr_flag = false)
    {
        // 模块类型
        $MODULE = xphp_get_config('module', 'MODULE_TYPE');
     ...
     * @return {}
     */
    private function getItemNode($d, $pId, $data_type, $inCopyFlag, $detail, $unifiedField, $no_check_flag = false, $name = '', $instanceName = '', $is_parent_flag = false, $p_incr_flag = false)
    {
        // 模块类型配置缓存（假设类中已缓存）
        static $MODULE = null;
        if ($MODULE === null) {
     ...
     * @return {}
     */
    private function getIncrFlag($task_uuid)
    {
        $result = false;
        $sql = "select mode from bd_time_strategy where task_uuid = '" . $task_uuid . "'";
        $data = $this->dbSelect($sql) ?? [];
        if (!empty($data) && count((array)$data) == 1 && $data[0]['mode'] == 2) {
            $result = true;
        }
        return $result;
    }
     ...
     * @return {}
     */
    private function getTaskNode($d, $data_type, $inCopyFlag, $taskAvailable, $pId = '', $click_show = true, $archive_flag = false)
    {
        $pIncrFlag = false;
        // #28339 取消永久增量+镜像副本上云组合 只对备份任务源处理
        if ((in_array($d['module_type'], [2, 5]) || ($d['module_type'] == 4 && $d['db_type'] == 15)) && $data_type == 1) {
            $pIncrFlag = $this->getIncrFlag($d['task_uuid']);
        }
        ...
        return [
            ...
            "name" => $task_node_name,
            "open" => !$click_show,
            "nocheck" => !((in_array($data_type, [1]) && $d['module_type'] == 2 && !$archive_flag) || !$click_show),
            "isParent" => empty($pId) && !(in_array($data_type, [1]) && $d['module_type'] == 2 && !$archive_flag) ? true : false,
            ...
            "get_item_flag" => in_array($data_type, [1]) && $d['module_type'] == 2,
            "p_incr_flag" => $pIncrFlag,
        ];
    }
        ...
        switch ($module_type) {
            case $MODULE['VM']:
                ...
                if (!empty($params['search'])) {
                    $whereSql .= " AND (bbt.task_name LIKE '%{$params['search']}%' OR vbt.vm_name LIKE '%{$params['search']}%') ";
                }
                if (!empty($item_uuid)) {
            ...
            case $MODULE['DB']:
                ...
                // 模糊匹配
                if (!empty($params['search'])) {
                    $whereSql .= " AND (bbt.task_name LIKE '%{$params['search']}%' OR dbt.dir_path LIKE '%{$params['search']}%') ";
                }
            ...
            case $MODULE['OS']:
                ...
                // 模糊匹配
                if (!empty($params['search'])) {
                    $whereSql .= " AND (bbt.task_name LIKE '%{$params['search']}%' OR obt.agent_ip LIKE '%{$params['search']}%' OR obt.os_name LIKE '%{$params['search']}%')";
                }
                if (!empty($item_uuid)) {
            ...
            case $MODULE['M365']:
                ...
                // 模糊匹配
                if (!empty($params['search'])) {
                    $whereSql .= " AND (bbt.task_name LIKE '%{$params['search']}%' mbt.organization_name LIKE '%{$params['search']}%') ";
                }
                if (!empty($item_uuid)) {
     ...
     * @return string
     */
    private function getBackupDataTaskSql($params)
    {
        $MODULE = xphp_get_config('module', 'MODULE_TYPE');
        // 副本源类型 1 备份数据| 2 备份任务  3 副本数据| 4 副本任务
        $COPY_SRC_TYPE = xphp_get_config('copy', 'COPY_SRC_TYPE');
        ...
        $sql = "SELECT
                    ...
                    bsr.storage_type,";
        // 查询条件
        $whereSql = "   WHERE bbt.rn = 1 ";
        // -- 按任务分组，按时间倒序打标签(1代表最新的)
        $tempSql = "
                FROM(
                    SELECT 
                        *,
                        ROW_NUMBER() OVER(PARTITION BY task_uuid ORDER BY timepoint DESC) as rn
                    FROM
                        bd_backup_timepoint 
                    WHERE
                        deleted_flag = {$FLAG['UNSET']}
                        AND import_flag = {$FLAG['UNSET']}
                        AND available_flag = {$FLAG['SET']} 
                        AND module_type = {$module_type}
                        AND chain_status IN  ({$CHAIN_STATUS['UNKNOWN']},{$CHAIN_STATUS['NORMAL']})
                    ) bbt ";
        switch ($module_type) {
            case $MODULE['VM']:
                $tempSql .= "
    ...
    private function getBackupDataTask($params)
    {
        ...
        //备份数据
        // 查询语句
        $sql = $this->getBackupDataTaskSql($params);
        $nodes = [];
        // 将副本的任务排列在最前面
        if (!empty($source_task_uuid)) {
            $sql .= " ORDER BY CASE WHEN bbt.task_uuid IN ('" . implode("','", $source_task_uuid) . "') THEN 0 ELSE 1 END,bbt.task_uuid ";
        }
        // dump($sql);
        $dataTask = $this->dbSelect($sql);
        // 获取当前所有任务节点相关的对象节点
        $task_uuid_arr = array_column((array)$dataTask, 'task_uuid');
        // 组装任务节点
     ...
     * @param mixed $data_type
     * @param mixed $task_offset
     * @param mixed $task_limit
     * @param mixed $task_uuid_arr
     * @return {}
     */
    private function getTaskNodeList($module_type, $dataTask, $data_type, $task_uuid_arr, $archive_flag = false)
    {
        $inCopyFlag = false;
        // 副本源类型 备份数据|任务  副本数据|任务
        $SOURCE_TYPE = xphp_get_config('copy', 'COPY_SRC_TYPE');
        ...
        foreach ($data as $point) {
            ...
            if ($point['backup_mode'] == $MODE['FULL'] || $module_type == $MODULE['M365']) {
                if (!in_array($time_point_uuid, $timepoint)) {
                    ...
                    $node[] = array(
                        "id" =>  $time_point_uuid,
                        "pId" =>  $pId,
                        "name" => $point['storage_status'] == 3 ? $node_name . '(' . xphp_get_lang('UI_PUBLIC_STORAGE_OFF') . ')' : $node_name,
                        ...
                        "chkDisabled" => $point['backup_mode'] != $MODE['FULL'] || $point['operation_status'] != 0 || $point['storage_status'] == 3,
                        "event_type" => "point",
                        "dataType" => $data_type,
                        "item_uuid" => $item_uuid,
            ...
            if (!in_array($time_point_uuid, $timepoint)) {
                ...
                $node[] = array(
                    ...
                    'chkDisabled' => $module_type == $MODULE['DB'] || $point['operation_status'] != 0 || $point['storage_status'] == 3,
                    "task_uuid" => $task_uuid,
     ...
     * 获取排除对象列表信息
     * @param mixed $task_uuid
     * @return array{copy_ignore_id_list: array, copy_ignore_object_list: array}
     */
    private function getCopyIgnoreItemList($task_uuid)
    {
        $sql = "SELECT * FROM copy_ignore_object_list WHERE task_uuid = '{$task_uuid}'";
        $data = $this->dbSelect($sql);
        $result = [];
        $uuidArr = [];
        if (!empty($data)) {
            foreach ($data as $d) {
                if (!isset($result[$d['source_task_uuid']])) {
                    $result[$d['source_task_uuid']] = [];
                }
                $result[$d['source_task_uuid']][$d['item_uuid']] = [
                    'task_uuid' => $d['task_uuid'],
                    'item_uuid' => $d['item_uuid'],
                    'parent_uuid' => $d['parent_uuid'],
                    'source_task_uuid' => $d['source_task_uuid'],
                ];
                $uuidArr = array_merge($uuidArr, [$d['item_uuid']]);
            }
        }
        return [
            'copy_ignore_object_list' => $result,
            'copy_ignore_id_list' => $uuidArr
        ];
    }
    ...
    public function getCopyTaskInfo($params)
    {
        ...
        $sql = "SELECT
                    ...
                    bt.storage_pool_uuid,
                    cl.source_storage_uuid,
                    cl.source_task_uuid,
                    ct.copy_mode,
                    ct.transport_ip,
                    ct.transport_port,
                    cl.sub_type,
                    ct.transport_data_encrypt,
                    bt.inc_mode,
                    ct.wan_accelerate_flag,
                    ct.chain_length,
                    ct.detail,
                    ct.detail,
                    ct.copy_week_flag,
                    ...
                    bsr.free_size,
                    bsr.storage_type,
                    bss.redundant_data_proportiont,
                    bss.data_container_size 
                FROM
                    copy_list cl,
                    copy_task ct,
                    bd_transport_strategy bts,
                    bd_storage_strategy bss,
                    bd_task bt
                    LEFT JOIN bd_storage_resource bsr ON bsr.storage_uuid = bt.storage_uuid 
                WHERE
                    bt.task_uuid = ct.task_uuid 
        ...
        $copy_ignore_info = $this->getCopyIgnoreItemList($taskUUID);
        if ($data) {
            ...
            $info = array(
                ...
                "auto_add_flag" => v1_parse_flag_to_bool($copy_info['auto_add_flag']),
                "copy_ignore_object_list" => json_encode($copy_ignore_info['copy_ignore_object_list']),
                "copy_ignore_id_list" => $copy_ignore_info['copy_ignore_id_list'],
            );
        }
        return $info;
    }
    ...
    public function getCopyEditInfo($task_uuid)
    {
        ...
        $source_task_uuid = [];
        $source_item_uuid = [];
        $sql =  "SELECT cl.item_id, cl.item_uuid, cl.parent_uuid, cl.item_name, cl.source_task_uuid, cl.source_storage_uuid, cl.specified_timepoint_list, cl.sub_type, bt.node_uuid,cl.archive_timepoint_uuid,cl.detail FROM copy_list cl, bd_task bt WHERE bt.task_uuid = cl.task_uuid  AND cl.task_uuid = ?";
        $data = $this->dbSelect($sql, array($task_uuid)) ?? [];
        if (!empty($data)) {
    ...
    private function searchCopyTask($params)
    {
        ...
        foreach ($data as $d) {
            ...
            // 可能存在多次副本同一对象的情况，需要加上task_uuid
            $itemNodeId = $d['task_uuid'] . $d['item_uuid'];
            if (!in_array($itemNodeId, $item_uuid_array)) {
