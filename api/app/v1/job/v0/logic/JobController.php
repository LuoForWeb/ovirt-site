...
class JobController extends Jobs
{
    ...
    public function getClass(array $taskuuid, $method = '', $params = []): array
    {
        ...
        foreach ($task as $item) {
            ...
            $recoveryArr = [
                ...
                "{$moduleTYpe['VOL_CDP']}-{$taskType['PLATFORM_RECOVERY']}", // 有代理实时跨平台恢
            ];
            ...
            if (
                $method == 'startJob' &&
                xphp_get_config('app', 'SYSTEM_INFO')['vendor'] == xphp_get_config('app', 'VENDOR_LIST')['gmp']
            ) {
                $this->taskLog(
                    $logParams,
                    'WEB_TASK_LOG_DESC_KEY_START_TASK_SUCCESS',
    ...
    public function delHistory($params = [])
    {
        ...
        foreach ($data as $item) {
            ...
            } elseif ($item['module_type'] == $module['FILE_COPY']) {
                $nodeuuid = $details[0]['node_uuid'];

                if (empty($nodeuuid)) { // 兼容旧的文件复制历史任务，没有在detial里存node_uuid，默认去拿主节点的uuid
        ...
        if (!empty($nodeinfo)) {
            foreach ($nodeinfo as $node) {
                ...
                if ($node['module_type'] == $module['FS']) { // 文件模块发到fs_server
                    $fsResult = $service->mbFSMsgs($node['nodeuuid'], $opName, $fsmsg);
                } else { // 文件复制模块发到 sync_fs_server
                    $fsResult = $service->mbFCMsgs($node['nodeuuid'], $opName, $fsmsg);
                }
            }
        }
        if (!empty($taskIDArr)) {
