 ...
 */
class SystemMonitor extends Base
{
    ...
    * ram_alarm
     *root_alarm
     */
    public function getSystemChart($params){
        //获取节点信息
        ...
        $info = array(
            ...
            'ramMsg' => array(
                //内存名字
                'name' => array(xphp_get_lang('UI_SYSTEM_MONITOR_RAM_PERCENTAGE')),
                //x轴时间
                'x_time' => array(),
                //y轴百分比
                'y_percent' => array(),
...
//             $info['cpuMsg']['x_time'][] = $each['monitor_time'];
            ...
            foreach ($cpuDetails as $cpuEach){
                $name_cpu = $cpuEach['name'].xphp_get_lang('UI_SYSTEM_MONITOR_PERCENTAGE_SYSTEM_CHART');
                if(!$cpu_name_flag){
...
//             $info['iopsMsg']['x_time'][] = $each['monitor_time'];
        ...
        $infoData = array(
            'success' => true,
            'code' => 0,
            'message' => xphp_get_lang('WEB_SYSTEM_MONITER_GET_DATA_SUCCESS'),
            'data' => $info,
        );
        return $infoData;
    }
     ...
     * @param string $startInfo 取的所有数据的第一个元素
     * @param string $endInfo 取的所有数据的最后一个元素
     * @param array $info 最后组装成的数据
     */
    ...
    public function getNodeUUid(){
        ...
        foreach ($result as $each){
            $name = "";
            if($each['node_type'] == 1){
                $name = xphp_get_lang('UI_PALTFORM_MASTER_NODE')."(".$each['ip'].")";
            }else{
                $name = $each['ip'];
            }
            //得到节点状态
            $nodeInfo = (new Node()) ->getNodeAllStatus($each['node_uuid']);
    ...
    public function getAlarmVal(){
        $infoData = array(
            'success' => true,
            'code' => 0,
            'message' =>  xphp_get_lang('WEB_SYSTEM_MONITER_GET_ALARM_MESSAGE'),
            'data' =>array(),
        );
        $info = array(
        ...
        $result = $this->dbSelect($sql,array(xphp_get_config('app','SETTINGS_CONF')['SYSTEM_MONITOR_PROGRESS']));
        //如果为空值 则表示没有设置告警信息
        if(empty($result)){
            $infoData['data'] = $info;
            return  $infoData;
    ...
    public function setAlarmVal($params){
        $infoData = array(
            'success' => true,
            'code' => 0,
            'message' => xphp_get_lang('WEB_SYSTEM_MONITER_SET_ALARM_MESSAGE'),
            'data' =>array(),
        );
        ...
        $settings_type = xphp_get_config('app','SETTINGS_CONF')['SYSTEM_MONITOR_PROGRESS'];
        //先删除相关设置再统一添加
    ...
    public function getBasicInfo($params){
        ...
        $infoData = array(
            'success' => true,
            'code' => 0,
            'message' => xphp_get_lang('WEB_SYSTEM_GET_NODE_INFO_SUCCESS'),
            'data' => $info,
        );
        return $infoData;
    ...
    public function getOSBasicInfo($node_uuid){
        ...
        //获取CPU名称型号主频
        $cardcmd .= "cat /proc/cpuinfo | grep name | cut -f2 -d: | awk 'NR==1';";
        //物理cpu个数
        ...
        //根分区总量
        $cardcmd .= "df -BG | grep -w '/$' | awk {'print $2'};";
        //根分区已使用
        $cardcmd .= "df -BG | grep -w '/$' | awk {'print $3'};";
        //根分区使用率
        $cardcmd .= "df -BG | grep -w '/$' | awk {'print $5'};";
        $msg = array(
            "command" => $cardcmd
        );
        ...
        if($mbResult['result']){
            ...
            $info = array(
                ...
                //内存总量
                'menTotal' => v1_calsize($BasicData[13]*1024,true),
                //根分区总大小
    ...
    public function getDiskInfo($node_uuid){
        ...
        $opName = 'NODE_SYS_OP_DO_CMD_WITH_DETAIL';
        //设备名称
        $cardcmdpart1 = "lsblk -d --output NAME,TYPE,SIZE;";
        $msgpart1 = array(
