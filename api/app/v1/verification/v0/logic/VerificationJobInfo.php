...
class VerificationJobInfo extends Base
{
    ...
    protected function getTimepointDes(string $timepointuuid, $info){
        ...
        $sql = "select bbt.timepoint, bbt.backup_mode,bbt.module_type, vol.end_timestamp from bd_backup_timepoint bbt 
                ...
                where (bbt.task_uuid = ? or dbt.depend_task_uuid = ?)  and (vbt.vm_uuid = '".$objectuuid."' or fbt.agent_uuid = '".$objectuuid."' or dbt.db_uuid = '".$objectuuid."' or obt.agent_uuid = '".$objectuuid."' or mbt.organization_uuid = '".$objectuuid."' or kbt.cluster_uuid = '".$objectuuid."' or agent.master_agent_uuid = '".$objectuuid."') ";
        $sqlParams = array($taskuuid, $taskuuid);
        if (!empty($timepointuuid)){
            $sql .= " and bbt.timepoint_uuid = ? ";
            $sqlParams = array($taskuuid, $taskuuid, $timepointuuid);
        }else{
...
//        $utils = Xphp::instance('Utils');
        ...
        foreach ($vmInfo as $d){
            ...
            //组装时间点信息
            $timepointDes = $d['timepoint']."(".xphp_get_desc('Pf','BACKUP_MODE_DES')[intval($d['backup_mode'])] .xphp_get_lang('WEB_PLATFORM_DES_BACKUP'). ")";

            //数据库模块单独处理日志备份和归档日志备份描述显示
            if(intval($d['module_type']) == xphp_get_config('module', 'MODULE_TYPE')['DB'] ){
                //以下是需要显示归档日志备份点的数据库类型
                $dbList = [2, 4, 5, 6, 7, 8, 10, 11, 12 ];
                if (intval($d['current_mode']) == xphp_get_config('task', 'BACKUP_MODE')['LOG'] && in_array(intval($d['submodule_type']),$dbList)){
                    $timepointDes = $d['timepoint']."(".xphp_get_desc('Pf','BACKUP_MODE_DES')[5] .xphp_get_lang('WEB_PLATFORM_DES_BACKUP'). ")";//显示归档日志备份
                }
            }
            $taskTypeDes = xphp_get_desc('Pf', 'MODULE_TYPE_DES')[intval($d['module_type'])]. xphp_get_desc('Pf', 'TASKTYPEDES')[intval($d['task_type'])];
            if(intval($d['task_type']) == xphp_get_config('task', 'TASKTYPE')['DB_BACKUP']){
                $taskTypeDes =  xphp_get_desc('Pf', 'TASKTYPEDES')[intval($d['task_type'])];
            }
            $info = ' <li>'.
                '<div style="margin-bottom:20px;">'.xphp_get_lang('UI_VERIFY_OBJECT').': '.$d['name'].'</div>'.
                '<div class="content-list" >'.
                '<p ><span>'.xphp_get_lang('UI_PUBLIC_TASK_RNAME').': </span><span>'.$d['task_name'].'</span></p>'.
                '<p ><span>'.xphp_get_lang('UI_PUBLIC_TASK_TYPE').': </span><span>'.$taskTypeDes.'</span></p>'.
                '<p ><span></span></p>'.
                '</div>'.
                '<div class="content-list" >'.
                ...
                '</div>'.
                '<div class="content-list">'.
                '<p ><span>'.xphp_get_lang('UI_DATA_TIMEPOINT').': </span><span>'.$timepointDes.'</span></p>'.
