...
if (!function_exists('getConfig')) {
    ...
    function getConfig(string $file): array
    {
        ...
        // 加密的配置文件数组
        $encryptArr = [
            APP_PATH . getXphpVersion() . '/config/app.php',
            APP_PATH . getXphpVersion() . '/config/database.php',
            APP_PATH . getXphpVersion() . '/config/email.php',
            APP_PATH . getXphpVersion() . '/config/socket.php',
            APP_PATH . getXphpVersion() . '/config/three_powers.php',
        ];
        if (!empty($arr) && in_array($file, $encryptArr)) {
            // 判读是否是加密过的
            // 读取 app/v1/config/app.php 配置文件里面的 ext 内容
            $arrs = require APP_PATH . getXphpVersion() . '/config/app.php';
            if ($arrs['ext'] != '.php') {
                // 表示加密了
                // 对数组进行解密
