...
class Tools
{
    ...
    {
        return preg_replace_callback('/\\\\u([0-9a-f]{4})/i', create_function('$matches', 'return iconv("UCS-2BE", "UTF-8", pack("H*", $matches[1]));'), json_encode($array));
        //return preg_replace_callback('/\\\\u([0-9a-f]{4})/i', create_function('$matches', 'return iconv(pack("H*", $matches[1]), "UTF-8", "UCS-2BE");'), json_encode($array));
    }
