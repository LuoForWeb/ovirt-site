...
class Gd
{
    ...
    public function open($imgname)
    {
        //检测图像文件
        if (!is_file($imgname)) {
            throw new \Exception(xphp_get_lang('WEB_IMAGE_NOT_EXISTS_IMAGE'));
        }
        ...
        if (false === $info || (IMAGETYPE_GIF === $info[2] && empty($info['bits']))) {
            throw new Exception(xphp_get_lang('WEB_UPLOAD_ILLEGAL_IMAGE_FILE'));
        }
    ...
    {
        if (empty($this->im)) {
            throw new Exception(xphp_get_lang('WEB_IMAGE_NO_IMAGE_CAN_SAVE'));
        }
    ...
    {
        if (empty($this->im)) {
            throw new Exception(xphp_get_lang('WEB_IMAGE_NO_SPECIFY_IMAGE'));
        }
    ...
    {
        if (empty($this->im)) {
            throw new Exception(xphp_get_lang('WEB_IMAGE_NO_CROPPED_IMAGE'));
        }
    ...
    {
        if (empty($this->im)) {
            throw new Exception(xphp_get_lang('WEB_IMAGE_NO_ABBREVIATED_IMAGE'));
        }
        ...
        switch ($type) {
            ...
            default:
                throw new Exception(xphp_get_lang('WEB_IMAGE_NOT_SUPPORTED_CROPPED_TYPE'));
        }
    ...
    public function water($source, $locate = THINKIMAGE_WATER_SOUTHEAST)
    {
        //资源检测
        if (empty($this->im)) {
            throw new Exception(xphp_get_lang('WEB_IMAGE_NO_WATERMARKED_IMAGE'));
        }

        if (!is_file($source)) {
            throw new Exception(xphp_get_lang('WEB_IMAGE_WATERMARKED_IMAGE_NOT_EXISTS'));
        }

        //获取水印图像信息
        $info = getimagesize($source);
        if (false === $info || (IMAGETYPE_GIF === $info[2] && empty($info['bits']))) {
            throw new Exception(xphp_get_lang('WEB_IMAGE_ILLEGAL_WATERMARKED_FILE'));
        }
        ...
        switch ($locate) {
            ...
            default:
                ...
                if (is_array($locate)) {
                    ...
                    throw new Exception(xphp_get_lang('WEB_IMAGE_NOT_SUPPORTED_WATERMARK_TYPE'));
                }
        }
    ...
    public function text($text, $font, $size, $color = '#00000000',
        ...
        //资源检测
        if (empty($this->im)) {
            throw new Exception(xphp_get_lang('WEB_IMAGE_NO_IMAGE_CAN_WRITE'));
        }

        if (!is_file($font)) {
            throw new Exception(xphp_get_lang('WEB_IMAGE_NOT_EXIST_FONT_FILE')."：{$font}");
        }
        ...
        switch ($locate) {
            ...
            default:
                ...
                if (is_array($locate)) {
                    ...
                    throw new Exception(xphp_get_lang('WEB_IMAGE_NOT_SUPPORTED_FONT_TYPE'));
                }
        }
        ...
        if (is_string($color) && 0 === strpos($color, '#')) {
            ...
            throw new Exception(xphp_get_lang('WEB_IMAGE_ERROR_COLOR'));
        }
