        ...
        pAjaxRequest({}, '/api/v1/jobs/history/' + row.job_uuid + '', 'GET', function (res) {
            ...
            if (data.info.job_type == CONF.TASK_TYPE.SURE_BACKUP) {
                ...
                if(data.list[0].verify_way == 1){
                    return $(element).append(`<tr><td>${LANG.UI_PUBLIC_NOTHING}</td></tr>`);
                }

                // 中止或错误任务不显示验证报告按钮
                if (!data.info.report_uuid) {
                    return $(element).append(`<tr><td>${LANG.UI_PUBLIC_NOTHING}</td></tr>`);
                }
                ...
                if (data.list && data.list.length > 0) {
                    ...
                    let tempHtml = '';
                    for (let i = 0; i < data.list.length; i++) {
                        let reportHtml = data.list[i].report_uuid ? `<a class="verifyReport" id="${data.list[i].report_uuid}">查看</a>` : `${LANG.UI_PUBLIC_NOTHING}`;
                        tempHtml += `<tr>
                                        <td>` + data.list[i].vm_name + `</td>
                                        <td>` + reportHtml + `</td>
                                    </tr>`;
                    ...
                    html = `<table><tr><td>${LANG.UI_PUBLIC_NOTHING}</td></tr></table>`;
                }
            ...
            $('.verifyReport').off().on('click', function(){
                let reportUUID = $(this).attr("id");
                Metronic.blockUI({target: '#history_verify_table', animate: true});
                JobReportDetail.init({'uuid': reportUUID, 'pre': 1, 'history': 1});
                Metronic.unblockUI('#history_verify_table');
            });
