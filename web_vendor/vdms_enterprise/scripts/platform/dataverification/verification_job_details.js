var VerificationJobDetail = function () {
    ...
    let documentFlag = true; // 是否设置了文件比对
    ...
    function addListener() {
        ...
        $('.open_verify_machine').off('click').on('click', function (){
            $('#show_iframe_url').hide();
            $('#show_iframe_url2').hide();
            if (!isInitUrl) {
                ...
                $('#show_iframe_url').show();
            }
            $('#verify_vir').click();
        });
        // 打开生产环境事件
        $('.open_prod_machine').off('click').on('click', function (){
            $('#show_iframe_url').hide();
            $('#show_iframe_url2').hide();
            if (!isInitUrl) {
                opereate(vm_uuid, vnc_url, 'show_iframe_url2');
                // 需要先请求接口，然后渲染
            } else {
                isInitUrl = true;
                $('#full-machine-block').show();
                $('#show_iframe_url2').show();
            }
            $('#product_vir').click();
        });
        // 生产和验证环境切换事件
        $('#verify_vir').off('click').on('click', function (){
            $(this).addClass('in-active');
            $(this).removeClass('in-default');
            $('#product_vir').removeClass('in-active');
            $('#product_vir').addClass('in-default');
            $('#show_iframe_url').show();
            $('#show_iframe_url2').hide();
        })
        $('#product_vir').off('click').on('click', function (){
            $(this).addClass('in-active');
            $(this).removeClass('in-default');
            $('#verify_vir').removeClass('in-active');
            $('#verify_vir').addClass('in-default');
            $('#show_iframe_url2').show();
            $('#show_iframe_url').hide();
        })
            ...
            pAjaxRequest(params, '/api/v1/verification/object_info', 'GET', (result) => {
                if (result.success){
                    ...
                    if (!documentFlag) {
                        ...
                        $('#filePath').html(doc);
                        $('#filePath').parent().show();
                    } else {
                        $('#filePath').html('该任务没有比对文件');
                    }
                    $('#filePath').parent().show();
    ...
    function buttonClickListener() {
        ...
        $(".screen-item .products .div-img").mouseenter(function(){
            ...
            var dom = $(this).closest('.screen-item-pic-job');
            dom.find('.show-screen-item-shadow-box').show(); // 显示遮罩
            dom.find('.show-screen-item-shadow-box .shadow-box').css({
                'height': height * 2 + 64
            });
            // 鼠标点击放大事件
    ...
    // 增加iframe
    function loadIframe_bak(id, url) {
    ...
    function loadIframe(id, url) {
        ...
        var container = document.getElementById(id);
        // 这里需要加上一个提示
        let tips = `<div style="position: fixed;left: calc(40% - 200px);margin-top: 4px;">
                        <div class="alert alert-block alert-info fade in mt15">
                        <button id="cencel" type="button" class="close" data-dismiss="alert"></button>
                        <h4 class="alert-heading"><strong style="font-size: 16px;">提示</strong></h4>
                        <ol class="alert-ol" style="font-size: 12px;">
                            <li>
                               进行截图时，请保持生产与验证环境处于开机与活跃状态。</li>
                            <li>
                                生产环境截图采用客户端会话方式，请勿最小化窗口</li>
                        </ol>
                    </div></div>`;
        container.innerHTML = tips; // 清空容器
        container.appendChild(iframe);

        // 标记是否已确认加载成功
        let loaded = false;

        // 监听 load 事件（仅当 iframe 开始加载时触发，即使显示的是浏览器错误页也可能触发）
        iframe.onload = function() {
            loaded = true;
            console.log('Iframe loaded (but may still show cert error)');

            // 尝试检测是否能访问内容（只有在同源且无证书问题时才可能成功）
            try {
                // 注意：跨域或证书错误都会导致 SecurityError
        ...
        // 设置超时检测：如果长时间未触发 onload 或无法访问内容，则提示用户
        setTimeout(() => {
            if (!loaded) {
                console.warn('Iframe did not load within timeout');
                maybeShowFallback();
            }
        }, 3000); // 3秒超时，可根据网络调整

        // 备用检测方法：尝试访问 location 失败则提示
        ...
        function showSecurityWarning() {
            // 防止重复添加警告
            if (container.querySelector('.cert-warning')) return;
            $('#show_iframe_url').html('');
            $('#show_iframe_url').css('background', '#ddd');
            const warningDiv = document.createElement('div');
            warningDiv.className = 'cert-warning';
        ...
        `;
            ...
            <strong>浏览器可能采用了安全策略，<a onclick="openAndMark('${url}')" style="
                margin-top: 8px;
                color: #0066cc;
                border: none;
                cursor: pointer;
            "> “点击这里” </a>进行授权</strong>
        `;
            container.style.position = 'relative'; // 确保定位上下文
            container.appendChild(warningDiv);

        }
        window.openAndMark = function (targetUrl) {
            // 打开新窗口让用户手动信任证书
    ...
    function calcDel(){
        ...
        $('.block-item-bottom').each(function (index){
            let pic = $(this).find('.screen-item-pic-job');
            pic.hide();
            pic.last().show();

            pic.find('.product-job .titles i').remove(); // 移除所有的
            pic.find('.verify-job .titles').find('i').remove(); // 移除所有的
            pic.find('.screen-time-job-del').find('i').remove(); // 移除所有的
            pic.find('.screen-desc .result').find('i').removeClass('i-active'); // 移除所有的

            if (report_status == 9) {
                pic.find('.screen-time-job-del').append('<i class="viconfont vicon-zujianshanchu"></i>'); // 给一个删除图标
                pic.last().find('.product-job .titles .labels-l').append('<i class="viconfont vicon-zujianshanchu"></i>'); // 给最后一个增加删除图标
                pic.last().find('.verify-job .titles .labels-l').append('<i class="viconfont vicon-zujianshanchu"></i>'); // 给最后一个增加删除图标
                pic.last().find('.screen-desc .result i').addClass('i-active');
            } else {
                // 移除截图的所有删除
                pic.find('.screen-time-job-del').find('i').remove(); // 移除所有的
                // 隐藏截屏的按钮
                pic.closest('.right-contents .block-item2').hide();
            }
        })
        ...
        addListenerDel();
        // 这里计算下下面的按钮切换事件
        const totalItems = $('#block_calc_page').find('.screen-item-pic-job').length;
    ...
    function renderPagination(totalItems, itemsPerPage, currentPage) {
        ...
        // 添加点击事件
        paginationContainer.find('a').off('click').click(function(e) {
            e.preventDefault();
            let $clickedLink = $(this);
            let newPage = parseInt($clickedLink.attr('data-num'));
            ...
            $('.screen-bottom').each(function (){
                let screenBottom  = $(this);
                let $blockItemBottom = screenBottom .closest('.block-item-bottom');
                let items = $blockItemBottom.find('.screen-item-pic-job');

                items.each(function(index) {
                    let itemPage = Math.floor(index / itemsPerPage) + 1;
                    $(this).toggle(itemPage === newPage);
                });
            })
    ...
    // 二次处理截图显示html
    function againCalc(html) {
        $('.block-item-bottom .screen-bottom').each(function(index) {
            // 克隆HTML并修改
            let thatss = $(this);
            let $clonedHtml = $(html).clone();
            let $doms = $clonedHtml.find('.screen-desc [type="radio"]');
            $doms.each(function (){
                let names = $(this).attr('name');
                $(this).attr('name', names + index);
            })
            thatss.before($clonedHtml);
        });
    }
    // 删除按钮事件
    function addListenerDel(){
        ...
        $('.screen-item-pic-job .product-job .titles').find('i').off('click').on('click', function() {
            ...
            bootbox.confirm({
                ...
                callback: function(result) {
                    ...
                    if ($that.closest('.screen-item-pic-job').find('.verify-job').length > 0) {
                        $that.closest('.product_flag').removeClass('product-job');
                        $that.closest('.product_flag').find('img').attr('src', '/img/platform/screen_job.png');
                        initScreenTag = 2;
                        // console.log('initScreenTag', initScreenTag)
                        // 找到当前元素所在的组
                        let $group = $that.closest('.screen-item-pic-job');
                        // 修改该组内按钮的颜色
                        $group.find('.screen-desc .result i').addClass('i-active'); // 改变按钮背景色为黄色
                        $group.find('input[type="radio"]').prop('checked', false);
                    } else {
                        $that.closest('.screen-item-pic-job').remove();
                        calcDel();
                        initScreenTag = 0;
                        checkIsEmpty();
                    }
        ...
        $('.screen-item-pic-job .verify-job .titles').find('i').off('click').on('click', function() {
            ...
            bootbox.confirm({
                ...
                callback: function(result) {
                    ...
                    if ($that.closest('.screen-item-pic-job').find('.product-job').length > 0) {
                        $that.closest('.verify_flag').removeClass('verify-job');
                        $that.closest('.verify_flag').find('img').attr('src', '/img/platform/screen_job.png');
                        initScreenTag = 1;
                        // console.log('initScreenTag', initScreenTag)
                        // 找到当前元素所在的组
                        let $group = $that.closest('.screen-item-pic-job');
                        // 修改该组内按钮的颜色
                        $group.find('.screen-desc .result i').addClass('i-active'); // 改变按钮背景色为黄色
                        // 方法1：推荐 - 移除所有 radio 的 checked 属性
                        $group.find('input[type="radio"]').prop('checked', false);
                    } else {
                        $that.closest('.screen-item-pic-job').remove();
                        calcDel();
        ...
        $('.screen-item-pic-job .screen-time-job-del>i').off('click').on('click', function() {
            ...
            bootbox.confirm({
                ...
                callback: function(result) {
                    if(!result) return;
                    let doms = $that.closest('.block-item-bottom');
                    const $currentItem = $that.closest('.screen-item-pic-job');
                    const $allItems = $that.closest('.block-item-bottom').find('.screen-item-pic-job');
                    // ✅ 判断当前项是不是最后一个
                    const isLastOne = $allItems.index($currentItem) === $allItems.length - 1;
        ...
        $('.screen-item-pic-job .screen-desc .result').find('i').off('click').on('click', function (){
            ...
            saveScreen(0, $(this).closest('.block-item-bottom'));
        })
    }

    // 保存截屏
    function saveScreen(type = 0, $dom){
        var screen_list = [];
        var checks = true;
        var msg = '';
        $('.screen-item-pic-job').css('border', 'none');
        if ($dom.find('.screen-item-pic-job').length>0) {
            // 获取所有的截屏
            $dom.find('.screen-item-pic-job').each(function(index) {
                ...
                if ((product == '' || verify == '') && type == 0) {
                    checks = false;
                    msg = '请完善第（' + (index + 1) + '）组截图后重试';
                    $(this).css('border', '1px solid red');
                    return false;
                }
                if (result == '' && type == 0) {
                    msg = '请完善第（'+ (index + 1) +'）组截图的比对结果后重试';
                    $(this).css('border', '1px solid red');
                    checks = false;
                    return false;
                }
                if ((result == '' || product == '' || verify == '') && type == 1) {
                    // 如果是删除触发的保存，那么此时如果截屏未完成，就不保存这些记录
                } else {
                    screen_list.push(
    ...
    function addListenerScreen(){
        $('#productScreen_job, #productScreen_job2').off('click').on('click', function() {
            ...
            // 请求接口，组装返回数据
            Metronic.blockUI({target: '#jobDetail',animate: true,cenrerY: true});
            Metronic.blockUI({target: '#full-machine-block',animate: true,cenrerY: true});
            pAjaxRequest({report_uuid: report_uuid}, "/api/v1/industry/screenshot/produce", "POST", function (result) {
                Metronic.unblockUI('#jobDetail');
                Metronic.unblockUI('#full-machine-block');
                if (result.code == 0) {
                    // 数据渲染
                    ...
                    if (initScreenTag == 2) {
                        // 存在验证截屏
                        initScreenTag = 0;
                        let doms = thats.closest('.right-contents').find('.block-item-bottom');
                        doms.find('.screen-item-pic-job').last().find('.product_flag').addClass('product-job');
                        doms.find('.screen-item-pic-job').last().find('.product_flag img').attr('src', data.url);
                        doms.find('.screen-item-pic-job').last().find('.product_flag .labels-r').html(data.create_time);
                    } else {
                        // 都不存在，那么存在生产截屏
                        initScreenTag = 1;
                        var pa = [
        ...
        $('#verifyScreen_job, #verifyScreen_job2').off('click').on('click', function() {
            // 验证截屏事件
            if ((vm_uuid == '' || vnc_url == '') || !isReady) {
                UIToastr.showWarning(LANG.UI_PUBLIC_TIPS, LANG.UI_PLATFORM_GMP_JOB_NOT_READY);
                return false;
            ...
            pAjaxRequest({report_uuid: report_uuid}, "/api/v1/industry/screenshot/verify", "POST", function (result) {
                ...
                if (result.code == 0) {
                    ...
                    if (initScreenTag == 1) {
                        ...
                        let doms = thats.closest('.right-contents').find('.block-item-bottom');
                        initScreenTag = 0;
                        doms.find('.screen-item-pic-job').last().find('.verify_flag').addClass('verify-job');
                        doms.find('.screen-item-pic-job').last().find('.verify_flag img').attr('src', data.url);
                        doms.find('.screen-item-pic-job').last().find('.verify_flag .labels-r').html(data.create_time);
                    } else {
                        // 都不存在，那么存在验证截屏
                        initScreenTag = 2;
                        var pa = [
        ...
        $('#allScreen_job, #allScreen_job2').off('click').on('click', function() {
            // 一键截屏事件
            if ((vm_uuid == '' || vnc_url == '') || !isReady) {
                UIToastr.showWarning(LANG.UI_PUBLIC_TIPS, LANG.UI_PLATFORM_GMP_JOB_NOT_READY);
                return false;
    ...
    var getScreenHtml = function (list, type = 0) {
        ...
        var html = '';
        // 渲染列表
        var k = $('#block_calc_page .screen-item-pic-job').length;
        for (var j in list) {
            k = k + 1;
    ...
    var initStageAndResult = function (data){
        ...
        documentFlag = data.document_flag;
        ...
        pAjaxRequest({}, "/api/v1/industry/job/"+_jobUuid+"/client/"+object_uuid, "GET", function (d) {
            ...
            if (d.code == 0) {
                ...
                if (flag) {
                    ...
                    // 截屏列表
                    againMakeScreen(data.screen_list);
                }
    ...
    // 重新渲染截图
    function againMakeScreen(screen_list){
        // 截屏列表
        var pa = [];
